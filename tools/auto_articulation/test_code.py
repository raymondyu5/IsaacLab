...
full_mesh_verts = np.array(full_mesh_dict["verts"]).reshape(-1, 3).astype(np.float32)
full_mesh_faces = np.array(full_mesh_dict["faces_verts_idx"]).reshape(-1, 3).astype(np.int32) - 1
full_mesh_vts = np.array(full_mesh_dict["verts_uvs"]).reshape(-1, 2).astype(np.float32)
full_mesh_fts = np.array(full_mesh_dict["faces_textures_idx"]).reshape(-1, 3).astype(np.int32) - 1
full_mesh_faces_tex_pos = full_mesh_vts[full_mesh_fts.reshape(-1)].reshape(-1, 3, 2)
...
drawers_handle_save_dir = os.path.join(ckpt_dir, "handle")
os.makedirs(drawers_internal_save_dir, exist_ok=True)
os.makedirs(drawers_handle_save_dir, exist_ok=True)
...
def get_material(root, stage, tex_path):
    ...
    diffuseTextureSampler = UsdShade.Shader.Define(stage, f"{root}/{matname}/diffuseTexture")
    diffuseTextureSampler.CreateIdAttr('UsdUVTexture')
    diffuseTextureSampler.CreateInput('file', Sdf.ValueTypeNames.Asset).Set(tex_path)
...
def add_box_mesh_revolute(stage, scale, translation, rotation, path, rigid=False, no_collision=False, box_grid_mn=(3, 3), internal_inverse=False, drawer_mesh_door=None):
    ...
    # add internal box
    internal_box_mesh_trimesh = create_drawer_internal_box_mesh(D = 0.5 * scale_factor, Sy=scale[1], Sz=scale[2], thickness=thickness * scale_factor, m = box_grid_mn[0], n = box_grid_mn[1], internal_inverse=internal_inverse)
...
def add_box_mesh_prismatic(stage, scale, translation, rotation, path, rigid=False, no_collision=False, drawer_mesh_door=None):
    mesh = UsdGeom.Mesh.Define(stage, path)

    if drawer_mesh_door is not None:
        box_mesh = trimesh.Trimesh(
            drawer_mesh_door[0], drawer_mesh_door[1]
        )
    else:
        box_mesh = trimesh.primitives.Box(extents=[scale[0], scale[1], scale[2]])

    # joint internal box mesh
    internal_box_mesh = create_drawer_internal_box_mesh(D = 0.2 * scale_factor, Sy=scale[1], Sz=scale[2], thickness=thickness * scale_factor, m = 1, n = 1, internal_inverse=False, top=False)
    drawer_i = int(os.path.basename(os.path.dirname(path)).split("_")[-1])
    ...
    texpos = np.concatenate([texpos, np.zeros((internal_box_mesh.faces.shape[0]*3, 2))], axis=0)
...
def add_mesh(stage, mesh_verts, mesh_faces, texcoords, path, material, rigid=False):
...
def create_drawer(stage, path, scale, inverse, internal_inverse, box_grid_mn, drawer_mesh_door):
    sy = scale[1]
    sz = scale[2]
    joint_pos = [0, -0.5*sy, 0]
    stage = add_box_mesh_revolute(stage, (1e-4, sy, sz), (0, 0, 0), (1, 0, 0, 0), f"{path}/drawer", rigid=True, no_collision=True, internal_inverse=internal_inverse, box_grid_mn=box_grid_mn, drawer_mesh_door=drawer_mesh_door)
...
def create_drawer_with_joint(stage, scale, translation, rotation, path, inverse=False, internal_inverse=False, box_grid_mn=(3, 3), drawer_mesh_door=None):
    stage = create_xform(stage, (1, 1, 1), translation, rotation, path)
    stage = create_drawer(stage, path, scale, inverse, internal_inverse, box_grid_mn, drawer_mesh_door)
...
def create_drawer_prismatic(stage, path, scale, drawer_mesh_door):
    ...
    sz = scale[2]
    joint_pos = [0, 0, 0]
    stage = add_box_mesh_prismatic(stage, (1e-4, sy, sz), (0, 0, 0), (1, 0, 0, 0), f"{path}/drawer", rigid=True, no_collision=True, drawer_mesh_door=drawer_mesh_door)
    stage = add_cube(stage, (0.001, 0.001, 0.001), (joint_pos[0] + 1e-4, joint_pos[1], joint_pos[2]), (1, 0, 0, 0), f"{path}/drawer_help",  no_collision=True)
    stage = add_prismatic(stage, f"{path}/drawer_joint", f"{path}/drawer_help", f"{path}/drawer", np.array([-1e-4, 0, 0]).reshape(3), (0, 0, 0))
    return stage 
...
def create_drawer_with_joint_prismatic(stage, scale, translation, rotation, path, drawer_mesh_door):
...
stage = Usd.Stage.CreateNew(os.path.join(ckpt_dir, "joint.usda"))
...
# for drawer_i in range(1):
    ...
    if interact_type in ["1.1", "2", "3.3"]:
        inside_x = np.logical_and(full_mesh_verts_transformed[:, 0:1] > door_depth * 1.0,
    ...
    if np.any(drawer_handles_local):
        drawer_mesh_handle = filter_mesh_from_vertices(drawer_handles_local, full_mesh_verts, full_mesh_faces, full_mesh_faces_tex_pos)

        filtered_mesh = trimesh.Trimesh(drawer_mesh_handle[0], drawer_mesh_handle[1], process=False)
        ...
        keep = np.zeros((drawer_mesh_handle[0].shape[0])).astype(np.bool_)
        keep[components[largest_cc].reshape(-1)] = True
        v_filtered, f_filtered, _ = filter_mesh_from_vertices(keep, filtered_mesh.vertices.copy(), filtered_mesh.faces.copy(), None)

        trimesh.exchange.export.export_mesh(trimesh.Trimesh(
            v_filtered,
            f_filtered
        ), os.path.join(drawers_handle_save_dir, f"handle_{drawer_i:0>2d}.ply"))
    ...
    drawer_mesh_door = filter_mesh_from_vertices(drawer_sub_verts_local, full_mesh_verts_transformed, full_mesh_faces, full_mesh_faces_tex_pos)
    ...
    drawer_mesh_door = (drawer_mesh_door_verts, drawer_mesh_door[1], drawer_mesh_door[2], material)
    ...
    if interact_type in ["1.2", "3.2"]:
        stage = create_drawer_with_joint(stage, (1, scale[1], scale[2]), (trans[0], trans[1], trans[2]), 
                                        ...
                                        f"{root}/drawer/drawer_{drawer_i:0>2d}", internal_inverse=True, box_grid_mn=box_grid_mn, drawer_mesh_door=drawer_mesh_door)
    elif interact_type in ["1.1", "3.1"]:
        stage = create_drawer_with_joint(stage, (1, scale[1], scale[2]), (trans[0], trans[1], trans[2]), 
                                        transformations.quaternion_from_euler(angles[0], angles[1], angles[2], axes='sxyz'), 
                                        f"{root}/drawer/drawer_{drawer_i:0>2d}", inverse=True, box_grid_mn=box_grid_mn, drawer_mesh_door=drawer_mesh_door)
    else:
        stage = create_drawer_with_joint_prismatic(stage, (1, scale[1], scale[2]), (trans[0], trans[1], trans[2]), 
                                        transformations.quaternion_from_euler(angles[0], angles[1], angles[2], axes='sxyz'), 
                                        f"{root}/drawer/drawer_{drawer_i:0>2d}", drawer_mesh_door=drawer_mesh_door)


drawer_sub_verts_all = np.logical_not(drawer_sub_verts_all)
no_drawer_mesh_verts, no_drawer_mesh_faces, no_drawer_mesh_tex_pos = filter_mesh_from_vertices(drawer_sub_verts_all, full_mesh_verts, full_mesh_faces, full_mesh_faces_tex_pos)

stage = add_mesh(stage, no_drawer_mesh_verts, no_drawer_mesh_faces, no_drawer_mesh_tex_pos, f"{root}/main", material)
