...
class ButtonCallback:
    ...
    def __init__(self, buttons: Union[int, List[int]],
...
class Config:
    ...
    def __init__(
        ...
        check_config(callback, dof_callback, dof_callback_arr, button_callback,
                     button_callback_arr)
        self.callback = callback
    ...
    def process(self, data):
        ...
        for name, (chan, b1, b2, flip) in self.__mappings.items():
            if data[0] == chan:
                ...
                if b1 < len(data) and b2 < len(data):
                    self.dict_state[name] = (flip *
                                             to_int16(data[b1], data[b2]) /
                                             float(self.axis_scale))
        ...
        for button_index, (chan, byte, bit) in enumerate(self.button_mapping):
            if data[0] == chan:
                ...
                mask = 1 << bit
                self.dict_state["buttons"][button_index] = (1 if
                                                            (data[byte] & mask)
                                                            != 0 else 0)
        ...
        if self.dof_callback_arr and dof_changed:
            ...
            for block_dof_callback in self.dof_callback_arr:
                ...
                if now >= self.dict_state_last[
                        axis_name] + block_dof_callback.sleep:
                    ...
                    if block_dof_callback.callback_minus:
                        ...
                        block_dof_callback.cafllback(self.tuple_state,
                                                     axis_val)
        ...
        if self.button_callback_arr and button_changed:
            ...
            for block_button_callback in self.button_callback_arr:
                ...
                # is one button
                    ...
                    if not self.tuple_state.buttons[
...
def read_all():
    ...
    Returns:
        ...
        None if the device is not open.
    """
    return [_active_device[i].read() for i in range(len(_active_device))
            ] if _active_device is not None else None
...
def openCfg(config: Config,
    ...
    return open(config.callback, config.dof_callback, config.dof_callback_arr,
                config.button_callback, config.button_callback_arr,
                set_nonblocking_loop, device, DeviceNumber)
...
def check_button_callback_arr(
        button_callback_arr: List[ButtonCallback]) -> List[ButtonCallback]:
    ...
    for num, butt_call in enumerate(button_callback_arr):
        ...
        if not callable(butt_call.callback):
            raise Exception(
                f"'ButtonCallback[{num}]:callback' is not callable")
...
def check_dof_callback_arr(
        dof_callback_arr: List[DofCallback]) -> List[DofCallback]:
    ...
    for num, dof_call in enumerate(dof_callback_arr):
        if not isinstance(dof_call, DofCallback):
            raise Exception(
                f"'DofCallback[{num}]' is not instance of 'DofCallback'")
        ...
        if not dof_call.callback_minus or not callable(
            ...
            raise Exception(
                f"'DofCallback[{num}]:callback_minus' is not callable")
...
def config_set_sep(callback=None,
                   dof_callback=None,
                   dof_callback_arr=None,
                   button_callback=None,
                   button_callback_arr=None):
    ...
    if _active_device is not None:
        _active_device.config_set_sep(callback, dof_callback, dof_callback_arr,
                                      button_callback, button_callback_arr)
...
if __name__ == "__main__":
    ...
    dev = open(dof_callback=print_state,
