...
def capture_test_output_with_timeout(cmd, timeout, env):
    ...
    try:
        ...
        while process.poll() is None:
            ...
            try:
                ready_fds, _, _ = select.select([stdout_fd, stderr_fd], [], [],
                                                0.1)
...
def create_timeout_test_case(test_file, timeout, stdout_data, stderr_data):
    ...
    test_suite = TestSuite(
        name=f"timeout_{os.path.splitext(os.path.basename(test_file))[0]}")
    test_case = TestCase(name="test_execution",
...
def run_individual_tests(test_files, workspace_root, isaacsim_ci):
    ...
    for test_file in test_files:
        ...
        timeout = (test_settings.PER_TEST_TIMEOUTS[file_name]
                   if file_name in test_settings.PER_TEST_TIMEOUTS else
                   test_settings.DEFAULT_TIMEOUT)
        ...
        if timed_out:
            ...
            timeout_suite = create_timeout_test_case(test_file, timeout,
...
def pytest_sessionstart(session):
    ...
    if hasattr(session.config, "option") and hasattr(session.config.option,
                                                     "filter_pattern"):
        filter_pattern = filter_pattern or getattr(session.config.option,
                                                   "filter_pattern", "")
    if hasattr(session.config, "option") and hasattr(session.config.option,
                                                     "exclude_pattern"):
        exclude_pattern = exclude_pattern or getattr(session.config.option,
                                                     "exclude_pattern", "")
    ...
    print(
        f"TEST_FILTER_PATTERN env var: '{os.environ.get('TEST_FILTER_PATTERN', 'NOT_SET')}'"
    )
    print(
        f"TEST_EXCLUDE_PATTERN env var: '{os.environ.get('TEST_EXCLUDE_PATTERN', 'NOT_SET')}'"
    )
    print("=" * 50)
            ...
            for file in files:
                if file.startswith("test_") and file.endswith(".py"):
                    ...
                    if filter_pattern and filter_pattern not in full_path:
                        print(
                            f"Skipping {full_path} (does not match include pattern: {filter_pattern})"
                        )
                        continue
                    ...
                    if exclude_pattern and exclude_pattern in full_path:
                        print(
                            f"Skipping {full_path} (matches exclude pattern: {exclude_pattern})"
                        )
                        continue
    ...
    failed_tests, test_status = run_individual_tests(test_files,
                                                     workspace_root,
                                                     isaacsim_ci)
    ...
    num_passing = len([
        test_path for test_path in test_files
        if test_status[test_path]["result"] == "passed"
    ])
    num_failing = len([
        test_path for test_path in test_files
        if test_status[test_path]["result"] == "FAILED"
    ])
    num_timeout = len([
        test_path for test_path in test_files
        if test_status[test_path]["result"] == "TIMEOUT"
    ])
    ...
    total_time = sum(
        [test_status[test_path]["time_elapsed"] for test_path in test_files])
    ...
    per_test_result_table = PrettyTable(
        field_names=["Test Path", "Result", "Time (s)", "# Tests"])
    per_test_result_table.align["Test Path"] = "l"
    per_test_result_table.align["Time (s)"] = "r"
    for test_path in test_files:
        num_tests_passed = (test_status[test_path]["tests"] -
    ...
    pytest.exit("Custom test execution completed",
                returncode=0 if num_failing == 0 else 1)
