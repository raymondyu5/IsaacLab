...
def load_annotations(uids: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    dir_ids = (set(object_paths[uid].split("/")[1]
                   for uid in uids) if uids is not None else
               [f"{i // 1000:03d}-{i % 1000:03d}" for i in range(160)])
    if len(dir_ids) > 10:
...
def _download_object(
    ...
    if local_path is None:
        ...
        local_path = os.path.join(local_path, object_path)
    ...
    if download_processes == 1:
        ...
        for uid in uids:
            ...
            if uid not in object_paths:
                warnings.warn(
                    f"Could not find object with uid {uid}. Skipping it.")
                continue
        ...
        for uid, object_path in uids_to_download:
            ...
            uid, local_path = _download_object(uid,
                                               object_path,
        ...
        if save_dir is not None:
            args_list = [(*arg, len(args), start_file_count, save_dir)
                         for arg in args]
        else:
