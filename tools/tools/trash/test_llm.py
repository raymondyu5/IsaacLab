...
class LMPAgent:
    ...
    def __init__(self, text_prompt_path, failure_path):
        self.prompt = self.parse_text_prompt(text_prompt_path)
        self.user_prompt = self.parse_text_prompt(failure_path)
...
agent = LMPAgent(
    ...
    "source/config/llm_prompt/block_stack/prompt_blockstack_feedback.txt")
agent.prompt_llm()
