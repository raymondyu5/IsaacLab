...
class AssembledBodies:
    ...
    def disassemble(self):
        ...
        if not self.is_assembled():
            carb.log_warn(
                "Cannot disassemble a robot that has already been disassembled"
            )
            return

        RobotAssembler.move_articulation_root(
            get_prim_at_path(self._attach_path), self._articulation_root)
    ...
    def get_fixed_joint_transform(self) -> Tuple[np.array, np.array]:
        ...
        if not self.is_assembled():
            carb.log_warn(
                "Fixed joint no longer exists in composed robot.  Robots have been disassembled."
            )
            return None, None
    ...
    def set_fixed_joint_transform(self, translation: np.array,
        ...
        if not self.is_assembled():
            carb.log_warn(
                "Fixed joint no longer exists in composed robot.  Rigid Bodies have been disassembled."
            )
            return
        fixed_joint = self.fixed_joint
        fixed_joint.GetLocalPos0Attr().Set(
            Gf.Vec3f(*translation.astype(float)))
        fixed_joint.GetLocalRot0Attr().Set(
            Gf.Quatf(*orientation.astype(float)))
        ...
        body0 = str(
            fixed_joint_prim.GetProperty("physics:body0").GetTargets()[0])
        body1 = str(
            fixed_joint_prim.GetProperty("physics:body1").GetTargets()[0])

        RobotAssembler._move_obj_b_to_local_pos(body0, self.attach_path, body1,
    ...
    def _unmask_collisions(self):
        if self._collision_mask is not None:
            [
                self._collision_mask.RemoveTarget(target)
                for target in self._collision_mask.GetTargets()
            ]
    ...
    def _refresh_asset(self, prim_path):
        ...
        if len(composed_payloads) != 0:
            ...
            omni.kit.commands.execute("RemovePayload",
                                      stage=stage,
                                      prim_path=prim_path,
                                      payload=payload)
        ...
        if len(composed_refs) != 0:
            ...
            omni.kit.commands.execute("RemoveReference",
                                      stage=stage,
...
class AssembledRobot:
    ...
    def disassemble(self):
        ...
        if art_b_prim.HasProperty("physxArticulation:articulationEnabled"):
            art_b_prim.GetProperty(
                "physxArticulation:articulationEnabled").Set(True)
    ...
    def set_fixed_joint_transform(self, translation: np.array,
        ...
        """
        self.assembled_robots.set_fixed_joint_transform(
            translation, orientation)
...
class RobotAssembler:
    ...
    def mask_collisions(self, prim_path_a: str,
                        prim_path_b: str) -> Usd.Relationship:
        ...
        """
        filteringPairsAPI = UsdPhysics.FilteredPairsAPI.Apply(
            get_prim_at_path(prim_path_a))
    ...
    def assemble_rigid_bodies(
        ...
        if attach_mount_frame == "":
            attach_mount_path = attach_path + "/assembler_attach_frame"
        ...
        articulation_root = get_prim_at_path(
            get_articulation_root_api_prim_path(attach_path))

        attach_prim = get_prim_at_path(attach_path)

        self._move_obj_b_to_local_pos(base_mount_path, attach_path,
                                      attach_mount_path, fixed_joint_offset,
                                      fixed_joint_orient)
        ...
        root_joints = [
            p for p in Usd.PrimRange(attach_prim) if self.is_root_joint(p)
        ]
        ...
        fixed_joint = self.create_fixed_joint(attach_mount_path,
                                              base_mount_path,
                                              attach_mount_path,
                                              fixed_joint_offset,
                                              fixed_joint_orient)
        ...
        if mask_all_collisions:
            ...
            collision_mask = self.mask_collisions(base_path_art_root,
                                                  attach_path)
        ...
        return AssembledBodies(base_path, attach_path, fixed_joint,
                               root_joints, articulation_root, collision_mask)
    ...
    def assemble_articulations(
        ...
        if single_robot:
            ...
            if art_b_prim.HasProperty("physxArticulation:articulationEnabled"):
                art_b_prim.GetProperty(
                    "physxArticulation:articulationEnabled").Set(False)
    ...
    def create_fixed_joint(
        ...
        fixed_joint.GetLocalPos0Attr().Set(
            Gf.Vec3f(*fixed_joint_offset.astype(np.float64)))
        fixed_joint.GetLocalRot0Attr().Set(
            Gf.Quatf(*fixed_joint_orient.astype(np.float64)))
        fixed_joint.GetLocalPos1Attr().Set(
            Gf.Vec3f(*np.zeros(3).astype(np.float64)))
        fixed_joint.GetLocalRot1Attr().Set(
            Gf.Quatf(*np.array([1, 0, 0, 0]).astype(np.float64)))
    ...
    @staticmethod
    def _move_obj_b_to_local_pos(base_mount_path, attach_path,
                                 attach_mount_path, rel_offset, rel_orient):
