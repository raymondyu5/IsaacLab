...
def overwrite_python_analysis_extra_paths(isaaclab_settings: str) -> str:
    ...
    """
    # isaac-sim settings
    isaacsim_vscode_filename = os.path.join(ISAACSIM_DIR, ".vscode",
                                            "settings.json")
    ...
    if os.path.exists(isaacsim_vscode_filename):
        ...
        settings = re.search(r"\"python.analysis.extraPaths\": \[.*?\]",
                             vscode_settings,
        ...
        path_names = [
            path_name for path_name in path_names if len(path_name) > 0
        ]
...
def main():
    ...
    isaaclab_vscode_template_filename = os.path.join(PROJECT_DIR, ".vscode",
                                                     "tools",
    ...
    isaaclab_settings = overwrite_python_analysis_extra_paths(
    ...
    isaaclab_vscode_filename = os.path.join(PROJECT_DIR, ".vscode",
                                            "settings.json")
    with open(isaaclab_vscode_filename, "w") as f:
        f.write(isaaclab_settings)

    # copy the launch.json file if it doesn't exist
    isaaclab_vscode_launch_filename = os.path.join(PROJECT_DIR, ".vscode",
                                                   "launch.json")
    ...
    if not os.path.exists(isaaclab_vscode_launch_filename):
        ...
        header_message = header_message.replace(
            isaaclab_vscode_template_filename,
