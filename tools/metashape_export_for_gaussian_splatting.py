...
def matrix_to_quat(m):
    ...
    if (tr > 0):
        s = 2 * math.sqrt(tr + 1)
        return Metashape.Vector([(m[2, 1] - m[1, 2]) / s,
                                 (m[0, 2] - m[2, 0]) / s,
    ...
    if (m[1, 1] > m[2, 2]):
        ...
        return Metashape.Vector([(m[0, 1] + m[1, 0]) / s, 0.25 * s,
                                 (m[1, 2] + m[2, 1]) / s,
                                 (m[0, 2] - m[2, 0]) / s])
    else:
        s = 2 * math.sqrt(1 + m[2, 2] - m[0, 0] - m[1, 1])
        return Metashape.Vector([(m[0, 2] + m[2, 0]) / s,
...
def get_valid_calib_region(calib):
    ...
    for r in range(max_dim):
        ...
        next_top = top if top_set else math.floor(calib.cy + h / 2 -
                                                  r * step_y)
        next_bottom = bottom if bottom_set else math.floor(calib.cy + h / 2 +
                                                           r * step_y)
        next_left = left if left_set else math.floor(calib.cx + w / 2 -
                                                     r * step_x)
        next_right = right if right_set else math.floor(calib.cx + w / 2 +
                                                        r * step_x)
        ...
        for v in range(2):
            for u in range(2):
                ...
                corner = Metashape.Vector([
                    next_right if u else next_left,
                    next_bottom if v else next_top
                ])
                corner.x += 0.5
                corner.y += 0.5

                step = Metashape.Vector(
                    [step_x if u else -step_x, step_y if v else -step_y])
                ...
                prev_pt = Metashape.Vector(
                    [prev_pt.x / prev_pt.z, prev_pt.y / prev_pt.z])

                dif = pt - prev_pt

                if (pt.norm() > max_tan or dif * step <= 0
                        or not calib_valid(calib, corner)):
                    if u:
                        right_set = True
                    else:
                        left_set = True
    ...
    if (new_w != w or new_h != h):
        print("Cropped initial calibration due to high distortion",
              str(w) + "x" + str(h), "->",
...
def rotate_vector(vec, axis, angle):
    ...
    return collinear + orthogonal0 * math.cos(angle) + orthogonal1 * math.sin(
        angle)
...
def axis_magnitude_rotation(axis):
    ...
    return Metashape.Matrix(
        (rotate_vector(x, axis, -angle), rotate_vector(y, axis, -angle),
         rotate_vector(z, axis, -angle)))
...
def compute_undistorted_calib(sensor, zero_cxy):
    ...
    (reg_top, reg_right, reg_bottom,
     reg_left) = get_valid_calib_region(calib_initial)
    ...
    if zero_cxy:
        ...
        rotation_vec = Metashape.Vector([
            math.tan((left_ang + right_ang) / 2),
            math.tan((top_ang + bottom_ang) / 2), 1
        ]).normalized()
        rotation_vec = Metashape.Vector.cross(Metashape.Vector((0, 0, 1)),
                                              rotation_vec)
        T1 = Metashape.Matrix.Rotation(axis_magnitude_rotation(rotation_vec))

    (halfht, halfwr, halfhb, halfwl) = compute_size(top, right, bottom, left,
                                                    T1)
    ...
    calib.f = f
    calib.width = min(math.floor(max_dim * 1.2),
                      math.floor(halfwl + halfwr) - 2 * border)
    calib.height = min(math.floor(max_dim * 1.2),
                       math.floor(halfht + halfhb) - 2 * border)
...
def check_undistorted_calib(sensor, calib, T1):
    ...
    for i in range(h):
        pt = calib_initial.project(
            T1.mulp(calib.unproject(Metashape.Vector([0.5, i + 0.5]))))
        left = min(left, pt.x)
        pt = calib_initial.project(
            T1.mulp(calib.unproject(Metashape.Vector([w - 0.5, i + 0.5]))))
        right = max(right, pt.x)
    for i in range(w):
        pt = calib_initial.project(
            T1.mulp(calib.unproject(Metashape.Vector([i + 0.5, 0.5]))))
        top = min(top, pt.y)
        pt = calib_initial.project(
            T1.mulp(calib.unproject(Metashape.Vector([i + 0.5, h - 0.5]))))
        bottom = max(bottom, pt.y)

    print(left, right, top, bottom)
    if (left < 0.5 or calib_initial.width - 0.5 < right or top < 0.5
            or calib_initial.height - 0.5 < bottom):
...
def get_coord_transform(frame, use_localframe):
    ...
    return (
        Metashape.Matrix.Translation(-fr_to_loc.mulp(frame.region.center)) *
        fr_to_loc)


def compute_undistorted_calibs(frame, zero_cxy):
    calibs = {
    }  # { sensor_key: ( sensor, undistorted calibration, undistorted camera transform ) }
    ...
    for (s_key, (sensor, calib, T1)) in calibs.items():
        print(sensor.key, calib.f, calib.width, calib.height, calib.cx,
              calib.cy)
...
def get_calibs(camera, calibs):
    ...
    if s_key not in calibs:
        cause = "unsupported" if (
            camera.sensor.type != Metashape.Sensor.Type.Frame and
            camera.sensor.type != Metashape.Sensor.Type.Fisheye) else "cropped"
        print("Camera " + camera.label + " (key = " + str(camera.key) +
              ") has " + cause + " sensor (key = " + str(s_key) + ")")
...
def get_filtered_track_structure(frame, folder, calibs):
    ...
    for cam in frame.cameras:
        ...
        for (i, proj) in enumerate(projections):
            ...
            good = False
            if (pt is not None):
                good = (0 <= pt.x and pt.x < calib1.width and 0 <= pt.y
                        and pt.y < calib1.height)
...
def save_points(params, frame, folder, calibs, tracks, images):
    ...
    with open(folder + "sparse/0/points3D.bin", "wb") as fout:
        ...
        for (track_id, [points, good_prjs, bad_prjs]) in tracks.items():
            ...
            num = (len(good_prjs) if only_good else len(good_prjs) +
                   len(bad_prjs))
            fout.write(u64(num))
...
class ExportSceneParams():
    ...
    def log(self):
        ...
        print("Using pinhole model instead of simple_pinhole:",
              self.use_pinhole_model)
        print("Using only uncropped projections:", self.only_good)


def export_for_gaussian_splatting(params=ExportSceneParams(),
                                  progress=QtWidgets.QProgressBar()):
    log_result = lambda x: print(
        "", x, "-----------------------------------", sep="\n")
    ...
    for chunk_id, (chunk_key, chunk_folder) in enumerate(chunk_dirs.items()):
        chunk = [
            ck for ck in Metashape.app.document.chunks if ck.key == chunk_key
        ]
        if (len(chunk) != 1):
        ...
        set_progress_frame = lambda n: set_progress(prog_step *
                                                    (chunk_id + n / frame_num))
        frame_cnt = 0
        ...
        for frame_id, frame in enumerate(chunk.frames):
            ...
            folder = chunk_folder + ("" if frame_num == 1 else "frame_" +
            ...
            (tracks,
             images) = get_filtered_track_structure(frame, folder, calibs)
...
class ExportSceneGUI(QtWidgets.QDialog):
    ...
    def __init__(self, parent):
        ...
        self.gbAdvanced.toggled.connect(
            lambda: QtCore.QTimer.singleShot(20, lambda: self.adjustSize()))
        ...
        self.buttons = [
            self.btnP1, self.btnQuit, self.radioBtn_allC, self.radioBtn_selC,
            self.radioBtn_allF, self.radioBtn_selF, self.zcxyBox,
            self.locFrameBox, self.imgQualSpBox, self.expImagesBox,
            self.expMasksBox
        ]
