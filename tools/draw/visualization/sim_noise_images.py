...
data_path = "/media/ensu/data/datasets/teleop_data/render_noise"
...
for object_name in object_list:
    ...
    for episode_name in episode_list:
        ...
        for i in range(0, link_sorted_pose.shape[-1]):
            os.makedirs(os.path.join(episode_dir, "noise", f"{i:05d}"),
                        exist_ok=True)
            per_noise = link_sorted_pose[:, :, i]

            for j, noise in enumerate(per_noise):

                color = synthesize_pc.render_pose(noise,
                                                  target_link_name,
                                                  render_object=False)
                cv2.imwrite(
                    os.path.join(
                        os.path.join(episode_dir, "noise", f"{i:05d}"),
                        f"{j:05d}.png"), color[..., :3][..., ::-1])
