...
class RlGamesVecEnvWrapper(IVecEnv):
    ...
    def __init__(
        ...
        if obs_groups is None:
            ...
            if not self.unwrapped.single_observation_space.get("policy"):
                raise KeyError(
                    "Policy observation group is expected if no explicit groups is defined"
                )
        ...
        elif (not self._concate_obs_groups
    ...
    def observation_space(self) -> gym.spaces.Box | gym.spaces.Dict:
        ...
        if not self._concate_obs_groups:
            ...
            shapes = [
                space.get(group).shape for group in self._obs_groups["obs"]
            ]
    ...
    def action_space(self) -> gym.Space:
        ...
        return gym.spaces.Box(-self._clip_actions, self._clip_actions,
                              action_space.shape)
    ...
    def state_space(self) -> gym.spaces.Box | gym.spaces.Dict | None:
        ...
        if not self._concate_obs_groups:
            state_space = {
                grp: gym.spaces.Box(-clip, clip,
                                    space.get(grp).shape)
                for grp in self._obs_groups["states"]
            }
            return gym.spaces.Dict(state_space)
        else:
            shapes = [
                space.get(group).shape for group in self._obs_groups["states"]
            ]
    ...
    def step(self, actions):  # noqa: D102
        ...
        extras = {
            k:
            v.to(device=self._rl_device, non_blocking=True) if hasattr(
                v, "to") else v
    ...
    def _process_obs(
        ...
        rl_games_obs = {
            "obs": {
                group: obs_dict[group]
                for group in self._obs_groups["obs"]
            }
        }
        if len(self._obs_groups["states"]) > 0:
            rl_games_obs["states"] = {
                group: obs_dict[group]
                for group in self._obs_groups["states"]
            }

        if self._concate_obs_groups:
            rl_games_obs["obs"] = self._obs_concat_fn(
                list(rl_games_obs["obs"].values()))
            if "states" in rl_games_obs:
                rl_games_obs["states"] = self._states_concat_fn(
                    list(rl_games_obs["states"].values()))

        return rl_games_obs


def make_concat_plan(
        shapes: list[tuple[int, ...]]) -> tuple[tuple[int, ...], Callable]:
    """
    ...
    if all(len(s) == rank for s in shapes) and rank > 1:
        ...
        if all(s[1:] == shapes[0][1:] for s in shapes):
            ...
            raise ValueError(
                f"Could not find a valid concatenation plan for rank {[(len(s),) for s in shapes]}"
            )
    else:
        raise ValueError(
            "Could not find a valid concatenation plan, please make sure all value share the same size"
        )
...
class RlGamesGpuEnv(IVecEnv):
    ...
    def __init__(self, config_name: str, num_actors: int, **kwargs):
        ...
        """
        self.env: RlGamesVecEnvWrapper = env_configurations.configurations[
            config_name]["env_creator"](**kwargs)
