...
def process_sb3_cfg(cfg: dict, num_envs: int) -> dict:
    ...
    def update_dict(hyperparams: dict[str, Any], depth: int) -> dict[str, Any]:
        ...
        if "n_minibatches" in hyperparams:
            hyperparams["batch_size"] = (hyperparams.get(
                "n_steps", 2048) * num_envs) // hyperparams["n_minibatches"]
...
class Sb3VecEnvWrapper(VecEnv):
    ...
    def _process_spaces(self):
        ...
        if isinstance(observation_space, gym.spaces.Dict):
            for obs_key, obs_space in observation_space.spaces.items():
                ...
                if is_image_space(obs_space,
                    ...
                    actually_normalized = np.all(
                        obs_space.low == -1.0) and np.all(
                            obs_space.high == 1.0)
                    if not actually_normalized:
                        if np.any(obs_space.low != 0) or np.any(
                                obs_space.high != 255):
                            raise ValueError(
                        ...
                        if not is_image_space_channels_first(obs_space):
                            ...
                            def tranp(img: torch.Tensor) -> torch.Tensor:
                                return img.permute(2, 0, 1) if len(
                                    img.shape) == 3 else img.permute(
                                        0, 3, 1, 2)
                    ...
                    def chained_processor(obs: torch.Tensor,
                                          procs=processors) -> Any:
                        for proc in procs:
                            obs = proc(obs)
                        return obs
        ...
        if isinstance(action_space,
            ...
            action_space = gym.spaces.Box(low=-100,
                                          high=100,
