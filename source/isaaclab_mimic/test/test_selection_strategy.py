...
def test_select_source_demo_identity_orientations_object_strategy(
        nearest_neighbor_object_strategy):
    ...
    src_object_poses_in_world_cluster_1 = [
        torch.eye(4) +
        torch.tensor([[0.0, 0.0, 0.0, i], [0.0, 0.0, 0.0, i],
                      [0.0, 0.0, 0.0, i], [0.0, 0.0, 0.0, -1.0]])
    ...
    src_subtask_datagen_infos = [
        DatagenInfo(object_poses={0: object_pose.unsqueeze(0)})
        for object_pose in src_object_poses_in_world
    ]
    ...
    random_index_cluster_1 = np.random.randint(
        0, len(src_object_poses_in_world_cluster_1))
    cluster_1_curr_object_pose = src_object_poses_in_world_cluster_1[
        random_index_cluster_1].clone()  # Use clone to avoid reference issues
    ...
    random_index_cluster_2 = np.random.randint(
        0, len(src_object_poses_in_world_cluster_2))
    cluster_2_curr_object_pose = src_object_poses_in_world_cluster_2[
        random_index_cluster_2].clone()  # Use clone to avoid reference issues
    # Randomly adjust the current pose within the maximum deviation
    ...
    assert np.all(
        np.array(selected_indices) > (
            ...
            1)), "Some selected indices are not part of cluster 2."


def test_select_source_demo_identity_orientations_robot_distance_strategy(
    ...
    transformed_eef_pose_cluster_1 = [
        torch.eye(4) +
        torch.tensor([[0, 0, 0, i], [0, 0, 0, i], [0, 0, 0, i], [0, 0, 0, -1]])
        for i in range(cluster_1_range_min, cluster_1_range_max)
    ]
    ...
    src_obj_in_world_poses = torch.stack([
        PoseUtils.generate_random_transformation_matrix(pos_boundary=10,
                                                        rot_boundary=(2 *
                                                                      np.pi))
    ...
    # Create DatagenInfo instances for these positions
    src_subtask_datagen_infos = [
        DatagenInfo(eef_pose=src_eef_in_world_pose.unsqueeze(0),
                    object_poses={0: src_obj_in_world_pose.unsqueeze(0)})
        for src_obj_in_world_pose, src_eef_in_world_pose in zip(
            src_obj_in_world_poses, src_eef_in_world_poses)
    ]
    ...
    # Set the current object pose to the first value of cluster 1 and add some noise
    random_index_cluster_1 = np.random.randint(
        0, len(transformed_eef_pose_cluster_1))
    ...
    # Set the current object pose to the first value of cluster 2 and add some noise
    random_index_cluster_2 = np.random.randint(
        0, len(transformed_eef_pose_cluster_2))
    ...
    # Assert that all selected indices are valid indices within cluster 2
    assert np.all(
        np.array(selected_indices) < transformed_eef_in_world_poses_tensor.
        shape[0]), "Some selected indices are not part of cluster 2."
    assert np.all(
        np.array(selected_indices) > (
