...
class DataGenInfoPool:
    ...
    def __init__(self,
        ...
        for eef_name, eef_subtask_configs in env_cfg.subtask_configs.items():
            ...
            self.subtask_start_offset_ranges[eef_name] = [
                subtask_config.subtask_start_offset_range
                for subtask_config in eef_subtask_configs
            ]
    ...
    def _add_episode(self, episode: EpisodeData):
        ...
        if "datagen_info" in ep_grp["obs"]:
            ...
            subtask_start_signals_dict = ep_grp["obs"]["datagen_info"].get(
        ...
        gripper_actions = self.env.actions_to_gripper_actions(
            ep_grp["actions"])
        ...
        for eef_name in self.subtask_term_signal_names.keys():
            ...
            for eef_subtask_index, eef_subtask_signal_name in enumerate(
                    self.subtask_term_signal_names[eef_name]):
                if self.env_cfg.datagen_config.use_skillgen:
                    ...
                    subtask_start_indicators = (
                        ep_datagen_info_obj.subtask_start_signals[
                            eef_subtask_signal_name].flatten().int())
                    # Compute the difference between consecutive elements to find the transition point.
                    diffs = subtask_start_indicators[
                        1:] - subtask_start_indicators[:-1]
                ...
                if eef_subtask_index == len(
                        self.subtask_term_signal_names[eef_name]) - 1:
                    ...
                    subtask_term_indicators = (
                        ep_datagen_info_obj.subtask_term_signals[
                            eef_subtask_signal_name].flatten().int())
                    diffs = subtask_term_indicators[
                        1:] - subtask_term_indicators[:-1]
            ...
            if self.env_cfg.datagen_config.use_skillgen:
                ...
                for i in range(len(eef_subtask_boundaries)):
                    ...
                    assert (eef_subtask_boundaries[i][0] +
                            self.subtask_start_offset_ranges[eef_name][i][1]
                    ...
                    assert (
                        eef_subtask_boundaries[i][1] +
                        self.subtask_term_offset_ranges[eef_name][i][1]
                        < eef_subtask_boundaries[i + 1][0] +
                        self.subtask_start_offset_ranges[eef_name][i + 1][0]
                ...
                for i in range(1, len(eef_subtask_boundaries)):
                    prev_max_offset_range = self.subtask_term_offset_ranges[
                        eef_name][i - 1][1]
                    # Make sure that subtasks are not overlapped with the largest offsets
                    assert (
                        eef_subtask_boundaries[i - 1][1] +
                        prev_max_offset_range < eef_subtask_boundaries[i][1] +
                        self.subtask_term_offset_ranges[eef_name][i][0]
