...
def update_nested_value(d: dict[str, Any], keys: list[str],
                        value: Any) -> None:
...
def get_parameter_input(
    ...
    if isinstance(current_val, (tuple, list)):
        ...
        help_text = widgets.HTML(
            value=
            f'<p style="color:gray">Allowed range: {allowed_range[:2]}</p>')
...
def interactive_update_randomizable_params(
    ...
    for key, allowed_range in param_config.items():
        ...
        if isinstance(allowed_range, dict):
            interactive_update_randomizable_params(event_term, event_term_name,
                                                   allowed_range, current_path,
                                                   env)
        else:
            try:
                ...
                print(
                    f"Key '{current_path}' not found in event_term.params; skipping."
                )
                continue
...
def get_env_name_from_dataset(input_file_path: str) -> str:
    ...
    if not os.path.exists(input_file_path):
        raise FileNotFoundError(
            f"The dataset file {input_file_path} does not exist.")
