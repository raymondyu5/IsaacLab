    ...
    def __init__(self, xr: bool):
        ...
        if self.xr:
            ...
            self._display_subtask = lambda text: show_instruction(
                ...
                "/_xr/stage/xrCamera",
                Gf.Vec3f(1.25, 0.3, -2),
                target_prim_path="/subtask_instruction")
            self._display_demo = lambda text: show_instruction(
                text,
                "/_xr/stage/xrCamera",
                Gf.Vec3f(-1.25, 0.3, -2),
                target_prim_path="/demo_complete")
        else:
            self.subtask_label = None
            self.demo_label = None
            self._display_subtask = lambda text: setattr(
                self.subtask_label, "text", text)
            self._display_demo = lambda text: setattr(self.demo_label, "text",
                                                      text)
...
def show_subtask_instructions(instruction_display: InstructionDisplay,
                              prev_subtasks: dict, obv: dict,
                              env_cfg: Any) -> None:
    """
    ...
    for subtask_config in subtask_configs:
        ...
        if current_state and not prev_state:
            instruction_display.show_subtask(
                f"Next objective: {subtask_config.next_subtask_description}")
    ...
    if all_false and subtask_configs:
        ...
        instruction_display.show_subtask(
            f"Current objective: {first_task.description}")
