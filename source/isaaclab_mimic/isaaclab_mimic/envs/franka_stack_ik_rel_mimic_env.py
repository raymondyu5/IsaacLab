...
class FrankaCubeStackIKRelMimicEnv(ManagerBasedRLMimicEnv):
    ...
    def get_robot_eef_pose(
        ...
        return PoseUtils.make_pose(eef_pos,
                                   PoseUtils.matrix_from_quat(eef_quat))
    ...
    def action_to_target_eef_pose(
        ...
        delta_rotation_angle = torch.linalg.norm(delta_rotation,
                                                 dim=-1,
                                                 keepdim=True)
        ...
        is_close_to_zero_angle = torch.isclose(
            ...
            torch.zeros_like(delta_rotation_angle)).squeeze(1)
        delta_rotation_axis[is_close_to_zero_angle] = torch.zeros_like(
            delta_rotation_axis)[is_close_to_zero_angle]

        delta_quat = PoseUtils.quat_from_angle_axis(
            delta_rotation_angle.squeeze(1), delta_rotation_axis).squeeze(0)
    ...
    def get_expected_attached_object(self, eef_name: str, subtask_index: int,
                                     env_cfg) -> str | None:
        """
