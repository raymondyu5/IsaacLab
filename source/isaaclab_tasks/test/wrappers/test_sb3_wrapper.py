        ...
        for task_spec in gym.registry.values():
            if "Isaac" in task_spec.id:
                cfg_entry_point = gym.spec(
                    task_spec.id).kwargs.get("sb3_cfg_entry_point")
            ...
            with self.subTest(task_name=task_name):
                ...
                with torch.inference_mode():
                    for _ in range(100):
                        ...
                        actions = 2 * np.random.rand(
                            env.num_envs, *env.action_space.shape) - 1
                        # apply actions
                        transition = env.step(actions)
                        # check signals
                        for data in transition:
                            self.assertTrue(self._check_valid_array(data),
        ...
        if isinstance(data, np.ndarray):
            ...
            for value in data.values():
                if isinstance(value, dict):
                    valid_array &= TestStableBaselines3VecEnvWrapper._check_valid_array(
                        value)
