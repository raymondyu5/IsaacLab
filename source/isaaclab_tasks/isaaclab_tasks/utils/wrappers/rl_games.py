...
class RlGamesVecEnvWrapper(IVecEnv):
    ...
    def __init__(self, env: ManagerBasedRLEnv | DirectRLEnv, rl_device: str,
                 clip_obs: float, clip_actions: float):
    ...
    def observation_space(self) -> gym.spaces.Box:
        ...
        return gym.spaces.Box(-self._clip_obs, self._clip_obs,
                              policy_obs_space.shape)
    ...
    def state_space(self) -> gym.spaces.Box | None:
        """Returns the :attr:`Env` :attr:`observation_space`."""
        # note: rl-games only wants single observation space
        critic_obs_space = self.unwrapped.single_observation_space.get(
            "critic")
    ...
    def _process_obs(
            self,
            obs_dict: VecEnvObs) -> torch.Tensor | dict[str, torch.Tensor]:
        ...
        if self.rlg_num_states > 0:
            ...
            try:
                ...
                raise NotImplementedError(
                    "Environment does not define key 'critic' for privileged observations."
                )
            # clip the states
