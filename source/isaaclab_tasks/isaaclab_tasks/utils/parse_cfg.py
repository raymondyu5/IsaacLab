...
def load_cfg_from_registry(task_name: str,
                           entry_point_key: str,
                           config_yaml=None) -> dict | ManagerBasedRLEnvCfg:
    ...
    cfg_entry_point = gym.spec(
        task_name.split(":")[-1]).kwargs.get(entry_point_key)
    ...
    if cfg_entry_point is None:
        ...
        for k in gym.spec(task_name.split(":")[-1]).kwargs:
            if k.endswith("_cfg_entry_point") and k != "env_cfg_entry_point":
                spec = (k.replace("_cfg_entry_point", "").replace(
                    "rl_games", "rl-games").replace("rsl_rl",
                                                    "rsl-rl").split("_"))
                agent = spec[0].replace("-", "_")
                algorithms = [
                    item.upper()
                    for item in (spec[1:] if len(spec) > 1 else ["PPO"])
                ]
        ...
        if os.path.exists(cfg_entry_point):
            ...
            if "class" in mod_name:
                ...
                mod_path = os.path.dirname(
                    importlib.import_module(mod_name).__file__)
                # obtain the configuration file path
                config_file = os.path.join(mod_path, config_yaml)
    ...
    #     mod_name = cfg_entry_point
    #     if "class" in mod_name:
    #         class_path = mod_name.split("'")[1]
    #         module_path, class_name = class_path.rsplit('.', 1)
    #         module = importlib.import_module(module_path)
    #         cfg_entry_point = getattr(module, class_name)
    #         cfg_cls = cfg_entry_point(config_yaml)
...
def parse_env_cfg(
        ...
        config_yaml: str | None = None) -> dict | ManagerBasedRLEnvCfg:
    ...
    cfg = load_cfg_from_registry(
        task_name.split(":")[-1], "env_cfg_entry_point", config_yaml)

    # load the default configuration

    cfg = load_cfg_from_registry(task_name, "env_cfg_entry_point", config_yaml)
...
def get_checkpoint_path(log_path: str,
                        run_dir: str = ".*",
                        checkpoint: str = ".*",
                        other_dirs: list[str] = None,
                        sort_alpha: bool = True) -> str:
    ...
    try:
        ...
        runs = [
            os.path.join(log_path, run) for run in os.scandir(log_path)
    ...
    model_checkpoints = [
        f for f in os.listdir(run_path) if re.match(checkpoint, f)
    ]
