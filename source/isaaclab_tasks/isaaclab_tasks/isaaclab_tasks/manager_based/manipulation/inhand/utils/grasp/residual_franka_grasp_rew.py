...
class ResidualFrankaGraspRew:
    ...
    def init_env(self, env_cfg, scene_cfg, rewards_cfg, hand_side, num_envs,
        ...
        SingleHandGraspRew(env_cfg, scene_cfg, rewards_cfg,
                           f"{hand_side}_hand_object", hand_side,
                           target_manipulated_object_pose, init_height,
                           events_cfg)


class SingleHandGraspRew:

    def __init__(self, env_cfg, scene_cfg, rewards_cfg, object_name, hand_side,
                 target_manipulated_object_pose, init_height, events_cfg):
        self.env_cfg = env_cfg

        self.scene_cfg = scene_cfg
        self.hand_side = hand_side
        self.object_name = object_name
        self.target_manipulated_object_pose = target_manipulated_object_pose
        ...
        self.init_robot_pose = torch.as_tensor(
            self.env_cfg["params"][f"{hand_side}_robot_pose"]).to(self.device)
    ...
    #     cfg = env.event_manager.get_term_cfg("reset_target_pose")
    #     owner_obj = cfg.func.__self__  # the object owning the method
    #     self.target_manipulated_object_pose = owner_obj.target_state  # access attribute from the object
    ...
    def setup_reward(self):
        ...
        grasp_rewards = RewTerm(
            ...
            weight=self.env_cfg["params"].get("reward_scale", 4.0),
        )
    ...
    def grasp_rewards(self, env):
        ...
        final_rewards = rewards_finger2object * 0.1 + rewards03_contact * 0.1 + rewards04_lift * 0.3 + rewards04_link6
    ...
    def liftobject_rewards(self, env):
        ...
        lift_or_not = ((self.object_pose[:, 2] - self.init_height)
                       >= 0.05).int()
        # scale_topple = self.lift_reward_scale * 20 + 1
        # topple_reward = torch.clip(
        #     torch.clip(still_or_not.int() * 2 - 1, -1, 0.2) * scale_topple, -1,
        #     1) * (
        #         (self.object_pose[:, 2] - self.init_height) >= 0.05).int() * 3
        topple_reward = torch.clip(1 - abs(self.target_in_object_angle) / 0.05,
                                   -1, 1) * lift_or_not * 3

        # thumb_contact = self.contact_or_not[:, 0].int()
        # thumb_lift_reward = (
        #     self.object_pose[:, 2] - self.init_height >= 0.04
        # ).int(
        # ) * thumb_contact * 1.0  # give reward for thumb contact for lifting
    ...
    def object2fingercontact_rewards(self, env):
        ...
        self.contact_or_not = self.contact_or_not.to(torch.float32)

        self.contact_or_not[:, 0] *= 4.0
        self.contact_or_not[:, 1:-1] *= 0.25
        self.contact_or_not[:, -1] *= 0.25

        return torch.sum(self.contact_or_not,
                         dim=1) * 1 + self.finger_rewards_scale * 1.0
    ...
    def finger2object_rewards(self, env):
        ...
        reward = torch.sum(reward, dim=1) / len(self.fingers_name_list) * 1.0
    ...
    def get_finger_info(self, env):
        ...
        for name in self.fingers_name_list:
            ...
            finger_pose[:, :3] -= self.init_robot_pose[..., :3]
    ...
    def get_target_object_pose(self, env, target_pose=None):
        try:  # TODO:reset target pose in env
            self.target_manipulated_object_pose = env.scene[
                self.object_name].data.target_state.clone()

        except:
            pass
        return self.target_manipulated_object_pose.clone()

    def get_object_info(self, env):

        self.object_pose = env.scene[
            self.object_name]._data.root_state_w[:, :7].clone()
        self.target_manipulated_object_pose = self.get_target_object_pose(env)
        self.object_pose[:, :3] -= self.init_robot_pose[..., :3]
