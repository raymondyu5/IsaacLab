...
class SingleHandGraspRew:
    ...
    def __init__(self, env_cfg, scene_cfg, rewards_cfg, object_name, hand_side,
        ...
        self.reset_init_height = init_height.clone()
    ...
    def grasp_rewards(self, env):
        ...
        final_rewards = rewards_finger2object * 0.30 + rewards03_contact_scaled + rewards04_lift * 1.0 + rewards04_link6 - joint_vel_penalty * 1.0e-3 - joint_limit_penalty * 6.0e-1 - action_rate_penalty * 5e-3  #+ reward05_ori_penalty * 0.1
    ...
    def liftobject_rewards(self, env):
        lift_reward = torch.clip(
            self.object_pose[:, 2] - self.reset_init_height, -0.1, 0.3) / 0.3
    ...
    def get_object_info(self, env):
        ...
        # delta_quat = math_utils.quat_mul(
        #     target_pose[:, 3:7].clone(),
        #     math_utils.quat_inv(env.scene[
        #         self.object_name].data.reset_root_state[..., 3:7].clone()))

        detla_euler = math_utils.euler_xyz_from_quat(delta_quat)
        # print(
        #     math_utils.euler_xyz_from_quat(delta_quat)[2],
        #     math_utils.euler_xyz_from_quat(delta_quat2)[2])
        delta_xy_rotation = torch.cat([
