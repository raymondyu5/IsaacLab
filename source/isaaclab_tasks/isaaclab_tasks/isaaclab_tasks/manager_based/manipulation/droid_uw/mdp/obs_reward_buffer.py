...
class RewardObsBuffer:
    ...
    def init_setting(self):
        self.success_condition = self.env_cfg["params"]["Task"][
            "success_condition"]
        self.placement_height = self.env_cfg["params"]["Task"]["placement"][
            "placement_height"]
        ...
        setattr(
            self.obs_target, "target_pos_dist",
            ObsTerm(
                func=self.target_pos_dist,
                params={
                    "taget_object_name": self.target_object_name,
                    "placement_object_name": self.placement_object_name,
                },
            ))

    def target_pos_dist(self, env, taget_object_name, placement_object_name):
        target_object_state = self.get_root_link_state(env, taget_object_name)
        placement_object_state = self.get_root_link_state(
            env, placement_object_name)

        target_pos_dist = target_object_state[:, :
                                              3] - placement_object_state[:, :3]
    ...
    def judge_object_inside_place_region(self, env):

        object_state = self.get_root_link_state(env, self.target_object_name)
        ...
        object_inside_place_region = (
            ...
            (object_state[:, 0] < placement_state[:, 0] +
             self.success_condition["bbox_region"][0]))
        return object_inside_place_region

    def object_inside_place_region(self, env, object_name: str):
        if env.episode_length_buf[
                0] < 100:  #self.env_cfg["params"]["Task"]["reset_horizon"] - 1:

            return torch.zeros(env.num_envs).to(env.device)

        object_inside_place_region = self.judge_object_inside_place_region(env)
        ...
        return torch.where(reward_condition, 8, 0) / env.step_dt + torch.where(
            object_inside_place_region, 0.5, 0) / env.step_dt

    def init_object_pos(self, env, object_name):
        object = env.scene[object_name]

        object_pos_w = object.data.root_link_state_w[..., :3].clone()
    ...
    def target_object_pos(self, env, object_name):
        try:
            if env.episode_length_buf[
                ...
                self.target_object_pose = self.get_root_link_state(
                    env, object_name)[:, :3]
                self.target_object_pose[:, 2] += 0.12
        except:
            if self.target_object_pose is None:
                self.target_object_pose = torch.zeros(
                    (env.num_envs, 3)).to(env.device)

        return self.target_object_pose
    ...
    def get_root_link_state(self, env, object_name):
        ...
        if object_name == self.placement_object_name:
            ...
            object_pose[:,
                        2] = self.init_object_height + 0.10 + object_pose[:,
                                                                          2] * 2
    ...
    def object_ee_distance(self,
        ...
        object_ee_dist_reward = torch.clip(
            (1 / torch.clip(object_ee_distance, 0.01, 10) -
             1 / self.init_ee_object_dist) * scaling_factor, -0.0, 20)
        ...
        object_inside_place_region = self.judge_object_inside_place_region(env)
        rewards = torch.where(object_inside_place_region, 20 * 1 / env.step_dt,
                              rewards)
    ...
    def object_is_lifted(self, env, object_name: str):
        ...
        if env.episode_length_buf[
            ...
            self.init_object_to_lift_dist = torch.linalg.norm(
                pick_object_state[:, :2] - target_pose[:, :2] -
                env.scene.env_origins[:, :2],
                dim=1)
            self.init_object_to_lift_height = target_pose[:,
                                                          2] - pick_object_state[:,
                                                                                 2]
        ...
        cur_object_to_lift_dist = torch.linalg.norm(
            ...
            dim=1)
        object_inside_place_region = self.judge_object_inside_place_region(env)
        lift_ratio = torch.clip(
            (pick_object_state[:, 2] - self.init_object_height) /
            target_pose[:, 2], 0.0, 1.0)

        lift_reward = lift_ratio * 10

        target_dist_factor = torch.where(lift_ratio > 0.5, 5, 1)

        target_pose_ratio = (
            self.init_object_to_lift_dist -
            cur_object_to_lift_dist) / self.init_object_to_lift_dist
        target_dist_factor = torch.where(
            target_pose_ratio < 0, 2,
            target_dist_factor)  # penalty for moving away from the target

        target_reward = torch.clip(target_pose_ratio, -0.5,
                                   1) * target_dist_factor
        target_reward = torch.where(object_inside_place_region, 5,
                                    target_reward)

        # negative_delta_height_index = torch.where(
        #     (self.init_object_to_lift_height -
        #      ((pick_object_state[:, 2] - self.init_object_height))) < 0.0,
        #     True, False)
        # lift_reward[negative_delta_height_index] = -2
        lift_reward = torch.where(object_inside_place_region, 10, lift_reward)
        ...
        return target_reward / env.step_dt + bonus_reward / env.step_dt + lift_reward / env.step_dt
    ...
    def object_in_between_gripper(self,
        ...
        reward = (in_between_reward + ation_penalty)[0]
        object_inside_place_region = self.judge_object_inside_place_region(env)
        reward = torch.where(object_inside_place_region, 3, reward)
        return reward
    ...
    def gripper_in_between_penalty(self, env, in_between_gripper):
        ...
        action_penalty = torch.where(
            ...
            torch.clip(-2 * torch.sign(actions + 0.02), -0.0, 1) * 2.0,
