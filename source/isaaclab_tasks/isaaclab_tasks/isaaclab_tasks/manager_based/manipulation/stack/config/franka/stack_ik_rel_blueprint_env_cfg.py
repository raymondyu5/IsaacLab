...
def image(
    ...
    if save_image_to_file:
        ...
        total_successes = 0
        if hasattr(env,
                   "recorder_manager") and env.recorder_manager is not None:
            total_successes = env.recorder_manager.exported_successful_episode_count

        for tile in range(images.shape[0]):
            tile_chw = torch.swapaxes(images[tile:tile + 1].unsqueeze(1), 1,
                                      -1).squeeze(-1)
            filename = (
...
@configclass
class FrankaCubeStackBlueprintEnvCfg(
        stack_joint_pos_env_cfg.FrankaCubeStackEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.scene.table_cam = CameraCfg(
            ...
            offset=CameraCfg.OffsetCfg(pos=(1.0, 0.0, 0.33),
        ...
        self.scene.table_high_cam = CameraCfg(
            ...
            spawn=sim_utils.PinholeCameraCfg(focal_length=24.0,
                                             focus_distance=400.0,
                                             horizontal_aperture=20.955,
                                             clipping_range=(1.5, 1.0e5)),
            offset=CameraCfg.OffsetCfg(pos=(1.4, 1.8, 1.2),
