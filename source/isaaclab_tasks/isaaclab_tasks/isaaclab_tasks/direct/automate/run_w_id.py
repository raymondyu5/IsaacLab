...
def update_task_param(task_cfg, assembly_id, if_sbc, if_log_eval):
    ...
    if_log_eval_pattern = re.compile(
        r"^(.*if_logging_eval\s*:\s*bool\s*=\s*).*$")
    ...
    for line in lines:
        if "assembly_id =" in line:
            ...
            line = eval_file_pattern.sub(
                r"\1'{}'".format(f"evaluation_{assembly_id}.h5"), line)
...
def main():
    ...
    parser.add_argument("--assembly_id",
                        type=str,
                        help="New assembly ID to set.")
    ...
    parser.add_argument("--log_eval",
                        action="store_true",
                        help="Log evaluation results.")
    ...
    parser.add_argument("--max_iterations",
                        type=int,
                        default=1500,
                        help="Number of iteration for policy learning.")
    args = parser.parse_args()

    update_task_param(args.cfg_path, args.assembly_id, args.train,
                      args.log_eval)
