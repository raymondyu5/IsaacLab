...
class HumanoidAmpEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        # load motion
        self._motion_loader = MotionLoader(motion_file=self.cfg.motion_file,
                                           device=self.device)

        # DOF and key body indexes
        key_body_names = ["right_hand", "left_hand", "right_foot", "left_foot"]
        self.ref_body_index = self.robot.data.body_names.index(
        ...
        self.amp_observation_buffer = torch.zeros(
            (self.num_envs, self.cfg.num_amp_observations,
    ...
    def _get_observations(self) -> dict:
        ...
        for i in reversed(range(self.cfg.num_amp_observations - 1)):
            self.amp_observation_buffer[:,
                                        i + 1] = self.amp_observation_buffer[:,
                                                                             i]
        # build AMP observation
        self.amp_observation_buffer[:, 0] = obs.clone()
        self.extras = {
            "amp_obs":
            self.amp_observation_buffer.view(-1, self.amp_observation_size)
        }
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.cfg.early_termination:
            died = self.robot.data.body_pos_w[:, self.ref_body_index,
                                              2] < self.cfg.termination_height
        else:
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        if self.cfg.reset_strategy == "default":
            root_state, joint_pos, joint_vel = self._reset_strategy_default(
                env_ids)
            ...
            raise ValueError(
                f"Unknown reset strategy: {self.cfg.reset_strategy}")
    ...
    def _reset_strategy_random(
        ...
        times = np.zeros(
            num_samples) if start else self._motion_loader.sample_times(
                num_samples)
        ...
        root_state[:, 0:
                   3] = body_positions[:,
                                       motion_torso_index] + self.scene.env_origins[
                                           env_ids]
        root_state[:,
                   2] += 0.15  # lift the humanoid slightly to avoid collisions with the ground
    ...
    def collect_reference_motions(
        ...
        times = (np.expand_dims(current_times, axis=-1) -
                 ...
                 np.arange(0, self.cfg.num_amp_observations)).flatten()
        # get motions
        (
            dof_positions,
            dof_velocities,
