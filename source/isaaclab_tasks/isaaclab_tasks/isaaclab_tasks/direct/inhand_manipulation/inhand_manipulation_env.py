...
class InHandManipulationEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        self.hand_dof_targets = torch.zeros(
            ...
            dtype=torch.float,
            device=self.device)
        self.prev_targets = torch.zeros((self.num_envs, self.num_hand_dofs),
                                        dtype=torch.float,
                                        device=self.device)
        self.cur_targets = torch.zeros((self.num_envs, self.num_hand_dofs),
                                       dtype=torch.float,
                                       device=self.device)
        ...
        # joint limits
        joint_pos_limits = self.hand.root_physx_view.get_dof_limits().to(
            self.device)
        self.hand_dof_lower_limits = joint_pos_limits[..., 0]
        self.hand_dof_upper_limits = joint_pos_limits[..., 1]

        # track goal resets
        self.reset_goal_buf = torch.zeros(self.num_envs,
                                          dtype=torch.bool,
                                          device=self.device)
        ...
        self.goal_pos = torch.zeros((self.num_envs, 3),
                                    dtype=torch.float,
                                    device=self.device)
        self.goal_pos[:, :] = torch.tensor([-0.2, -0.45, 0.68],
                                           device=self.device)
    ...
    def _apply_action(self) -> None:
        ...
        self.prev_targets[:, self.
                          actuated_dof_indices] = self.cur_targets[:, self.
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        self.extras["log"][
            "consecutive_successes"] = self.consecutive_successes.mean()
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        delta_max = self.hand_dof_upper_limits[
            env_ids] - self.hand.data.default_joint_pos[env_ids]
        ...
        dof_pos = self.hand.data.default_joint_pos[
            env_ids] + self.cfg.reset_dof_pos_noise * rand_delta

        dof_vel_noise = sample_uniform(-1.0,
                                       1.0, (len(env_ids), self.num_hand_dofs),
                                       device=self.device)
        dof_vel = self.hand.data.default_joint_vel[
    ...
    def _compute_intermediate_values(self):
        ...
        self.fingertip_velocities = self.hand.data.body_vel_w[:, self.
                                                              finger_bodies]
    ...
    def compute_reduced_observations(self):
        ...
        obs = torch.cat(
            (
                self.fingertip_pos.view(self.num_envs,
                                        self.num_fingertips * 3),
                self.object_pos,
    ...
    def compute_full_observations(self):
        obs = torch.cat(
            (
                # hand
                unscale(self.hand_dof_pos, self.hand_dof_lower_limits,
                ...
                self.fingertip_velocities.view(self.num_envs,
    ...
    def compute_full_state(self):
        states = torch.cat(
            (
                ...
                self.fingertip_force_sensors.view(self.num_envs,
                                                  self.num_fingertips * 6),
                # actions
                self.actions,
            ),
...
def rotation_distance(object_rot, target_rot):
    ...
    return 2.0 * torch.asin(
        torch.clamp(torch.norm(quat_diff[:, 1:4], p=2, dim=-1),
                    max=1.0))  # changed quat convention
...
def compute_rewards(
    ...
    resets = torch.where(goal_dist >= fall_dist, torch.ones_like(reset_buf),
                         reset_buf)
