...
def set_default_joint_pose(
    ...
    asset.data.default_joint_pos = torch.tensor(default_pose,
                                                device=env.device).repeat(
                                                    env.num_envs, 1)
...
def randomize_joint_by_gaussian_offset(
    ...
    joint_pos += math_utils.sample_gaussian(mean, std, joint_pos.shape,
                                            joint_pos.device)
...
def randomize_scene_lighting_domelight(
    ...
    if env.cfg.eval_type in ["light_color", "all"]:
        ...
        new_color = sample_random_color(base=default_color,
...
def sample_object_poses(
    ...
    for i in range(num_objects):
        for j in range(max_sample_tries):
            sample = [
                random.uniform(range[0], range[1]) for range in range_list
            ]
    ...
    for cur_env in env_ids.tolist():
        ...
        for asset_idx in range(len(asset_cfgs)):
            ...
            object_states = torch.stack(
                [out_focus_state] * asset.num_objects).to(device=env.device)
            pose_tensor = torch.tensor([pose_list[asset_idx]],
                                       device=env.device)
            positions = pose_tensor[:, 0:3] + env.scene.env_origins[cur_env,
                                                                    0:3]
            orientations = math_utils.quat_from_euler_xyz(
            ...
            asset.write_object_state_to_sim(object_state=object_states,
                                            env_ids=torch.tensor(
                                                [cur_env], device=env.device))
...
def randomize_visual_texture_material(
    ...
    """
    if hasattr(env.cfg,
               "eval_mode") and (not env.cfg.eval_mode or env.cfg.eval_type
                                 not in [f"{asset_cfg.name}_texture", "all"]):
        return
    ...
    if not hasattr(asset, "cfg"):
        prims_group = rep.get.prims(
            path_pattern=f"{asset.prim_paths[0]}/visuals")
    else:
        prims_group = rep.get.prims(
