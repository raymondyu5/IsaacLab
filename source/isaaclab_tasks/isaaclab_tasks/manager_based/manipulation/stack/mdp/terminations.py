...
def cubes_stacked(
    ...
    stacked = torch.logical_and(xy_dist_c12 < xy_threshold, xy_dist_c23
                                < xy_threshold)
    stacked = torch.logical_and(h_dist_c12 - height_diff < height_threshold,
                                stacked)
    stacked = torch.logical_and(pos_diff_c12[:, 2] < 0.0, stacked)
    stacked = torch.logical_and(h_dist_c23 - height_diff < height_threshold,
                                stacked)
    ...
    if hasattr(env.scene, "surface_grippers") and len(
        ...
        if hasattr(env.cfg, "gripper_joint_names"):
            ...
            assert len(
                gripper_joint_ids
            ) == 2, "Terminations only support parallel gripper for now"
