...
def cube_positions_in_world_frame(
    ...
    return torch.cat((cube_1.data.root_pos_w, cube_2.data.root_pos_w,
...
def instance_randomize_cube_positions_in_world_frame(
    ...
    for env_id in range(env.num_envs):
        cube_1_pos_w.append(cube_1.data.object_pos_w[
            env_id, env.rigid_objects_in_focus[env_id][0], :3])
        cube_2_pos_w.append(cube_2.data.object_pos_w[
            env_id, env.rigid_objects_in_focus[env_id][1], :3])
        cube_3_pos_w.append(cube_3.data.object_pos_w[
            env_id, env.rigid_objects_in_focus[env_id][2], :3])
...
def cube_orientations_in_world_frame(
    ...
    return torch.cat((cube_1.data.root_quat_w, cube_2.data.root_quat_w,
                      cube_3.data.root_quat_w),
                     dim=1)
...
def instance_randomize_cube_orientations_in_world_frame(
    ...
    cube_3_quat_w = []
    for env_id in range(env.num_envs):
        cube_1_quat_w.append(cube_1.data.object_quat_w[
            env_id, env.rigid_objects_in_focus[env_id][0], :4])
        cube_2_quat_w.append(cube_2.data.object_quat_w[
            env_id, env.rigid_objects_in_focus[env_id][1], :4])
        cube_3_quat_w.append(cube_3.data.object_quat_w[
            env_id, env.rigid_objects_in_focus[env_id][2], :4])
...
def ee_frame_pos(
    ...
    ee_frame: FrameTransformer = env.scene[ee_frame_cfg.name]
    ee_frame_pos = ee_frame.data.target_pos_w[:,
                                              0, :] - env.scene.env_origins[:,
                                                                            0:3]
...
def gripper_pos(
    ...
    if hasattr(env.scene, "surface_grippers") and len(
            env.scene.surface_grippers) > 0:
        ...
        if hasattr(env.cfg, "gripper_joint_names"):
            ...
            assert len(
                gripper_joint_ids
            ) == 2, "Observation gripper_pos only support parallel gripper for now"
            finger_joint_1 = robot.data.joint_pos[:,
                                                  gripper_joint_ids[0]].clone(
                                                  ).unsqueeze(1)
            finger_joint_2 = -1 * robot.data.joint_pos[:, gripper_joint_ids[
                1]].clone().unsqueeze(1)
...
def object_grasped(
    ...
    if hasattr(env.scene, "surface_grippers") and len(
        ...
        suction_cup_status = surface_gripper.state.view(
            -1, 1)  # 1: closed, 0: closing, -1: open
        suction_cup_is_closed = (suction_cup_status == 1).to(torch.float32)
        grasped = torch.logical_and(suction_cup_is_closed, pose_diff
                                    < diff_threshold)
        ...
        if hasattr(env.cfg, "gripper_joint_names"):
            ...
            assert len(
                gripper_joint_ids
            ) == 2, "Observations only support parallel gripper for now"

            grasped = torch.logical_and(
                pose_diff < diff_threshold,
                torch.abs(robot.data.joint_pos[:, gripper_joint_ids[0]] -
                          torch.tensor(env.cfg.gripper_open_val,
...
def object_stacked(
    ...
    stacked = torch.logical_and(xy_dist < xy_threshold,
    ...
    if hasattr(env.scene, "surface_grippers") and len(
        ...
        if hasattr(env.cfg, "gripper_joint_names"):
            ...
            raise ValueError(
                "No gripper_joint_names found in environment config")
