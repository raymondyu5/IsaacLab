    ...
    * If the operation is "scale", the values are the scaling factors for the joint limits, i.e. the joint range
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        for joint_name, joint_range in cfg.params["position_range"].items():
            ...
            if use_default_offset:
                self._pos_ranges[joint_ids] += default_joint_pos[
                    joint_ids].unsqueeze(1)

        # store the joint pos ids (used later to sample the joint positions)
        self._pos_joint_ids = torch.tensor(pos_joint_ids,
                                           device=self._pos_ranges.device)
        ...
        for joint_name, joint_range in cfg.params["velocity_range"].items():
            ...
            if operation == "abs":
                ...
                if joint_range[0] is not None:
                    self._vel_ranges[
                        joint_ids,
                        0] = joint_range[0] * self._vel_ranges[joint_ids, 0]
                if joint_range[1] is not None:
                    self._vel_ranges[
                        joint_ids,
                        1] = joint_range[1] * self._vel_ranges[joint_ids, 1]
            else:
                raise ValueError(
            ...
            # add the default offset
            if use_default_offset:
                self._vel_ranges[joint_ids] += default_joint_vel[
                    joint_ids].unsqueeze(1)

        # store the joint vel ids (used later to sample the joint positions)
        self._vel_joint_ids = torch.tensor(vel_joint_ids,
                                           device=self._vel_ranges.device)
    ...
    def __call__(
        ...
        if len(self._pos_joint_ids) > 0:
            ...
            joint_pos = joint_pos.clamp(joint_pos_limits[:, 0],
                                        joint_pos_limits[:, 1])
        ...
        self._asset.write_joint_state_to_sim(joint_pos,
                                             joint_vel,
                                             env_ids=env_ids)
