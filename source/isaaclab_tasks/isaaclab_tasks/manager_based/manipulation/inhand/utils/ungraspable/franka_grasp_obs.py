...
class FrankaGraspObs:
    ...
    def __init__(
        ...
        self.num_object = len(list(env_cfg["params"]["RigidObject"].keys()))
        for index, object_name in enumerate(
                list(env_cfg["params"]["RigidObject"].keys())):
        ...
        self.spawn_object_mesh = env_cfg["params"]["spawn_rigid_objects"][
            "init"]
        if self.spawn_object_mesh:
            self.init_object_mesh()
    ...
    def setup_obs(self):
        ...
        setattr(self.obs_cfg, "manipulated_object_pose",
    ...
    def get_object_points(self, env):
        ...
        transformed_points = math_utils.transform_points(
            init_object_points, self.object_pose[:, :3], self.object_pose[:,
                                                                          3:7])
