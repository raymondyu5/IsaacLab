...
class SingleHandGraspObs:
    ...
    def __init__(self, env_cfg, obs_cfg, scene_cfg, object_name, hand_side,
                 target_manipulated_object_pose, events_cfg):
    ...
    def setup_obs(self):
        ...
        setattr(self.obs_cfg, f"{self.hand_side}_target_object_pose",
                target_object_pose)
    ...
    def get_target_object_pose(self, env, target_pose=None):
        try:  # TODO:reset target pose in env
            self.target_manipulated_object_pose = env.scene[
                self.object_name].data.target_state.clone()[..., :7]
            # self._markers = VisualizationMarkers(self.goal_object_cfg)
            # self._markers.visualize(
            #     translations=self.target_manipulated_object_pose[:, :3] +
            #     env.scene.env_origins,
            #     orientations=self.target_manipulated_object_pose[:, 3:7],
            # )

            self.target_manipulated_object_pose[:, 2] += env.scene[
                f"{self.hand_side}_hand"]._data.root_state_w[:, 2].clone()

        except:
            pass
        return self.target_manipulated_object_pose.clone()[..., :7]

    def target_object_pose(self, env, target_pose=None):
        target_pose = self.get_target_object_pose(env)
