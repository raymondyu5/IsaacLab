...
def sample_object_poses(
    ...
    for i in range(num_objects):
        ...
        if sample_pose is None:
            y_min, y_max = pose_range.get("y", (-0.2, 0.2))
            ...
            fallback_sample = [
                random.uniform(*pose_range.get("x", (0.0, 0.0))),
                first_index * y_min,
                random.uniform(*pose_range.get("z", (0.0, 0.0))),
                random.uniform(*pose_range.get("roll", (0.0, 0.0))),
                random.uniform(*pose_range.get("pitch", (0.0, 0.0))),
                random.uniform(*pose_range.get("yaw", (0.0, 0.0))),
            ]
...
def randomize_object_pose(
    ...
    for cur_env in env_ids.tolist():
        ...
        for i in range(len(asset_cfgs)):
            ...
            positions[:, 2] += env.scene["table_block"].data.reset_root_state[
                cur_env, 2]
...
class MultiRootStateCfg():
    ...
    def reset_multi_root_state_uniform(
        ...
        pick_root_states = pick_asset.data.default_root_state[env_ids].clone()
        pick_root_states[:, 2] += env.scene[
            "table_block"]._data.root_state_w[:, 2].clone()
        ...
        randomize_object_pose(
            ...
            min_separation=min_separation,
            pose_range=self.pose_pick_range,
...
def configure_multi_cluster_rigid_obs(object, env_cfg):
    ...
    for handness in ["right", "left"]:
        ...
        place_object = multi_cluster_rigid.get(f"{handness}_hand_place_object",
                                               None)
        ...
        target_object_pose = env_cfg["params"].get(
            "target_manipulated_object_pose", None)
