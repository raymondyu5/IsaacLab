...
class SingleHandGraspRew:
    ...
    def grasp_rewards(self, env):
        ...
        place_rewards = torch.zeros_like(grasp_rewards).to(env.device)
        if env.unwrapped.episode_length_buf[0] < 5:
            self.grasp_sucess = torch.zeros(env.num_envs).to(env.device).bool()
            self.target_height = self.object_pose[:, 2].clone()
        else:

            self.grasp_sucess |= torch.linalg.norm(
                self.object_target_dev[:, :2], dim=1) < 0.05
            ...
            if self.grasp_sucess.sum() > 0:
                ...
                place_rewards[place_success] = 80

        return grasp_rewards * 0.5 + place_rewards * 0.5
