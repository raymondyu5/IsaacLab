...
class MultiRootStateCfg():
    ...
    def reset_multi_root_state_uniform(
        ...
        # poses
        range_list = copy.deepcopy(pose_range)

        ranges = torch.tensor(range_list).to(env.device).to(torch.float32)
        ...
        target_state[:, 2] += env.scene["table_block"].data.reset_root_state[:,
                                                                             2]
...
def configure_multi_cluster_rigid_obs(object, env_cfg):
    ...
    for object_name, object_config in multi_cluster_object.items():
        ...
        for rigid_object in all_rigid_list:
            if rigid_object not in multi_cluster_object[
                    f"{handness}_hand_object"]["objects_list"]:
                env_cfg["params"]["RigidObject"].pop(rigid_object)

        adr = env_cfg["params"].get("adr", None)
        adr_factor = 1.0
        ...
        multi_state = MultiRootStateCfg(object_name,
                                        ...
                                        distractor_name, adr_factor)
        reset_func = EventTerm(func=multi_state.reset_multi_root_state_uniform,
                               mode="reset",
                               params={
                                   "default_pose_range":
                                   multi_state.default_pose_range,
                                   "pose_range": multi_state.pose_range,
                               })
