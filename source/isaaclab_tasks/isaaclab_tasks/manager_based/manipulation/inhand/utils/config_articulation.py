...
class MultiRootStateCfg():
    ...
    def reset_default_scale(self, env, env_ids, relative_child_path=None):
        ...
        asset: ArticulationObject = env.scene[self.object_name]
        env_ids = env_ids.cpu()
...
def get_articulation_state(env, object_name):
    """
...
def configure_reset_articulation_obs(object, object_name, env_cfg):
    ...
    reset = EventTerm(
        ...
        params={
            ...
            object_name,
            "pose_range":
            env_cfg["params"]["ArticulationObject"][object_name]["pose_range"]
        })
    setattr(object.events, f"reset_{object_name}", reset)


def configure_multi_articulation_obs(object, env_cfg):

    num_rigid_objects = len(
    ...
    multi_state = MultiRootStateCfg(object_name,
                                    env_cfg["params"]["ArticulationObject"],
                                    num_envs)
...
def configure_articulation_obs(object, env_cfg):

    rigid_objects = env_cfg['params'].get('ArticulationObject', {})
        ...
        for object_name, object_config in rigid_objects.items():
            ...
            if object_config.get("load_from_folder", False):
                ...
                for name in object_names:
                    object_state = ObsTerm(func=get_articulation_state,
                ...
                configure_reset_articulation_obs(object, object_name, env_cfg)
