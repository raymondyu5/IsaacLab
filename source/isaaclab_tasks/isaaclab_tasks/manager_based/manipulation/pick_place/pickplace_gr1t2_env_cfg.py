...
class ObjectTableSceneCfg(InteractiveSceneCfg):
    ...
    object = RigidObjectCfg(
        ...
        init_state=RigidObjectCfg.InitialStateCfg(pos=[-0.45, 0.45, 0.9996],
                                                  rot=[1, 0, 0, 0]),
        spawn=UsdFileCfg(
            usd_path=
...
class ActionsCfg:
    ...
    upper_body_ik = PinkInverseKinematicsActionCfg(
        ...
        controller=PinkIKControllerCfg(
            ...
            False,  # Determines whether to pink solver will fail due to a joint limit violation
            ...
            fixed_input_tasks=[],
            xr_enabled=bool(
                carb.settings.get_settings().get("/app/xr/enabled")),
        ),
    )
                ...
                OpenXRDeviceCfg(
                    retargeters=[
                        GR1T2RetargeterCfg(
                            ...
                            num_open_xr_hand_joints=2 *
                            self.NUM_OPENXR_HAND_JOINTS,
                            sim_device=self.sim.device,
                            hand_joint_names=self.actions.upper_body_ik.
                            hand_joint_names,
                        ),
                    ],
                    sim_device=self.sim.device,
