...
def move_to_target_bonus(
    ...
    heading_proj = obs.base_heading_proj(env, target_pos,
                                         asset_cfg).squeeze(-1)
...
class progress_reward(ManagerTermBase):
    ...
    def reset(self, env_ids: torch.Tensor):
        ...
        target_pos = torch.tensor(self.cfg.params["target_pos"],
                                  device=self.device)
        to_target_pos = target_pos - asset.data.root_pos_w[env_ids, :3]
        # reward terms
        self.potentials[env_ids] = -torch.norm(to_target_pos, p=2,
                                               dim=-1) / self._env.step_dt
    ...
    def __call__(
        ...
        self.potentials[:] = -torch.norm(to_target_pos, p=2,
                                         dim=-1) / env.step_dt
...
class joint_pos_limits_penalty_ratio(ManagerTermBase):
    ...
    def __init__(self, env: ManagerBasedRLEnv, cfg: RewardTermCfg):
        ...
        self.gear_ratio = torch.ones(env.num_envs,
                                     asset.num_joints,
                                     device=env.device)
        index_list, _, value_list = string_utils.resolve_matching_names_values(
            cfg.params["gear_ratio"], asset.joint_names)
        self.gear_ratio[:, index_list] = torch.tensor(value_list,
                                                      device=env.device)
    ...
    def __call__(
        ...
        violation_amount = (torch.abs(joint_pos_scaled) -
                            threshold) / (1 - threshold)
        violation_amount = violation_amount * self.gear_ratio_scaled

        return torch.sum(
            (torch.abs(joint_pos_scaled) > threshold) * violation_amount,
            dim=-1)
...
class power_consumption(ManagerTermBase):
    ...
    def __call__(
        ...
        return torch.sum(
            torch.abs(env.action_manager.action * asset.data.joint_vel *
