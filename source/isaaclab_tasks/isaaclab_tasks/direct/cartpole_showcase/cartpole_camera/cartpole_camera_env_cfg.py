...
def get_tiled_camera_cfg(data_type: str,
                         width: int = 100,
                         height: int = 100) -> TiledCameraCfg:
...
class BoxBoxEnvCfg(CartpoleCameraEnvCfg):
    ...
    * Action space (``~gymnasium.spaces.Box`` with shape (1,))
    ...
    observation_space = spaces.Box(
        low=float("-inf"),
        high=float("inf"),
        shape=(tiled_camera.height, tiled_camera.width,
               3))  # or for simplicity: [height, width, 3]
    action_space = spaces.Box(low=-1.0, high=1.0,
                              shape=(1, ))  # or for simplicity: 1 or [1]
...
class BoxMultiDiscreteEnvCfg(CartpoleCameraEnvCfg):
    ...
    * Action space (``~gymnasium.spaces.MultiDiscrete`` with 2 discrete spaces)
    ...
    observation_space = spaces.Box(
        ...
        shape=(tiled_camera.height, tiled_camera.width,
               3))  # or for simplicity: [height, width, 3]
    action_space = spaces.MultiDiscrete([3,
                                         2])  # or for simplicity: [{3}, {2}]
    ...
    observation_space = spaces.Dict(
        {
            ...
            spaces.Box(low=float("-inf"),
                       high=float("inf"),
                       shape=(tiled_camera.height, tiled_camera.width, 3)),
        }
