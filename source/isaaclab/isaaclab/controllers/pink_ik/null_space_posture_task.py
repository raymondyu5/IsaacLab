    ...
    **Task Integration:**
    ...
    def compute_error(self, configuration: Configuration) -> np.ndarray:
        ...
        if self.target_q is None:
            raise ValueError(
                "No posture target has been set. Call set_target() first.")
    ...
    def compute_jacobian(self, configuration: Configuration) -> np.ndarray:
        ...
        J_frame_tasks = [
            configuration.get_frame_jacobian(frame_name)
        ...
        np.fill_diagonal(JJT,
                         JJT.diagonal() + self.PSEUDOINVERSE_DAMPING_FACTOR**2)
