...
class PinkIKController:
    ...
    def __init__(self, cfg: PinkIKControllerCfg, device: str):
        ...
        self.robot_wrapper = RobotWrapper.BuildFromURDF(cfg.urdf_path,
                                                        cfg.mesh_path,
                                                        root_joint=None)
        self.pink_configuration = Configuration(self.robot_wrapper.model,
        ...
        pink_joint_names = self.robot_wrapper.model.names.tolist()[
            1:]  # Skip the root and universal joints
    ...
    def reorder_array(self, input_array: list[float],
                      reordering_array: list[int]) -> list[float]:
    ...
    def compute(
        ...
        joint_positions_pink = np.array(
            self.reorder_array(curr_joint_pos,
                               self.isaac_lab_to_pink_ordering))
        ...
        try:
            velocity = solve_ik(self.pink_configuration,
            ...
            return torch.tensor(curr_joint_pos,
                                device=self.device,
                                dtype=torch.float32)
        ...
        joint_vel_isaac_lab = torch.tensor(
            self.reorder_array(pink_joint_angle_changes,
                               self.pink_to_isaac_lab_ordering),
            device=self.device,
            dtype=torch.float,
        )
