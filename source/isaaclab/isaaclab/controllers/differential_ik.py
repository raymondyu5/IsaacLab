...
class DifferentialIKController:
    ...
    def __init__(self, cfg: DifferentialIKControllerCfg, num_envs: int,
                 device: str):
        ...
        # -- input command
        self._command = torch.zeros(self.num_envs,
                                    self.action_dim,
                                    device=self._device)
    ...
    def compute(self, ee_pos: torch.Tensor, ee_quat: torch.Tensor,
    ...
    def _compute_delta_joint_pos(self, delta_pose: torch.Tensor,
        ...
        if self.cfg.ik_method == "pinv":  # Jacobian pseudo-inverse
            ...
            S_inv = 1.0 / S
            S_inv = torch.where(S > min_singular_value, S_inv,
                                torch.zeros_like(S_inv))
            jacobian_pinv = (
                torch.transpose(Vh, dim0=1, dim1=2)[:, :, :6]
                @ torch.diag_embed(S_inv) @ torch.transpose(U, dim0=1, dim1=2))
            delta_joint_pos = k_val * jacobian_pinv @ delta_pose.unsqueeze(-1)
            delta_joint_pos = delta_joint_pos.squeeze(-1)
            ...
            # computation
            jacobian_T = torch.transpose(jacobian, dim0=1, dim1=2)
            lambda_matrix = (lambda_val**2) * torch.eye(n=jacobian.shape[1],
                                                        device=self._device)
            delta_joint_pos = (
                jacobian_T
