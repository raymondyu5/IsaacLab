...
class SensorBase(ABC):
    ...
    def update(self, dt: float, force_recompute: bool = False):
        ...
        if force_recompute or self._is_visualizing or (self.cfg.history_length
                                                       > 0):
            self._update_outdated_buffers()
    ...
    def _initialize_impl(self):
        ...
        self._is_outdated = torch.ones(self._num_envs,
                                       dtype=torch.bool,
                                       device=self._device)
    ...
    def _on_prim_deletion(self, prim_path: str) -> None:
        ...
        result = re.match(pattern="^" + "/".join(
            self.cfg.prim_path.split("/")[:prim_path.count("/") + 1]) + "$",
                          string=prim_path)
        if result:
            self._clear_callbacks()
