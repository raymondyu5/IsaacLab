...
class FrameTransformer(SensorBase):
    ...
    def find_bodies(
        ...
        """
        return string_utils.resolve_matching_names(name_keys,
    ...
    def _initialize_impl(self):
        ...
        source_frame_offset_pos = torch.tensor(
            self.cfg.source_frame_offset.pos, device=self.device)
        ...
        if is_identity_pose(source_frame_offset_pos, source_frame_offset_quat):
            logger.debug(
                f"No offset application needed for source frame as it is identity: {self.cfg.prim_path}"
            )
            self._apply_source_frame_offset = False
        else:
            logger.debug(
                f"Applying offset to source frame as it is not identity: {self.cfg.prim_path}"
            )
            # Store offsets as tensors (duplicating each env's offsets for ease of multiplication later)
            self._source_frame_offset_pos = source_frame_offset_pos.unsqueeze(
        ...
        frame_offsets = [None] + [
            target_frame.offset for target_frame in self.cfg.target_frames
        ]
        frame_types = ["source"] + ["target"] * len(self.cfg.target_frames)
        for frame, prim_path, offset, frame_type in zip(
                frames, frame_prim_paths, frame_offsets, frame_types):
            # Find correct prim
            ...
            for prim in matching_prims:
                ...
                if body_name in body_names_to_frames:
                    ...
                    if body_names_to_frames[body_name][
                            "type"] == "source" and frame_type == "target":
        ...
        tracked_prim_paths = [
            body_names_to_frames[body_name]["prim_path"]
            for body_name in body_names_to_frames.keys()
        ]
        tracked_body_names = [
            body_name for body_name in body_names_to_frames.keys()
        ]
        ...
        self._frame_physx_view = self._physics_sim_view.create_rigid_body_view(
            body_names_regex)
        ...
        if "env_" in all_prim_paths[0]:
            ...
            self._per_env_indices = [
                index for index, _ in sorted(
                    list(enumerate(all_prim_paths)),
                    key=lambda x: extract_env_num_and_prim_path(x[1]))
            ]

            # Only need 0th env as the names and their ordering are the same across environments
            sorted_prim_paths = [
                all_prim_paths[index] for index in self._per_env_indices
            ...
            self._per_env_indices = [
                index for index, _ in sorted(enumerate(all_prim_paths),
                                             key=lambda x: x[1])
            ]
            sorted_prim_paths = [
                all_prim_paths[index] for index in self._per_env_indices
        ...
        self._target_frame_body_names = [
            prim_path.split("/")[-1] for prim_path in sorted_prim_paths
        ]

        # -- source frame
        self._source_frame_body_name = self.cfg.prim_path.split("/")[-1]
        source_frame_index = self._target_frame_body_names.index(
            self._source_frame_body_name)
        ...
        self._source_frame_body_ids = torch.arange(
            self._num_envs) * len(tracked_body_names) + source_frame_index
        ...
        if self._source_is_also_target_frame:
            ...
            self._target_frame_body_ids = all_ids[
                ~torch.isin(all_ids, self._source_frame_body_ids)]
        ...
        for i, body_name in enumerate(self._target_frame_body_names):
            for frame in body_names_to_frames[body_name]["frames"]:
                ...
                if frame in target_offsets:
                    target_frame_offset_pos.append(
                        target_offsets[frame]["pos"])
                    target_frame_offset_quat.append(
                        target_offsets[frame]["quat"])
        ...
        duplicate_frame_indices = torch.tensor(duplicate_frame_indices,
                                               device=self.device)
        if self._source_is_also_target_frame:
        ...
        if self._apply_target_frame_offset:
            ...
            self._target_frame_offset_pos = torch.stack(
                target_frame_offset_pos).repeat(self._num_envs, 1)
            self._target_frame_offset_quat = torch.stack(
                target_frame_offset_quat).repeat(self._num_envs, 1)

        # fill the data buffer
        self._data.target_frame_names = self._target_frame_names
        self._data.source_pos_w = torch.zeros(self._num_envs,
                                              3,
                                              device=self._device)
        self._data.source_quat_w = torch.zeros(self._num_envs,
                                               4,
                                               device=self._device)
        self._data.target_pos_w = torch.zeros(self._num_envs,
                                              len(duplicate_frame_indices),
                                              3,
                                              device=self._device)
        self._data.target_quat_w = torch.zeros(self._num_envs,
                                               len(duplicate_frame_indices),
                                               4,
                                               device=self._device)
        self._data.target_pos_source = torch.zeros_like(
            self._data.target_pos_w)
        self._data.target_quat_source = torch.zeros_like(
            self._data.target_quat_w)
    ...
    def _update_buffers_impl(self, env_ids: Sequence[int]):
        ...
        duplicated_target_frame_pos_w = target_frames[
            self._duplicate_frame_indices, :3]
        duplicated_target_frame_quat_w = target_frames[
            self._duplicate_frame_indices, 3:]
        ...
        target_pos_source, target_quat_source = subtract_frame_transforms(
            source_pos_w.unsqueeze(1).expand(-1, total_num_frames,
                                             -1).reshape(-1, 3),
            source_quat_w.unsqueeze(1).expand(-1, total_num_frames,
                                              -1).reshape(-1, 4),
            target_pos_w,
            target_quat_w,
        )
        ...
        self._data.target_quat_w[:] = target_quat_w.view(
            -1, total_num_frames, 4)
        self._data.target_pos_source[:] = target_pos_source.view(
            -1, total_num_frames, 3)
        self._data.target_quat_source[:] = target_quat_source.view(
            -1, total_num_frames, 4)
    ...
    def _debug_vis_callback(self, event):
        ...
        frames_pos = torch.cat(
            [self._data.source_pos_w,
             self._data.target_pos_w.view(-1, 3)],
            dim=0)
        frames_quat = torch.cat(
            [self._data.source_quat_w,
             self._data.target_quat_w.view(-1, 4)],
            dim=0)

        # Get the all connecting lines between frames pose
        lines_pos, lines_quat, lines_length = self._get_connecting_lines(
            start_pos=self._data.source_pos_w.repeat_interleave(
                self._data.target_pos_w.size(1), dim=0),
    ...
    def _get_connecting_lines(
        ...
        default_direction = torch.tensor([0.0, 0.0, 1.0],
        ...
        rotation_axis = torch.where(
            ...
            torch.tensor([1.0, 0.0, 0.0],
                         device=self.device).expand(start_pos.size(0), -1),
        )
