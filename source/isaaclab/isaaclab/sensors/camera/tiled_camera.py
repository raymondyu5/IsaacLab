...
class TiledCamera(Camera):
    ...
    def _initialize_impl(self):
        ...
        rp = rep.create.render_product_tiled(cameras=self._view.prim_paths,
                                             tile_resolution=(self.cfg.width,
                                                              self.cfg.height))
    ...
    def _update_buffers_impl(self, env_ids: Sequence[int]):
        ...
        for data_type, annotator in self._annotators.items():
            ...
            if isinstance(tiled_data_buffer, np.ndarray):
                ...
                tiled_data_buffer = wp.array(tiled_data_buffer,
                                             device=self.device)
            else:
            ...
            if ((data_type == "semantic_segmentation"
                 and self.cfg.colorize_semantic_segmentation)
                    or (data_type == "instance_segmentation_fast"
                        and self.cfg.colorize_instance_segmentation)
                    or (data_type == "instance_id_segmentation_fast"
                        and self.cfg.colorize_instance_id_segmentation)):
                tiled_data_buffer = wp.array(ptr=tiled_data_buffer.ptr,
                                             shape=(*tiled_data_buffer.shape,
                                                    4),
                                             dtype=wp.uint8,
                                             device=self.device)
            ...
            wp.launch(
                ...
                inputs=[
                    ...
                    wp.from_torch(
                        self._data.output[data_type]),  # zero-copy alias
            ...
            if (data_type == "distance_to_camera"
                    or data_type == "distance_to_image_plane" or data_type
                    == "depth") and self.cfg.depth_clipping_behavior != "none":
                self._data.output[data_type][torch.isinf(
                    self._data.output[data_type])] = (
