...
def convert_camera_intrinsics_to_usd(
        ...
        width: int,
        height: int,
        focal_length: float | None = None) -> dict:
    ...
    if abs(f_x - f_y) > 1e-4:
        logger.warning(
            "Camera non square pixels are not supported by Omniverse. The average of f_x and f_y are used."
        )

    # warn about aperture offsets
    if abs((c_x - float(width) / 2) > 1e-4
           or (c_y - float(height) / 2) > 1e-4):
        logger.warning(
