...
def check_file_path(path: str) -> Literal[0, 1, 2]:
    ...
    elif omni.client.stat(path.replace(os.sep,
                                       "/"))[0] == omni.client.Result.OK:
        return 2
    else:
        return 0


def retrieve_file_path(path: str,
                       download_dir: str | None = None,
                       force_download: bool = True) -> str:
...
def check_usd_path_with_timeout(usd_path: str,
                                timeout: float = 300,
                                log_interval: float = 30) -> bool:
    ...
    while not task.done():
        ...
        if now >= next_log_time:
            ...
            if first_log:
                logger.warning(
                    f"Checking server availability for USD path: {usd_path} (timeout: {timeout}s)"
                )
                first_log = False
            logger.warning(
                f"Waiting for server response... ({elapsed}s elapsed)")
            next_log_time += log_interval
        loop.run_until_complete(
            asyncio.sleep(0.1))  # Yield to allow async work
...
async def _is_usd_path_available(usd_path: str, timeout: float) -> bool:
    ...
    """
    try:
        result, _ = await asyncio.wait_for(omni.client.stat_async(usd_path),
                                           timeout=timeout)
        return result == omni.client.Result.OK
    except asyncio.TimeoutError:
        logger.warning(
            f"Timed out after {timeout}s while checking for USD: {usd_path}")
        return False
    except Exception as ex:
        logger.warning(
            f"Exception during USD file check: {type(ex).__name__}: {ex}")
        return False
