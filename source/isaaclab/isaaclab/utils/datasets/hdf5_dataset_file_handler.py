...
class HDF5DatasetFileHandler(DatasetFileHandlerBase):
    ...
    def load_episode(self, episode_name: str,
                     device: str) -> EpisodeData | None:
        ...
        def load_dataset_helper(group):
            ...
            for key in group:
                if isinstance(group[key], h5py.Group):
                    ...
                    data[key] = torch.tensor(np.array(group[key]),
                                             device=device)
            return data
    ...
    def write_episode(self, episode: EpisodeData, demo_id: int | None = None):
        ...
        if "actions" in episode.data:
            h5_episode_group.attrs["num_samples"] = len(
                episode.data["actions"])
        else:
        ...
        self._hdf5_data_group.attrs["total"] += h5_episode_group.attrs[
            "num_samples"]
