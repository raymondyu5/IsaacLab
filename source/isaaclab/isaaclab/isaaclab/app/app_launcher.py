...
class AppLauncher:
    ...
    def __init__(self,
        ...
        if len(kwargs) > 0:
            if not set(kwargs.keys()).isdisjoint(launcher_args.keys()):
                overlapping_args = set(kwargs.keys()).intersection(
                    launcher_args.keys())
                raise ValueError(
        ...
        self._livestream: Literal[
            0, 1, 2]  # 0: Disabled, 1: WebRTC public, 2: WebRTC private
    ...
    def add_app_launcher_args(parser: argparse.ArgumentParser) -> None:
        ...
        * ``kit_args`` (str): Optional command line arguments to be passed to Omniverse Kit directly.
        ...
        if len(parser._actions) > 0 and isinstance(
                parser._actions[0], argparse._HelpAction):  # type: ignore
    ...
    def _config_resolution(self, launcher_args: dict):
        ...
        livestream_arg, livestream_env = self._resolve_livestream_settings(
            launcher_args)
        self._resolve_headless_settings(launcher_args, livestream_arg,
                                        livestream_env)
    ...
    def _resolve_livestream_settings(self,
                                     launcher_args: dict) -> tuple[int, int]:
    ...
    def _resolve_headless_settings(self, launcher_args: dict,
                                   livestream_arg: int, livestream_env: int):
    ...
    def _resolve_camera_settings(self, launcher_args: dict):
        ...
        enable_cameras_arg = launcher_args.get(
            "enable_cameras",
            AppLauncher._APPLAUNCHER_CFG_INFO["enable_cameras"][1])
    ...
    def _resolve_xr_settings(self, launcher_args: dict):
        ...
        xr_arg = launcher_args.get("xr",
                                   AppLauncher._APPLAUNCHER_CFG_INFO["xr"][1])
        xr_valid_vals = {0, 1}
    ...
    def _resolve_viewport_settings(self, launcher_args: dict):
        ...
        if self._headless and not self._livestream and not launcher_args.get(
                "video", False):
    ...
    def _resolve_device_settings(self, launcher_args: dict):
        ...
        if "distributed" in launcher_args and launcher_args["distributed"]:
            ...
            num_threads_per_process = num_cpu_cores // int(
    ...
    def _resolve_experience_file(self, launcher_args: dict):
        ...
        if self.is_isaac_sim_version_4_5():
            isaaclab_app_exp_path = os.path.join(isaaclab_app_exp_path,
                                                 "isaacsim_4_5")
        ...
        if self._sim_experience_file == "":
            ...
            if self._enable_cameras and not self._xr:
                ...
                if self._headless:
                    ...
                    self._sim_experience_file = os.path.join(
                        isaaclab_app_exp_path, "isaaclab.python.xr.openxr.kit")
            ...
            option_1_app_exp_path = os.path.join(kit_app_exp_path,
                                                 self._sim_experience_file)
            option_2_app_exp_path = os.path.join(isaaclab_app_exp_path,
                                                 self._sim_experience_file)
    ...
    def _create_app(self):
        ...
        self._app = SimulationApp(self._sim_app_config,
                                  experience=self._sim_experience_file)
        ...
        if len(self._livestream_args) > 0:
            sys.argv = [
                arg for arg in sys.argv if arg not in self._livestream_args
            ]
    ...
    def _load_extensions(self):
        ...
        carb_settings_iface.set_bool("/isaaclab/render/offscreen",
        ...
        # when the render() method is called.
        carb_settings_iface.set_bool("/isaaclab/render/active_viewport",
        ...
        carb_settings_iface.set_bool("/physics/fabricUpdateTransformations",
    ...
    def _set_rendering_mode_settings(self, launcher_args: dict) -> None:
        ...
        set_carb_setting(carb_settings, "/isaaclab/rendering/rendering_mode",
                         rendering_mode)
    ...
    def _set_animation_recording_settings(self, launcher_args: dict) -> None:
        ...
        carb_settings = carb.settings.get_settings()
        set_carb_setting(carb_settings, "/isaaclab/anim_recording/enabled",
                         recording_enabled)
        set_carb_setting(carb_settings, "/isaaclab/anim_recording/start_time",
                         start_time)
        set_carb_setting(carb_settings, "/isaaclab/anim_recording/stop_time",
                         stop_time)
    ...
    def is_isaac_sim_version_4_5(self) -> bool:
        if not hasattr(self, "_is_sim_ver_4_5"):
            ...
            version_path = os.path.abspath(
                os.path.join(os.path.dirname(isaacsim.__file__),
                             "../../VERSION"))
            ...
            try:
                ...
                # Case 2: Single argument
                    ...
                    # Case 2b: Tuple of tuples (4x4 matrix) OR List of lists (4x4 matrix)
                          ...
                          and all(isinstance(row, tuple) for row in arg)) or (
                              isinstance(arg, list) and len(arg) == 4
                              and all(isinstance(row, list) for row in arg)):
                        float_list = [
                            float(item) for row in arg for item in row
                        ]
                    ...
                    # Case 2d: Another matrix-like object with elements accessible via indexing
                    elif hasattr(arg, "__getitem__") and hasattr(
                            arg, "__len__"):
                        with contextlib.suppress(IndexError, TypeError):
                            ...
                            elif len(arg) == 4 and all(
                                    len(row) == 4 for row in arg):
                                float_list = [
                                    float(arg[i][j]) for i in range(4)
                                    for j in range(4)
                                ]
                                original_matrix4d(self, *float_list)
                                return
