...
class UrdfConverter(AssetConverterBase):
    ...
    def _get_urdf_import_config(
            self) -> isaacsim.asset.importer.urdf.ImportConfig:
        ...
        import_config.set_collision_from_visuals(
            self.cfg.collision_from_visuals)
    ...
    def _set_joints_drive_type(self):
        ...
        if isinstance(self.cfg.joint_drive.drive_type, str):
            for joint in self._robot_model.joints.values():
                joint.drive.set_drive_type(
                    drive_type_mapping[self.cfg.joint_drive.drive_type])
            ...
            for joint_name, drive_type in self.cfg.joint_drive.drive_type.items(
                ...
                matches = [
                    s for s in self._robot_model.joints.keys()
                    if re.search(joint_name, s)
                ]
                if not matches:
                    raise ValueError(
    ...
    def _set_joints_drive_target_type(self):
        ...
        if isinstance(self.cfg.joint_drive.target_type, str):
            for joint in self._robot_model.joints.values():
                joint.drive.set_target_type(
                    target_type_mapping[self.cfg.joint_drive.target_type])
    ...
    def _set_joint_drive_gains(self):
        ...
        if isinstance(self.cfg.joint_drive.gains,
                      UrdfConverterCfg.JointDriveCfg.PDGainsCfg):
            # stiffness
            if isinstance(self.cfg.joint_drive.gains.stiffness, (float, int)):
                for joint in self._robot_model.joints.values():
                    self._set_joint_drive_stiffness(
                        joint, self.cfg.joint_drive.gains.stiffness)
            ...
            if isinstance(self.cfg.joint_drive.gains.damping, (float, int)):
                for joint in self._robot_model.joints.values():
                    self._set_joint_drive_damping(
                        joint, self.cfg.joint_drive.gains.damping)
        ...
        # set the gains from natural frequency and damping ratio
                ...
                self.cfg.joint_drive.gains,
                UrdfConverterCfg.JointDriveCfg.NaturalFrequencyGainsCfg):
            # damping ratio
            if isinstance(self.cfg.joint_drive.gains.damping_ratio,
                          (float, int)):
                for joint in self._robot_model.joints.values():
            ...
            if isinstance(self.cfg.joint_drive.gains.natural_frequency,
                          (float, int)):
                for joint in self._robot_model.joints.values():
            ...
            elif isinstance(self.cfg.joint_drive.gains.natural_frequency,
                            dict):
                for joint_name, natural_frequency in self.cfg.joint_drive.gains.natural_frequency.items(
                    ...
                    for match in matches:
                        ...
                        self._set_joint_drive_gains_from_natural_frequency(
                            joint)
