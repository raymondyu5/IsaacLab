...
class MeshConverter(AssetConverterBase):
    ...
    * ``{prim_path}/geometry`` - The prim that contains the mesh and optionally the materials if configured.
    ...
    def _convert_asset(self, cfg: MeshConverterCfg):
        ...
        mesh_file_basename, mesh_file_format = os.path.basename(
            cfg.asset_path).split(".")
        ...
        asyncio.get_event_loop().run_until_complete(
            self._convert_mesh_to_usd(in_file=cfg.asset_path,
                                      out_file=self.usd_path))
        ...
        prim = temp_stage.DefinePrim(f"/{mesh_file_basename}/geometry",
                                     "Xform")
        ...
        # translation
        translate_op = geom_xform.AddTranslateOp(
        ...
        if cfg.make_instanceable:
            ...
            export_prim_to_file(
                path=os.path.join(self.usd_dir,
                                  self.usd_instanceable_meshes_path),
        ...
        if cfg.mass_props is not None:
            schemas.define_mass_properties(prim_path=xform_prim.GetPath(),
                                           cfg=cfg.mass_props,
                                           stage=stage)
    ...
    @staticmethod
    async def _convert_mesh_to_usd(in_file: str,
                                   out_file: str,
                                   load_materials: bool = True) -> bool:
        ...
        task = instance.create_converter_task(in_file, out_file, None,
                                              converter_context)
