...
def safe_set_attribute_on_usd_schema(schema_api: Usd.APISchemaBase, name: str,
                                     value: Any, camel_case: bool):
    ...
    if attr is not None:
        ...
        logger.error(
            f"Attribute '{attr_name}' does not exist on prim '{schema_api.GetPath()}'."
        )
        raise TypeError(
            f"Attribute '{attr_name}' does not exist on prim '{schema_api.GetPath()}'."
        )
...
def safe_set_attribute_on_usd_prim(prim: Usd.Prim, attr_name: str, value: Any,
                                   camel_case: bool):
    ...
    elif isinstance(value, (tuple, list)) and len(value) == 3 and any(
            isinstance(v, float) for v in value):
        sdf_type = Sdf.ValueTypeNames.Float3
    elif isinstance(value, (tuple, list)) and len(value) == 2 and any(
            isinstance(v, float) for v in value):
...
def clone(func: Callable) -> Callable:
    ...
    def wrapper(prim_path: str | Sdf.Path, cfg: SpawnerCfg, *args, **kwargs):
        ...
        if not prim_path.startswith("/"):
            raise ValueError(
                f"Prim path '{prim_path}' is not global. It must start with '/'."
            )
        ...
        if hasattr(
                cfg,
                "activate_contact_sensors") and cfg.activate_contact_sensors:
...
def bind_physics_material(
    ...
    if not (has_physics_scene_api or has_collider or has_deformable_body
            or has_particle_system):
        logger.debug(
    ...
    material_binding_api.Bind(material,
                              ...
                              materialPurpose="physics")  # type: ignore
    # return success
    return True
...
def export_prim_to_file(
    ...
    if not source_prim_path.startswith("/"):
        raise ValueError(
            f"Source prim path '{source_prim_path}' is not global. It must start with '/'."
        )
    if target_prim_path is not None and not target_prim_path.startswith("/"):
        raise ValueError(
            f"Target prim path '{target_prim_path}' is not global. It must start with '/'."
        )
    ...
    UsdGeom.SetStageMetersPerUnit(target_stage,
    ...
    Sdf.CopySpec(source_layer, source_prim_path, target_layer,
                 target_prim_path)
...
def make_uninstanceable(prim_path: str | Sdf.Path,
                        stage: Usd.Stage | None = None):
    ...
    while len(all_prims) > 0:
        ...
        all_prims += child_prim.GetFilteredChildren(
            Usd.TraverseInstanceProxies())


def resolve_prim_pose(
    prim: Usd.Prim,
    ref_prim: Usd.Prim | None = None
    ...
    if ref_prim is not None:
        ...
        if not ref_prim.IsValid():
            raise ValueError(
                f"Ref prim at path '{ref_prim.GetPath().pathString}' is not valid."
            )
        # get ref prim xform
    ...
    prim_quat = [
        prim_tf.ExtractRotationQuat().real,
        *prim_tf.ExtractRotationQuat().imaginary
    ]
...
def get_all_matching_child_prims(
    ...
    while len(all_prims_queue) > 0:
        ...
        if depth is None or current_depth < depth:
            ...
            if traverse_instance_prims:
                children = child_prim.GetFilteredChildren(
                    Usd.TraverseInstanceProxies())
            else:
                children = child_prim.GetChildren()
            # add children to list
            all_prims_queue += [(child, current_depth + 1)
                                for child in children]

    return output_prims


def find_first_matching_prim(
        prim_path_regex: str,
        stage: Usd.Stage | None = None) -> Usd.Prim | None:
...
def find_matching_prims(prim_path_regex: str,
                        stage: Usd.Stage | None = None) -> list[Usd.Prim]:
...
def find_matching_prim_paths(prim_path_regex: str,
                             stage: Usd.Stage | None = None) -> list[str]:
...
def find_global_fixed_joint_prim(
        ...
        stage: Usd.Stage | None = None) -> UsdPhysics.Joint | None:
...
def select_usd_variants(prim_path: str,
                        variants: object | dict[str, str],
                        stage: Usd.Stage | None = None):
...
def attach_stage_to_usd_context(attaching_early: bool = False):
    ...
    is_rendering_enabled = get_carb_setting(
        carb_setting, "/physics/fabricUpdateTransformations")
...
def use_stage(stage: Usd.Stage) -> Generator[None, None, None]:
    ...
    if isaac_sim_version < 5:
        logger.warning(
            "[Compat] Isaac Sim < 5.0 does not support thread-local stage contexts. Skipping use_stage()."
        )
        yield  # no-op
    else:
...
class RateLimitFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        if msg_key not in self.last_emitted or (
                now - self.last_emitted[msg_key]) > self.interval:
