...
class RigidObjectCollection(AssetBase):
    ...
    def __init__(self, cfg: RigidObjectCollectionCfg):
        ...
        for rigid_object_cfg in self.cfg.rigid_objects.values():
            ...
            asset_path_is_regex = re.match(r"^[a-zA-Z0-9/_]+$",
                                           asset_path) is None
            # spawn the asset
            ...
            matching_prims = sim_utils.find_matching_prims(
                rigid_object_cfg.prim_path)
        ...
        if self.has_external_wrench:
            if self.uses_external_wrench_positions:
                self.root_physx_view.apply_forces_and_torques_at_position(
                    ...
                    torque_data=self.reshape_data_to_view(
                        self._external_torque_b),
                    position_data=self.reshape_data_to_view(
                        self._external_wrench_positions_b),
    ...
    def find_objects(
            ...
            preserve_order: bool = False) -> tuple[torch.Tensor, list[str]]:
    ...
    def write_object_state_to_sim(
        ...
        """
        self.write_object_link_pose_to_sim(object_state[..., :7],
                                           env_ids=env_ids,
                                           object_ids=object_ids)
        self.write_object_com_velocity_to_sim(object_state[..., 7:],
                                              env_ids=env_ids,
                                              object_ids=object_ids)
    ...
    def write_object_com_state_to_sim(
        ...
        Args:
            ...
            object_ids: Object indices. If None, then all indices are used.
        """
        self.write_object_com_pose_to_sim(object_state[..., :7],
                                          env_ids=env_ids,
                                          object_ids=object_ids)
        self.write_object_com_velocity_to_sim(object_state[..., 7:],
                                              env_ids=env_ids,
    ...
    def write_object_link_state_to_sim(
        ...
        self.write_object_link_pose_to_sim(object_state[..., :7],
                                           env_ids=env_ids,
                                           object_ids=object_ids)
        self.write_object_link_velocity_to_sim(object_state[..., 7:],
                                               env_ids=env_ids,
                                               object_ids=object_ids)
    ...
    def write_object_pose_to_sim(
        ...
        Args:
            ...
            object_ids: Object indices. If None, then all indices are used.
        """
        self.write_object_link_pose_to_sim(object_pose,
                                           env_ids=env_ids,
                                           object_ids=object_ids)
    ...
    def write_object_link_pose_to_sim(
        ...
        self._data.object_link_pose_w[env_ids[:, None],
                                      object_ids] = object_pose.clone()
        ...
        if self._data._object_link_state_w.data is not None:
            self._data.object_link_state_w[
                env_ids[:, None], object_ids, :7] = object_pose.clone()
        if self._data._object_state_w.data is not None:
            self._data.object_state_w[env_ids[:, None],
                                      object_ids, :7] = object_pose.clone()
        if self._data._object_com_state_w.data is not None:
            # get CoM pose in link frame
            com_pos_b = self.data.object_com_pos_b[env_ids[:, None],
                                                   object_ids]
            com_quat_b = self.data.object_com_quat_b[env_ids[:, None],
                                                     object_ids]
            ...
            self._data.object_com_state_w[env_ids[:, None],
                                          object_ids, :3] = com_pos
            self._data.object_com_state_w[env_ids[:, None], object_ids,
                                          3:7] = com_quat

        # convert the quaternion from wxyz to xyzw
        poses_xyzw = self._data.object_link_pose_w.clone()
        poses_xyzw[..., 3:] = math_utils.convert_quat(poses_xyzw[..., 3:],
                                                      to="xyzw")
    ...
    def write_object_com_pose_to_sim(
        ...
        # set into internal buffers
        self._data.object_com_pose_w[env_ids[:, None],
                                     object_ids] = object_pose.clone()
        # update these buffers only if the user is using them. Otherwise this adds to overhead.
        if self._data._object_com_state_w.data is not None:
            self._data.object_com_state_w[
                env_ids[:, None], object_ids, :7] = object_pose.clone()
        ...
        object_link_pose = torch.cat((object_link_pos, object_link_quat),
                                     dim=-1)
        self.write_object_link_pose_to_sim(object_link_pose,
                                           env_ids=env_ids,
                                           object_ids=object_ids)
    ...
    def write_object_velocity_to_sim(
        ...
        Args:
            ...
            object_ids: Object indices. If None, then all indices are used.
        """
        self.write_object_com_velocity_to_sim(object_velocity,
                                              env_ids=env_ids,
                                              object_ids=object_ids)
    ...
    def write_object_com_velocity_to_sim(
        ...
        # note: we need to do this here since tensors are not set into simulation until step.
        # set into internal buffers
        self._data.object_com_vel_w[env_ids[:, None],
                                    object_ids] = object_velocity.clone()
        # update these buffers only if the user is using them. Otherwise this adds to overhead.
        if self._data._object_com_state_w.data is not None:
            self._data.object_com_state_w[env_ids[:, None], object_ids,
                                          7:] = object_velocity.clone()
        if self._data._object_state_w.data is not None:
            self._data.object_state_w[env_ids[:, None], object_ids,
                                      7:] = object_velocity.clone()
        if self._data._object_link_state_w.data is not None:
            self._data.object_link_state_w[env_ids[:, None], object_ids,
                                           7:] = object_velocity.clone()
        ...
        # set into simulation
        view_ids = self._env_obj_ids_to_view_ids(env_ids, object_ids)
        self.root_physx_view.set_velocities(self.reshape_data_to_view(
            self._data.object_com_vel_w),
                                            indices=view_ids)
    ...
    def write_object_link_velocity_to_sim(
        ...
        # set into internal buffers
        self._data.object_link_vel_w[env_ids[:, None],
                                     object_ids] = object_velocity.clone()
        # update these buffers only if the user is using them. Otherwise this adds to overhead.
        if self._data._object_link_state_w.data is not None:
            self._data.object_link_state_w[env_ids[:, None], object_ids,
        ...
        object_com_velocity[..., :3] += torch.linalg.cross(
            ...
            math_utils.quat_apply(quat, com_pos_b),
            dim=-1)

        # write center of mass velocity to sim
        self.write_object_com_velocity_to_sim(object_com_velocity,
                                              env_ids=env_ids,
                                              object_ids=object_ids)
    ...
    def set_external_force_and_torque(
        ...
        if positions is not None:
            ...
            self._external_wrench_positions_b[env_ids[:, None],
                                              object_ids] = positions
        else:
            if self.uses_external_wrench_positions:
                self._external_wrench_positions_b[env_ids[:, None],
                                                  object_ids] = 0.0
    ...
    def reshape_view_to_data(self, data: torch.Tensor) -> torch.Tensor:
        ...
        """
        return torch.einsum(
            "ijk -> jik", data.reshape(self.num_objects, self.num_instances,
                                       -1))
    ...
    def _initialize_impl(self):
        ...
        for name, rigid_object_cfg in self.cfg.rigid_objects.items():
            ...
            if len(articulation_prims) != 0:
                if articulation_prims[0].GetAttribute(
                        "physxArticulation:articulationEnabled").Get():
                    raise RuntimeError(
            ...
            root_prim_path_expr = rigid_object_cfg.prim_path + root_prim_path[
                len(template_prim_path):]
        ...
        self._data = RigidObjectCollectionData(self.root_physx_view,
                                               self.num_objects, self.device)
    ...
    def _create_buffers(self):
        ...
        # constants
        self._ALL_ENV_INDICES = torch.arange(self.num_instances,
                                             dtype=torch.long,
                                             device=self.device)
        self._ALL_OBJ_INDICES = torch.arange(self.num_objects,
                                             dtype=torch.long,
                                             device=self.device)
    ...
    def _process_cfg(self):
        ...
        for rigid_object_cfg in self.cfg.rigid_objects.values():
            ...
            default_object_state = (torch.tensor(default_object_state,
                                                 dtype=torch.float,
    ...
    def _env_obj_ids_to_view_ids(
            ...
            object_ids: Sequence[int] | slice | torch.Tensor) -> torch.Tensor:
        ...
        return (object_ids.unsqueeze(1) * self.num_instances +
                env_ids).flatten()
    ...
    def _on_prim_deletion(self, prim_path: str) -> None:
        ...
        for prim_path_expr in self._prim_paths:
            result = re.match(pattern="^" + "/".join(
                prim_path_expr.split("/")[:prim_path.count("/") + 1]) + "$",
                              string=prim_path)
            if result:
                self._clear_callbacks()
                return
