...
class DeformableObject(AssetBase):
    ...
    def write_nodal_state_to_sim(self,
    ...
    def write_nodal_pos_to_sim(self,
        ...
        # set into simulation
        self.root_physx_view.set_sim_nodal_positions(self._data.nodal_pos_w,
                                                     indices=physx_env_ids)

    def write_nodal_velocity_to_sim(self,
        ...
        # set into simulation
        self.root_physx_view.set_sim_nodal_velocities(self._data.nodal_vel_w,
                                                      indices=physx_env_ids)

    def write_nodal_kinematic_target_to_sim(self,
        ...
        # set into simulation
        self.root_physx_view.set_sim_kinematic_targets(
            self._data.nodal_kinematic_target, indices=physx_env_ids)

    """
    Operations - Helper.
    """

    def transform_nodal_pos(self,
                            nodal_pos: torch.tensor,
        ...
        return math_utils.transform_points(nodal_pos, pos,
                                           quat) + mean_nodal_pos
    ...
    def _initialize_impl(self):
        ...
        root_prims = sim_utils.get_all_matching_child_prims(
            template_prim_path,
            predicate=lambda prim: prim.HasAPI(PhysxSchema.
        ...
        if root_prim.HasAPI(UsdShade.MaterialBindingAPI):
            ...
            material_paths = UsdShade.MaterialBindingAPI(
                root_prim).GetDirectBindingRel("physics").GetTargets()
        ...
        # -- object view
        self._root_physx_view = self._physics_sim_view.create_soft_body_view(
            root_prim_path_expr.replace(".*", "*"))

        # Return if the asset is not found
        if self._root_physx_view._backend is None:
            raise RuntimeError(
                f"Failed to create deformable body at: {self.cfg.prim_path}. Please check PhysX logs."
            )
        ...
        if material_prim is not None:
            ...
            if template_prim_path in material_prim_path:
                material_prim_path_expr = self.cfg.prim_path + material_prim_path[
                    len(template_prim_path):]
            else:
        ...
        if self._material_physx_view is not None:
            logger.info(
                f"Deformable material initialized at: {material_prim_path_expr}"
            )
            logger.info(
                f"Number of instances: {self._material_physx_view.count}")
        else:
            logger.info(
                "No deformable material found. Material properties will be set to default values."
            )
    ...
    def _create_buffers(self):
        ...
        self._data.default_nodal_state_w = torch.cat(
            (nodal_positions, nodal_velocities), dim=-1)
    ...
    def _debug_vis_callback(self, event):
        ...
        if num_enabled == 0:
            ...
            positions = self.data.nodal_kinematic_target[targets_enabled][
                ..., :3]
