...
class SurfaceGripper(AssetBase):
    ...
    def update(self, dt: float) -> None:
        ...
        self._gripper_state = torch.tensor(
            state_list, dtype=torch.float32, device=self._device) - 1.0
    ...
    def write_data_to_sim(self) -> None:
        ...
        indices = (torch.argwhere(
            torch.logical_or(self._gripper_command < -0.3,
                             self._gripper_command
                             > 0.3)).to(torch.int32).tolist())
        # Write to the SurfaceGripperView if there are any indices to write to
        if len(indices) > 0:
            self._gripper_view.apply_gripper_action(
                self._gripper_command.tolist(), indices)

    def set_grippers_command(self,
                             states: torch.Tensor,
                             indices: torch.Tensor | None = None) -> None:
    ...
    def _initialize_impl(self) -> None:
        ...
        if template_prim is None:
            raise RuntimeError(
                f"Failed to find prim for expression: '{self._cfg.prim_path}'."
            )
        ...
        gripper_prim_path_expr = self._cfg.prim_path + gripper_prim_path[
        ...
        self._gripper_view = GripperView(self._prim_expr, )
        ...
        logger.info(
            f"Surface gripper initialized at: {self._cfg.prim_path} with root '{gripper_prim_path_expr}'."
        )
    ...
    def _create_buffers(self) -> None:
        ...
        self._gripper_state = torch.zeros(self._num_envs,
                                          device=self._device,
                                          dtype=torch.float32)
        self._gripper_command = torch.zeros(self._num_envs,
                                            device=self._device,
                                            dtype=torch.float32)
        self._ALL_INDICES = torch.arange(self._num_envs,
                                         device=self._device,
                                         dtype=torch.long)

        self._max_grip_distance = torch.zeros(self._num_envs,
                                              device=self._device,
                                              dtype=torch.float32)
        self._coaxial_force_limit = torch.zeros(self._num_envs,
                                                device=self._device,
                                                dtype=torch.float32)
        self._shear_force_limit = torch.zeros(self._num_envs,
                                              device=self._device,
                                              dtype=torch.float32)
        self._retry_interval = torch.zeros(self._num_envs,
                                           device=self._device,
                                           dtype=torch.float32)
    ...
    def _process_cfg(self) -> None:
        ...
        try:
            max_grip_distance = gripper_prim.GetAttribute(
                "isaac:maxGripDistance").Get()
        ...
        try:
            coaxial_force_limit = gripper_prim.GetAttribute(
                "isaac:coaxialForceLimit").Get()
        except Exception as e:
            warnings.warn(
        ...
        try:
            shear_force_limit = gripper_prim.GetAttribute(
                "isaac:shearForceLimit").Get()
        except Exception as e:
            warnings.warn(
        ...
        try:
            retry_interval = gripper_prim.GetAttribute(
                "isaac:retryInterval").Get()
        except Exception as e:
            warnings.warn(
        ...
        self._max_grip_distance = self.parse_gripper_parameter(
            self._cfg.max_grip_distance, max_grip_distance)
        self._coaxial_force_limit = self.parse_gripper_parameter(
            self._cfg.coaxial_force_limit, coaxial_force_limit)
        self._shear_force_limit = self.parse_gripper_parameter(
            self._cfg.shear_force_limit, shear_force_limit)
        self._retry_interval = self.parse_gripper_parameter(
            self._cfg.retry_interval, retry_interval)

    """
    Helper functions.
    """

    def parse_gripper_parameter(self,
                                cfg_value: float | int | tuple | None,
                                default_value: float | int | tuple | None,
                                ndim: int = 0) -> torch.Tensor:
        ...
        if cfg_value is not None:
            if isinstance(cfg_value, (float, int)):
                ...
                if len(cfg_value) == ndim:
                    param[:] = torch.tensor(cfg_value,
                                            dtype=torch.float,
                                            device=self._device)
                else:
                    raise ValueError(
                        f"Invalid number of values for parameter. Got: {len(cfg_value)}\nExpected: {ndim}"
                    )
            else:
                raise TypeError(
                    f"Invalid type for parameter value: {type(cfg_value)}. " +
            ...
            if isinstance(default_value, (float, int)):
                ...
                assert len(
                    default_value
                ) == ndim, f"Expected {ndim} values, got {len(default_value)}"
                param[:] = torch.tensor(default_value,
                                        dtype=torch.float,
                                        device=self._device)
            else:
                raise TypeError(
                    f"Invalid type for default value: {type(default_value)}. "
                    + "Expected float or Tensor.")
        else:
            raise ValueError(
                "The parameter value is None and no default value is provided."
            )
