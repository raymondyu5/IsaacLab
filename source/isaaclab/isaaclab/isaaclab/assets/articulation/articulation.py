...
class Articulation(AssetBase):
    ...
    def reset(self, env_ids: Sequence[int] | None = None):
        ...
        reset_root_state = self.data.reset_root_state[env_ids].clone()

        self.write_root_pose_to_sim(reset_root_state[:, :7], env_ids=env_ids)
        self.write_root_velocity_to_sim(reset_root_state[:, 7:] * 0,
                                        env_ids=env_ids)
        ...
        if self.has_external_wrench:
            if self.uses_external_wrench_positions:
                self.root_physx_view.apply_forces_and_torques_at_position(
                    ...
                    position_data=self._external_wrench_positions_b.view(
                        -1, 3),
    ...
    def find_fixed_tendons(
        ...
        # find tendons
        return string_utils.resolve_matching_names(name_keys, tendon_subsets,
                                                   preserve_order)
    ...
    def write_root_com_state_to_sim(self,
                                    root_state: torch.Tensor,
                                    env_ids: Sequence[int] | None = None):
    ...
    def write_root_link_state_to_sim(self,
                                     root_state: torch.Tensor,
                                     env_ids: Sequence[int] | None = None):
        ...
        """
        self.write_root_link_pose_to_sim(root_state[:, :7], env_ids=env_ids)
        self.write_root_link_velocity_to_sim(root_state[:, 7:],
                                             env_ids=env_ids)
    ...
    def write_root_link_pose_to_sim(self,
                                    root_pose: torch.Tensor,
                                    env_ids: Sequence[int] | None = None):
        ...
        if self._data._root_link_state_w.data is not None:
            self._data.root_link_state_w[
                env_ids, :7] = self._data.root_link_pose_w[env_ids]
        if self._data._root_state_w.data is not None:
            self._data.root_state_w[
                env_ids, :7] = self._data.root_link_pose_w[env_ids]

        # convert root quaternion from wxyz to xyzw
        root_poses_xyzw = self._data.root_link_pose_w.clone()
        root_poses_xyzw[:, 3:] = math_utils.convert_quat(root_poses_xyzw[:,
                                                                         3:],
                                                         to="xyzw")
        ...
        # set into simulation
        self.root_physx_view.set_root_transforms(root_poses_xyzw,
                                                 indices=physx_env_ids)

    def write_root_com_pose_to_sim(self,
                                   root_pose: torch.Tensor,
                                   env_ids: Sequence[int] | None = None):
        ...
        if self._data._root_com_state_w.data is not None:
            self._data.root_com_state_w[
                local_env_ids, :7] = self._data.root_com_pose_w[local_env_ids]
        ...
        self.write_root_link_pose_to_sim(root_pose=root_link_pose,
                                         env_ids=env_ids)
    ...
    def write_root_velocity_to_sim(self,
        ...
        Args:
            ...
            env_ids: Environment indices. If None, then all indices are used.
        """
        self.write_root_com_velocity_to_sim(root_velocity=root_velocity,
                                            env_ids=env_ids)

    def write_root_com_velocity_to_sim(self,
                                       root_velocity: torch.Tensor,
                                       env_ids: Sequence[int] | None = None):
        """Set the root center of mass velocity over selected environment indices into the simulation.
        ...
        if self._data._root_com_state_w.data is not None:
            self._data.root_com_state_w[
                env_ids, 7:] = self._data.root_com_vel_w[env_ids]
        if self._data._root_state_w.data is not None:
            self._data.root_state_w[env_ids,
                                    7:] = self._data.root_com_vel_w[env_ids]
        # make the acceleration zero to prevent reporting old values
        self._data.body_acc_w[env_ids] = 0.0

        # set into simulation
        self.root_physx_view.set_root_velocities(self._data.root_com_vel_w,
                                                 indices=physx_env_ids)

    def write_root_link_velocity_to_sim(self,
                                        root_velocity: torch.Tensor,
                                        env_ids: Sequence[int] | None = None):
        ...
        if self._data._root_link_state_w.data is not None:
            self._data.root_link_state_w[
                local_env_ids, 7:] = self._data.root_link_vel_w[local_env_ids]
        ...
        root_com_velocity[:, :3] += torch.linalg.cross(root_com_velocity[:,
                                                                         3:],
        ...
        self.write_root_com_velocity_to_sim(root_velocity=root_com_velocity,
                                            env_ids=env_ids)
    ...
    def write_joint_state_to_sim(
        ...
        """
        # set into simulation
        self.write_joint_position_to_sim(position,
                                         joint_ids=joint_ids,
                                         env_ids=env_ids)
        self.write_joint_velocity_to_sim(velocity,
                                         joint_ids=joint_ids,
                                         env_ids=env_ids)
    ...
    def write_joint_position_to_sim(
        ...
        self._data._body_com_state_w.timestamp = -1.0
        # set into simulation
        self.root_physx_view.set_dof_positions(self._data.joint_pos,
                                               indices=physx_env_ids)
    ...
    def write_joint_velocity_to_sim(
        ...
        # set into simulation
        self.root_physx_view.set_dof_velocities(self._data.joint_vel,
                                                indices=physx_env_ids)
    ...
    def write_joint_damping_to_sim(
        ...
        # set into simulation
        self.root_physx_view.set_dof_dampings(self._data.joint_damping.cpu(),
                                              indices=physx_env_ids.cpu())
    ...
    def write_joint_position_limit_to_sim(
        ...
        if torch.any((self._data.default_joint_pos[env_ids,
                                                   joint_ids] < limits[..., 0])
                     | (self._data.default_joint_pos[env_ids,
                                                     joint_ids] > limits[...,
                                                                         1])):
        ...
        # set into simulation
        self.root_physx_view.set_dof_limits(self._data.joint_pos_limits.cpu(),
                                            indices=physx_env_ids.cpu())
        ...
        joint_pos_mean = (self._data.joint_pos_limits[..., 0] +
                          self._data.joint_pos_limits[..., 1]) / 2
        joint_pos_range = self._data.joint_pos_limits[
            ..., 1] - self._data.joint_pos_limits[..., 0]
        soft_limit_factor = self.cfg.soft_joint_pos_limit_factor
        # add to data
        self._data.soft_joint_pos_limits[
            ...,
            0] = joint_pos_mean - 0.5 * joint_pos_range * soft_limit_factor
        self._data.soft_joint_pos_limits[
            ...,
            1] = joint_pos_mean + 0.5 * joint_pos_range * soft_limit_factor
    ...
    def write_joint_velocity_limit_to_sim(
        ...
        self.root_physx_view.set_dof_max_velocities(
            self._data.joint_vel_limits.cpu(), indices=physx_env_ids.cpu())
    ...
    def write_joint_effort_limit_to_sim(
        ...
        self.root_physx_view.set_dof_max_forces(
            self._data.joint_effort_limits.cpu(), indices=physx_env_ids.cpu())
    ...
    def write_joint_armature_to_sim(
        ...
        # set into simulation
        self.root_physx_view.set_dof_armatures(self._data.joint_armature.cpu(),
                                               indices=physx_env_ids.cpu())
    ...
    def write_joint_friction_coefficient_to_sim(
        ...
        if env_ids != slice(None) and joint_ids != slice(None):
            env_ids = env_ids[:, None]
        # set into internal buffers
        self._data.joint_friction_coeff[env_ids,
                                        joint_ids] = joint_friction_coeff
        ...
        if joint_dynamic_friction_coeff is not None:
            self._data.joint_dynamic_friction_coeff[
                env_ids, joint_ids] = joint_dynamic_friction_coeff
        if joint_viscous_friction_coeff is not None:
            self._data.joint_viscous_friction_coeff[
                env_ids, joint_ids] = joint_viscous_friction_coeff
        ...
        if int(get_version()[2]) < 5:
            self.root_physx_view.set_dof_friction_coefficients(
                self._data.joint_friction_coeff.cpu(),
                indices=physx_envs_ids_cpu)
        else:
            friction_props = self.root_physx_view.get_dof_friction_properties()
            friction_props[physx_envs_ids_cpu, :,
                           0] = self._data.joint_friction_coeff[
                               physx_envs_ids_cpu, :].cpu()

            # only set dynamic and viscous friction if provided
            if joint_dynamic_friction_coeff is not None:
                friction_props[physx_envs_ids_cpu, :,
                               1] = self._data.joint_dynamic_friction_coeff[
                                   physx_envs_ids_cpu, :].cpu()

            # only set viscous friction if provided
            if joint_viscous_friction_coeff is not None:
                friction_props[physx_envs_ids_cpu, :,
                               2] = self._data.joint_viscous_friction_coeff[
                                   physx_envs_ids_cpu, :].cpu()

            self.root_physx_view.set_dof_friction_properties(
                friction_props, indices=physx_envs_ids_cpu)
    ...
    def write_joint_dynamic_friction_coefficient_to_sim(
        ...
        if int(get_version()[2]) < 5:
            omni.log.warn(
                "Setting joint dynamic friction coefficients are not supported in Isaac Sim < 5.0"
            )
            return
        # resolve indices
        physx_env_ids = env_ids
        ...
        # set into simulation
        friction_props = self.root_physx_view.get_dof_friction_properties()
        friction_props[physx_env_ids.cpu(), :,
                       1] = self._data.joint_dynamic_friction_coeff[
                           physx_env_ids, :].cpu()
        self.root_physx_view.set_dof_friction_properties(
            friction_props, indices=physx_env_ids.cpu())
    ...
    def set_external_force_and_torque(
        ...
        # -- body_ids
        if body_ids is None:
            body_ids = torch.arange(self.num_bodies,
                                    dtype=torch.long,
                                    device=self.device)
        elif isinstance(body_ids, slice):
            body_ids = torch.arange(self.num_bodies,
                                    dtype=torch.long,
                                    device=self.device)[body_ids]
        elif not isinstance(body_ids, torch.Tensor):
            body_ids = torch.tensor(body_ids,
                                    dtype=torch.long,
                                    device=self.device)
        ...
        indices = body_ids.repeat(len(env_ids),
                                  1) + env_ids.unsqueeze(1) * self.num_bodies
        ...
        if positions is not None:
            ...
            self._external_wrench_positions_b.flatten(
                0, 1)[indices] = positions.flatten(0, 1)
        else:
    ...
    def set_joint_position_target(self,
                                  target: torch.Tensor,
                                  joint_ids: Sequence[int] | slice
                                  | None = None,
                                  env_ids: Sequence[int] | None = None):
    ...
    def set_joint_effort_target(self,
                                target: torch.Tensor,
                                joint_ids: Sequence[int] | slice | None = None,
                                env_ids: Sequence[int] | None = None):
    ...
    def set_fixed_tendon_stiffness(
        ...
        if env_ids != slice(None) and fixed_tendon_ids != slice(None):
            env_ids = env_ids[:, None]
        # set stiffness
        self._data.fixed_tendon_stiffness[env_ids,
                                          fixed_tendon_ids] = stiffness
    ...
    def set_fixed_tendon_limit_stiffness(
        ...
        self._data.fixed_tendon_limit_stiffness[
            env_ids, fixed_tendon_ids] = limit_stiffness
    ...
    def set_fixed_tendon_rest_length(
        ...
        if env_ids != slice(None) and fixed_tendon_ids != slice(None):
            env_ids = env_ids[:, None]
        # set rest_length
        self._data.fixed_tendon_rest_length[env_ids,
                                            fixed_tendon_ids] = rest_length
    ...
    def set_spatial_tendon_stiffness(
        ...
        if env_ids != slice(None) and spatial_tendon_ids != slice(None):
            env_ids = env_ids[:, None]
        # set stiffness
        self._data.spatial_tendon_stiffness[env_ids,
                                            spatial_tendon_ids] = stiffness
    ...
    def set_spatial_tendon_damping(
        ...
        if env_ids != slice(None) and spatial_tendon_ids != slice(None):
            env_ids = env_ids[:, None]
        # set damping
        self._data.spatial_tendon_damping[env_ids,
                                          spatial_tendon_ids] = damping
    ...
    def set_spatial_tendon_limit_stiffness(
        ...
        self._data.spatial_tendon_limit_stiffness[
            env_ids, spatial_tendon_ids] = limit_stiffness
    ...
    def _initialize_impl(self):
        ...
        if self.cfg.articulation_root_prim_path is not None:
            ...
            first_env_matching_prim = sim_utils.find_first_matching_prim(
                self.cfg.prim_path)
            if first_env_matching_prim is None:
                raise RuntimeError(
                    f"Failed to find prim for expression: '{self.cfg.prim_path}'."
                )
            first_env_matching_prim_path = first_env_matching_prim.GetPath(
            ).pathString
            ...
            first_env_root_prims = sim_utils.get_all_matching_child_prims(
                ...
                predicate=lambda prim: prim.HasAPI(UsdPhysics.
                                                   ArticulationRootAPI),
            ...
            first_env_root_prim_path = first_env_root_prims[0].GetPath(
            ).pathString
            root_prim_path_relative_to_prim_path = first_env_root_prim_path[
                len(first_env_matching_prim_path):]
        ...
        if self._root_physx_view._backend is None:
            raise RuntimeError(
                f"Failed to create articulation at: {root_prim_path_expr}. Please check PhysX logs."
            )
        ...
        if int(get_version()[2]) < 5:
            ...
            self._root_physx_view.get_spatial_tendon_stiffnesses = lambda: torch.empty(
                0, device=self.device)
            self._root_physx_view.get_spatial_tendon_dampings = lambda: torch.empty(
                0, device=self.device)
            self._root_physx_view.get_spatial_tendon_limit_stiffnesses = lambda: torch.empty(
                0, device=self.device)
            self._root_physx_view.get_spatial_tendon_offsets = lambda: torch.empty(
                0, device=self.device)
    ...
    def _create_buffers(self):
        # constants
        self._ALL_INDICES = torch.arange(self.num_instances,
                                         dtype=torch.long,
                                         device=self.device)
        ...
        self._external_wrench_positions_b = torch.zeros_like(
        ...
        # -- joint properties
        self._data.default_joint_pos_limits = self.root_physx_view.get_dof_limits(
        ).to(self.device).clone()
        self._data.default_joint_stiffness = self.root_physx_view.get_dof_stiffnesses(
        ).to(self.device).clone()
        self._data.default_joint_damping = self.root_physx_view.get_dof_dampings(
        ).to(self.device).clone()
        self._data.default_joint_armature = self.root_physx_view.get_dof_armatures(
        ).to(self.device).clone()
        if int(get_version()[2]) < 5:
            self._data.default_joint_friction_coeff = (
                self.root_physx_view.get_dof_friction_coefficients().to(
                    self.device).clone())
            self._data.default_joint_dynamic_friction_coeff = torch.zeros_like(
                self._data.default_joint_friction_coeff)
            self._data.default_joint_viscous_friction_coeff = torch.zeros_like(
                self._data.default_joint_friction_coeff)
        else:
            friction_props = self.root_physx_view.get_dof_friction_properties()
            self._data.default_joint_friction_coeff = friction_props[:, :,
                                                                     0].to(
                                                                         self.
                                                                         device
                                                                     ).clone()
            self._data.default_joint_dynamic_friction_coeff = friction_props[:, :, 1].to(
                self.device).clone()
            self._data.default_joint_viscous_friction_coeff = friction_props[:, :, 2].to(
                self.device).clone()

        self._data.joint_pos_limits = self._data.default_joint_pos_limits.clone(
        )
        self._data.joint_vel_limits = self.root_physx_view.get_dof_max_velocities(
        ).to(self.device).clone()
        self._data.joint_effort_limits = self.root_physx_view.get_dof_max_forces(
        ).to(self.device).clone()
        ...
        self._data.joint_pos_target = torch.zeros(self.num_instances,
                                                  self.num_joints,
                                                  device=self.device)
        self._data.joint_vel_target = torch.zeros_like(
            self._data.joint_pos_target)
        self._data.joint_effort_target = torch.zeros_like(
            self._data.joint_pos_target)
        # -- joint commands (sent to the simulation after actuator processing)
        self._joint_pos_target_sim = torch.zeros_like(
            self._data.joint_pos_target)
        self._joint_vel_target_sim = torch.zeros_like(
            self._data.joint_pos_target)
        self._joint_effort_target_sim = torch.zeros_like(
            self._data.joint_pos_target)

        # -- computed joint efforts from the actuator models
        self._data.computed_torque = torch.zeros_like(
            self._data.joint_pos_target)
        self._data.applied_torque = torch.zeros_like(
            self._data.joint_pos_target)

        # -- other data that are filled based on explicit actuator models
        self._data.soft_joint_vel_limits = torch.zeros(self.num_instances,
                                                       self.num_joints,
                                                       device=self.device)
        self._data.gear_ratio = torch.ones(self.num_instances,
                                           self.num_joints,
                                           device=self.device)
        ...
        soft_limit_factor = self.cfg.soft_joint_pos_limit_factor
        # add to data
        self._data.soft_joint_pos_limits = torch.zeros(self.num_instances,
                                                       self.num_joints,
                                                       2,
                                                       device=self.device)
        self._data.soft_joint_pos_limits[
        ...
        self._data.reset_joint_pos = self._data.joint_pos.clone()
        if self.cfg.articulation_cfg:
            if "ee_name" in self.cfg.articulation_cfg.keys():  # robot setting

                index, _ = self.find_bodies(
                    self.cfg.articulation_cfg["ee_name"])

                ee_pose_w = torch.squeeze(self.data.body_state_w[:, index, :7],
                                          dim=1)
                root_pose_w = self.data.root_state_w[:, :7]

                init_ee_euler = torch.as_tensor(
                    self.cfg.articulation_cfg["init_ee_euler"])[None].to(
                        self.device).repeat_interleave(self.num_instances, 0)

                transformed_quat = math_utils.quat_from_euler_xyz(
                    init_ee_euler[:, 0], init_ee_euler[:, 1], init_ee_euler[:,
                                                                            2])
                root_pose_w[:,
                            3:7] = math_utils.quat_mul(transformed_quat,
                                                       root_pose_w[:, 3:7])
                ...
                ee_quat_b[
                    :,
                ] = torch.as_tensor([0., 1.0, 0.0, 0.0]).to(self.device)
                self._data.default_ee_pose = torch.cat([ee_pose_b, ee_quat_b],
                                                       dim=1)
            elif "name" in self.cfg.articulation_cfg.keys():
                joint_limits = self.root_physx_view.get_dof_limits()
                for index, name in enumerate(self.joint_names):
                    if "prismatic" in name:
                        joint_limits[:, index] *= self.cfg.articulation_cfg[
                            "scale"][0]
                self.root_physx_view.set_dof_limits(joint_limits.cpu(),
    ...
    def _process_cfg(self):
        ...
        # -- joint state
        self._data.default_joint_pos = torch.zeros(self.num_instances,
                                                   self.num_joints,
                                                   device=self.device)
        self._data.default_joint_vel = torch.zeros_like(
        ...
        self._data.reset_root_state = default_root_state.repeat(
            self.num_instances, 1)
    ...
    def _process_actuators_cfg(self):
        ...
        for actuator_name, actuator_cfg in self.cfg.actuators.items():
            ...
            joint_ids, joint_names = self.find_joints(
            ...
            actuator: ActuatorBase = actuator_cfg.class_type(
                ...
                dynamic_friction=self._data.
                default_joint_dynamic_friction_coeff[:, joint_ids],
                viscous_friction=self._data.
                default_joint_viscous_friction_coeff[:, joint_ids],
            ...
            if isinstance(actuator, ImplicitActuator):
                ...
                self.write_joint_stiffness_to_sim(
                    0.0, joint_ids=actuator.joint_indices)
                self.write_joint_damping_to_sim(
                    0.0, joint_ids=actuator.joint_indices)

            # Set common properties into the simulation
            self.write_joint_effort_limit_to_sim(
                actuator.effort_limit_sim, joint_ids=actuator.joint_indices)
            self.write_joint_velocity_limit_to_sim(
                actuator.velocity_limit_sim, joint_ids=actuator.joint_indices)
            ...
            self._data.default_joint_stiffness[:, actuator.
                                               joint_indices] = actuator.stiffness
            self._data.default_joint_damping[:, actuator.
                                             joint_indices] = actuator.damping
            self._data.default_joint_armature[:, actuator.
                                              joint_indices] = actuator.armature
            self._data.default_joint_friction_coeff[:, actuator.
                                                    joint_indices] = actuator.friction
            if int(get_version()[2]) >= 5:
                self._data.default_joint_dynamic_friction_coeff[:, actuator.
                                                                joint_indices] = actuator.dynamic_friction
                self._data.default_joint_viscous_friction_coeff[:, actuator.
                                                                joint_indices] = actuator.viscous_friction

        # perform some sanity checks to ensure actuators are prepared correctly
        total_act_joints = sum(actuator.num_joints
                               for actuator in self.actuators.values())
        ...
        if self.cfg.actuator_value_resolution_debug_print:
            t = PrettyTable([
                "Group", "Property", "Name", "ID", "USD Value",
                "ActutatorCfg Value", "Applied"
            ])
            for actuator_group, actuator in self.actuators.items():
                ...
                for property, resolution_details in actuator.joint_property_resolution_table.items(
                    ...
                    for prop_idx, resolution_detail in enumerate(
                            resolution_details):
    ...
    def _process_tendons(self):
        ...
        if self.num_fixed_tendons > 0 or self.num_spatial_tendons > 0:
            ...
            for j in range(self.num_joints):
                ...
                elif joint.GetPrim().HasAPI(
                        PhysxSchema.PhysxTendonAttachmentRootAPI
                ) or joint.GetPrim().HasAPI(
            ...
            self._data.default_fixed_tendon_stiffness = self.root_physx_view.get_fixed_tendon_stiffnesses(
            ).clone()
            self._data.default_fixed_tendon_damping = self.root_physx_view.get_fixed_tendon_dampings(
            ).clone()
            self._data.default_fixed_tendon_limit_stiffness = (
                self.root_physx_view.get_fixed_tendon_limit_stiffnesses(
                ).clone())
            self._data.default_fixed_tendon_pos_limits = self.root_physx_view.get_fixed_tendon_limits(
            ).clone()
            self._data.default_fixed_tendon_rest_length = self.root_physx_view.get_fixed_tendon_rest_lengths(
            ).clone()
            self._data.default_fixed_tendon_offset = self.root_physx_view.get_fixed_tendon_offsets(
            ).clone()
            self._data.default_spatial_tendon_stiffness = self.root_physx_view.get_spatial_tendon_stiffnesses(
            ).clone()
            self._data.default_spatial_tendon_damping = self.root_physx_view.get_spatial_tendon_dampings(
            ).clone()
            self._data.default_spatial_tendon_limit_stiffness = (
                self.root_physx_view.get_spatial_tendon_limit_stiffnesses(
                ).clone())
            self._data.default_spatial_tendon_offset = self.root_physx_view.get_spatial_tendon_offsets(
            ).clone()
    ...
    def _apply_actuator_model(self):
        ...
        for actuator in self.actuators.values():
            ...
            control_action = ArticulationActions(
                joint_positions=self._data.joint_pos_target[:, actuator.
                                                            joint_indices],
                joint_velocities=self._data.joint_vel_target[:, actuator.
                                                             joint_indices],
                joint_efforts=self._data.joint_effort_target[:, actuator.
                                                             joint_indices],
            ...
            if control_action.joint_positions is not None:
                self._joint_pos_target_sim[:, actuator.
                                           joint_indices] = control_action.joint_positions
            if control_action.joint_velocities is not None:
                self._joint_vel_target_sim[:, actuator.
                                           joint_indices] = control_action.joint_velocities
            if control_action.joint_efforts is not None:
                self._joint_effort_target_sim[:, actuator.
                                              joint_indices] = control_action.joint_efforts
            # update state of the actuator model
            # -- torques
            self._data.computed_torque[:, actuator.
                                       joint_indices] = actuator.computed_effort
            self._data.applied_torque[:, actuator.
                                      joint_indices] = actuator.applied_effort
            # -- actuator data
            self._data.soft_joint_vel_limits[:, actuator.
                                             joint_indices] = actuator.velocity_limit
            # TODO: find a cleaner way to handle gear ratio. Only needed for variable gear ratio actuators.
            if hasattr(actuator, "gear_ratio"):
                self._data.gear_ratio[:, actuator.
                                      joint_indices] = actuator.gear_ratio
    ...
    def _validate_cfg(self):
        ...
        joint_pos_limits = self.root_physx_view.get_dof_limits()[0].to(
            self.device)
        out_of_range = self._data.default_joint_pos[0] < joint_pos_limits[:, 0]
        out_of_range |= self._data.default_joint_pos[0] > joint_pos_limits[:,
                                                                           1]
        violated_indices = torch.nonzero(out_of_range,
        ...
        joint_max_vel = self.root_physx_view.get_dof_max_velocities()[0].to(
            self.device)
        out_of_range = torch.abs(
            self._data.default_joint_vel[0]) > joint_max_vel
        violated_indices = torch.nonzero(out_of_range,
                                         as_tuple=False).squeeze(-1)
        if len(violated_indices) > 0:
            # prepare message for violated joints
    ...
    def _log_articulation_info(self):
        ...
        if int(get_version()[2]) < 5:
            static_frictions = self.root_physx_view.get_dof_friction_coefficients(
            )[0].tolist()
        else:
            friction_props = self.root_physx_view.get_dof_friction_properties()
        ...
        velocity_limits = self.root_physx_view.get_dof_max_velocities(
        )[0].tolist()
        ...
        joint_table.custom_format[
            "Position Limits"] = lambda f, v: f"[{v[0]:.3f}, {v[1]:.3f}]"
        ...
        if self.num_fixed_tendons > 0:
            ...
            ft_dampings = self.root_physx_view.get_fixed_tendon_dampings(
            )[0].tolist()
            # -- limits
            ft_limit_stiffnesses = self.root_physx_view.get_fixed_tendon_limit_stiffnesses(
            )[0].tolist()
            ft_limits = self.root_physx_view.get_fixed_tendon_limits(
            )[0].tolist()
            ft_rest_lengths = self.root_physx_view.get_fixed_tendon_rest_lengths(
            )[0].tolist()
            ft_offsets = self.root_physx_view.get_fixed_tendon_offsets(
            )[0].tolist()
            # create table for term information
            ...
            joint_table.custom_format[
                "Limits"] = lambda f, v: f"[{v[0]:.3f}, {v[1]:.3f}]"
            # add info on each term
        ...
        if self.num_spatial_tendons > 0:
            # -- gains
            st_stiffnesses = self.root_physx_view.get_spatial_tendon_stiffnesses(
            )[0].tolist()
            st_dampings = self.root_physx_view.get_spatial_tendon_dampings(
            )[0].tolist()
            # -- limits
            st_limit_stiffnesses = self.root_physx_view.get_spatial_tendon_limit_stiffnesses(
            )[0].tolist()
            st_offsets = self.root_physx_view.get_spatial_tendon_offsets(
            )[0].tolist()
            # create table for term information
            tendon_table = PrettyTable()
    ...
    def set_fixed_tendon_limit(
        ...
        self.set_fixed_tendon_position_limit(limit,
