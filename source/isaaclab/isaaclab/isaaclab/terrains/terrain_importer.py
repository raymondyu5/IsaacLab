...
class TerrainImporter:
    ...
    def __init__(self, cfg: TerrainImporterCfg):
        ...
        # store inputs
        self.cfg = cfg
        self.device = sim_utils.SimulationContext.instance(
        ).device  # type: ignore
        ...
        if self.cfg.terrain_type == "generator":
            ...
            raise ValueError(
                f"Terrain type '{self.cfg.terrain_type}' not available.")
    ...
    def configure_env_origins(self,
    ...
    def update_env_origins(self, env_ids: torch.Tensor, move_up: torch.Tensor,
    ...
    def _compute_env_origins_curriculum(self, num_envs: int,
                                        origins: torch.Tensor) -> torch.Tensor:
        ...
        self.terrain_levels = torch.randint(0,
                                            max_init_level + 1, (num_envs, ),
                                            device=self.device)
        self.terrain_types = torch.div(torch.arange(num_envs,
                                                    device=self.device),
                                       (num_envs / num_cols),
    ...
    def _compute_env_origins_grid(self, num_envs: int,
                                  env_spacing: float) -> torch.Tensor:
        ...
        ii, jj = torch.meshgrid(torch.arange(num_rows, device=self.device),
                                torch.arange(num_cols, device=self.device),
                                indexing="ij")
        env_origins[:, 0] = -(ii.flatten()[:num_envs] -
                              (num_rows - 1) / 2) * env_spacing
        env_origins[:, 1] = (jj.flatten()[:num_envs] -
                             (num_cols - 1) / 2) * env_spacing
        env_origins[:, 2] = 0.0
        return env_origins
