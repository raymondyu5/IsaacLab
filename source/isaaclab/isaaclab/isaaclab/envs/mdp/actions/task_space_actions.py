...
class DifferentialInverseKinematicsAction(ActionTerm):
    ...
    def __init__(self, cfg: actions_cfg.DifferentialInverseKinematicsActionCfg,
                 env: ManagerBasedEnv):
        ...
        self._ik_controller = DifferentialIKController(cfg=self.cfg.controller,
                                                       num_envs=self.num_envs,
                                                       device=self.device)

        # create tensors for raw and processed actions
        self._raw_actions = torch.zeros(self.num_envs,
                                        self.action_dim,
                                        device=self.device)
        self._processed_actions = torch.zeros_like(self.raw_actions)

        # save the scale as tensors
        self._scale = torch.zeros((self.num_envs, self.action_dim),
                                  device=self.device)
        self._scale[:] = torch.tensor(self.cfg.scale, device=self.device)

        # convert the fixed offsets to torch tensors of batched shape
        if self.cfg.body_offset is not None:
            self._offset_pos = torch.tensor(self.cfg.body_offset.pos,
                                            device=self.device).repeat(
                                                self.num_envs, 1)
            self._offset_rot = torch.tensor(self.cfg.body_offset.rot,
                                            device=self.device).repeat(
                                                self.num_envs, 1)
        else:
        ...
        if self.cfg.clip is not None:
            if isinstance(cfg.clip, dict):
                self._clip = torch.tensor(
                    [[-float("inf"), float("inf")]],
                    device=self.device).repeat(self.num_envs, self.action_dim,
                                               1)
                index_list, _, value_list = string_utils.resolve_matching_names_values(
                    self.cfg.clip, self._joint_names)
                self._clip[:, index_list] = torch.tensor(value_list,
                                                         device=self.device)
            else:
                raise ValueError(
    ...
    def jacobian_b(self) -> torch.Tensor:
        ...
        base_rot_matrix = math_utils.matrix_from_quat(
            math_utils.quat_inv(base_rot))
    ...
    def IO_descriptor(self) -> GenericActionIODescriptor:
        ...
        self._IO_descriptor.extras[
            "controller_cfg"] = self.cfg.controller.__dict__
        self._IO_descriptor.extras[
            "body_offset"] = self.cfg.body_offset.__dict__
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        self._ik_controller.set_command(self._processed_actions, ee_pos_curr,
                                        ee_quat_curr)
    ...
    def _compute_frame_jacobian(self):
        ...
        if self.cfg.body_offset is not None:
            ...
            jacobian[:, 3:, :] = torch.bmm(
                math_utils.matrix_from_quat(self._offset_rot), jacobian[:,
                                                                        3:, :])
...
class OperationalSpaceControllerAction(ActionTerm):
    ...
    def __init__(self, cfg: actions_cfg.OperationalSpaceControllerActionCfg,
                 env: ManagerBasedEnv):
        # initialize the action term
        super().__init__(cfg, env)
        ...
        if ("wrench_abs" in self.cfg.controller_cfg.target_types
        ...
        if self.cfg.task_frame_rel_path is not None:
            ...
            self._task_frame_transformer = FrameTransformer(
                task_frame_transformer_cfg)
            ...
            self._task_frame_pose_b = torch.zeros(self.num_envs,
                                                  7,
                                                  device=self.device)
        else:
        ...
        self._osc = OperationalSpaceController(cfg=self.cfg.controller_cfg,
                                               num_envs=self.num_envs,
                                               device=self.device)
        ...
        self._jacobian_b = torch.zeros(self.num_envs,
                                       6,
                                       self._num_DoF,
                                       device=self.device)
        self._mass_matrix = torch.zeros(self.num_envs,
                                        self._num_DoF,
                                        self._num_DoF,
                                        device=self.device)
        self._gravity = torch.zeros(self.num_envs,
                                    self._num_DoF,
                                    device=self.device)
        ...
        self._ee_pose_b_no_offset = torch.zeros(
            self.num_envs, 7,
            device=self.device)  # The original ee without offset
        ...
        self._ee_force_w = torch.zeros(
            self.num_envs, 3,
            device=self.device)  # Only the forces are used for now
        self._ee_force_b = torch.zeros(
            self.num_envs, 3,
            device=self.device)  # Only the forces are used for now

        # create tensors for the joint states
        self._joint_pos = torch.zeros(self.num_envs,
                                      self._num_DoF,
                                      device=self.device)
        self._joint_vel = torch.zeros(self.num_envs,
                                      self._num_DoF,
                                      device=self.device)

        # create the joint effort tensor
        self._joint_efforts = torch.zeros(self.num_envs,
                                          self._num_DoF,
                                          device=self.device)

        # save the scale as tensors
        self._position_scale = torch.tensor(self.cfg.position_scale,
                                            device=self.device)
        self._orientation_scale = torch.tensor(self.cfg.orientation_scale,
                                               device=self.device)
        self._wrench_scale = torch.tensor(self.cfg.wrench_scale,
                                          device=self.device)
        self._stiffness_scale = torch.tensor(self.cfg.stiffness_scale,
                                             device=self.device)
        self._damping_ratio_scale = torch.tensor(self.cfg.damping_ratio_scale,
                                                 device=self.device)
    ...
    def IO_descriptor(self) -> GenericActionIODescriptor:
        ...
        if self.cfg.clip is not None:
            ...
            self._IO_descriptor.clip = None
        self._IO_descriptor.extras[
            "controller_cfg"] = self.cfg.controller_cfg.__dict__
        self._IO_descriptor.extras[
            "body_offset"] = self.cfg.body_offset.__dict__
        return self._IO_descriptor
    ...
    def apply_actions(self):
        ...
        self._asset.set_joint_effort_target(self._joint_efforts,
    ...
    def _first_RigidObject_child_path(self):
        ...
        rigid_child_prim_path = self._asset.cfg.prim_path + "/" + rigid_child_prim_path.split(
            "/")[-1]
    ...
    def _resolve_command_indexes(self):
        ...
        if (self.cfg.controller_cfg.impedance_mode == "variable_kp"
    ...
    def _compute_dynamic_quantities(self):
        ...
        self._gravity[:] = self._asset.root_physx_view.get_gravity_compensation_forces(
        )[:, self._joint_ids]
    ...
    def _compute_ee_jacobian(self):
        ...
        if self.cfg.body_offset is not None:
            ...
            self._jacobian_b[:, 0:3, :] += torch.bmm(
                -math_utils.skew_symmetric_matrix(self._offset_pos),
                self._jacobian_b[:, 3:, :])  # type: ignore
            # -- rotational part
            # w_link = R_link_ee @ w_ee
    ...
    def _compute_ee_pose(self):
        ...
        # Obtain quantities from simulation
        self._ee_pose_w[:,
                        0:3] = self._asset.data.body_pos_w[:,
                                                           self._ee_body_idx]
        self._ee_pose_w[:,
                        3:7] = self._asset.data.body_quat_w[:,
                                                            self._ee_body_idx]
        # Compute the pose of the ee body in the root frame
        self._ee_pose_b_no_offset[:, 0:
                                  3], self._ee_pose_b_no_offset[:, 3:7] = math_utils.subtract_frame_transforms(
        ...
        if self.cfg.body_offset is not None:
            self._ee_pose_b[:, 0:
                            3], self._ee_pose_b[:, 3:
                                                7] = math_utils.combine_frame_transforms(
                                                    self.
                                                    _ee_pose_b_no_offset[:,
                                                                         0:3],
                                                    self.
                                                    _ee_pose_b_no_offset[:,
                                                                         3:7],
    ...
    def _compute_ee_velocity(self):
        ...
        if self.cfg.body_offset is not None:
            ...
            self._ee_vel_b[:, :3] += torch.cross(self._ee_vel_b[:, 3:],
                                                 r_offset_b,
                                                 dim=-1)
    ...
    def _compute_ee_force(self):
        ...
        if self._contact_sensor is not None:
            ...
            self._ee_force_w[:] = self._contact_sensor.data.net_forces_w[:,
                                                                         0, :]  # type: ignore
    ...
    def _compute_task_frame_pose(self):
        ...
        if self._task_frame_transformer is not None and self._task_frame_pose_b is not None:
            ...
            self._task_frame_pose_b[:, :
                                    3], self._task_frame_pose_b[:, 3:] = math_utils.subtract_frame_transforms(
                                        self._asset.data.root_pos_w,
                                        self._asset.data.root_quat_w,
                                        self._task_frame_transformer.data.
                                        target_pos_w[:, 0, :],
                                        self._task_frame_transformer.data.
                                        target_quat_w[:, 0, :],
                                    )
    ...
    def _preprocess_actions(self, actions: torch.Tensor):
        ...
        if self._pose_abs_idx is not None:
            self._processed_actions[:, self._pose_abs_idx:self._pose_abs_idx +
                                    3] *= self._position_scale
            self._processed_actions[:,
                                    self._pose_abs_idx + 3:self._pose_abs_idx +
                                    7] *= self._orientation_scale
        if self._pose_rel_idx is not None:
            self._processed_actions[:, self._pose_rel_idx:self._pose_rel_idx +
                                    3] *= self._position_scale
            self._processed_actions[:,
                                    self._pose_rel_idx + 3:self._pose_rel_idx +
                                    6] *= self._orientation_scale
        if self._wrench_abs_idx is not None:
            self._processed_actions[:,
                                    self._wrench_abs_idx:self._wrench_abs_idx +
                                    6] *= self._wrench_scale
        if self._stiffness_idx is not None:
            self._processed_actions[:,
                                    self._stiffness_idx:self._stiffness_idx +
                                    6] *= self._stiffness_scale
            self._processed_actions[:,
                                    self._stiffness_idx:self._stiffness_idx +
                                    6] = torch.clamp(
                                        self.
                                        _processed_actions[:, self.
                                                           _stiffness_idx:self.
                                                           _stiffness_idx + 6],
                                        min=self.cfg.controller_cfg.
                                        motion_stiffness_limits_task[0],
                                        max=self.cfg.controller_cfg.
                                        motion_stiffness_limits_task[1],
                                    )
        if self._damping_ratio_idx is not None:
            self._processed_actions[:, self._damping_ratio_idx:self.
                                    _damping_ratio_idx +
                                    6] *= self._damping_ratio_scale
            self._processed_actions[:, self._damping_ratio_idx:self.
                                    _damping_ratio_idx + 6] = torch.clamp(
                                        self.
                                        _processed_actions[:, self.
                                                           _damping_ratio_idx:
                                                           self.
                                                           ...
                                                           6],
                                        min=self.cfg.controller_cfg.
                                        motion_damping_ratio_limits_task[0],
                                        max=self.cfg.controller_cfg.
                                        motion_damping_ratio_limits_task[1],
                                    )
