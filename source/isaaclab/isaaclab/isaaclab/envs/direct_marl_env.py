...
class DirectMARLEnv(gym.Env):
    ...
    def __init__(self,
        ...
        if self.cfg.action_noise_model:
            self._action_noise_model: dict[AgentID, NoiseModel] = {
                agent:
                noise_model.class_type(noise_model,
        ...
        if self.cfg.observation_noise_model:
            self._observation_noise_model: dict[AgentID, NoiseModel] = {
                ...
                noise_model.class_type(noise_model,
                                       num_envs=self.num_envs,
                                       device=self.device)
                for agent, noise_model in
                self.cfg.observation_noise_model.items()
                if noise_model is not None
            }
    ...
    def max_episode_length(self):
        ...
        return math.ceil(self.max_episode_length_s /
    ...
    def step(self, actions: dict[AgentID, ActionType]) -> EnvStepReturn:
        ...
        self.reset_buf[:] = math.prod(
            self.terminated_dict.values()) | math.prod(
        ...
        if self.cfg.observation_noise_model:
            for agent, obs in self.obs_dict.items():
                if agent in self._observation_noise_model:
                    self.obs_dict[agent] = self._observation_noise_model[
                        agent](obs)
    ...
    def state(self) -> StateType | None:
        ...
        if isinstance(self.cfg.state_space, int) and self.cfg.state_space < 0:
            self.state_buf = torch.cat([
                self.obs_dict[agent].reshape(self.num_envs, -1)
    ...
    def render(self, recompute: bool = False) -> np.ndarray | None:
        ...
        if self.render_mode == "human" or self.render_mode is None:
            ...
            rgb_data = np.frombuffer(rgb_data,
                                     dtype=np.uint8).reshape(*rgb_data.shape)
            # return the rgb data
            # note: initially the renderer is warming up and returns empty data
            if rgb_data.size == 0:
                return np.zeros((self.cfg.viewer.resolution[1],
                                 self.cfg.viewer.resolution[0], 3),
                                dtype=np.uint8)
            else:
                return rgb_data[:, :, :3]
        else:
    ...
    def set_debug_vis(self, debug_vis: bool) -> bool:
        ...
        if debug_vis:
            ...
            if self._debug_vis_handle is None:
                ...
                self._debug_vis_handle = app_interface.get_post_update_event_stream(
                ).create_subscription_to_pop(lambda event, obj=weakref.proxy(
                    self): obj._debug_vis_callback(event))
        else:
    ...
    def _configure_env_spaces(self):
        ...
        if self.cfg.num_actions is not None:
            logger.warning(
                "DirectMARLEnvCfg.num_actions is deprecated. Use DirectMARLEnvCfg.action_spaces instead."
            )
        ...
        if self.cfg.num_states is not None:
            logger.warning(
                "DirectMARLEnvCfg.num_states is deprecated. Use DirectMARLEnvCfg.state_space instead."
            )
        ...
        self.actions = {
            agent:
            sample_space(self.action_spaces[agent],
                         self.sim.device,
    ...
    def _reset_idx(self, env_ids: Sequence[int]):
        ...
        if self.cfg.events:
            if "reset" in self.event_manager.available_modes:
                ...
                self.event_manager.apply(mode="reset",
                                         env_ids=env_ids,
    ...
    def _get_states(self) -> StateType:
        ...
        """
        raise NotImplementedError(
            f"Please implement the '_get_states' method for {self.__class__.__name__}."
        )
