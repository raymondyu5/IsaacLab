...
class InteractiveScene:
    ...
    * For tasks that require having separate assets in the environments, ``replicate_physics`` would have to
    ...
    def __init__(self, cfg: InteractiveSceneCfg):
        ...
        self._ALL_INDICES = torch.arange(self.cfg.num_envs,
                                         dtype=torch.long,
                                         device=self.device)
        ...
        if not self.cfg.replicate_physics:
            ...
            self._default_env_origins = torch.tensor(env_origins,
                                                     device=self.device,
                                                     dtype=torch.float32)
        else:
        ...
        if self._is_scene_setup_from_cfg():
            ...
            if (not self.cfg.replicate_physics
                    and self.cfg.filter_collisions) or self.device == "cpu":
    ...
    def clone_environments(self, copy_from_source: bool = False):
        ...
        has_multi_assets = carb_settings_iface.get(
    ...
    def device(self) -> str:
        ...
        return sim_utils.SimulationContext.instance(
        ).device  # pyright: ignore [reportOptionalMemberAccess]
    ...
    def reset_to(
        ...
        for asset_name, articulation in self._articulations.items():
            ...
            articulation.write_root_velocity_to_sim(root_velocity,
                                                    env_ids=env_ids)
            # joint state
            joint_position = asset_state["joint_position"].clone()
            joint_velocity = asset_state["joint_velocity"].clone()
            articulation.write_joint_state_to_sim(joint_position,
                                                  joint_velocity,
                                                  env_ids=env_ids)
            ...
            articulation.set_joint_position_target(joint_position,
                                                   env_ids=env_ids)
            articulation.set_joint_velocity_target(joint_velocity,
                                                   env_ids=env_ids)
        # deformable objects
        for asset_name, deformable_object in self._deformable_objects.items():
            ...
            deformable_object.write_nodal_pos_to_sim(nodal_position,
                                                     env_ids=env_ids)
            deformable_object.write_nodal_velocity_to_sim(nodal_velocity,
                                                          env_ids=env_ids)
        # rigid objects
        for asset_name, rigid_object in self._rigid_objects.items():
            ...
            rigid_object.write_root_velocity_to_sim(root_velocity,
                                                    env_ids=env_ids)
        # surface grippers
    ...
    def _is_scene_setup_from_cfg(self) -> bool:
        ...
        """
        return any(not (asset_name in InteractiveSceneCfg.__dataclass_fields__
    ...
    def _add_entities_from_cfg(self):
        ...
        for asset_name, asset_cfg in self.cfg.__dict__.items():
            ...
            if isinstance(asset_cfg, TerrainImporterCfg):
                ...
                self._articulations[asset_name] = asset_cfg.class_type(
                    asset_cfg)
            elif isinstance(asset_cfg, DeformableObjectCfg):
                self._deformable_objects[asset_name] = asset_cfg.class_type(
                    asset_cfg)
            elif isinstance(asset_cfg, RigidObjectCfg):
                self._rigid_objects[asset_name] = asset_cfg.class_type(
                    asset_cfg)
                ...
                for rigid_object_cfg in asset_cfg.rigid_objects.values():
                    rigid_object_cfg.prim_path = rigid_object_cfg.prim_path.format(
                        ENV_REGEX_NS=self.env_regex_ns)
                self._rigid_object_collections[
                    asset_name] = asset_cfg.class_type(asset_cfg)
                for rigid_object_cfg in asset_cfg.rigid_objects.values():
                    if hasattr(rigid_object_cfg, "collision_group"
                ...
                self._surface_grippers[asset_name] = asset_cfg.class_type(
                    asset_cfg)
            ...
            if hasattr(asset_cfg,
                       "collision_group") and asset_cfg.collision_group == -1:
                asset_paths = sim_utils.find_matching_prim_paths(
                    asset_cfg.prim_path)
                self._global_prim_paths += asset_paths
