...
class GridCloner(Cloner):
    ...
    def get_clone_transforms(
        ...
        if position_offsets is not None:
            if len(position_offsets) != num_clones:
                raise ValueError(
                    "Dimension mismatch between position_offsets and prim_paths!"
                )
        ...
        if orientation_offsets is not None:
            if len(orientation_offsets) != num_clones:
                raise ValueError(
                    "Dimension mismatch between orientation_offsets and prim_paths!"
                )
            # convert to numpy array
        ...
        self._num_per_row = int(np.sqrt(
            num_clones)) if self._num_per_row == -1 else self._num_per_row
        num_rows = np.ceil(num_clones / self._num_per_row)
        if eval_mode:
            num_rows = 1
            self._num_per_row = num_clones
        ...
        for i in range(num_clones):
            ...
            if orientation_offsets is not None:
                orientation = (
                    Gf.Quatd(orientation_offsets[i][0].item(),
                             Gf.Vec3d(orientation_offsets[i][1:].tolist())) *
                    orientation)
    ...
    def clone(
        ...
        positions, orientations = self.get_clone_transforms(
            num_clones, position_offsets, orientation_offsets, eval_mode)
