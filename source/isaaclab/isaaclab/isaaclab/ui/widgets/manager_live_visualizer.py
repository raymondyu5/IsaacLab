...
class ManagerLiveVisualizer(UiVisualizerBase):
    ...
    def __init__(self,
                 manager: ManagerBase,
                 cfg: ManagerLiveVisualizerCfg = ManagerLiveVisualizerCfg()):
    ...
    def _set_env_selection_impl(self, env_idx: int):
        ...
        if env_idx > 0 and env_idx < self._manager.num_envs:
            ...
            logger.warning(
                f"Environment index is out of range (0, {self._manager.num_envs - 1})"
            )
    ...
    def _debug_vis_callback(self, event):
        ...
        for (_, term), vis in zip(
                self._manager.get_active_iterable_terms(env_idx=self._env_idx),
    ...
    def _set_debug_vis_impl(self, debug_vis: bool):
        ...
        if debug_vis:
            ...
            if not hasattr(
                    self,
                    "_debug_vis_handle") or self._debug_vis_handle is None:
        ...
        with self._vis_frame:
            with omni.ui.VStack():
                ...
                for name, term in self._manager.get_active_iterable_terms(
                    ...
                    if name in self.term_names or len(self.term_names) == 0:
                        ...
                        with frame:
                            ...
                            # create an image plot for 2d and greater data (i.e. mono and rgb images)
                                ...
                                image = ImagePlot(image=numpy.array(term),
                                                  label=name)
...
class DefaultManagerBasedEnvLiveVisCfg:
    ...
    observation_live_vis = ManagerLiveVisualizerCfg(
        manager_name="observation_manager")
...
class DefaultManagerBasedRLEnvLiveVisCfg(DefaultManagerBasedEnvLiveVisCfg):
    ...
    curriculum_live_vis = ManagerLiveVisualizerCfg(
        manager_name="curriculum_manager")
    ...
    termination_live_vis = ManagerLiveVisualizerCfg(
        manager_name="termination_manager")
    ...
    def _prepare_terms(self):
        ...
        for term_name, term_cfg in cfg_items:
            ...
            if isinstance(term_cfg, ManagerLiveVisualizerCfg):
                ...
                self._manager_visualizers[
                    term_cfg.manager_name] = ManagerLiveVisualizer(
                        manager=manager, cfg=term_cfg)
            else:
                raise TypeError(
