...
class ImagePlot(UIWidgetWrapper):
    ...
    def update_image(self, image: np.ndarray):
        ...
        if self._curr_mode == "Normalization":
            ...
            if image.ndim == 3 and image.shape[2] == 3:
                logger.warning(
                    "Colorization mode is only available for single channel images"
                )
            else:
                image = (image - image.min()) / (image.max() - image.min())
        ...
        if image.ndim == 2 or (image.ndim == 3 and image.shape[2] == 1):
            ...
            image = np.dstack(
                (image, np.full((height, width, 1), 255, dtype=np.uint8)))

        self._byte_provider.set_bytes_data(image.flatten().data,
                                           [width, height])
    ...
    def _get_unit_description(self,
                              min_value: float,
                              max_value: float,
        ...
        return (
            ...
            (f" Median: {median_value * self._unit_scale:.2f} {self._unit_name}"
             if median_value is not None else ""))
    ...
    def _build_widget(self):
        ...
        with omni.ui.VStack(spacing=3):
            with omni.ui.HStack():
                ...
                with omni.ui.Frame(width=self._aspect_ratio *
                                   self._widget_height,
                                   height=self._widget_height):
                    self._base_plot = omni.ui.ImageWithProvider(
                        self._byte_provider)

            if self._show_min_max:
                self._min_max_label = omni.ui.Label(
                    self._get_unit_description(0, 0))
