    ...
    def _build_ui(self):
        ...
        with ui.ZStack():
            ...
            with ui.VStack(alignment=ui.Alignment.CENTER):
                self._ui_label = ui.Label(self._text,
                                          style=self._style,
                                          alignment=ui.Alignment.CENTER)


def compute_widget_dimensions(text: str, font_size: float, max_width: float,
                              min_width: float) -> tuple[float, float, str]:
...
def show_instruction(
    ...
    if target_prim_path in camera_facing_widget_container:
        container, current_text = camera_facing_widget_container[
            target_prim_path]
    ...
    width, height, wrapped_text = compute_widget_dimensions(
        text, font_size, max_width, min_width)
    ...
    if copied_prim is not None:
        space_stack.append(
            SpatialSource.new_prim_path_source(target_prim_path))
    ...
    if display_duration:
        timer = asyncio.get_event_loop().call_later(
            display_duration,
            functools.partial(hide_instruction, target_prim_path))
...
def update_instruction(target_prim_path: str = "/newPrim", text: str = ""):
    ...
    if container_data:
        ...
        if current_text != text:
            ...
            components = getattr(manipulator,
                                 "_ComposableManipulator__components")
            if len(components) > 0:
                ...
                if simple_text_widget and simple_text_widget.component and simple_text_widget.component.widget:
                    ...
                    simple_text_widget.component.widget.set_label_text(
                        wrapped_text)
                # Update the stored text in the global dictionary
                camera_facing_widget_container[target_prim_path] = (container,
                                                                    text)
                return True
