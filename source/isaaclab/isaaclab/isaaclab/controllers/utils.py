...
def convert_usd_to_urdf(usd_path: str,
                        output_path: str,
                        force_conversion: bool = True) -> tuple[str, str]:
    ...
    if not os.path.exists(urdf_output_path) or not os.path.exists(
            urdf_meshes_output_dir) or force_conversion:
...
def change_revolute_to_fixed(urdf_path: str,
    ...
    for joint in fixed_joints:
        ...
        if verbose:
            ...
            if old_str not in content:
                logger.warning(
                    f"Error: Could not find revolute joint named '{joint}' in URDF file"
                )
...
def change_revolute_to_fixed_regex(urdf_path: str,
                                   fixed_joints: list[str],
                                   verbose: bool = False):
    """Change revolute joints to fixed joints in a URDF file.
    ...
    revolute_joints = re.findall(r'<joint name="([^"]+)" type="revolute">',
                                 content)
