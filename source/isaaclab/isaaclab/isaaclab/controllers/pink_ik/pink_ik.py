...
class PinkIKController:
    ...
    def __init__(self, cfg: PinkIKControllerCfg, robot_cfg: ArticulationCfg,
                 device: str, controlled_joint_indices: list[int]):
        ...
        if cfg.joint_names is None:
            raise ValueError(
                "joint_names must be provided in the configuration")
        if cfg.all_joint_names is None:
            raise ValueError(
                "all_joint_names must be provided in the configuration")
    ...
    def _validate_consistency(self, cfg: PinkIKControllerCfg,
                              controlled_joint_indices: list[int]) -> None:
        ...
        actual_joint_names = [
            cfg.all_joint_names[idx] for idx in controlled_joint_indices
        ]
        if actual_joint_names != cfg.joint_names:
            mismatches = []
            for i, (actual, expected) in enumerate(
                    zip(actual_joint_names, cfg.joint_names)):
                if actual != expected:
                    mismatches.append(
    ...
    def _setup_joint_ordering_mappings(self):
        ...
        self.isaac_lab_to_pink_ordering = np.array([
            isaac_lab_joint_names.index(pink_joint)
            for pink_joint in pink_joint_names
        ])
        self.pink_to_isaac_lab_ordering = np.array([
            pink_joint_names.index(isaac_lab_joint)
            for isaac_lab_joint in isaac_lab_joint_names
        ])
        ...
        if isaac_lab_controlled_joint_names is None:
            raise ValueError(
                "isaac_lab_controlled_joint_names should not be None")

        self.isaac_lab_to_pink_controlled_ordering = np.array([
            isaac_lab_controlled_joint_names.index(pink_joint)
            for pink_joint in pink_controlled_joint_names
        ])
        self.pink_to_isaac_lab_controlled_ordering = np.array([
            pink_controlled_joint_names.index(isaac_lab_joint)
            for isaac_lab_joint in isaac_lab_controlled_joint_names
        ])
    ...
    def compute(
        ...
        curr_controlled_joint_pos = [
            curr_joint_pos[i] for i in self.controlled_joint_indices
        ]
        ...
        try:
            ...
            return torch.tensor(curr_controlled_joint_pos,
                                device=self.device,
                                dtype=torch.float32)
        ...
        target_joint_pos = torch.add(
            joint_vel_isaac_lab,
            torch.tensor(curr_controlled_joint_pos,
                         device=self.device,
                         dtype=torch.float32))
