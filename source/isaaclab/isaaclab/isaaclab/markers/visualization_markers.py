...
class VisualizationMarkers:
    ...
    def __init__(self, cfg: VisualizationMarkersCfg):
        ...
        self._instancer_manager = UsdGeom.PointInstancer.Define(
        ...
        if len(self.cfg.markers) == 0:
            raise ValueError(
                f"The `cfg.markers` cannot be empty. Received: {self.cfg.markers}"
            )
        ...
        self._instancer_manager.GetProtoIndicesAttr().Set(
            list(range(self.num_prototypes)))
        self._instancer_manager.GetPositionsAttr().Set([Gf.Vec3f(0.0)] *
                                                       self.num_prototypes)
    ...
    def is_visible(self) -> bool:
        ...
        """
        return self._instancer_manager.GetVisibilityAttr().Get(
    ...
    def visualize(
        ...
        if translations is not None:
            ...
            if translations.shape[1] != 3 or len(translations.shape) != 2:
                raise ValueError(
                    f"Expected `translations` to have shape (M, 3). Received: {translations.shape}."
                )
            # apply translations
            self._instancer_manager.GetPositionsAttr().Set(
                Vt.Vec3fArray.FromNumpy(translations))
        ...
        if orientations is not None:
            ...
            if orientations.shape[1] != 4 or len(orientations.shape) != 2:
                raise ValueError(
                    f"Expected `orientations` to have shape (M, 4). Received: {orientations.shape}."
                )
            # roll orientations from (w, x, y, z) to (x, y, z, w)
            # internally USD expects (x, y, z, w)
            orientations = convert_quat(orientations, to="xyzw")
            # apply orientations
            self._instancer_manager.GetOrientationsAttr().Set(
                Vt.QuathArray.FromNumpy(orientations))
            # update number of markers
        ...
        if scales is not None:
            ...
            if scales.shape[1] != 3 or len(scales.shape) != 2:
                raise ValueError(
                    f"Expected `scales` to have shape (M, 3). Received: {scales.shape}."
                )
            # apply scales
            self._instancer_manager.GetScalesAttr().Set(
        ...
        if marker_indices is not None or num_markers != self._count:
            ...
            if marker_indices is not None:
                ...
                if len(marker_indices.shape) != 1:
                    raise ValueError(
                        f"Expected `marker_indices` to have shape (M,). Received: {marker_indices.shape}."
                    )
                # apply proto indices
                self._instancer_manager.GetProtoIndicesAttr().Set(
                    Vt.IntArray.FromNumpy(marker_indices))
                # update number of markers
                ...
                if num_markers == 0:
                    raise ValueError(
                        "Number of markers cannot be zero! Hint: The function was called with no inputs?"
                    )
    ...
    def _add_markers_prototypes(self, markers_cfg: dict[str,
        ...
        for name, cfg in markers_cfg.items():
            ...
            self._instancer_manager.GetPrototypesRel().AddTarget(
                marker_prim_path)
    ...
    def _process_prototype_prim(self, prim: Usd.Prim):
        ...
        if not prim.IsValid():
            raise ValueError(
                f"Prim at path '{prim.GetPrimAtPath()}' is not valid.")
