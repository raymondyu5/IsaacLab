...
class Se3Gamepad(DeviceBase):
    ...
    * gripper: a binary command to open or close the gripper.
    ...
    def __del__(self):
        ...
        self._input.unsubscribe_to_gamepad_events(self._gamepad,
                                                  self._gamepad_sub)
    ...
    def _on_gamepad_event(self, event, *args, **kwargs):
        ...
        if event.input in self._INPUT_STICK_VALUE_MAPPING:
            direction, axis, value = self._INPUT_STICK_VALUE_MAPPING[
                event.input]
        ...
        if event.input in self._INPUT_DPAD_VALUE_MAPPING:
            direction, axis, value = self._INPUT_DPAD_VALUE_MAPPING[
                event.input]
    ...
    def _create_key_bindings(self):
        ...
        self._INPUT_STICK_VALUE_MAPPING = {
            ...
            # upward command
            carb.input.GamepadInput.RIGHT_STICK_UP: (0, 2,
                                                     self.pos_sensitivity),
            # downward command
            carb.input.GamepadInput.RIGHT_STICK_DOWN: (1, 2,
                                                       self.pos_sensitivity),
            # yaw command (positive)
            carb.input.GamepadInput.RIGHT_STICK_RIGHT: (0, 5,
                                                        self.rot_sensitivity),
            # yaw command (negative)
            carb.input.GamepadInput.RIGHT_STICK_LEFT: (1, 5,
                                                       self.rot_sensitivity),
        }
