...
class OpenXRDevice(DeviceBase):
    ...
    * "RESET": Reset the tracking and signal simulation reset
    ...
    def __init__(
        ...
        self._vc_subscription = (XRCore.get_singleton().get_message_bus(
    ...
    def __del__(self):
        ...
        """
        if hasattr(self,
                   "_vc_subscription") and self._vc_subscription is not None:
    ...
    def _get_raw_data(self) -> Any:
        ...
        """
        return {
            self.TrackingTarget.HAND_LEFT:
            self._calculate_joint_poses(
                XRCore.get_singleton().get_input_device("/user/hand/left"),
                self._previous_joint_poses_left,
            ),
            self.TrackingTarget.HAND_RIGHT:
            self._calculate_joint_poses(
                XRCore.get_singleton().get_input_device("/user/hand/right"),
                self._previous_joint_poses_right,
            ),
            self.TrackingTarget.HEAD:
            self._calculate_headpose(),
        }
    ...
    def _calculate_joint_poses(
            self, hand_device: Any,
            previous_joint_poses: dict[str,
        ...
        for joint_name, joint_pose in joint_poses.items():
            if joint_name in HAND_JOINT_NAMES:
                ...
                previous_joint_poses[joint_name] = np.array([
                    position[0], position[1], position[2], quatw, quati[0],
                    quati[1], quati[2]
                ],
                                                            dtype=np.float32)
