...
class ManusViveIntegration:
    ...
    def __init__(self):
        ...
        self.rot_adjust = Gf.Matrix3d().SetRotate(
    ...
    def get_all_device_data(self) -> dict:
        ...
        return {
            "manus_gloves": self._transform_manus_data(manus_data,
                                                       scene_T_wrist),
    ...
    def _initialize_coordinate_transformation(self):
        ...
        try:
            ...
            # Choose a mapping
            choose_A = None
            if len(self._pairA_candidates) == 0 and len(
                    self._pairB_candidates) >= min_frames:
                choose_A = False
            elif len(self._pairB_candidates) == 0 and len(
                    self._pairA_candidates) >= min_frames:
                choose_A = True
            elif len(self._pairA_trans_errs) >= min_frames and len(
                    self._pairB_trans_errs) >= min_frames:
                errA = get_pairing_error(self._pairA_trans_errs,
                                         self._pairA_rot_errs)
                errB = get_pairing_error(self._pairB_trans_errs,
                                         self._pairB_rot_errs)
                if errA < errB and errA < tolerance:
                    choose_A = True
                elif errB < errA and errB < tolerance:
                    choose_A = False
                elif len(self._pairA_trans_errs) % 10 == 0 or len(
                        self._pairB_trans_errs) % 10 == 0:
            ...
            logger.error(
                f"Failed to initialize coordinate transformation: {e}")
    ...
    def _transform_vive_data(self, device_data: dict) -> dict:
        ...
        for joint_name, joint_data in device_data.items():
            ...
            if self.scene_T_lighthouse_static is not None:
                transformed_matrix = pose_to_matrix(
                    joint_data) * self.scene_T_lighthouse_static
    ...
    def _get_scene_T_wrist_matrix(self, vive_data: dict) -> dict:
        ...
        """
        scene_T_wrist = {
            "left": Gf.Matrix4d().SetIdentity(),
            "right": Gf.Matrix4d().SetIdentity()
        }
        ...
        if self._vive_left_id is not None:
            scene_T_wrist["left"] = Rcorr * pose_to_matrix(
                vive_data[self._vive_left_id])
        if self._vive_right_id is not None:
            scene_T_wrist["right"] = Rcorr * pose_to_matrix(
                vive_data[self._vive_right_id])
        return scene_T_wrist

    def _transform_manus_data(self, manus_data: dict,
                              scene_T_wrist: dict) -> dict:
        ...
        for joint_name, joint_data in manus_data.items():
            ...
            joint_mat = Rcorr * pose_to_matrix(
                joint_data) * scene_T_wrist[hand]
        ...
        transformed_data["right_25"] = self._get_palm(transformed_data,
                                                      "right")
        return transformed_data
    ...
    def _get_palm(self, transformed_data: dict, hand: str) -> dict:
        ...
        pos = (np.array(metacarpal["position"]) +
               np.array(proximal["position"])) / 2.0
        return {
            "position": [pos[0], pos[1], pos[2]],
            "orientation": metacarpal["orientation"]
        }


def compute_delta_errors(a: Gf.Matrix4d,
                         b: Gf.Matrix4d) -> tuple[float, float]:
...
def average_transforms(mats: list[Gf.Matrix4d]) -> Gf.Matrix4d:
    ...
    if norm <= 1e-12:
        ...
        qn = acc_q / norm
        quat_avg = Gf.Quatd(float(qn[0]),
                            Gf.Vec3d(float(qn[1]), float(qn[2]), float(qn[3])))
...
def select_mode_cluster(mats: list[Gf.Matrix4d],
                        trans_thresh_m: float = 0.03,
                        rot_thresh_deg: float = 10.0) -> list[Gf.Matrix4d]:
...
def get_openxr_wrist_matrix(hand: str) -> Gf.Matrix4d:
    ...
    hand = hand.lower()
    try:
        hand_device = XRCore.get_singleton().get_input_device(
            f"/user/hand/{hand}")
...
def get_vive_wrist_ids(vive_data: dict) -> tuple[str, str]:
    ...
    if len(
            wm_ids
    ) >= 2:  # Assumes the first two vive trackers are the wrist trackers
