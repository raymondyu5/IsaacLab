...
def create_teleop_device(
        device_name: str,
        devices_cfg: dict[str, DeviceCfg],
        callbacks: dict[str, Callable] | None = None) -> DeviceBase:
    ...
    if device_name not in devices_cfg:
        raise ValueError(
            f"Device '{device_name}' not found in teleop device configurations"
        )
    ...
    if cfg_type not in DEVICE_MAP:
        raise ValueError(
            f"Unsupported device configuration type: {cfg_type.__name__}")
    ...
    retargeters = []
    if hasattr(device_cfg,
               "retargeters") and device_cfg.retargeters is not None:
        try:
            ...
            for retargeter_cfg in device_cfg.retargeters:
                ...
                if cfg_type in RETARGETER_MAP:
                    retargeters.append(
                        RETARGETER_MAP[cfg_type](retargeter_cfg))
                else:
                    raise ValueError(
                        f"Unknown retargeter configuration type: {cfg_type.__name__}"
                    )
