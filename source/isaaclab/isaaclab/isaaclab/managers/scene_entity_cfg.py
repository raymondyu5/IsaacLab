...
class SceneEntityCfg:
    ...
    def resolve(self, scene: InteractiveScene):
        ...
        if self.name not in scene.keys():
            raise ValueError(
                f"The scene entity '{self.name}' does not exist. Available entities: {scene.keys()}."
            )
    ...
    def _resolve_joint_names(self, scene: InteractiveScene):
        ...
        if self.joint_names is not None or self.joint_ids != slice(None):
            ...
            # -- from joint names to joint indices
                ...
                if len(
                        self.joint_ids
                ) == entity.num_joints and self.joint_names == entity.joint_names:
            ...
            # -- from joint indices to joint names
                ...
                if isinstance(self.joint_ids, int):
                    self.joint_ids = [self.joint_ids]
                self.joint_names = [
                    entity.joint_names[i] for i in self.joint_ids
                ]

    def _resolve_fixed_tendon_names(self, scene: InteractiveScene):
        # convert tendon names to indices based on regex
        if self.fixed_tendon_names is not None or self.fixed_tendon_ids != slice(
                None):
            entity: Articulation = scene[self.name]
            # -- if both are not their default values, check if they are valid
            if self.fixed_tendon_names is not None and self.fixed_tendon_ids != slice(
                    None):
                ...
                fixed_tendon_names = [
                    entity.fixed_tendon_names[i] for i in self.fixed_tendon_ids
                ]
            ...
            # -- from fixed tendon names to fixed tendon indices
                ...
                if (len(self.fixed_tendon_ids) == entity.num_fixed_tendons and
                        self.fixed_tendon_names == entity.fixed_tendon_names):
    ...
    def _resolve_body_names(self, scene: InteractiveScene):
        ...
        if self.body_names is not None or self.body_ids != slice(None):
            ...
            # -- from body names to body indices
                ...
                if len(
                        self.body_ids
                ) == entity.num_bodies and self.body_names == entity.body_names:
    ...
    def _resolve_object_collection_names(self, scene: InteractiveScene):
        ...
        if self.object_collection_names is not None or self.object_collection_ids != slice(
                None):
            entity: RigidObjectCollection = scene[self.name]
            # -- if both are not their default values, check if they are valid
            if self.object_collection_names is not None and self.object_collection_ids != slice(
                    None):
                ...
                object_ids, _ = entity.find_objects(
                    ...
                    preserve_order=self.preserve_order)
                object_names = [
                    entity.object_names[i] for i in self.object_collection_ids
                ]
