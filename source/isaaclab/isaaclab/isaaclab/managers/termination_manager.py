...
class TerminationManager(ManagerBase):
    ...
    * **Terminated**: This signal is set to true if the environment has reached a terminal state defined by the
    ...
    def __init__(self, cfg: object, env: ManagerBasedRLEnv):
        ...
        self._truncated_buf = torch.zeros(self.num_envs,
                                          device=self.device,
                                          dtype=torch.bool)
    ...
    def reset(self,
        ...
        for i, key in enumerate(self._term_names):
            # store information
            extras["Episode_Termination/" +
                   key] = last_episode_done_stats[i].item()
    ...
    def get_active_iterable_terms(
        ...
        for i, key in enumerate(self._term_names):
            terms.append(
                (key, [self._term_dones[env_idx, i].float().cpu().item()]))
        return terms
