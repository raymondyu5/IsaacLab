...
class RecorderTerm(ManagerTermBase):
    ...
    * Post-step recording: This callback is invoked at the end of `env.step()` when all the managers are processed.
    ...
    def record_post_step(
            self) -> tuple[str | None, torch.Tensor | dict | None]:
...
class RecorderManager(ManagerBase):
    ...
    def __init__(self, cfg: object, env: ManagerBasedEnv):
        ...
        if not isinstance(cfg, RecorderManagerBaseCfg):
            raise TypeError(
                "Configuration for the recorder manager is not of type RecorderManagerBaseCfg."
            )
        ...
        if cfg.dataset_export_mode != DatasetExportMode.EXPORT_NONE:
            ...
            self._dataset_file_handler.create(os.path.join(
                cfg.dataset_export_dir_path, cfg.dataset_filename),
                                              env_name=env_name)
        ...
        if cfg.dataset_export_mode == DatasetExportMode.EXPORT_SUCCEEDED_FAILED_IN_SEPARATE_FILES:
            ...
            self._failed_episode_dataset_file_handler.create(os.path.join(
                cfg.dataset_export_dir_path, f"{cfg.dataset_filename}_failed"),
                                                             env_name=env_name)
    ...
    def reset(self,
              env_ids: Sequence[int] | None = None) -> dict[str, torch.Tensor]:
    ...
    def set_success_to_episodes(self, env_ids: Sequence[int] | None,
    ...
    def record_pre_reset(self,
                         ...
                         force_export_or_skip=None) -> None:
        ...
        success_results = torch.zeros(len(env_ids),
                                      dtype=bool,
        ...
        if hasattr(self._env, "termination_manager"):
            if "success" in self._env.termination_manager.active_terms:
                success_results |= self._env.termination_manager.get_term(
                    "success")[env_ids]
        self.set_success_to_episodes(env_ids, success_results)

        if force_export_or_skip or (force_export_or_skip is None
                                    and self.cfg.export_in_record_pre_reset):
    ...
    def export_episodes(self,
                        env_ids: Sequence[int] | None = None,
                        demo_ids: Sequence[int] | None = None) -> None:
        ...
        if demo_ids is not None:
            ...
            if len(demo_ids) != len(env_ids):
                raise ValueError(
                    f"Length of demo_ids ({len(demo_ids)}) must match length of env_ids ({len(env_ids)})"
                )
            # Check for duplicate demo_ids
            if len(set(demo_ids)) != len(demo_ids):
                duplicates = [
                    x for i, x in enumerate(demo_ids) if demo_ids.index(x) != i
                ]
                raise ValueError(
                    f"demo_ids must be unique. Found duplicates: {list(set(duplicates))}"
                )

        # Export episode data through dataset exporter
        need_to_flush = False

        if any(env_id in self._episodes
        ...
        for i, env_id in enumerate(env_ids):
            if env_id in self._episodes and not self._episodes[
                    env_id].is_empty():
                self._episodes[env_id].pre_export()

                episode_succeeded = self._episodes[env_id].success
                target_dataset_file_handler = None
                if (self.cfg.dataset_export_mode
                        == DatasetExportMode.EXPORT_ALL) or (
                            self.cfg.dataset_export_mode
                            == DatasetExportMode.EXPORT_SUCCEEDED_ONLY
                            and episode_succeeded):
                ...
                if target_dataset_file_handler is not None:
                    ...
                    current_demo_id = demo_ids[
                        i] if demo_ids is not None else None
                    target_dataset_file_handler.write_episode(
                        self._episodes[env_id], current_demo_id)
                    need_to_flush = True
                # Update episode count
                if episode_succeeded:
                    self._exported_successful_episode_count[env_id] = (
                        self._exported_successful_episode_count.get(env_id,
                                                                    0) + 1)
                else:
                    self._exported_failed_episode_count[
                        env_id] = self._exported_failed_episode_count.get(
                            env_id, 0) + 1
