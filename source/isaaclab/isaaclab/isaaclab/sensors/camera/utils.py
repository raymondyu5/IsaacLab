...
def create_pointcloud_from_rgbd(
    ...
    rgb: torch.Tensor | wp.array | np.ndarray
    ...
    if num_channels not in [3, 4]:
        raise ValueError(
            f"Invalid number of channels: {num_channels} != 3 or 4.")
    ...
    points_xyz = create_pointcloud_from_depth(intrinsic_matrix,
                                              depth,
                                              True,
                                              position,
                                              orientation,
    ...
    if rgb is not None:
        if isinstance(rgb, (np.ndarray, torch.Tensor, wp.array)):
            ...
            points_rgb = torch.Tensor((rgb, ) * num_points,
                                      device=device,
                                      dtype=torch.uint8)
        else:
            # default color is white
            points_rgb = torch.Tensor(((0, 0, 0), ) * num_points,
                                      device=device,
                                      dtype=torch.uint8)
    else:
        points_rgb = torch.Tensor(((0, 0, 0), ) * num_points,
    ...
    pts_idx_to_keep = torch.all(torch.logical_and(~torch.isnan(points_xyz),
                                                  ~torch.isinf(points_xyz)),
                                dim=1)
    ...
    if num_channels == 4:
        points_rgb = torch.nn.functional.pad(points_rgb, (0, 1),
                                             mode="constant",
                                             value=1.0)
...
def save_images_to_file(images: torch.Tensor, file_path: str):
    ...
    save_image(
        make_grid(torch.swapaxes(images.unsqueeze(1), 1, -1).squeeze(-1),
                  nrow=round(images.shape[0]**0.5)), file_path)
