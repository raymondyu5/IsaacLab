...
from isaaclab.sensors.camera import randomization
...
class Camera(SensorBase):
    ...
    def __init__(self, cfg: CameraCfg):
        ...
        sensor_path_is_regex = re.match(r"^[a-zA-Z0-9/_]+$",
                                        sensor_path) is None
        ...
        if self.cfg.spawn is not None:
            ...
            rot = torch.tensor(self.cfg.offset.rot,
                               dtype=torch.float32,
            ...
            # spawn the asset
            self.cfg.spawn.func(self.cfg.prim_path,
                                self.cfg.spawn,
        ...
        if len(matching_prims) == 0:
            raise RuntimeError(
                f"Could not find prim with path {self.cfg.prim_path}.")
    ...
    def __del__(self):
        ...
        for _, annotators in self._rep_registry.items():
            for annotator, render_product_path in zip(
                    annotators, self._render_product_paths):
    ...
    def set_intrinsic_matrices(self,
        ...
        for i, intrinsic_matrix in zip(env_ids, matrices):
            ...
            params = sensor_utils.convert_camera_intrinsics_to_usd(
                intrinsic_matrix=intrinsic_matrix.reshape(-1),
                height=height,
                width=width,
    ...
    def set_world_poses(
        ...
        if orientations is not None:
            if isinstance(orientations, np.ndarray):
                orientations = torch.from_numpy(orientations).to(
                    device=self._device)
    ...
    def set_world_poses_from_view(self,
                                  eyes: torch.Tensor,
        ...
        orientations = quat_from_matrix(
            create_rotation_matrix_from_view(eyes,
                                             targets,
                                             up_axis,
                                             device=self._device))
    ...
    def _initialize_impl(self):
        ...
        self._view = XFormPrim(self.cfg.prim_path,
        ...
        self._ALL_INDICES = torch.arange(self._view.count,
                                         device=self._device,
                                         dtype=torch.long)
        # Create frame count buffer
        self._frame = torch.zeros(self._view.count,
                                  device=self._device,
                                  dtype=torch.long)
        ...
        for cam_prim_path in self._view.prim_paths:
            ...
            if isinstance(self.cfg.semantic_filter, list):
                semantic_filter_predicate = ":*; ".join(
                    self.cfg.semantic_filter) + ":*"
            elif isinstance(self.cfg.semantic_filter, str):
                semantic_filter_predicate = self.cfg.semantic_filter
            else:
                raise ValueError(
                    f"Semantic types must be a list or a string. Received: {self.cfg.semantic_filter}."
                )
            ...
            for name in self.cfg.data_types:
                ...
                if name == "semantic_segmentation":
                    init_params = {
                        ...
                        "mapping":
                        json.dumps(self.cfg.semantic_segmentation_mapping),
                    }
            ...
            for name, annotators in self._rep_registry.items():
                ...
                if name == "distance_to_camera":
                    self._data.output[name][
                        self._data.output[name] >
                        self.cfg.spawn.clipping_range[1]] = torch.inf
                # apply defined clipping behavior
                if (name == "distance_to_camera"
                        or name == "distance_to_image_plane"
                    ) and self.cfg.depth_clipping_behavior != "none":
                    self._data.output[name][torch.isinf(
                        self._data.output[name])] = (
                            0.0 if self.cfg.depth_clipping_behavior == "zero"
    ...
    def _create_buffers(self):
        ...
        # -- pose of the cameras
        self._data.pos_w = torch.zeros((self._view.count, 3),
                                       device=self._device)
        self._data.pos_local = torch.zeros((self._view.count, 3),
                                           device=self._device)
        self._data.quat_w_world = torch.zeros((self._view.count, 4),
                                              device=self._device)
        self._data.quat_local = torch.as_tensor(
            [[1, 0, 0, 0]] * self._view.count,
            device=self._device,
        ).to(torch.float32)
        self._data.quat_w = torch.as_tensor(
            [[1, 0, 0, 0]] * self._view.count,
            device=self._device,
        ).to(torch.float32)
        # -- intrinsic matrix
        self._data.intrinsic_matrices = torch.zeros((self._view.count, 3, 3),
                                                    device=self._device)
    ...
    def _create_annotator_data(self):
        ...
        for name, annotators in self._rep_registry.items():
            ...
            if name == "distance_to_camera":
                self._data.output[name][self._data.output[name] > self.cfg.
                                        spawn.clipping_range[1]] = torch.inf
            ...
            if (name == "distance_to_camera"
                ...
                self._data.output[name][torch.isinf(
                    self._data.output[name])] = (
                        0.0 if self.cfg.depth_clipping_behavior == "zero" else
                        self.cfg.spawn.clipping_range[1])

    def _process_annotator_output(
            self, name: str, output: Any) -> tuple[torch.tensor, dict | None]:
    ...
    def randonmize_camera_pose(self, env_ids, time_step, env_origins):

        translations, quat = randomization.sample_camera_positions(
            len(env_ids), self.cfg.customize_camera_setting['radius_range'],
            self.cfg.customize_camera_setting['z_min'],
            self.cfg.customize_camera_setting["base_rot"],
            self.cfg.customize_camera_setting["origin"], time_step)
        # self.set_world_poses(positions=translations,
        #                      orientations=quat,
        #                      env_ids=env_ids)

        eyes, orientations = self.set_world_poses_from_view(
            eyes=translations.to(self.device) + env_origins.to(self.device),
            targets=torch.as_tensor(
                self.cfg.customize_camera_setting["origin"])
            [None].repeat_interleave(self._num_envs, 0).to(self.device) +
            env_origins.to(self.device),
            env_ids=env_ids)
    ...
    def _update_poses(self, env_ids: Sequence[int]):
        ...
        local_poses, local_quat = self._view.get_local_poses(env_ids)

        self._data.pos_local[env_ids] = local_poses
        self._data.pos_w[env_ids] = poses
        self._data.quat_w[env_ids] = quat

        self._data.quat_local[
            env_ids] = convert_camera_frame_orientation_convention(
                local_quat, origin="opengl", target="world")
        self._data.quat_w_world[
            env_ids] = convert_camera_frame_orientation_convention(
                quat, origin="opengl", target="world")

    def pad_point_clouds(self, point_clouds, max_points=50000):
        """
            Pads a list of point clouds with zeros to make them the same size.
            
            Args:
            - point_clouds (list of torch.Tensor): List of point clouds of different sizes.
            - max_points (int): The maximum number of points in the batch.
            
            Returns:
            - padded_point_clouds (torch.Tensor): Padded point clouds of shape (batch_size, max_points, point_dim).
            """
        padded_point_clouds = []
        for pc in point_clouds:
            num_points = pc.shape[0]
            # Create a padded point cloud filled with zeros
            padded_pc = torch.zeros((max_points, pc.shape[1]),
                                    device=pc.device)
            # Copy the original point cloud into the padded tensor

            padded_pc[:max_points, :] = pc[:max_points, :]
