...
class ActuatorBase(ABC):
    ...
    def __init__(
        ...
        for param_name, usd_val in to_check:
            ...
            setattr(self, param_name,
                    self._parse_joint_parameter(cfg_val, usd_val))
            new_val = getattr(self, param_name)

            allclose = (torch.all(new_val == usd_val) if isinstance(
                usd_val, (float, int)) else torch.allclose(new_val, usd_val))
            if cfg_val is None or not allclose:
                self._record_actuator_resolution(
                    cfg_val=getattr(self.cfg, param_name),
                    new_val=new_val[
                        0],  # new val always has the shape of (num_envs, num_joints)
                    usd_val=usd_val,
        ...
        self.velocity_limit = self._parse_joint_parameter(
            self.cfg.velocity_limit, self.velocity_limit_sim)
        ...
        self.effort_limit = self._parse_joint_parameter(
            self.cfg.effort_limit, effort_default)

        # create commands buffers for allocation
        self.computed_effort = torch.zeros(self._num_envs,
                                           self.num_joints,
                                           device=self._device)
    ...
    @abstractmethod
    def compute(self, control_action: ArticulationActions,
    ...
    def _record_actuator_resolution(self, cfg_val, new_val, usd_val,
        ...
        ids = joint_ids if isinstance(joint_ids, torch.Tensor) else list(
            range(len(joint_names)))
        for idx, name in enumerate(joint_names):
            cfg_val_log = "Not Specified" if cfg_val is None else float(
                new_val[idx])
            default_usd_val = usd_val if isinstance(
                usd_val, (float, int)) else float(usd_val[0][idx])
            applied_val_log = default_usd_val if cfg_val is None else float(
                new_val[idx])
            table.append([
                name,
                int(ids[idx]), default_usd_val, cfg_val_log, applied_val_log
            ])

    def _parse_joint_parameter(
            self, cfg_value: float | dict[str, float] | None,
        ...
        param = torch.zeros(self._num_envs,
                            self.num_joints,
                            device=self._device)
        # parse the parameter
