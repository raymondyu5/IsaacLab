...
class TerrainGenerator:
    ...
    def __init__(self, cfg: TerrainGeneratorCfg, device: str = "cpu"):
        ...
        # -- terrain mesh
        transform = np.eye(4)
        transform[:2, -1] = -self.cfg.size[
            0] * self.cfg.num_rows * 0.5, -self.cfg.size[
                1] * self.cfg.num_cols * 0.5
        self.terrain_mesh.apply_transform(transform)
        # -- terrain origins
        self.terrain_origins += transform[:3, -1]
        # -- valid patches
        terrain_origins_torch = torch.tensor(self.terrain_origins,
                                             dtype=torch.float,
    ...
    def _generate_random_terrains(self):
        ...
        for index in range(self.cfg.num_rows * self.cfg.num_cols):
            ...
            # generate terrain
            mesh, origin = self._get_terrain_mesh(difficulty,
    ...
    def _generate_curriculum_terrains(self):
        ...
        for index in range(self.cfg.num_cols):
            sub_index = np.min(
                np.where(index / self.cfg.num_cols +
                         0.001 < np.cumsum(proportions))[0])
        ...
        for sub_col in range(self.cfg.num_cols):
            for sub_row in range(self.cfg.num_rows):
                ...
                difficulty = (sub_row +
                              self.np_rng.uniform()) / self.cfg.num_rows
    ...
    def _add_terrain_border(self):
        ...
        inner_size = (self.cfg.num_rows * self.cfg.size[0],
        ...
        # border mesh
        border_meshes = make_border(border_size,
                                    inner_size,
    ...
    def _add_sub_terrain(self, mesh: trimesh.Trimesh, origin: np.ndarray,
                         row: int, col: int,
    ...
    def _get_terrain_mesh(
            ...
            cfg: SubTerrainBaseCfg) -> tuple[trimesh.Trimesh, np.ndarray]:
        ...
        sub_terrain_cache_dir = os.path.join(self.cfg.cache_dir,
                                             sub_terrain_hash)
        sub_terrain_obj_filename = os.path.join(sub_terrain_cache_dir,
                                                "mesh.obj")
        sub_terrain_csv_filename = os.path.join(sub_terrain_cache_dir,
                                                "origin.csv")
        sub_terrain_meta_filename = os.path.join(sub_terrain_cache_dir,
                                                 "cfg.yaml")
        ...
        if self.cfg.use_cache:
            ...
            np.savetxt(sub_terrain_csv_filename,
                       origin,
                       delimiter=",",
                       header="x,y,z")
