...
class Cloner:
    ...
    def replicate_physics(self, source_prim_path: str, prim_paths: list,
                          base_env_path: str, root_path: str):
        ...
        num_replications = len(
            prim_paths) if replicate_first else len(prim_paths) - 1
        ...
        def replicationAttachEndFn(stageId):
            get_physx_replicator_interface().replicate(stageId,
                                                       source_prim_path,
                                                       num_replications)
        ...
        get_physx_replicator_interface().register_replicator(
            stageId, replicationAttachFn, replicationAttachEndFn,
            hierarchyRenameFn)
    ...
    def clone(
        ...
        if positions is not None:
            if len(positions) != len(prim_paths):
                raise ValueError(
                    "Dimension mismatch between positions and prim_paths!")
        ...
        if orientations is not None:
            if len(orientations) != len(prim_paths):
                raise ValueError(
                    "Dimension mismatch between orientations and prim_paths!")
            # convert to numpy array
        ...
        T_l_p = Gf.Transform()
        T_l_p.SetMatrix(
            Gf.Matrix4d(np.matmul(T_l_w, np.linalg.inv(T_p_w)).tolist()))
        ...
        if "xformOp:scale" in properties:
            current_scale = Gf.Vec3d(
                source_prim.GetAttribute("xformOp:scale").Get())
        ...
        if "xformOp:translate" not in properties:
            ...
            xform_op_translate = UsdGeom.XformOp(
                source_prim.GetAttribute("xformOp:translate"))
        ...
        if "xformOp:orient" not in properties:
            ...
            xform_op_rot = UsdGeom.XformOp(
                source_prim.GetAttribute("xformOp:orient"))
            if source_prim.GetAttribute(
                    "xformOp:orient").GetTypeName() == "quatf":
        ...
        if "xformOp:scale" not in properties:
            ...
            xform_op_scale = UsdGeom.XformOp(
                source_prim.GetAttribute("xformOp:scale"))
        xform_op_scale.Set(current_scale)
        # set xform op order
        xformable.SetXformOpOrder(
            [xform_op_translate, xform_op_rot, xform_op_scale])
        ...
        with Sdf.ChangeBlock():
            for i, prim_path in enumerate(prim_paths):
                if prim_path != source_prim_path:
                    ...
                    env_spec = Sdf.CreatePrimInLayer(
                        self._stage.GetRootLayer(), prim_path)
                    stack = UsdGeom.Xform(
                        self._stage.GetPrimAtPath(
                            source_prim_path)).GetPrim().GetPrimStack()

                    if copy_from_source:
                        Sdf.CopySpec(env_spec.layer,
                                     Sdf.Path(source_prim_path),
                                     env_spec.layer, Sdf.Path(prim_path))
                    else:
                    ...
                    if translate_spec is None:
                        translate_spec = Sdf.AttributeSpec(
                            env_spec, "xformOp:translate",
                            Sdf.ValueTypeNames.Double3)
                    translate_spec.default = translation

                    orient_spec = env_spec.GetAttributeAtPath(
                        prim_path + ".xformOp:orient")
                    if orient_spec is None:
                        orient_spec = Sdf.AttributeSpec(
                            env_spec, "xformOp:orient",
                            Sdf.ValueTypeNames.Quatd)
                    orient_spec.default = orientation

                    scale_spec = env_spec.GetAttributeAtPath(prim_path +
                                                             ".xformOp:scale")
                    if scale_spec is None:
                        scale_spec = Sdf.AttributeSpec(
                            env_spec, "xformOp:scale",
                            Sdf.ValueTypeNames.Double3)
                    scale_spec.default = current_scale

                    op_order_spec = env_spec.GetAttributeAtPath(
                        prim_path + ".xformOpOrder")
                    if op_order_spec is None:
                        op_order_spec = Sdf.AttributeSpec(
                            env_spec, UsdGeom.Tokens.xformOpOrder,
        ...
        if replicate_physics and has_clones:
            self.replicate_physics(source_prim_path, prim_paths, base_env_path,
                                   root_path)
        else:
    ...
    def filter_collisions(self,
        ...
        physx_scene = PhysxSchema.PhysxSceneAPI(
            self._stage.GetPrimAtPath(physicsscene_path))
        ...
        collision_scope = UsdGeom.Scope.Define(self._stage,
                                               collision_root_path)
        ...
        with Sdf.ChangeBlock():
            if len(global_paths) > 0:
                ...
                global_collision_group = Sdf.PrimSpec(
                    self._stage.GetRootLayer().GetPrimAtPath(
                        collision_root_path),
                    "global_group",
                    Sdf.SpecifierDef,
                ...
                global_includes_rel = Sdf.RelationshipSpec(
                    global_collision_group, "collection:colliders:includes",
                    False)
                ...
                # each other.
                global_filtered_groups.targetPathList.Append(
                    global_collision_group_path)
            ...
            for i, prim_path in enumerate(prim_paths):
                ...
                if len(global_paths) > 0:
                    filtered_groups.targetPathList.Append(
                        global_collision_group_path)
                    global_filtered_groups.targetPathList.Append(
