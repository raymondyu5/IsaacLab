...
def spawn_multi_asset(
    ...
    for index, asset_cfg in enumerate(cfg.assets_cfg):
        ...
        attr_names = [
            "mass_props", "rigid_props", "collision_props",
            "activate_contact_sensors", "deformable_props"
        ]
    ...
    with Sdf.ChangeBlock():
        for index, prim_path in enumerate(prim_paths):
            ...
            # copy the proto prim
            Sdf.CopySpec(env_spec.layer, Sdf.Path(proto_path), env_spec.layer,
                         Sdf.Path(prim_path))
...
def spawn_multi_usd_file(
    ...
    return spawn_multi_asset(prim_path, multi_asset_cfg, translation,
                             orientation, clone_in_fabric, replicate_physics)
