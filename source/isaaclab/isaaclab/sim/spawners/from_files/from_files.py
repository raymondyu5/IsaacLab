...
def spawn_from_urdf(
    ...
    **kwargs,
    ...
    return _spawn_from_usd_file(prim_path, urdf_loader.usd_path, cfg,
    ...
    **kwargs,
    ...
    if cfg.physics_material is not None:
        ...
        if collision_prim is not None:
            ...
            omni.log.warn(
                f"No collision prim with type 'Plane' found under '{prim_path}'. "
                "Physics material will not be applied to ground plane."
            )
    ...
    if prim_utils.is_prim_path_valid(f"{prim_path}/Environment"):
        ...
        prim_utils.set_prim_property(f"{prim_path}/Environment",
                                     "xformOp:scale", scale)
    ...
    if prim_utils.is_prim_path_valid(f"{prim_path}/SphereLight"):
        ...
        omni.kit.commands.execute("ToggleVisibilitySelectedPrims",
                                  selected_paths=[f"{prim_path}/SphereLight"],
                                  stage=stage)
...
def _spawn_from_usd_file(
    ...
    **kwargs,
    ...
    if not check_usd_path_with_timeout(usd_path):
        if "4.5" in usd_path:
            usd_5_0_path = usd_path.replace("http",
                                            "https").replace("/4.5", "/5.0")
            if not check_usd_path_with_timeout(usd_5_0_path):
                raise FileNotFoundError(
                    f"USD file not found at path at either: '{usd_path}' or '{usd_5_0_path}'."
                )
            usd_path = usd_5_0_path
        else:
            raise FileNotFoundError(
                f"USD file not found at path at: '{usd_path}'.")
    ...
    if cfg.articulation_props is not None:
        schemas.modify_articulation_root_properties(prim_path,
                                                    cfg.articulation_props)
    # modify tendon properties
    if cfg.fixed_tendons_props is not None:
        schemas.modify_fixed_tendon_properties(prim_path,
                                               cfg.fixed_tendons_props)
    if cfg.spatial_tendons_props is not None:
        schemas.modify_spatial_tendon_properties(prim_path,
    ...
    if cfg.deformable_props is not None:
        schemas.modify_deformable_body_properties(prim_path,
