...
def modify_articulation_root_properties(
        prim_path: str,
        cfg: schemas_cfg.ArticulationRootPropertiesCfg,
        stage: Usd.Stage | None = None) -> bool:
    ...
    physx_articulation_api = PhysxSchema.PhysxArticulationAPI(
        articulation_prim)
    ...
    for attr_name, value in cfg.items():
        safe_set_attribute_on_usd_schema(physx_articulation_api,
                                         attr_name,
                                         value,
                                         camel_case=True)
    ...
    if fix_root_link is not None:
        ...
        if existing_fixed_joint_prim is not None:
            ...
            logger.info(
                f"Creating a fixed joint for the articulation: '{prim_path}'.")
            ...
            physx_utils.createJoint(stage=stage,
                                    joint_type="Fixed",
                                    from_prim=None,
            ...
            # copy the attributes
            # -- usd attributes
            usd_articulation_api = UsdPhysics.ArticulationRootAPI(
                articulation_prim)
...
def define_rigid_body_properties(prim_path: str,
...
@apply_nested
def modify_rigid_body_properties(prim_path: str,
                                 cfg: schemas_cfg.RigidBodyPropertiesCfg,
                                 stage: Usd.Stage | None = None) -> bool:
    ...
    if not physx_rigid_body_api:
        physx_rigid_body_api = PhysxSchema.PhysxRigidBodyAPI.Apply(
            rigid_body_prim)
    ...
    for attr_name in ["rigid_body_enabled", "kinematic_enabled"]:
        ...
        safe_set_attribute_on_usd_schema(usd_rigid_body_api,
                                         attr_name,
                                         value,
                                         camel_case=True)
    # set into PhysX API
    for attr_name, value in cfg.items():
        safe_set_attribute_on_usd_schema(physx_rigid_body_api,
                                         attr_name,
                                         value,
                                         camel_case=True)
    # success
...
def define_collision_properties(prim_path: str,
...
@apply_nested
def modify_collision_properties(prim_path: str,
                                cfg: schemas_cfg.CollisionPropertiesCfg,
                                stage: Usd.Stage | None = None) -> bool:
    ...
    if not physx_collision_api:
        physx_collision_api = PhysxSchema.PhysxCollisionAPI.Apply(
            collider_prim)
    ...
    for attr_name in ["collision_enabled"]:
        value = cfg.pop(attr_name, None)
        safe_set_attribute_on_usd_schema(usd_collision_api,
                                         attr_name,
                                         value,
                                         camel_case=True)
    # set into PhysX API
    for attr_name, value in cfg.items():
        safe_set_attribute_on_usd_schema(physx_collision_api,
                                         attr_name,
                                         value,
                                         camel_case=True)
    # success
...
def define_mass_properties(prim_path: str,
                           cfg: schemas_cfg.MassPropertiesCfg,
                           stage: Usd.Stage | None = None):
...
@apply_nested
def modify_mass_properties(prim_path: str,
                           cfg: schemas_cfg.MassPropertiesCfg,
                           stage: Usd.Stage | None = None) -> bool:
    ...
    for attr_name in ["mass", "density"]:
        value = cfg.pop(attr_name, None)
        safe_set_attribute_on_usd_schema(usd_physics_mass_api,
                                         attr_name,
                                         value,
                                         camel_case=True)
    # success
...
def activate_contact_sensors(prim_path: str,
                             threshold: float = 0.0,
    ...
    while len(all_prims) > 0:
        ...
        if child_prim.HasAPI(UsdPhysics.RigidBodyAPI):
            ...
            if not child_prim.HasAPI(PhysxSchema.PhysxContactReportAPI):
                logger.debug(
                    f"Adding contact report API to prim: '{child_prim.GetPrimPath()}'"
                )
                cr_api = PhysxSchema.PhysxContactReportAPI.Apply(child_prim)
            else:
                logger.debug(
                    f"Contact report API already exists on prim: '{child_prim.GetPrimPath()}'"
                )
                cr_api = PhysxSchema.PhysxContactReportAPI.Get(
                    stage, child_prim.GetPrimPath())
...
@apply_nested
def modify_joint_drive_properties(prim_path: str,
                                  cfg: schemas_cfg.JointDrivePropertiesCfg,
                                  stage: Usd.Stage | None = None) -> bool:
    ...
    if prim.HasAPI(PhysxSchema.PhysxTendonAxisAPI) and not prim.HasAPI(
    ...
    for attr_name in ["max_velocity"]:
        ...
        safe_set_attribute_on_usd_schema(physx_joint_api,
                                         attr_name,
                                         value,
                                         camel_case=True)
    # set into USD API
    for attr_name, attr_value in cfg.items():
        ...
        safe_set_attribute_on_usd_schema(usd_drive_api,
                                         attr_name,
                                         attr_value,
                                         camel_case=True)
...
@apply_nested
def modify_fixed_tendon_properties(prim_path: str,
    ...
    has_root_fixed_tendon = tendon_prim.HasAPI(
        PhysxSchema.PhysxTendonAxisRootAPI)
    ...
    for schema_name in tendon_prim.GetAppliedSchemas():
        ...
        physx_tendon_axis_api = PhysxSchema.PhysxTendonAxisRootAPI(
            tendon_prim, instance_name)

        # convert to dict
        cfg = cfg.to_dict()
        # set into PhysX API
        for attr_name, value in cfg.items():
            safe_set_attribute_on_usd_schema(physx_tendon_axis_api,
                                             attr_name,
                                             value,
                                             camel_case=True)
    # success
...
def modify_spatial_tendon_properties(
        prim_path: str,
        cfg: schemas_cfg.SpatialTendonPropertiesCfg,
        stage: Usd.Stage | None = None) -> bool:
    ...
    has_spatial_tendon = tendon_prim.HasAPI(
        PhysxSchema.PhysxTendonAttachmentRootAPI) or tendon_prim.HasAPI(
    ...
    for schema_name in tendon_prim.GetAppliedSchemas():
        ...
        if "PhysxTendonAttachmentRootAPI" in schema_name:
            instance_name = schema_name.split(":")[-1]
            physx_tendon_spatial_api = PhysxSchema.PhysxTendonAttachmentRootAPI(
                tendon_prim, instance_name)
        elif "PhysxTendonAttachmentLeafAPI" in schema_name:
            instance_name = schema_name.split(":")[-1]
            physx_tendon_spatial_api = PhysxSchema.PhysxTendonAttachmentLeafAPI(
                tendon_prim, instance_name)
        else:
            continue
        # convert to dict
        ...
        # set into PhysX API
        for attr_name, value in cfg.items():
            safe_set_attribute_on_usd_schema(physx_tendon_spatial_api,
                                             attr_name,
                                             value,
                                             camel_case=True)
    # success
...
def define_deformable_body_properties(
    ...
    if len(matching_prims) == 0:
        raise ValueError(
            f"Could not find any mesh in '{prim_path}'. Please check asset.")
...
def modify_deformable_body_properties(
    ...
    ) or not PhysxSchema.PhysxDeformableBodyAPI(deformable_body_prim):
        return False

    # retrieve the physx deformable-body api
    physx_deformable_body_api = PhysxSchema.PhysxDeformableBodyAPI(
        deformable_body_prim)
    # retrieve the physx deformable api
    physx_deformable_api = PhysxSchema.PhysxDeformableAPI(
        physx_deformable_body_api)
    ...
    status = deformable_utils.add_physx_deformable_body(stage,
                                                        prim_path=prim_path,
                                                        **attr_kwargs)
    ...
    for attr_name, value in cfg.items():
        if attr_name in ["rest_offset", "contact_offset"]:
            ...
            safe_set_attribute_on_usd_schema(physx_deformable_api,
                                             attr_name,
                                             value,
                                             camel_case=True)
...
def extract_mesh_collision_api_and_attrs(cfg):
    ...
    # We have no custom attributes
        ...
        raise ValueError(
            "Args are specified but the USD Mesh API doesn't support them!")
    ...
    if use_usd_api:
        ...
        custom_attrs["Approximation"] = mesh_collision_appx_type[0].lower(
        ) + mesh_collision_appx_type[1:]
    elif use_phsyx_api:
        api_func = cfg.physx_func
    else:
        raise ValueError(
            "Either USD or PhysX API should be used for mesh collision approximation!"
        )
...
def modify_mesh_collision_properties(
    ...
    for attr_name, value in custom_attrs.items():
        ...
        if attr_name == "Attribute":
            ...
            camel_case = True
        safe_set_attribute_on_usd_schema(mesh_collision_api,
                                         attr_name,
                                         value,
