...
class SimulationContext(_SimulationContext):
    ...
    * adding and removing callbacks to different simulation events such as physics stepping, rendering, etc.
    ...
    def __init__(self, cfg: SimulationCfg | None = None):
        ...
        self._livestream_gui = self.carb_settings.get(
        ...
        self._gravity_tensor = torch.tensor(self.cfg.gravity,
                                            dtype=torch.float32,
        ...
        if not builtins.ISAAC_LAUNCHED_FROM_TERMINAL:
            ...
            self._app_control_on_stop_handle = timeline_event_stream.create_subscription_to_pop_by_type(
                ...
                lambda *args, obj=weakref.proxy(self): obj.
                _app_control_on_stop_handle_fn(*args),
                order=15,
            )
        else:
    ...
    def get_version(self) -> tuple[int, int, int]:
        ...
        * Patch version (int): This is the patch number of the release (e.g. 0).
        """
        return int(self._isaacsim_version[2]), int(
            self._isaacsim_version[3]), int(self._isaacsim_version[4])
    ...
    def _apply_physics_settings(self):
        ...
        set_carb_setting(self.carb_settings,
    ...
    def _apply_render_settings_from_cfg(self):
        ...
        rendering_mode = get_carb_setting(
            self.carb_settings, "/isaaclab/rendering/rendering_mode")
        ...
        if carb_settings is not None:
            for key, value in carb_settings.items():
                if "_" in key:
                    key = "/" + key.replace(
                        "_", "/")  # convert from python variable style string
                elif "." in key:
                    key = "/" + key.replace(
        ...
        if get_carb_setting(self.carb_settings,
                            "/rtx/rendermode").lower() == "raytracedlighting":
            set_carb_setting(self.carb_settings, "/rtx/rendermode",
                             "RaytracedLighting")
    ...
    def _set_additional_physx_params(self):
        ...
        self.cfg.physics_material.func(material_path,
                                       self.cfg.physics_material)
    ...
    def _update_anim_recording(self):
        ...
        if self._anim_recording_started_timestamp is not None:
            anim_recording_total_time = time.time(
            ) - self._anim_recording_started_timestamp
    ...
    def _setup_anim_recording(self):
        ...
        self._anim_recording_timestamp = datetime.now().strftime(
            "%Y_%m_%d_%H%M%S")
        self._anim_recording_output_dir = (os.path.join(
            repo_path, "anim_recordings",
            self._anim_recording_timestamp).replace("\\", "/").rstrip("/") +
                                           "/")
    ...
    def _update_usda_start_time(self, file_path, start_time):
        ...
        content = re.sub(r"startTimeCode\s*=\s*\d+",
                         f"startTimeCode = {new_start_time_code}", content)
    ...
    def _finish_anim_recording(self):
        ...
        # Save stage to disk
        stage_path = os.path.join(self._anim_recording_output_dir,
                                  "stage_simulation.usdc")
        stage_utils.save_stage(stage_path, save_and_reload_in_place=False)

        # Find the latest ovd file not named tmp.ovd
        ovd_files = [
            f for f in glob.glob(
                os.path.join(self._anim_recording_output_dir, "*.ovd"))
    ...
    def _setup_logger(self):
        ...
        if self.cfg.save_logs_to_file:
            ...
            log_file_path = os.path.join(
                temp_dir,
                f"isaaclab_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log")
...
def build_simulation_context(
    ...
    try:
        ...
        if add_lighting or (auto_add_lighting and sim.has_gui()):
            ...
            cfg.func(prim_path="/World/defaultDomeLight",
                     cfg=cfg,
