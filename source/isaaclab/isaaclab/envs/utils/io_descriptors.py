...
def _make_descriptor(**kwargs: Any) -> GenericObservationIODescriptor:
    ...
    field_names = {
        f.name
        for f in dataclasses.fields(GenericObservationIODescriptor)
    }
...
def generic_io_descriptor(
    ...
    **descriptor_kwargs: Any,
) -> Callable[[Callable[Concatenate[ManagerBasedEnv, P], R]], Callable[
        Concatenate[ManagerBasedEnv, P], R]]:
    """
    Decorator factory for generic IO descriptors.
    ...
    if callable(on_inspect):
        ...
        inspect_hooks: list[Callable[..., Any]] = list(on_inspect
                                                       or [])  # handles None
    ...
    def _apply(
        ...
        def wrapper(env: ManagerBasedEnv, *args: P.args,
                    **kwargs: P.kwargs) -> R:
...
def record_shape(output: torch.Tensor,
                 descriptor: GenericObservationIODescriptor, **kwargs) -> None:
...
def record_joint_names(output: torch.Tensor,
...
def record_body_names(output: torch.Tensor,
                      descriptor: GenericObservationIODescriptor,
                      **kwargs) -> None:
...
def export_articulations_data(
        env: ManagerBasedEnv) -> dict[str, dict[str, list[float]]]:
    ...
    for articulation_name, articulation in env.scene.articulations.items():
        ...
        articulation_joint_data[articulation_name]["default_joint_pos"] = (
            articulation.data.default_joint_pos[0].detach().cpu().numpy(
            ).tolist())
        articulation_joint_data[articulation_name]["default_joint_vel"] = (
            articulation.data.default_joint_vel[0].detach().cpu().numpy(
            ).tolist())
        articulation_joint_data[articulation_name][
            "default_joint_pos_limits"] = (
                articulation.data.default_joint_pos_limits[0].detach().cpu(
                ).numpy().tolist())
        articulation_joint_data[articulation_name]["default_joint_damping"] = (
            articulation.data.default_joint_damping[0].detach().cpu().numpy(
            ).tolist())
        articulation_joint_data[articulation_name][
            "default_joint_stiffness"] = (
                articulation.data.default_joint_stiffness[0].detach().cpu(
                ).numpy().tolist())
        articulation_joint_data[articulation_name][
            "default_joint_friction"] = (
                articulation.data.default_joint_friction[0].detach().cpu(
                ).numpy().tolist())
        articulation_joint_data[articulation_name][
            "default_joint_armature"] = (
                articulation.data.default_joint_armature[0].detach().cpu(
                ).numpy().tolist())
