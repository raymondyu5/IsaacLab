...
class BinaryJointAction(ActionTerm):
    ...
    def __init__(self, cfg: actions_cfg.BinaryJointActionCfg,
        ...
        self._open_command[index_list] = torch.tensor(value_list,
                                                      device=self.device)
        ...
        self._close_command[index_list] = torch.tensor(value_list,
                                                       device=self.device)
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        if actions.dtype == torch.bool:
            ...
            binary_mask = actions < 0
        # compute the command
        self._processed_actions = torch.where(binary_mask, self._close_command,
                                              self._open_command)
        if self.cfg.clip is not None:
...
class BinaryJointVelocityAction(BinaryJointAction):
    ...
    def apply_actions(self):
        self._asset.set_joint_velocity_target(self._processed_actions,
                                              joint_ids=self._joint_ids)
...
class AbsBinaryJointPositionAction(BinaryJointAction):
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        if self.cfg.positive_threshold:
            ...
            binary_mask = actions < self.cfg.threshold
        # compute the command
        self._processed_actions = torch.where(binary_mask, self._open_command,
                                              self._close_command)
        if self.cfg.clip is not None:
            self._processed_actions = torch.clamp(self._processed_actions,
                                                  min=self._clip[:, :, 0],
