...
class JointPositionToLimitsAction(ActionTerm):
    ...
    def __init__(self, cfg: actions_cfg.JointPositionToLimitsActionCfg,
                 env: ManagerBasedEnv):
...
class EMAJointPositionToLimitsAction(JointPositionToLimitsAction):
    ...
    def __init__(self, cfg: actions_cfg.EMAJointPositionToLimitsActionCfg,
                 env: ManagerBasedEnv):
        # initialize the action term
        super().__init__(cfg, env)
        ...
        if isinstance(cfg.alpha, float):
            ...
            self._alpha = torch.ones((env.num_envs, self.action_dim),
                                     device=self.device)
            # resolve the dictionary config
            ...
            self._alpha[:, index_list] = torch.tensor(value_list,
                                                      device=self.device)
        else:
            raise ValueError(
    ...
    def IO_descriptor(self) -> GenericActionIODescriptor:
        ...
        if isinstance(self._alpha, float):
            ...
            self._IO_descriptor.alpha = self._alpha[0].detach().cpu().numpy(
            ).tolist()
        else:
            raise ValueError(
