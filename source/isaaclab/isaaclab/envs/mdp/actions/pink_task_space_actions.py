...
class PinkInverseKinematicsAction(ActionTerm):
    ...
    def __init__(self, cfg: pink_actions_cfg.PinkInverseKinematicsActionCfg,
                 env: ManagerBasedEnv):
    ...
    def _initialize_joint_info(self) -> None:
        ...
        self._isaaclab_all_joint_ids = list(
            range(len(self._asset.data.joint_names)))
        ...
        for _ in range(self._env.num_envs):
            self._ik_controllers.append(
                PinkIKController(
                    ...
                    device=self.device,
                    controlled_joint_indices=self.
                    _isaaclab_controlled_joint_ids,
                ))
    ...
    def _initialize_helper_tensors(self) -> None:
        ...
        articulation_data = self._env.scene[
            self.cfg.controller.articulation_name].data
        ...
        num_frame_tasks = sum(
            1 for task in self._ik_controllers[0].cfg.variable_input_tasks
            if isinstance(task, FrameTask))
        self._num_frame_tasks = num_frame_tasks
        self._controlled_frame_poses = torch.zeros(num_frame_tasks,
                                                   self.num_envs,
                                                   4,
                                                   4,
                                                   device=self.device)
        ...
        self._base_link_frame_buffer = torch.zeros(self.num_envs,
                                                   4,
                                                   4,
                                                   device=self.device)
    ...
    def _extract_controlled_frame_poses(self,
    ...
    def _transform_poses_to_base_link_frame(
            self, poses: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        transformed_poses = math_utils.pose_in_A_to_pose_in_B(
            poses, base_link_inv)
    ...
    def _set_task_targets(
            self, transformed_poses: tuple[torch.Tensor,
                                           torch.Tensor]) -> None:
        ...
        for env_index, ik_controller in enumerate(self._ik_controllers):
            for frame_task_index, task in enumerate(
                ...
                target.translation = positions[frame_task_index,
                                               env_index, :].cpu().numpy()
                target.rotation = rotation_matrices[
                    frame_task_index, env_index, :].cpu().numpy()
    ...
    def _apply_gravity_compensation(self) -> None:
        ...
        if not self._asset.cfg.spawn.rigid_props.disable_gravity:
            ...
            if self._asset.is_fixed_base:
                gravity = torch.zeros_like(self._asset.root_physx_view.
            ...
            # Apply gravity compensation to arm joints
            self._asset.set_joint_effort_target(gravity,
                                                self._controlled_joint_ids)
    ...
    def _compute_ik_solutions(self) -> torch.Tensor:
        ...
        for env_index, ik_controller in enumerate(self._ik_controllers):
            ...
            current_joint_pos = self._asset.data.joint_pos.cpu().numpy(
            )[env_index]

            # Compute IK solution
            joint_pos_des = ik_controller.compute(current_joint_pos,
                                                  self._sim_dt)
    ...
    def reset(self, env_ids: Sequence[int] | None = None) -> None:
        ...
        """
        self._raw_actions[env_ids] = torch.zeros(self.action_dim,
                                                 device=self.device)
