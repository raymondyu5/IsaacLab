...
def randomize_rigid_body_scale(
    ...
    if isinstance(scale_range, dict):
        range_list = [
            scale_range.get(key, (1.0, 1.0)) for key in ["x", "y", "z"]
        ]
        ...
        rand_samples = math_utils.sample_uniform(*scale_range,
                                                 (len(env_ids), 1),
                                                 device="cpu")
    ...
    with Sdf.ChangeBlock():
        for i, env_id in enumerate(env_ids):
            ...
            if not has_scale_attr:
                scale_spec = Sdf.AttributeSpec(prim_spec,
            ...
            if not has_scale_attr:
                op_order_spec = prim_spec.GetAttributeAtPath(prim_path +
                                                             ".xformOpOrder")
                if op_order_spec is None:
                    op_order_spec = Sdf.AttributeSpec(
                        prim_spec, UsdGeom.Tokens.xformOpOrder,
...
class randomize_rigid_body_material(ManagerTermBase):
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        static_friction_range = cfg.params.get("static_friction_range",
                                               (1.0, 1.0))
        dynamic_friction_range = cfg.params.get("dynamic_friction_range",
                                                (1.0, 1.0))
        ...
        ranges = torch.tensor(range_list, device="cpu")
        self.material_buckets = math_utils.sample_uniform(ranges[:, 0],
                                                          ranges[:, 1],
                                                          (num_buckets, 3),
                                                          device="cpu")
        ...
        if make_consistent:
            self.material_buckets[:,
                                  1] = torch.min(self.material_buckets[:, 0],
    ...
    def __call__(
        ...
        bucket_ids = torch.randint(0,
                                   num_buckets,
                                   (len(env_ids), total_num_shapes),
                                   device="cpu")
        ...
        if self.num_shapes_per_body is not None:
            ...
            for body_id in self.asset_cfg.body_ids:
                ...
                materials[
                    env_ids,
                    start_idx:end_idx] = material_samples[:, start_idx:end_idx]
        else:
...
class randomize_rigid_body_mass(ManagerTermBase):
    ...
    def __call__(
        ...
        if self.asset_cfg.body_ids == slice(None):
            body_ids = torch.arange(self.asset.num_bodies,
                                    dtype=torch.int,
                                    device="cpu")
        else:
            body_ids = torch.tensor(self.asset_cfg.body_ids,
                                    dtype=torch.int,
                                    device="cpu")
        ...
        masses[env_ids[:, None],
               body_ids] = self.asset.data.default_mass[env_ids[:, None],
                                                        body_ids].clone()
        ...
        if recompute_inertia:
            ...
            ratios = masses[env_ids[:, None],
                            body_ids] / self.asset.data.default_mass[
            ...
            if isinstance(self.asset, Articulation):
                ...
                inertias[env_ids] = self.asset.data.default_inertia[
                    env_ids] * ratios
...
def randomize_rigid_body_com(
    ...
    if asset_cfg.body_ids == slice(None):
        body_ids = torch.arange(asset.num_bodies,
                                dtype=torch.int,
                                device="cpu")
    else:
        body_ids = torch.tensor(asset_cfg.body_ids,
                                dtype=torch.int,
                                device="cpu")
...
def randomize_rigid_body_collider_offsets(
    ...
    if contact_offset_distribution_params is not None:
        ...
        asset.root_physx_view.set_contact_offsets(contact_offset,
                                                  env_ids.cpu())
...
def randomize_physics_scene_gravity(
    ...
    physics_sim_view: physx.SimulationView = sim_utils.SimulationContext.instance(
    ).physics_sim_view
...
class randomize_actuator_gains(ManagerTermBase):
    ...
    def __call__(
        ...
        def randomize(data: torch.Tensor,
                      params: tuple[float, float]) -> torch.Tensor:
        ...
        for actuator in self.asset.actuators.values():
            ...
            if stiffness_distribution_params is not None:
                ...
                stiffness[:,
                          actuator_indices] = self.asset.data.default_joint_stiffness[
                              env_ids][:, global_indices].clone()
            ...
            if damping_distribution_params is not None:
                ...
                damping[:,
                        actuator_indices] = self.asset.data.default_joint_damping[
                            env_ids][:, global_indices].clone()
...
class randomize_joint_parameters(ManagerTermBase):
    ...
    def __call__(
        ...
        if self.asset_cfg.joint_ids == slice(None):
            ...
            joint_ids = torch.tensor(self.asset_cfg.joint_ids,
                                     dtype=torch.int,
                                     device=self.asset.device)
        ...
        if friction_distribution_params is not None:
            ...
            if major_version >= 5:
                ...
                dynamic_friction_coeff = torch.clamp(dynamic_friction_coeff,
                                                     min=0.0)
                viscous_friction_coeff = torch.clamp(viscous_friction_coeff,
                                                     min=0.0)

                # Ensure dynamic ≤ static (same shape before indexing)
                dynamic_friction_coeff = torch.minimum(dynamic_friction_coeff,
                                                       friction_coeff)

                # Index once at the end
                dynamic_friction_coeff = dynamic_friction_coeff[env_ids[:,
                                                                        None],
                                                                joint_ids]
                viscous_friction_coeff = viscous_friction_coeff[env_ids[:,
                                                                        None],
                                                                joint_ids]
            else:
        ...
        if armature_distribution_params is not None:
            ...
            self.asset.write_joint_armature_to_sim(armature[env_ids[:, None],
                                                            joint_ids],
                                                   joint_ids=joint_ids,
                                                   env_ids=env_ids)
...
class randomize_fixed_tendon_parameters(ManagerTermBase):
    ...
    def __call__(
        ...
        if self.asset_cfg.fixed_tendon_ids == slice(None):
            ...
            tendon_ids = torch.tensor(self.asset_cfg.fixed_tendon_ids,
                                      dtype=torch.int,
                                      device=self.asset.device)
        ...
        if stiffness_distribution_params is not None:
            ...
            self.asset.set_fixed_tendon_stiffness(
                stiffness[env_ids[:, None], tendon_ids], tendon_ids, env_ids)
        ...
        if damping_distribution_params is not None:
            ...
            self.asset.set_fixed_tendon_damping(
                damping[env_ids[:, None], tendon_ids], tendon_ids, env_ids)
        ...
        if limit_stiffness_distribution_params is not None:
            ...
            self.asset.set_fixed_tendon_limit_stiffness(
                limit_stiffness[env_ids[:, None], tendon_ids], tendon_ids,
                env_ids)
        ...
        if lower_limit_distribution_params is not None or upper_limit_distribution_params is not None:
            ...
            self.asset.set_fixed_tendon_position_limit(tendon_limits,
                                                       tendon_ids, env_ids)
        ...
        if rest_length_distribution_params is not None:
            ...
            self.asset.set_fixed_tendon_rest_length(
                rest_length[env_ids[:, None], tendon_ids], tendon_ids, env_ids)
        ...
        if offset_distribution_params is not None:
            ...
            self.asset.set_fixed_tendon_offset(
                offset[env_ids[:, None], tendon_ids], tendon_ids, env_ids)
...
def apply_external_force_torque(
    ...
    num_bodies = len(asset_cfg.body_ids) if isinstance(
        asset_cfg.body_ids, list) else asset.num_bodies
    ...
    asset.set_external_force_and_torque(forces,
                                        torques,
                                        env_ids=env_ids,
...
def push_by_setting_velocity(
    ...
    vel_w += math_utils.sample_uniform(ranges[:, 0],
                                       ranges[:, 1],
                                       vel_w.shape,
                                       device=asset.device)
...
def reset_root_state_with_random_orientation(
    ...
    * :attr:`velocity_range` - a dictionary of velocity ranges for each axis and rotation. The keys of the dictionary
    ...
    positions = root_states[:, 0:3] + env.scene.env_origins[
        env_ids] + rand_samples
    orientations = math_utils.random_orientation(len(env_ids),
                                                 device=asset.device)
...
def reset_root_state_from_terrain(
    ...
    * :attr:`velocity_range` - a dictionary of velocity ranges for each axis and rotation. The keys of the dictionary
    ...
    ids = torch.randint(0,
                        valid_positions.shape[2],
                        size=(len(env_ids), ),
                        device=env.device)
    positions = valid_positions[terrain.terrain_levels[env_ids],
                                terrain.terrain_types[env_ids], ids]
    ...
    orientations = math_utils.quat_from_euler_xyz(rand_samples[:, 0],
                                                  rand_samples[:, 1],
                                                  rand_samples[:, 2])
...
def reset_joints_by_scale(
    ...
    joint_pos = asset.data.default_joint_pos[iter_env_ids,
                                             asset_cfg.joint_ids].clone()
    joint_vel = asset.data.default_joint_vel[iter_env_ids,
                                             asset_cfg.joint_ids].clone()

    # scale these values randomly
    joint_pos *= math_utils.sample_uniform(*position_range, joint_pos.shape,
                                           joint_pos.device)
    joint_vel *= math_utils.sample_uniform(*velocity_range, joint_vel.shape,
                                           joint_vel.device)

    # clamp joint pos to limits
    joint_pos_limits = asset.data.soft_joint_pos_limits[iter_env_ids,
                                                        asset_cfg.joint_ids]
    joint_pos = joint_pos.clamp_(joint_pos_limits[..., 0],
                                 joint_pos_limits[..., 1])
    # clamp joint vel to limits
    joint_vel_limits = asset.data.soft_joint_vel_limits[iter_env_ids,
                                                        asset_cfg.joint_ids]
    joint_vel = joint_vel.clamp_(-joint_vel_limits, joint_vel_limits)

    # set into the physics simulation
    asset.write_joint_state_to_sim(joint_pos,
                                   joint_vel,
...
def reset_nodal_state_uniform(
    ...
    * It samples the root velocity from the given ranges and sets them into the physics simulation.
    ...
    # velocities
    range_list = [
        velocity_range.get(key, (0.0, 0.0)) for key in ["x", "y", "z"]
    ]
    ranges = torch.tensor(range_list, device=asset.device)
    rand_samples = math_utils.sample_uniform(ranges[:, 0],
                                             ranges[:, 1],
...
def reset_scene_to_default(env: ManagerBasedEnv,
                           env_ids: torch.Tensor,
    ...
    for rigid_object in env.scene.rigid_objects.values():
        ...
        default_root_state = rigid_object.data.default_root_state[
            env_ids].clone()
    ...
    for articulation_asset in env.scene.articulations.values():
        ...
        default_root_state[:, 0:3] += env.scene.env_origins[env_ids]
        # set into the physics simulation
        articulation_asset.write_root_pose_to_sim(default_root_state[:, :7],
                                                  env_ids=env_ids)
        articulation_asset.write_root_velocity_to_sim(default_root_state[:,
                                                                         7:],
                                                      env_ids=env_ids)
        ...
        default_joint_vel = articulation_asset.data.default_joint_vel[
            env_ids].clone()
        # set into the physics simulation
        articulation_asset.write_joint_state_to_sim(default_joint_pos,
                                                    default_joint_vel,
                                                    env_ids=env_ids)
        # reset joint targets if required
        if reset_joint_targets:
            articulation_asset.set_joint_position_target(default_joint_pos,
                                                         env_ids=env_ids)
            articulation_asset.set_joint_velocity_target(default_joint_vel,
                                                         env_ids=env_ids)
    # deformable objects
    for deformable_object in env.scene.deformable_objects.values():
        # obtain default and set into the physics simulation
        nodal_state = deformable_object.data.default_nodal_state_w[
            env_ids].clone()
...
class randomize_visual_texture_material(ManagerTermBase):
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        version = re.match(r"^(\d+\.\d+\.\d+)",
                           rep.__file__.split("/")[-5][21:]).group(1)
        ...
        if compare_versions(version, "1.12.4") < 0:
            ...
            texture_rotation = tuple(
                math.degrees(angle) for angle in texture_rotation)
            ...
            def rep_texture_randomization():
                ...
                with prims_group:
                    rep.randomizer.texture(
                        ...
                        project_uvw=True,
                        texture_rotate=rep.distribution.uniform(
                            *texture_rotation),
                    )
            ...
            # acquire stage
            stage = get_current_stage()
            prims_group = rep.functional.get.prims(path_pattern=prim_path,
                                                   stage=stage)
    ...
    def __call__(
        ...
        if compare_versions(version, "1.12.4") < 0:
            ...
            # read parameters from the configuration
            texture_paths = texture_paths if texture_paths else self._cfg.params.get(
                "texture_paths")
            texture_rotation = (texture_rotation if texture_rotation else
                                self._cfg.params.get("texture_rotation",
                                                     (0.0, 0.0)))
            ...
            random_textures = self.texture_rng.generator.choice(texture_paths,
                                                                size=num_prims)
            ...
            rep.functional.modify.attribute(self.material_prims,
                                            "diffuse_texture", random_textures)
            rep.functional.modify.attribute(self.material_prims,
                                            "texture_rotate", random_rotations)
...
class randomize_visual_color(ManagerTermBase):
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        if compare_versions(version, "1.12.4") < 0:
            ...
            stage = get_current_stage()
            prims_group = rep.functional.get.prims(path_pattern=mesh_prim_path,
                                                   stage=stage)
    ...
    def __call__(
        ...
        if compare_versions(version, "1.12.4") < 0:
            ...
            num_prims = len(self.material_prims)
            random_colors = self.color_rng.generator.uniform(colors[0],
                                                             colors[1],
                                                             size=(num_prims,
                                                                   3))
...
def _randomize_prop_by_op(
    ...
    if operation == "add":
        data[dim_0_ids, dim_1_ids] += dist_fn(*distribution_parameters,
                                              (n_dim_0, n_dim_1),
                                              device=data.device)
...
def reset_joints_by_values(
    env,
    env_ids: torch.Tensor,
    joint_pose: tuple[float, float],
    ...
    robot._data.reset_joint_pos = joint_pose.to(
        env.device).unsqueeze(0).repeat_interleave(env.num_envs, 0)

    robot.root_physx_view.set_dof_positions(robot._data.reset_joint_pos,
                                            indices=env_ids)


def interpolate_joints(q1, q2, num_samples=10):
    """
    Linearly interpolate between two joint configurations.
    Args:
        q1 (torch.Tensor): shape (d,)
        q2 (torch.Tensor): shape (d,)
        num_samples (int): number of points including endpoints
    Returns:
        torch.Tensor: shape (num_samples, d)
    """

    alphas = torch.distributions.Beta(5.0, 5.0).sample((num_samples, 1))
    # alphas = torch.rand(num_samples, 1, device=q1.device)

    q1 = q1.unsqueeze(0)  # (1, d)
    q2 = q2.unsqueeze(0)  # (1, d)
    return (1 - alphas) * q1 + alphas * q2


def reset_joints_by_interpolate(
        env: ManagerBasedEnv,
        env_ids: torch.Tensor,
        position_range: tuple[float, float],
    ...
    joint_pos = asset.data.reset_joint_pos[env_ids,
                                           asset_cfg.joint_ids].clone()
    joint_pos[:, :7] = interpolate_joints(interpolation_range[0],
    ...
    asset.root_physx_view.set_dof_positions(
        joint_pos,
        indices=torch.arange(len(env_ids), device=asset.device),
    )


def randomize_robot_ee_pose(
    env,
    env_ids,
    init_ee_pose,
    pose_range=[0.06, 0.5],
    ...
    positions = math_utils.sample_uniform(-pose_range[0],
                                          pose_range[0], (len(env_ids), 3),
                                          device=robot.device)

    # sample random orientations
    euler_angles = math_utils.sample_uniform(-pose_range[1],
                                             pose_range[1], (len(env_ids), 3),
                                             device=robot.device)

    init_ee_pose = init_ee_pose.to(env.device).unsqueeze(0).repeat_interleave(
        len(env_ids), 0)
    randomize_ee_pose = torch.cat(math_utils.apply_delta_pose(
        init_ee_pose[:, :3], init_ee_pose[:, 3:7],
        torch.cat([positions, euler_angles], dim=-1)),
                                  dim=-1)
    setattr(robot.data, "randomize_ee_pose", randomize_ee_pose)


def reset_robot_root_state(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor,
    root_pose: tuple[float, float],
    asset_name: str = "robot",
):
    """Reset the robot joints with offsets around the default position and velocity by the given ranges.
    ...
    robot._data.reset_root_state[..., :7] = root_pose.to(
        env.device).unsqueeze(0).repeat_interleave(env.num_envs, 0)
    robot._data.reset_root_state[:, :3] += env.scene.env_origins[env_ids]

    robot.write_root_pose_to_sim(robot._data.reset_root_state[..., :7],
                                 env_ids=env_ids)


def reset_rigid_articulation(
    env,
    env_ids,
    target_name,
    pose_range,
