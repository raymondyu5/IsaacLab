...
class CameraObservation:
        ...
        if self.extract_seg_pc:
            ...
            if seg_mask_tensor is not None:
                if self.downsampled_points > 0:
                    ...
                    bg_mask = torch.where(seg_mask_flatten == 1, True,
                                          False)  # Background mask
    ...
    def process_camera_data(self, env: ManagerBasedRLEnv) -> dict:
        ...
        print("[CameraObservation] Starting process_camera_data...")
        rgb_data, segementation_data, intrinsic_params, depth, position, orientation, extrinsic_orientation, id2lables = self.extract_raw_camera_data(
            env)
        print(f"[CameraObservation] extract_raw_camera_data took {time.time() - start_time:.2f}s")
        ...
        if self.extract_seg_rgb or self.extract_rgb:  #or self.extract_rgb:
            ...
            self.process_rgb_and_segmentation(result, rgb_data,
                                              ...
                                              self.num_cameras)
            print(f"[CameraObservation] process_rgb_and_segmentation took {time.time() - rgb_start:.2f}s")
        ...
        if self.extract_all_pc or self.extract_seg_pc:
            ...
            self.process_point_cloud(env, result, rgb_data, depth,
                                     ...
                                     pc_idx_per_env, env.num_envs,
                                     self.num_cameras, segementation_data)
            print(f"[CameraObservation] process_point_cloud took {time.time() - pc_start:.2f}s")
        ...
        print(f"[CameraObservation] Total process_camera_data took {time.time() - start_time:.2f}s")
        return result
