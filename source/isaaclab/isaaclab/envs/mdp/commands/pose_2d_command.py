...
class UniformPose2dCommand(CommandTerm):
    ...
    def __init__(self, cfg: UniformPose2dCommandCfg, env: ManagerBasedEnv):
        ...
        self.metrics["error_pos"] = torch.zeros(self.num_envs,
                                                device=self.device)
        self.metrics["error_heading"] = torch.zeros(self.num_envs,
                                                    device=self.device)
    ...
    def command(self) -> torch.Tensor:
        ...
        return torch.cat(
            [self.pos_command_b,
             self.heading_command_b.unsqueeze(1)], dim=1)
    ...
    def _update_metrics(self):
        ...
        self.metrics["error_pos_2d"] = torch.norm(
            self.pos_command_w[:, :2] - self.robot.data.root_pos_w[:, :2],
            dim=1)
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        if self.cfg.simple_heading:
            ...
            target_vec = self.pos_command_w[
                env_ids] - self.robot.data.root_pos_w[env_ids]
            ...
            curr_to_target = wrap_to_pi(
                target_direction - self.robot.data.heading_w[env_ids]).abs()
            curr_to_flipped_target = wrap_to_pi(
                flipped_target_direction -
                self.robot.data.heading_w[env_ids]).abs()
            ...
            self.heading_command_w[env_ids] = r.uniform_(
    ...
    def _update_command(self):
        ...
        self.heading_command_b[:] = wrap_to_pi(self.heading_command_w -
                                               self.robot.data.heading_w)
...
class TerrainBasedPose2dCommand(UniformPose2dCommand):
    ...
    def __init__(self, cfg: TerrainBasedPose2dCommandCfg,
                 env: ManagerBasedEnv):
        # initialize the base class
        super().__init__(cfg, env)
