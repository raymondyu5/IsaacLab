...
def command_resample(env: ManagerBasedRLEnv,
                     command_name: str,
                     num_resamples: int = 1) -> torch.Tensor:
    ...
    return torch.logical_and((command.time_left <= env.step_dt),
                             (command.command_counter == num_resamples))
...
def bad_orientation(
    ...
    return torch.acos(-asset.data.projected_gravity_b[:,
                                                      2]).abs() > limit_angle
...
def joint_pos_out_of_limit(
    ...
    out_of_upper_limits = torch.any(asset.data.joint_pos[:,
                                                         asset_cfg.joint_ids]
                                    > limits[..., 1],
                                    dim=1)
    out_of_lower_limits = torch.any(asset.data.joint_pos[:,
                                                         asset_cfg.joint_ids]
                                    < limits[..., 0],
                                    dim=1)
...
def joint_vel_out_of_limit(
    ...
    return torch.any(torch.abs(asset.data.joint_vel[:, asset_cfg.joint_ids])
...
def illegal_contact(env: ManagerBasedRLEnv, threshold: float,
    ...
    return torch.any(torch.max(torch.norm(
