...
class is_terminated_term(ManagerTermBase):
    ...
    * attr:`term_keys`: The termination terms to penalize. This can be a string, a list of strings
    ...
    def __call__(self,
                 ...
                 term_keys: str | list[str] = ".*") -> torch.Tensor:
...
def flat_orientation_l2(
    ...
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject = env.scene[asset_cfg.name]
    return torch.sum(torch.square(asset.data.projected_gravity_b[:, :2]),
                     dim=1)
...
def body_lin_acc_l2(
    ...
    return torch.sum(torch.norm(
        asset.data.body_lin_acc_w[:, asset_cfg.body_ids, :], dim=-1),
                     dim=1)
...
def joint_vel_l1(env: ManagerBasedRLEnv,
    ...
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    return torch.sum(torch.abs(asset.data.joint_vel[:, asset_cfg.joint_ids]),
                     dim=1)
...
def joint_vel_l2(
    ...
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    return torch.sum(torch.square(asset.data.joint_vel[:,
                                                       asset_cfg.joint_ids]),
                     dim=1)
...
def joint_acc_l2(
    ...
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    return torch.sum(torch.square(asset.data.joint_acc[:,
                                                       asset_cfg.joint_ids]),
                     dim=1)
...
def joint_deviation_l1(
    ...
    angle = asset.data.joint_pos[:, asset_cfg.
...
def joint_pos_limits(
    ...
    out_of_limits = -(
        ...
        soft_ratio[0]).clip(max=0.0)
    out_of_limits += (
        asset.data.joint_pos[:, asset_cfg.joint_ids] -
        asset.data.soft_joint_pos_limits[:, asset_cfg.joint_ids, 1] *
        soft_ratio[1]).clip(min=0.0)

    return torch.sum(abs(out_of_limits), dim=1)
...
def action_rate_l2(env: ManagerBasedRLEnv, dim=6) -> torch.Tensor:
    """Penalize the rate of change of the actions using L2 squared kernel."""

    return torch.sum(torch.square(env.action_manager.action[:, :dim] -
                                  env.action_manager.prev_action[:, :dim]),
                     dim=1)
...
def contact_forces(env: ManagerBasedRLEnv, threshold: float,
    ...
    violation = torch.max(torch.norm(
