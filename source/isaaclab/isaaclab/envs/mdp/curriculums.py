...
class modify_env_param(ManagerTermBase):
    ...
    def _process_accessors(self, root: ManagerBasedRLEnv,
                           path: str) -> tuple[callable, callable]:
        ...
        self._last_path = path_parts[
            -1]  # for "a.b[2].c", this is "c", while for "a.b[2]" it is 2
        ...
        if isinstance(self._container, tuple):
            ...
            set_value = lambda val: setattr(self._container, self._last_path,
                                            val)  # noqa: E731
        else:
            raise TypeError(
