...
class Se2Gamepad(DeviceBase):
    ...
    def advance(self) -> torch.Tensor:
        ...
        return torch.tensor(numpy_result,
                            dtype=torch.float32,
    ...
    def _on_gamepad_event(self, event: carb.input.GamepadEvent, *args,
                          **kwargs):
    ...
    def _create_key_bindings(self):
        ...
        self._INPUT_STICK_VALUE_MAPPING = {
            ...
            # left command
            carb.input.GamepadInput.LEFT_STICK_LEFT: (1, 1,
                                                      self.v_y_sensitivity),
            # yaw command (positive)
            carb.input.GamepadInput.RIGHT_STICK_RIGHT:
            (0, 2, self.omega_z_sensitivity),
            # yaw command (negative)
            carb.input.GamepadInput.RIGHT_STICK_LEFT:
            (1, 2, self.omega_z_sensitivity),
        }
