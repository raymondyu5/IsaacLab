...
class G1TriHandDexRetargeting:
    ...
    def __init__(
        ...
        left_hand_urdf_path:
        str = f"{ISAACLAB_NUCLEUS_DIR}/Controllers/LocomanipulationAssets/unitree_g1_dexpilot_asset/G1_left_hand.urdf",
        right_hand_urdf_path:
        str = f"{ISAACLAB_NUCLEUS_DIR}/Controllers/LocomanipulationAssets/unitree_g1_dexpilot_asset/G1_right_hand.urdf",
    ):
        ...
        local_left_urdf_path = retrieve_file_path(left_hand_urdf_path,
                                                  force_download=True)
        local_right_urdf_path = retrieve_file_path(right_hand_urdf_path,
                                                   force_download=True)

        left_config_path = os.path.join(config_dir, left_hand_config_filename)
        right_config_path = os.path.join(config_dir,
                                         right_hand_config_filename)

        # Update the YAML files with the correct URDF paths
        self._update_yaml_with_urdf_path(left_config_path,
                                         local_left_urdf_path)
        self._update_yaml_with_urdf_path(right_config_path,
                                         local_right_urdf_path)

        self._dex_left_hand = RetargetingConfig.load_from_file(
            left_config_path).build()
        self._dex_right_hand = RetargetingConfig.load_from_file(
    ...
    def _update_yaml_with_urdf_path(self, yaml_path: str, urdf_path: str):
        ...
        try:
            ...
            if "retargeting" in config:
                ...
                logger.warning(
                    f"Unable to find 'retargeting' section in {yaml_path}")
    ...
    def compute_ref_value(self, joint_position: np.ndarray,
                          indices: np.ndarray,
                          retargeting_type: str) -> np.ndarray:
        ...
        if retargeting_type == "POSITION":
            ...
            ref_value = joint_position[task_indices, :] - joint_position[
                origin_indices, :]
            return ref_value
    ...
    def get_hand_indices(self, robot) -> np.ndarray:
        ...
        """
        return np.array(
            [robot.dof_names.index(name) for name in self.dof_names],
            dtype=np.int64)
    ...
    def compute_left(self, left_hand_poses: dict[str,
        ...
        if left_hand_poses is not None:
            left_hand_q = self.compute_one_hand(left_hand_poses,
                                                self._dex_left_hand,
                                                _OPERATOR2MANO_LEFT)
        else:
    ...
    def compute_right(self, right_hand_poses: dict[str,
        ...
        if right_hand_poses is not None:
            right_hand_q = self.compute_one_hand(right_hand_poses,
                                                 self._dex_right_hand,
