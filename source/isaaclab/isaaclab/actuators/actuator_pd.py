    ...
    * Corner velocity (:math:`V_{c}`): The velocity where the torque-speed curve intersects with continuous torque.
    ...
    def __init__(self, cfg: DCMotorCfg, *args, **kwargs):
        ...
        if self.cfg.saturation_effort is None:
            raise ValueError(
                "The saturation_effort must be provided for the DC motor actuator model."
            )
        ...
        self._vel_at_effort_lim = self.velocity_limit * (
            1 + self.effort_limit / self._saturation_effort)
        ...
        if self.cfg.velocity_limit is None:
            raise ValueError(
                "The velocity limit must be provided for the DC motor actuator model."
            )
...
class DelayedPDActuator(IdealPDActuator):
    ...
    def __init__(self, cfg: DelayedPDActuatorCfg, *args, **kwargs):
        ...
        self.positions_delay_buffer = DelayBuffer(cfg.max_delay,
                                                  self._num_envs,
                                                  device=self._device)
        self.velocities_delay_buffer = DelayBuffer(cfg.max_delay,
                                                   self._num_envs,
                                                   device=self._device)
        self.efforts_delay_buffer = DelayBuffer(cfg.max_delay,
                                                self._num_envs,
                                                device=self._device)
        # all of the envs
...
class RemotizedPDActuator(DelayedPDActuator):
    ...
    def __init__(
        ...
        self._joint_parameter_lookup = torch.tensor(cfg.joint_parameter_lookup,
                                                    device=device)
        # define remotized joint torque limit
        self._torque_limit = LinearInterpolation(self.angle_samples,
                                                 self.max_torque_samples,
                                                 device=device)
    ...
    def compute(self, control_action: ArticulationActions,
        ...
        control_action.joint_efforts = torch.clamp(
            control_action.joint_efforts,
            min=-abs_torque_limits,
            max=abs_torque_limits)
