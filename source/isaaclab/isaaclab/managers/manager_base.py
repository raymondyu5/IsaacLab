...
class ManagerBase(ABC):
    ...
    def __init__(self, cfg: object, env: ManagerBasedEnv):
        ...
        if not self._env.sim.is_playing():
            ...
            self._resolve_terms_handle = timeline_event_stream.create_subscription_to_pop_by_type(
                ...
                lambda event, obj=weakref.proxy(self): obj.
    ...
    def _resolve_common_term_cfg(self,
                                 term_name: str,
        ...
        * Initializing the term if it is a class.
        ...
        if not callable(term_cfg.func):
            raise AttributeError(
                f"The term '{term_name}' is not callable. Received: {term_cfg.func}"
            )
    ...
    def _process_term_cfg_at_play(self, term_name: str,
                                  term_cfg: ManagerTermBaseCfg):
        ...
        for key, value in term_cfg.params.items():
            if isinstance(value, SceneEntityCfg):
                ...
                try:
                    ...
                    raise ValueError(
                        f"Error while parsing '{term_name}:{key}'. {e}")
