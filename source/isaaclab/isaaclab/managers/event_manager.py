...
class EventManager(ManagerBase):
        ...
        for mode in self._mode_term_names:
            ...
            if mode == "interval":
                ...
                for index, (name, cfg) in enumerate(
                        zip(self._mode_term_names[mode],
        ...
        if "interval" in self._mode_term_cfgs:
            for index, term_cfg in enumerate(self._mode_term_cfgs["interval"]):
                ...
                if not term_cfg.is_global_time:
                    ...
                    sampled_interval = torch.rand(
                        num_envs, device=self.device) * (upper - lower) + lower
                    self._interval_term_time_left[index][
                        env_ids] = sampled_interval
    ...
    def apply(
        ...
        if mode not in self._mode_term_names:
            logger.warning(
                f"Event mode '{mode}' is not defined. Skipping event.")
            return
        ...
        for index, term_cfg in enumerate(self._mode_term_cfgs[mode]):
            if mode == "interval":
                ...
                if term_cfg.is_global_time:
                    if time_left < 1e-6:
                        lower, upper = term_cfg.interval_range_s
                        sampled_interval = torch.rand(1) * (upper -
                                                            lower) + lower
                        self._interval_term_time_left[
                            index][:] = sampled_interval
                    ...
                    if len(valid_env_ids) > 0:
                        ...
                        self._interval_term_time_left[index][
                            valid_env_ids] = sampled_time

                        # call the event term
                        term_cfg.func(self._env, valid_env_ids,
                                      **term_cfg.params)
            elif mode == "reset":
                ...
                if min_step_count == 0:
                    ...
                    valid_trigger |= (last_triggered_step
                                      == 0) & ~triggered_at_least_once
                    ...
                    if len(valid_env_ids) > 0:
                        self._reset_term_last_triggered_once[index][
                            valid_env_ids] = True
                        self._reset_term_last_triggered_step_id[index][
                            valid_env_ids] = global_env_step_count
    ...
    def _prepare_terms(self):
        ...
        for term_name, term_cfg in cfg_items:
            ...
            if inspect.isclass(
                    term_cfg.func) and term_cfg.mode == "prestartup":
                logger.info(
                    f"Initializing term '{term_name}' with class '{term_cfg.func.__name__}'."
                )
            ...
            if term_cfg.mode == "interval":
                ...
                if term_cfg.is_global_time:
                    ...
                    lower, upper = term_cfg.interval_range_s
                    time_left = torch.rand(self.num_envs, device=self.device
                                           ) * (upper - lower) + lower
                    self._interval_term_time_left.append(time_left)
            # -- reset mode
