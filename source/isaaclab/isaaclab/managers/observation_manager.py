...
class ObservationManager(ManagerBase):
    ...
    def __init__(self, cfg: object, env: ManagerBasedEnv):
        ...
        self._group_obs_dim: dict[str, tuple[int, ...]
                                  | list[tuple[int, ...]]] = dict()
            ...
            if self._group_obs_concatenate[group_name]:
                try:
                    term_dims = torch.stack([
                        torch.tensor(dims, device="cpu")
                        for dims in group_term_dims
                    ],
                                            dim=0)
                    if len(term_dims.shape) > 1:
                        if self._group_obs_concatenate_dim[group_name] >= 0:
                            dim = self._group_obs_concatenate_dim[
                                group_name] - 1  # account for the batch offset
                        else:
        ...
        self._obs_buffer: dict[str, torch.Tensor
    ...
    def get_active_iterable_terms(
            self, env_idx: int) -> Sequence[tuple[str, Sequence[float]]]:
        ...
        obs_buffer: dict[str, torch.Tensor
                         | dict[str, torch.Tensor]] = self._obs_buffer
        ...
        for group_name, _ in self._group_obs_dim.items():
            if not self.group_obs_concatenate[group_name]:
                for name, term in obs_buffer[group_name].items():
                    terms.append((group_name + "-" + name,
                                  term[env_idx].cpu().tolist()))
                continue
    ...
    @property
    def group_obs_dim(
            self) -> dict[str, tuple[int, ...] | list[tuple[int, ...]]]:
    ...
    def get_IO_descriptors(self,
                           group_names_to_export: list[str] = ["policy"]):
        ...
        for group_name in self._group_obs_term_names:
            ...
            obs_terms = zip(group_term_names,
            ...
            for term_name, term_cfg in obs_terms:
                ...
                try:
                    ...
                    for k, v in term_cfg.__dict__.items():
                        ...
                        if k in [
                                "modifiers", "clip", "scale", "history_length",
                                "flatten_history_dim"
                        ]:
                            overloads[k] = v
                    ...
                    print(
                        f"Error getting IO descriptor for term '{term_name}' in group '{group_name}': {e}"
                    )
        ...
        formatted_data = {
            k: v
            for k, v in formatted_data.items() if k in group_names_to_export
        }
    ...
    def reset(self, env_ids: Sequence[int] | None = None) -> dict[str, float]:
        ...
        for group_name, group_cfg in self._group_obs_class_term_cfgs.items():
            ...
            for term_name in self._group_obs_term_names[group_name]:
                if term_name in self._group_obs_term_history_buffer[
                        group_name]:
                    self._group_obs_term_history_buffer[group_name][
                        term_name].reset(batch_ids=env_ids)
    ...
    def compute(
        ...
        for group_name in self._group_obs_term_names:
            obs_buffer[group_name] = self.compute_group(
                group_name, update_history=update_history)
    ...
    def compute_group(
        ...
        for term_name, term_cfg in obs_terms:
            ...
            data: torch.Tensor = term_cfg.func(self._env, **term_cfg.params)

            if isinstance(data, dict):

                group_obs.pop(term_name)
                for key in data.keys():
                    ...
                    if term_cfg.clip:
                        obs = obs.clip_(min=term_cfg.clip[0],
                                        max=term_cfg.clip[1])
                    if term_cfg.scale:
                ...
                if term_cfg.history_length > 0:
                    circular_buffer = self._group_obs_term_history_buffer[
        ...
        if self._group_obs_concatenate[group_name]:
            ...
            return torch.cat(list(group_obs.values()),
                             dim=self._group_obs_concatenate_dim[group_name])
        else:
            return group_obs
    ...
    def serialize(self) -> dict:
        ...
        output = {
            group_name: {
                term_name: (term_cfg.func.serialize() if isinstance(
                    term_cfg.func, ManagerTermBase) else {
                        "cfg": class_to_dict(term_cfg)
                    })
    ...
    def _prepare_terms(self):
        ...
        self._group_obs_class_term_cfgs: dict[
            str, list[ObservationTermCfg]] = dict()
        ...
        self._group_obs_class_instances: list[modifiers.ModifierBase
        ...
        for group_name, group_cfg in group_cfg_items:
            ...
            self._group_obs_concatenate[
                group_name] = group_cfg.concatenate_terms
            self._group_obs_concatenate_dim[group_name] = (
                group_cfg.concatenate_dim + 1 if group_cfg.concatenate_dim >= 0
            ...
            # iterate over all the terms in each group
            print(f"[ObservationManager] Processing group: {group_name}")
            for term_name, term_cfg in group_cfg_items:
                print(f"[ObservationManager] Found term: {term_name}")
                ...
                if term_name in [
                    ...
                    print(f"[ObservationManager] Skipping non-obs setting: {term_name}")
                    continue
                # check for non config
                if term_cfg is None:
                    print(f"[ObservationManager] Skipping None config: {term_name}")
                    continue
                ...
                self._resolve_common_term_cfg(f"{group_name}/{term_name}",
                                              term_cfg,
                                              min_argc=1)
                ...
                import time
                print(f"[ObservationManager] Calling observation term: {term_name}")
                term_start = time.time()
                data = term_cfg.func(self._env, **term_cfg.params)
                print(f"[ObservationManager] Term '{term_name}' first call took {time.time() - term_start:.2f}s")

                if isinstance(data, dict):
                    print(f"[ObservationManager] Term '{term_name}' returned dict with keys: {list(data.keys())}")
                    for key in data.keys():
                        if isinstance(data[key], list):
                            ...
                            obs_dims = tuple(data[key].shape)
                            self._group_obs_term_dim[group_name].append(
                                obs_dims[1:])
                            self._group_obs_term_names[group_name].append(key)

                else:
                    print(f"[ObservationManager] Term '{term_name}' returned non-dict, calling again...")
                    second_call_start = time.time()
                    obs_dims = tuple(
                        term_cfg.func(self._env, **term_cfg.params).shape)
                    print(f"[ObservationManager] Term '{term_name}' second call took {time.time() - second_call_start:.2f}s")
                ...
                if term_cfg.scale is not None:
                    ...
                    if isinstance(term_cfg.scale, tuple) and len(
                            term_cfg.scale) != obs_dims[1]:
                        raise ValueError(
                            f"Scale for observation term '{term_name}' in group '{group_name}'"
                    ...
                    term_cfg.scale = torch.tensor(term_cfg.scale,
                                                  dtype=torch.float,
                    ...
                    for mod_cfg in term_cfg.modifiers:
                        ...
                        if isinstance(mod_cfg, modifiers.ModifierCfg):
                            ...
                            if inspect.isclass(mod_cfg.func):
                                if not issubclass(mod_cfg.func,
                                ...
                                self._group_obs_class_instances.append(
                                    mod_cfg.func)
                        else:
                            raise TypeError(
                ...
                if term_cfg.noise is not None and isinstance(
                        term_cfg.noise, noise.NoiseModelCfg):
                ...
                if isinstance(term_cfg.func, ManagerTermBase):
                    self._group_obs_class_term_cfgs[group_name].append(
                        term_cfg)
            ...
            self._group_obs_term_history_buffer[
                group_name] = group_entry_history_buffer
