...
class RigidObjectCollectionData:
    ...
    def __init__(self, root_physx_view: physx.RigidBodyView, num_objects: int,
                 device: str):
        ...
        self._root_physx_view: physx.RigidBodyView = weakref.proxy(
            root_physx_view)
        ...
        self.GRAVITY_VEC_W = gravity_dir.repeat(self.num_instances,
                                                self.num_objects, 1)
        self.FORWARD_VEC_B = torch.tensor(
            (1.0, 0.0, 0.0),
            device=self.device).repeat(self.num_instances, self.num_objects, 1)
    ...
    def object_link_pose_w(self):
        ...
        if self._object_link_pose_w.timestamp < self._sim_timestamp:
            ...
            pose = self._reshape_view_to_data(
                self._root_physx_view.get_transforms().clone())
    ...
    def object_link_vel_w(self):
        ...
        if self._object_link_vel_w.timestamp < self._sim_timestamp:
            ...
            velocity[..., :3] += torch.linalg.cross(
                velocity[..., 3:],
                math_utils.quat_apply(self.object_link_quat_w,
                                      -self.object_com_pos_b),
                dim=-1)
            # set the buffer data and timestamp
    ...
    def object_com_pose_w(self):
        ...
        if self._object_com_pose_w.timestamp < self._sim_timestamp:
            ...
            pos, quat = math_utils.combine_frame_transforms(
                self.object_link_pos_w, self.object_link_quat_w,
                self.object_com_pos_b, self.object_com_quat_b)
            # set the buffer data and timestamp
    ...
    def object_com_vel_w(self):
        ...
        if self._object_com_vel_w.timestamp < self._sim_timestamp:
            self._object_com_vel_w.data = self._reshape_view_to_data(
                self._root_physx_view.get_velocities())
    ...
    def object_state_w(self):
        ...
        if self._object_state_w.timestamp < self._sim_timestamp:
            self._object_state_w.data = torch.cat(
                (self.object_link_pose_w, self.object_com_vel_w), dim=-1)
    ...
    def object_link_state_w(self):
        ...
        if self._object_link_state_w.timestamp < self._sim_timestamp:
            self._object_link_state_w.data = torch.cat(
                (self.object_link_pose_w, self.object_link_vel_w), dim=-1)
    ...
    def object_com_state_w(self):
        ...
        if self._object_com_state_w.timestamp < self._sim_timestamp:
            self._object_com_state_w.data = torch.cat(
                (self.object_com_pose_w, self.object_com_vel_w), dim=-1)
    ...
    def object_com_acc_w(self):
        ...
        if self._object_com_acc_w.timestamp < self._sim_timestamp:
            self._object_com_acc_w.data = self._reshape_view_to_data(
                self._root_physx_view.get_accelerations())
    ...
    def projected_gravity_b(self):
        ...
        return math_utils.quat_apply_inverse(self.object_link_quat_w,
                                             self.GRAVITY_VEC_W)
    ...
    def heading_w(self):
        ...
        """
        forward_w = math_utils.quat_apply(self.object_link_quat_w,
                                          self.FORWARD_VEC_B)
