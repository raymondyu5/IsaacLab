...
class ArticulationData:
    ...
    def __init__(self, root_physx_view: physx.ArticulationView, device: str):
        ...
        self._root_physx_view: physx.ArticulationView = weakref.proxy(
            root_physx_view)
        ...
        self._previous_joint_vel = self._root_physx_view.get_dof_velocities(
        ).clone()
    ...
    def root_link_vel_w(self) -> torch.Tensor:
        ...
        if self._root_link_vel_w.timestamp < self._sim_timestamp:
            ...
            vel[:, :3] += torch.linalg.cross(vel[:, 3:],
    ...
    def root_com_pose_w(self) -> torch.Tensor:
        ...
        if self._root_com_pose_w.timestamp < self._sim_timestamp:
            ...
            pos, quat = math_utils.combine_frame_transforms(
                self.root_link_pos_w, self.root_link_quat_w,
                self.body_com_pos_b[:, 0], self.body_com_quat_b[:, 0])
            # set the buffer data and timestamp
    ...
    def body_link_pose_w(self) -> torch.Tensor:
        ...
        if self._body_link_pose_w.timestamp < self._sim_timestamp:
            ...
            poses[..., 3:7] = math_utils.convert_quat(poses[..., 3:7],
                                                      to="wxyz")
            # set the buffer data and timestamp
    ...
    def body_link_vel_w(self) -> torch.Tensor:
        ...
        if self._body_link_vel_w.timestamp < self._sim_timestamp:
            ...
            velocities[..., :3] += torch.linalg.cross(
                velocities[..., 3:],
                math_utils.quat_apply(self.body_link_quat_w,
                                      -self.body_com_pos_b),
                dim=-1)
            # set the buffer data and timestamp
    ...
    def body_com_pose_w(self) -> torch.Tensor:
        ...
        if self._body_com_pose_w.timestamp < self._sim_timestamp:
            # apply local transform to center of mass frame
            pos, quat = math_utils.combine_frame_transforms(
                self.body_link_pos_w, self.body_link_quat_w,
                self.body_com_pos_b, self.body_com_quat_b)
            # set the buffer data and timestamp
    ...
    def joint_acc(self):
        ...
        if self._joint_acc.timestamp < self._sim_timestamp:
            ...
            self._joint_acc.data = (self.joint_vel -
                                    self._previous_joint_vel) / time_elapsed
    ...
    def projected_gravity_b(self):
        ...
        return math_utils.quat_apply_inverse(self.root_link_quat_w,
                                             self.GRAVITY_VEC_W)
    ...
    def heading_w(self):
        ...
        """
        forward_w = math_utils.quat_apply(self.root_link_quat_w,
                                          self.FORWARD_VEC_B)
