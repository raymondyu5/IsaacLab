...
class VisualizationManager:
    ...
    StandardCallback = Callable[["VisualizationManager", "DataCollector"],
                                None]
    EventCallback = Callable[["VisualizationManager", "DataCollector", Any],
                             None]
    ...
    def display_widget(self, text: str, name: str, args: dict[str,
                                                              Any]) -> None:
    ...
    def __init__(self, data_collector: DataCollector):
        ...
        self._rules_on_period: dict[
            VisualizationManager.TimeCountdown,
            VisualizationManager.StandardCallback] = {}
        self._rules_on_event: dict[
            str, list[VisualizationManager.EventCallback]] = {}
        self._rules_on_change: dict[
            str, list[VisualizationManager.StandardCallback]] = {}
    ...
    def register_callback(self, trigger: TriggerType, arg: dict,
        ...
        match trigger:
            case TriggerType.TRIGGER_ON_PERIOD:
                ...
                if isinstance(period, float) and isinstance(
                        initial_countdown, float):
                    timer = VisualizationManager.TimeCountdown(
                        period=period, initial_countdown=initial_countdown)
            ...
            case TriggerType.TRIGGER_ON_EVENT:
                ...
                if isinstance(event, str):
                    ...
                    if callbacks is None:
                        ...
                        # Type cast since we've validated the signature
                        self._rules_on_event[event].append(
            ...
            case TriggerType.TRIGGER_ON_CHANGE:
                ...
                if isinstance(variable_name, str):
                    ...
                    if callbacks is None:
                        # Type cast since we've validated the signature
                        self._rules_on_change[variable_name] = [
                            callback
                        ]  # type: ignore
                    else:
                        # Type cast since we've validated the signature
                        self._rules_on_change[variable_name].append(
                            callback)  # type: ignore
    ...
    def _validate_callback_signature(self, trigger: TriggerType,
        ...
        try:
            ...
            logger.warning(
                f"Could not validate callback signature for {trigger.name}: {e}"
            )
...
class XRVisualization:
    ...
    def __init__(self):
        ...
        raise RuntimeError(
            "Use VisualizationInterface classmethods instead of direct instantiation"
        )

    @classmethod
    def __create_instance(
        cls,
        manager: type[VisualizationManager] = VisualizationManager
    ) -> XRVisualization:
    ...
    def _register(self) -> bool:
        ...
        if sim is not None:
            sim.add_render_callback("visualization_render_callback",
                                    self.update_loop)
    ...
    @classmethod
    def register_callback(cls, trigger: TriggerType, arg: dict,
                          callback: VisualizationManager.CallbackType) -> None:
        ...
        """
        instance = cls.__get_instance()
        instance._visualization_manager.register_callback(
            trigger, arg, callback)
