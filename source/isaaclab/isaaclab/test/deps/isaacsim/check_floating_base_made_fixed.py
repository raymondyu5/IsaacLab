...
parser = argparse.ArgumentParser(
    description=
    "This script shows the issue in Isaac Sim with making a floating robot fixed."
)
parser.add_argument("--headless",
                    action="store_true",
                    help="Run in headless mode.")
parser.add_argument("--fix-base",
                    action="store_true",
                    help="Whether to fix the base of the robot.")
# parse the arguments
...
def main():
    ...
    if args_cli.fix_base:
        ...
        if not root_prim.HasAPI(UsdPhysics.RigidBodyAPI):
            raise RuntimeError(
                "The root prim does not have the RigidBodyAPI applied.")
        ...
        # -- physx attributes
        root_physx_articulation_api = PhysxSchema.PhysxArticulationAPI(
            root_prim)
    ...
    while simulation_app.is_running():
        ...
        if count % 20 == 0:
            ...
            print("Fixed base: ",
                  robot_view._physics_view.shared_metatype.fixed_base)
