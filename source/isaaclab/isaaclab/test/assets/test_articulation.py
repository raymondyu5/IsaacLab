    ...
    if articulation_type == "humanoid":
        articulation_cfg = ArticulationCfg(
            spawn=sim_utils.UsdFileCfg(
                usd_path=
                f"{ISAAC_NUCLEUS_DIR}/Robots/IsaacSim/Humanoid/humanoid_instanceable.usd"
            ),
        ...
        articulation_cfg = ArticulationCfg(
            ...
            spawn=sim_utils.UsdFileCfg(
                usd_path=
                f"{ISAAC_NUCLEUS_DIR}/Robots/IsaacSim/SimpleArticulation/revolute_articulation.usd",
        ...
        articulation_cfg = ArticulationCfg(
            spawn=sim_utils.UsdFileCfg(
                usd_path=
                f"{ISAAC_NUCLEUS_DIR}/IsaacLab/Tests/spatial_tendons.usd", ),
            actuators={
                "joint":
                ImplicitActuatorCfg(
                    joint_names_expr=[".*"],
...
def generate_articulation(articulation_cfg: ArticulationCfg,
                          num_articulations: int,
                          device: str) -> tuple[Articulation, torch.tensor]:
    ...
    for i in range(num_articulations):
        prim_utils.create_prim(f"/World/Env_{i}",
                               "Xform",
                               translation=translations[i][:3])
...
def sim(request):
    ...
    with build_simulation_context(device=device,
                                  auto_add_lighting=True,
                                  gravity_enabled=gravity_enabled,
                                  add_ground_plane=add_ground_plane) as sim:
...
def test_initialization_floating_base_non_root(sim, num_articulations, device,
    ...
    articulation_cfg = generate_articulation_cfg(articulation_type="humanoid",
                                                 stiffness=0.0,
                                                 damping=0.0)
    articulation, _ = generate_articulation(articulation_cfg,
                                            num_articulations,
                                            device=sim.device)
    ...
    prim_path_body_names = [
        path.split("/")[-1]
        for path in articulation.root_physx_view.link_paths[0]
    ]
...
def test_initialization_floating_base(sim, num_articulations, device,
    ...
    assert articulation.data.default_mass.shape == (num_articulations,
                                                    articulation.num_bodies)
    assert articulation.data.default_inertia.shape == (num_articulations,
                                                       articulation.num_bodies,
                                                       9)
...
def test_initialization_fixed_base(sim, num_articulations, device):
    ...
    articulation, translations = generate_articulation(articulation_cfg,
                                                       num_articulations,
                                                       device=device)
    ...
    for _ in range(10):
        ...
        torch.testing.assert_close(articulation.data.root_state_w,
                                   default_root_state)
...
def test_joint_pos_limits(sim, num_articulations, device, add_ground_plane):
    ...
    limits = torch.zeros(num_articulations,
                         articulation.num_joints,
                         2,
                         device=device)
    limits[..., 0] = (
        torch.rand(num_articulations, articulation.num_joints, device=device) +
        5.0) * -1.0
    limits[..., 1] = torch.rand(
        num_articulations, articulation.num_joints, device=device) + 5.0
    ...
    torch.testing.assert_close(articulation._data.default_joint_pos,
                               default_joint_pos)

    # Set new joint limits with indexing
    env_ids = torch.arange(1, device=device)
    joint_ids = torch.arange(2, device=device)
    limits = torch.zeros(env_ids.shape[0],
                         joint_ids.shape[0],
                         2,
                         device=device)
    limits[..., 0] = (torch.rand(
        env_ids.shape[0], joint_ids.shape[0], device=device) + 5.0) * -1.0
    limits[..., 1] = torch.rand(
        env_ids.shape[0], joint_ids.shape[0], device=device) + 5.0
    articulation.write_joint_position_limit_to_sim(limits,
                                                   env_ids=env_ids,
                                                   joint_ids=joint_ids)
    ...
    limits = torch.zeros(num_articulations,
                         ...
                         device=device)
    limits[..., 0] = torch.rand(
        num_articulations, articulation.num_joints, device=device) * -0.1
    limits[..., 1] = torch.rand(
        num_articulations, articulation.num_joints, device=device) * 0.1
    articulation.write_joint_position_limit_to_sim(limits)

    # Check if all values are within the bounds
    within_bounds = (articulation._data.default_joint_pos
                     >= limits[..., 0]) & (articulation._data.default_joint_pos
                                           <= limits[..., 1])
    ...
    limits = torch.zeros(env_ids.shape[0],
                         ...
                         device=device)
    limits[..., 0] = torch.rand(
        env_ids.shape[0], joint_ids.shape[0], device=device) * -0.1
    limits[..., 1] = torch.rand(
        env_ids.shape[0], joint_ids.shape[0], device=device) * 0.1
    articulation.write_joint_position_limit_to_sim(limits,
                                                   env_ids=env_ids,
    ...
    # Check if all values are within the bounds
    within_bounds = (
        articulation._data.default_joint_pos[env_ids][:, joint_ids]
        >= limits[..., 0]) & (articulation._data.default_joint_pos[env_ids]
                              [:, joint_ids] <= limits[..., 1])
    assert torch.all(within_bounds)
...
def test_joint_effort_limits(sim, num_articulations, device, add_ground_plane):
    ...
    articulation._data.computed_torque.fill_(
        100.0)  # pretend controller commanded 100
    articulation._data.applied_torque.fill_(
...
def test_external_force_buffer(sim, num_articulations, device):
    ...
    for step in range(5):
        ...
        external_wrench_b = torch.zeros(articulation.num_instances,
                                        len(body_ids),
                                        6,
                                        device=sim.device)
        external_wrench_positions_b = torch.zeros(articulation.num_instances,
                                                  len(body_ids),
                                                  3,
                                                  device=sim.device)
        ...
        for i in range(num_articulations):
            ...
            assert articulation._external_wrench_positions_b[
                i, 0, 0].item() == position
            assert articulation._use_global_wrench_frame == (step == 0
                                                             or step == 3)
...
def test_external_force_on_single_body(sim, num_articulations, device):
    ...
    for _ in range(5):
        ...
        articulation.reset()
        # apply force
        articulation.set_external_force_and_torque(external_wrench_b[..., :3],
                                                   external_wrench_b[..., 3:],
                                                   body_ids=body_ids)
        # perform simulation
        for _ in range(100):
...
@pytest.mark.parametrize("device", ["cuda:0", "cpu"])
@pytest.mark.isaacsim_ci
def test_external_force_on_multiple_bodies_at_position(sim, num_articulations,
                                                       device):
    ...
    for _ in range(5):
        ...
        for i in range(num_articulations):
            # since there is a moment applied on the articulation, the articulation should rotate
            assert torch.abs(articulation.data.root_ang_vel_w[i,
                                                              2]).item() > 0.1
...
def test_loading_gains_from_usd(sim, num_articulations, device):
    ...
    expected_stiffness = torch.zeros(articulation.num_instances,
                                     articulation.num_joints,
                                     device=articulation.device)
    expected_stiffness[:,
                       indices_list] = torch.tensor(values_list,
                                                    device=articulation.device)
    # -- Damping values
    expected_damping = {
        ".*_waist.*": 5.0,
    ...
    torch.testing.assert_close(articulation.actuators["body"].stiffness,
                               expected_stiffness)
    torch.testing.assert_close(articulation.actuators["body"].damping,
                               expected_damping)
...
@pytest.mark.isaacsim_ci
def test_setting_velocity_limit_implicit(sim, num_articulations, device,
                                         vel_limit_sim, vel_limit,
                                         add_ground_plane):
    ...
    physx_vel_limit = articulation.root_physx_view.get_dof_max_velocities().to(
        device)
    # check data buffer
    torch.testing.assert_close(articulation.data.joint_velocity_limits,
                               physx_vel_limit)
...
@pytest.mark.parametrize("vel_limit", [1e2, None])
@pytest.mark.isaacsim_ci
def test_setting_velocity_limit_explicit(sim, num_articulations, device,
                                         vel_limit_sim, vel_limit):
    ...
    if vel_limit is not None:
        ...
        torch.testing.assert_close(actuator_vel_limit,
                                   expected_actuator_vel_limit)
...
@pytest.mark.isaacsim_ci
def test_setting_effort_limit_implicit(sim, num_articulations, device,
...
@pytest.mark.isaacsim_ci
def test_setting_effort_limit_explicit(sim, num_articulations, device,
                                       effort_limit_sim, effort_limit):
    ...
    actuator_effort_limit_sim = articulation.actuators[
        "joint"].effort_limit_sim
    ...
    if effort_limit is not None:
        ...
        # check actuator is set
        torch.testing.assert_close(actuator_effort_limit,
        ...
        expected_actuator_effort_limit = torch.full_like(
            physx_effort_limit, usd_default_effort_limit)
        torch.testing.assert_close(actuator_effort_limit,
                                   expected_actuator_effort_limit)
    ...
    expected_effort_limit = torch.full_like(physx_effort_limit, limit)
    torch.testing.assert_close(actuator_effort_limit_sim,
...
def test_body_root_state(sim, num_articulations, device, with_offset):
    ...
    articulation_cfg = generate_articulation_cfg(
        articulation_type="single_joint_implicit")
    articulation, env_pos = generate_articulation(articulation_cfg,
                                                  num_articulations, device)
    env_idx = torch.tensor([x for x in range(num_articulations)],
                           device=device)
    # Check that boundedness of articulation is correct
    assert ctypes.c_long.from_address(id(
        articulation)).value == 1, "Boundedness of articulation is incorrect"
    # Play sim
    sim.reset()
    # Check if articulation is initialized
    ...
    new_com = torch.tensor(offset,
                           device=device).repeat(num_articulations, 1, 1)
    com[:, 1, :3] = new_com.squeeze(-2)
    articulation.root_physx_view.set_coms(com.cpu(), env_idx.cpu())

    # check they are set
    torch.testing.assert_close(articulation.root_physx_view.get_coms(),
                               com.cpu())
    ...
    for i in range(50):
        ...
        if with_offset:
            ...
            # LINK state
            # pose
            torch.testing.assert_close(root_state_w[..., :7],
                                       root_link_state_w[..., :7])
            torch.testing.assert_close(body_state_w[..., :7],
                                       body_link_state_w[..., :7])

            # lin_vel arm
            lin_vel_gt = torch.zeros(num_articulations,
                                     num_bodies,
                                     3,
                                     device=device)
            ...
            torch.testing.assert_close(lin_vel_gt[:, 0, :],
                                       root_link_state_w[..., 7:10],
                                       atol=1e-3,
                                       rtol=1e-1)
            # linear velocity of pendulum link should be
            torch.testing.assert_close(lin_vel_gt,
                                       body_link_state_w[..., 7:10],
                                       atol=1e-3,
                                       rtol=1e-1)

            # ang_vel
            torch.testing.assert_close(root_state_w[..., 10:],
                                       root_link_state_w[..., 10:])
            torch.testing.assert_close(body_state_w[..., 10:],
                                       body_link_state_w[..., 10:])

            # COM state
            # position and orientation shouldn't match for the _state_com_w but everything else will
            pos_gt = torch.zeros(num_articulations,
                                 num_bodies,
                                 3,
                                 device=device)
            ...
            torch.testing.assert_close(pos_gt[:, 0, :],
                                       root_com_state_w[..., :3],
                                       atol=1e-3,
                                       rtol=1e-1)
            torch.testing.assert_close(pos_gt,
                                       body_com_state_w[..., :3],
                                       atol=1e-3,
                                       rtol=1e-1)
            ...
            com_quat_w = math_utils.quat_mul(body_link_state_w[..., 3:7],
                                             com_quat_b)
            torch.testing.assert_close(com_quat_w, body_com_state_w[..., 3:7])
            torch.testing.assert_close(com_quat_w[:, 0, :],
                                       root_com_state_w[..., 3:7])
...
@pytest.mark.isaacsim_ci
def test_write_root_state(sim, num_articulations, device, with_offset,
                          state_location, gravity_enabled):
    ...
    rand_state[..., 3:7] = torch.nn.functional.normalize(rand_state[..., 3:7],
                                                         dim=-1)
    ...
    for i in range(10):
        ...
        if state_location == "com":
            torch.testing.assert_close(rand_state,
                                       articulation.data.root_com_state_w)
        elif state_location == "link":
            torch.testing.assert_close(rand_state,
                                       articulation.data.root_link_state_w)
...
def test_body_incoming_joint_wrench_b_single_joint(sim, num_articulations,
    ...
    gravity = torch.tensor(sim.cfg.gravity,
                           device="cpu").repeat(num_articulations, 1).view(
                               (num_articulations, 3))
    ...
    expected_wrench[:, :3] = math_utils.quat_apply(
        ...
        weight_vector_w.to(device) + math_utils.quat_apply(
            quat_w[:, 1, :], external_force_vector_b[:, 1, :]),
    )
    expected_wrench[:, 3:] = math_utils.quat_apply(
        math_utils.quat_conjugate(quat_w[:, 0, :]),
        torch.cross(
            ...
            weight_vector_w.to(device) + math_utils.quat_apply(
                quat_w[:, 1, :], external_force_vector_b[:, 1, :]),
            dim=-1,
        ) + math_utils.quat_apply(quat_w[:, 1, :],
                                  external_torque_vector_b[:, 1, :]),
    )
...
@pytest.mark.parametrize("gravity_enabled", [False])
@pytest.mark.isaacsim_ci
def test_write_joint_state_data_consistency(sim, num_articulations, device,
                                            gravity_enabled):
    """Test the setters for root_state using both the link frame and center of mass as reference frame.
    ...
    pos_dist = Uniform(articulation.data.joint_pos_limits[..., 0],
                       articulation.data.joint_pos_limits[..., 1])
    vel_dist = Uniform(-articulation.data.joint_vel_limits,
                       articulation.data.joint_vel_limits)
    ...
    assert torch.count_nonzero(
        original_body_states[:,
                             1:] != articulation.data.body_state_w[:, 1:]) > (
                                 len(original_body_states[:, 1:]) / 2)
    # validate body - link consistency
    torch.testing.assert_close(articulation.data.body_state_w[..., :7],
    ...
    torch.testing.assert_close(articulation.data.body_state_w[..., 10:],
    ...
    torch.testing.assert_close(expected_com_pos.view(len(env_idx), -1, 3),
                               articulation.data.body_com_pos_w)
    torch.testing.assert_close(expected_com_quat.view(len(env_idx), -1, 4),
                               articulation.data.body_com_quat_w)

    # validate body - com consistency
    torch.testing.assert_close(articulation.data.body_state_w[..., 7:10],
                               articulation.data.body_com_lin_vel_w)
    torch.testing.assert_close(articulation.data.body_state_w[..., 10:],
                               articulation.data.body_com_ang_vel_w)

    # validate pos_w, quat_w, pos_b, quat_b is consistent with pose_w and pose_b
    expected_com_pose_w = torch.cat(
        (articulation.data.body_com_pos_w, articulation.data.body_com_quat_w),
        dim=2)
    expected_com_pose_b = torch.cat(
        (articulation.data.body_com_pos_b, articulation.data.body_com_quat_b),
        dim=2)
    expected_body_pose_w = torch.cat(
        (articulation.data.body_pos_w, articulation.data.body_quat_w), dim=2)
    expected_body_link_pose_w = torch.cat((articulation.data.body_link_pos_w,
                                           articulation.data.body_link_quat_w),
                                          dim=2)
    torch.testing.assert_close(articulation.data.body_com_pose_w,
                               expected_com_pose_w)
    torch.testing.assert_close(articulation.data.body_com_pose_b,
                               expected_com_pose_b)
    torch.testing.assert_close(articulation.data.body_pose_w,
                               expected_body_pose_w)
    torch.testing.assert_close(articulation.data.body_link_pose_w,
                               expected_body_link_pose_w)

    # validate pose_w is consistent state[..., :7]
    torch.testing.assert_close(articulation.data.body_pose_w,
                               articulation.data.body_state_w[..., :7])
    torch.testing.assert_close(articulation.data.body_vel_w,
                               articulation.data.body_state_w[..., 7:])
    torch.testing.assert_close(articulation.data.body_link_pose_w,
                               articulation.data.body_link_state_w[..., :7])
    torch.testing.assert_close(articulation.data.body_com_pose_w,
                               articulation.data.body_com_state_w[..., :7])
    torch.testing.assert_close(articulation.data.body_vel_w,
                               articulation.data.body_state_w[..., 7:])
...
def test_write_joint_frictions_to_sim(sim, num_articulations, device,
    ...
    # apply action to the articulation
    dynamic_friction = torch.rand(num_articulations,
                                  articulation.num_joints,
                                  device=device)
    viscous_friction = torch.rand(num_articulations,
                                  articulation.num_joints,
                                  device=device)
    friction = torch.rand(num_articulations,
                          articulation.num_joints,
                          device=device)
    ...
    if int(get_version()[2]) >= 5:
        ...
        assert torch.allclose(joint_dynamic_friction_coeff_sim,
                              dynamic_friction.cpu())
        assert torch.allclose(joint_viscous_friction_coeff_sim,
                              viscous_friction.cpu())
    else:
    ...
    if int(get_version()[2]) >= 5:
        ...
        dynamic_friction_2 = torch.rand(num_articulations,
                                        articulation.num_joints,
                                        device=device)
        viscous_friction_2 = torch.rand(num_articulations,
                                        articulation.num_joints,
                                        device=device)
        friction_2 = torch.rand(num_articulations,
                                articulation.num_joints,
                                device=device)
        ...
        assert torch.allclose(friction_viscous_coeff_sim_2,
                              viscous_friction_2.cpu())
        assert torch.allclose(friction_dynamic_coef_sim_2,
                              dynamic_friction_2.cpu())
