...
def grilled_chicken_with_yoghurt_and_bbq(env,
                                         hot: bool,
                                         bland: float,
                                         bbq: bool = False):
...
def grilled_chicken_image(env, bland: float, channel: int = 1):
    return bland * torch.ones(
        env.num_envs, 128, 256, channel, device=env.device)
...
def setup_env():
    ...
    env = namedtuple("ManagerBasedEnv",
                     ["num_envs", "device", "data", "dt", "sim"])(
                         num_envs, device, MyDataClass(num_envs,
                                                       device), dt, sim)
...
def test_str(setup_env):
    ...
    class MyObservationManagerCfg:
        ...
        class SampleGroupCfg(ObservationGroupCfg):
            ...
            term_3 = ObservationTermCfg(func=grilled_chicken_with_bbq,
                                        scale=5,
                                        params={"bbq": True})
            term_4 = ObservationTermCfg(func=grilled_chicken_with_yoghurt,
                                        scale=1.0,
                                        params={
                                            "hot": False,
                                            "bland": 2.0
...
def test_str_with_history(setup_env):
    ...
    class MyObservationManagerCfg:
        ...
        class SampleGroupCfg(ObservationGroupCfg):
            ...
            term_1 = ObservationTermCfg(func=grilled_chicken,
                                        scale=10,
...
def test_config_equivalence(setup_env):
    ...
    class MyObservationManagerCfg:
        ...
        class SampleGroupCfg(ObservationGroupCfg):
            ...
            my_term = ObservationTermCfg(func=grilled_chicken_with_bbq,
                                         scale=5,
                                         params={"bbq": True})
            her_term = ObservationTermCfg(func=grilled_chicken_with_yoghurt,
                                          scale=1.0,
                                          params={
                                              "hot": False,
                                              "bland": 2.0
    ...
    class MyObservationManagerAnnotatedCfg:
        ...
        policy: ObservationGroupCfg = SampleGroupCfg()
        critic: ObservationGroupCfg = SampleGroupCfg(concatenate_terms=False,
                                                     her_term=None)
...
def test_config_terms(setup_env):
    ...
    class MyObservationManagerCfg:
        ...
        class SampleGroupCfg(ObservationGroupCfg):
            ...
            term_1 = ObservationTermCfg(func=grilled_chicken, scale=10)
            term_2 = ObservationTermCfg(func=grilled_chicken_with_curry,
                                        scale=0.0,
                                        params={"hot": False})
        ...
        class SampleMixedGroupCfg(ObservationGroupCfg):
            ...
            term_2 = ObservationTermCfg(func=grilled_chicken_image,
                                        scale=1.5,
                                        params={"bland": 0.5})
        ...
        class SampleImageGroupCfg(ObservationGroupCfg):
            ...
            term_1 = ObservationTermCfg(func=grilled_chicken_image,
                                        scale=1.5,
                                        params={
                                            "bland": 0.5,
                                            "channel": 1
...
def test_compute(setup_env):
    ...
    assert torch.equal(
        env.data.pos_w * torch.tensor(pos_scale_tuple, device=env.device),
        obs_critic[:, :3])
...
def test_compute_with_history(setup_env):
    ...
    expected_obs_term_1_data = torch.ones(env.num_envs,
                                          4 * HISTORY_LENGTH,
                                          device=env.device)
    ...
    torch.testing.assert_close(expected_obs_data_t0[reset_env_ids],
                               obs_policy[reset_env_ids])
...
def test_compute_with_group_history(setup_env):
    ...
    expected_obs_term_1_data = torch.ones(env.num_envs,
                                          4 * GROUP_HISTORY_LENGTH,
                                          device=env.device)
    expected_obs_term_2_data = lin_vel_w_data(env).repeat(
        1, GROUP_HISTORY_LENGTH)
    expected_obs_data_t0 = torch.concat(
        (expected_obs_term_1_data, expected_obs_term_2_data), dim=-1)
    torch.testing.assert_close(expected_obs_data_t0, obs_policy)
    # test that the history buffer holds previous data
...
def test_invalid_observation_config(setup_env):
    ...
    class MyObservationManagerCfg:
        ...
        class PolicyCfg(ObservationGroupCfg):
            ...
            term_1 = ObservationTermCfg(func=grilled_chicken_with_bbq,
                                        scale=0.1,
                                        params={"hot": False})
            term_2 = ObservationTermCfg(func=grilled_chicken_with_yoghurt,
                                        scale=2.0,
                                        params={"hot": False})
...
def test_callable_class_term(setup_env):
    ...
    class MyObservationManagerCfg:
        ...
        class PolicyCfg(ObservationGroupCfg):
            ...
            term_1 = ObservationTermCfg(func=grilled_chicken, scale=10)
            term_2 = ObservationTermCfg(func=complex_function_class,
                                        scale=0.2,
                                        params={"interval": 0.5})
    ...
    for _ in range(num_exec_count):
        observations = obs_man.compute()
    assert observations["policy"][0, -1].item() == pytest.approx(
        0.2 * 0.5 * (num_exec_count + 1))

    # check reset works
    obs_man.reset(env_ids=[0, 4, 9, 14, 19])
    observations = obs_man.compute()
    assert observations["policy"][0, -1].item() == pytest.approx(0.2 * 0.5)
    assert observations["policy"][1, -1].item() == pytest.approx(
        0.2 * 0.5 * (num_exec_count + 2))
...
def test_modifier_compute(setup_env):
    ...
    modifier_1 = modifiers.ModifierCfg(func=modifiers.bias,
                                       params={"value": 1.0})
    modifier_2 = modifiers.ModifierCfg(func=modifiers.scale,
                                       params={"multiplier": 2.0})
    modifier_3 = modifiers.ModifierCfg(func=modifiers.clip,
                                       params={"bounds": (-0.5, 0.5)})
    ...
    class MyObservationManagerCfg:
        ...
        class PolicyCfg(ObservationGroupCfg):
            ...
            term_2 = ObservationTermCfg(func=pos_w_data,
                                        modifiers=[modifier_1])
            term_3 = ObservationTermCfg(func=pos_w_data,
                                        modifiers=[modifier_1, modifier_4])
    ...
    assert torch.equal(2.0 * (obs_critic["term_1"] + 1.0),
                       obs_critic["term_3"])
...
def test_modifier_invalid_config(setup_env):
    ...
    modifier = modifiers.ModifierCfg(func=modifiers.clip,
                                     params={
                                         "min": -0.5,
                                         "max": 0.5
                                     })
...
def test_concatenate_dim(setup_env):
    ...
    torch.testing.assert_close(obs_policy[:, :, :256, :], obs_policy[:, :,
                                                                     256:, :])
