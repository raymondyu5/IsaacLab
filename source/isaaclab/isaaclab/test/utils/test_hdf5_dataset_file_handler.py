...
def create_test_episode(device):
    ...
    test_episode.add("obs/policy/term1",
                     torch.tensor([1, 2, 3, 4, 5], device=device))
    test_episode.add("obs/policy/term1",
                     torch.tensor([6, 7, 8, 9, 10], device=device))
    test_episode.add("obs/policy/term1",
                     torch.tensor([11, 12, 13, 14, 15], device=device))
...
def test_write_and_load_episode(temp_dir, device):
    ...
    for episode_name in loaded_episode_names:
        loaded_episode = dataset_file_handler.load_episode(episode_name,
                                                           device=device)
        ...
        assert torch.equal(loaded_episode.get_initial_state(),
                           test_episode.get_initial_state())
