...
def test_get_all_matching_child_prims():
    ...
    prim_utils.create_prim("/World/Floor/Box",
                           "Cube",
                           position=np.array([75, 75, -150.1]),
                           attributes={"size": 300})
    ...
    isaaclab_result = sim_utils.get_all_matching_child_prims(
        "/World", predicate=lambda x: x.GetTypeName() == "Cube")
    ...
    isaaclab_result = sim_utils.get_all_matching_child_prims(
        ...
        predicate=lambda x: x.GetTypeName() == "Mesh")
    assert len(isaaclab_result) == 1
    assert isaaclab_result[0].GetPrimPath(
    ) == "/World/Franka/panda_hand/visuals/panda_hand"
...
def test_get_first_matching_child_prim():
    ...
    assert isaaclab_result is not None
    assert isaaclab_result.GetPrimPath(
    ) == "/World/env_1/Franka/panda_link0/visuals/panda_link0"
...
def test_find_matching_prim_paths():
    ...
    for index in range(2048):
        ...
        prim_utils.create_prim(f"/World/Floor_{index}",
                               "Cube",
                               position=random_pos,
                               attributes={"size": 2.0})
        prim_utils.create_prim(f"/World/Floor_{index}/Sphere",
                               "Sphere",
                               attributes={"radius": 10})
        prim_utils.create_prim(f"/World/Floor_{index}/Sphere/childSphere",
                               "Sphere",
                               attributes={"radius": 1})
        prim_utils.create_prim(f"/World/Floor_{index}/Sphere/childSphere2",
                               "Sphere",
                               attributes={"radius": 1})

    # test leaf paths
    isaac_sim_result = prim_utils.find_matching_prim_paths(
        "/World/Floor_.*/Sphere")
    isaaclab_result = sim_utils.find_matching_prim_paths(
        "/World/Floor_.*/Sphere")
    assert isaac_sim_result == isaaclab_result
    ...
    isaac_sim_result = prim_utils.find_matching_prim_paths(
        "/World/Floor_.*/Sphere/childSphere.*")
    isaaclab_result = sim_utils.find_matching_prim_paths(
        "/World/Floor_.*/Sphere/childSphere.*")
    assert isaac_sim_result == isaaclab_result
...
def test_find_global_fixed_joint_prim():
    ...
    assert sim_utils.find_global_fixed_joint_prim(
        "/World/Franka_Isaac") is not None
    ...
    assert sim_utils.find_global_fixed_joint_prim("/World/Franka") is not None
    assert sim_utils.find_global_fixed_joint_prim(
        "/World/Franka", check_enabled_only=True) is None
...
def test_resolve_prim_pose():
    ...
    for i in range(num_objects):
        ...
        quat = quat if np.sign(rand_quats[i, 0,
                                          0]) == np.sign(quat[0]) else -quat
        ...
        pos, quat = np.array(pos), np.array(quat)
        quat = quat if np.sign(rand_quats[i, 1,
                                          0]) == np.sign(quat[0]) else -quat
        ...
        pos, quat = sim_utils.resolve_prim_pose(geometry_prim,
                                                ref_prim=xform_prim)
        pos, quat = np.array(pos), np.array(quat)
        quat = quat if np.sign(rand_quats[i, 2,
                                          0]) == np.sign(quat[0]) else -quat
        np.testing.assert_allclose(pos,
                                   rand_positions[i, 2] * rand_scales[i, 1],
                                   atol=1e-3)
...
def test_resolve_prim_scale():
    ...
    for i in range(num_objects):
        ...
        np.testing.assert_allclose(scale,
                                   rand_scales[i, 1] * rand_scales[i, 2],
                                   atol=1e-5)
        # dummy prim
