...
def sim_config():
    ...
    extension_path = get_extension_path_from_name(
        "isaacsim.asset.importer.urdf")
    # default configuration
    config = UrdfConverterCfg(
        asset_path=
        f"{extension_path}/data/urdf/robots/franka_description/robots/panda_arm_hand.urdf",
        fix_base=True,
        joint_drive=UrdfConverterCfg.JointDriveCfg(
            gains=UrdfConverterCfg.JointDriveCfg.PDGainsCfg(stiffness=400.0,
                                                            damping=40.0)),
    )
    # Simulation time-step
    dt = 0.01
    # Load kit helper
    sim = SimulationContext(physics_dt=dt,
                            rendering_dt=dt,
...
def test_no_change(sim_config):
    ...
    new_time_usd_file_created = os.stat(
        new_urdf_converter.usd_path).st_mtime_ns
...
def test_config_drive_type(sim_config):
    ...
    np.testing.assert_array_equal(drive_stiffness,
                                  config.joint_drive.gains.stiffness)
    np.testing.assert_array_equal(drive_damping,
                                  config.joint_drive.gains.damping)
