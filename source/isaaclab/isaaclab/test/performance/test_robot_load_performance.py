...
def test_robot_load_performance(test_config, device):
    ...
    with build_simulation_context(device=device) as sim:
        ...
        with Timer(f"{test_config['name']} load time for device {device}"
                   ) as timer:
            robot = Articulation(test_config["robot_cfg"].replace(
                prim_path="/World/Robots_.*/Robot"))  # noqa: F841
            sim.reset()
