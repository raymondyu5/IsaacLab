...
def test_openxr_constructors(mock_environment, mocker):
    ...
    mocker.patch.object(device_mod, "XRCore",
                        mock_environment["omni"].kit.xr.core.XRCore)
    mocker.patch.object(
        device_mod, "XRPoseValidityFlags",
...
def test_create_teleop_device_with_retargeters(mock_environment, mocker):
    ...
    device_cfg = OpenXRDeviceCfg(
        xr_cfg=xr_cfg, retargeters=[retargeter_cfg1, retargeter_cfg2])
    ...
    retargeter_mod = importlib.import_module(
...
def test_create_teleop_device_unsupported_config():
    ...
    with pytest.raises(ValueError,
                       match="Unsupported device configuration type"):
