...
def test_grid_clone_env_origins(device, env_spacing, num_envs):
    ...
    with build_simulation_context(device=device,
        ...
        torch.testing.assert_close(terrain_importer_origins,
                                   grid_cloner_origins,
                                   rtol=1e-5,
                                   atol=1e-5)
...
def test_plane(device, use_custom_material):
    ...
    with build_simulation_context(device=device,
        ...
        visual_material = PreviewSurfaceCfg(
            diffuse_color=(1.0, 0.0, 0.0)) if use_custom_material else None
        # Handler for terrains importing
        terrain_importer_cfg = terrain_gen.TerrainImporterCfg(
            prim_path="/World/ground",
            terrain_type="plane",
...
def test_usd(device):
    ...
    with build_simulation_context(device=device,
        ...
        terrain_importer_cfg = terrain_gen.TerrainImporterCfg(
            ...
            terrain_type="usd",
            usd_path=
            f"{ISAAC_NUCLEUS_DIR}/Environments/Terrains/rough_plane.usd",
            num_envs=1,
            env_spacing=1.0,
        )
        terrain_importer = TerrainImporter(terrain_importer_cfg)
...
def test_ball_drop(device):
    ...
    with build_simulation_context(device=device,
        ...
        ball_view = RigidPrim("/World/envs/env_.*/ball",
        ...
        max_velocity_z = torch.max(
            torch.abs(ball_view.get_linear_velocities()[:, 2]))
...
def _obtain_collision_mesh(
        mesh_prim_path: str,
        mesh_type: Literal["Mesh", "Plane"]) -> trimesh.Trimesh | None:
    ...
    if mesh_prim.GetTypeName() == "Mesh":
        ...
        faces = np.asarray(mesh_prim.GetFaceVertexIndicesAttr().Get()).reshape(
            -1, 3)
...
def _obtain_grid_cloner_env_origins(num_envs: int, env_spacing: float,
                                    device: str) -> torch.Tensor:
    """Obtain the env origins generated by IsaacSim GridCloner (grid_cloner.py)."""
    # create grid cloner
    cloner = GridCloner(spacing=env_spacing)
    cloner.define_base_env("/World/envs")
    envs_prim_paths = cloner.generate_paths("/World/envs/env",
                                            num_paths=num_envs)
    prim_utils.define_prim("/World/envs/env_0")
    # clone envs using grid cloner
    env_origins = cloner.clone(source_prim_path="/World/envs/env_0",
...
def _populate_scene(sim: SimulationContext,
                    num_balls: int = 2048,
                    geom_sphere: bool = False):
    ...
    if geom_sphere:
        # -- Ball physics
        _ = DynamicSphere(prim_path="/World/envs/env_0/ball",
        ...
        cube_prim_path = omni.kit.commands.execute("CreateMeshPrimCommand",
                                                   prim_type="Sphere")[1]
        prim_utils.move_prim(cube_prim_path, "/World/envs/env_0/ball")
        # -- Ball physics
        SingleRigidPrim(prim_path="/World/envs/env_0/ball",
                        mass=0.5,
                        scale=(0.5, 0.5, 0.5),
    ...
    # -- Ball material
    sphere_geom = SingleGeometryPrim(prim_path="/World/envs/env_0/ball",
                                     collision=True)
    visual_material = PreviewSurface(
        prim_path="/World/Looks/ballColorMaterial",
    ...
    cloner.filter_collisions(physics_scene_path,
                             "/World/collisions",
                             prim_paths=envs_prim_paths,
