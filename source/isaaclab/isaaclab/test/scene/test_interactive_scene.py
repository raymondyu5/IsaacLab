...
def test_scene_entity_isolation(device, setup_scene):
    ...
    setattr(scene_cfg, "sensor",
            ContactSensorCfg(prim_path="/World/envs/env_.*/Robot"))

    scene_list = []
    # create two InteractiveScene instances
    for _ in range(2):
        with build_simulation_context(device=device,
                                      dt=sim.get_physics_dt()) as _:
...
def test_relative_flag(device, setup_scene):
    ...
    assert_state_different(scene.get_state(is_relative=False),
    ...
    scene["robot"].write_joint_state_to_sim(
        position=torch.rand_like(scene["robot"].data.joint_pos),
        velocity=torch.rand_like(scene["robot"].data.joint_pos))
...
def assert_state_equal(s1: dict, s2: dict, path=""):
    ...
    """
    assert set(s1.keys()) == set(
        s2.keys()), f"Key mismatch at {path}: {s1.keys()} vs {s2.keys()}"
    for k in s1:
        v1, v2 = s1[k], s2[k]
        ...
        if isinstance(v1, dict):
            ...
            assert isinstance(v1, torch.Tensor) and isinstance(
                v2, torch.Tensor), f"Expected tensors at {subpath}"
            if not torch.equal(v1, v2):
                diff = (v1 - v2).abs().max()
                pytest.fail(
                    f"Tensor mismatch at {subpath}, max abs diff = {diff}")
