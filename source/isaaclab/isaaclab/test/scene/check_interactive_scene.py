...
parser = argparse.ArgumentParser(
    description="This script demonstrates how to use the scene interface.")
...
def main():
    ...
    with Timer("Setup scene"):
        scene = InteractiveScene(
            MySceneCfg(num_envs=args_cli.num_envs,
                       env_spacing=5.0,
                       lazy_sensor_update=False))

    # Check that parsing happened as expected
    assert len(scene.env_prim_paths
               ) == args_cli.num_envs, "Number of environments does not match."
    ...
    robot_1_actions = scene.articulations[
        "robot_1"].data.default_joint_pos.clone()
    robot_2_actions = scene.articulations[
        "robot_2"].data.default_joint_pos.clone()
    ...
    while simulation_app.is_running():
        ...
        if count % 50 == 0:
            ...
            count = 0
            # reset root state
            root_state = scene.articulations[
                "robot_1"].data.default_root_state.clone()
            ...
            # -- set root state
            # -- robot 1
            scene.articulations["robot_1"].write_root_pose_to_sim(
                root_state[:, :7])
            scene.articulations["robot_1"].write_root_velocity_to_sim(
                root_state[:, 7:])
            scene.articulations["robot_1"].write_joint_state_to_sim(
                joint_pos, joint_vel)
            # -- robot 2
            root_state[:, 1] += 1.0
            scene.articulations["robot_2"].write_root_pose_to_sim(
                root_state[:, :7])
            scene.articulations["robot_2"].write_root_velocity_to_sim(
                root_state[:, 7:])
            scene.articulations["robot_2"].write_joint_state_to_sim(
                joint_pos, joint_vel)
            # reset buffers
            scene.reset()
            print(">>>>>>>> Reset!")
        ...
        for _ in range(4):
            # set joint targets
            scene.articulations["robot_1"].set_joint_position_target(
                robot_1_actions)
            scene.articulations["robot_2"].set_joint_position_target(
                robot_2_actions)
            # write data to sim
