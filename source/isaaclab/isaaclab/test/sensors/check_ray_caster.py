...
parser.add_argument("--num_envs",
                    type=int,
                    default=128,
                    help="Number of environments to clone.")
parser.add_argument(
    "--terrain_type",
    type=str,
    default="generator",
...
def design_scene(sim: SimulationContext, num_envs: int = 2048):
    ...
    cfg = sim_utils.SphereCfg(
        ...
        visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.0, 0.0,
                                                                   1.0)),
    )
...
def main():
    ...
    while simulation_app.is_running():
        ...
        if step_count % 500 == 0:
            ...
            # reset the balls
            ball_view.set_world_poses(ball_initial_positions[reset_indices],
                                      ball_initial_orientations[reset_indices],
                                      indices=reset_indices)
            ball_view.set_velocities(ball_initial_velocities[reset_indices],
                                     indices=reset_indices)
            # reset the sensor
        ...
        with Timer(
                f"Ray-caster update with {num_envs} x {ray_caster.num_rays} rays"
        ):
