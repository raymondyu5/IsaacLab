...
CUBE_CFG = ContactSensorRigidObjectCfg(
    ...
    spawn=sim_utils.CuboidCfg(
        ...
        visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.4, 0.6,
                                                                   0.4)),
    ),
...
SPHERE_CFG = ContactSensorRigidObjectCfg(
    ...
    spawn=sim_utils.SphereCfg(
        ...
        collision_props=sim_utils.CollisionPropertiesCfg(
            collision_enabled=True, ),
        activate_contact_sensors=True,
        visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.4, 0.4,
                                                                   0.6)),
    ),
    init_state=RigidObjectCfg.InitialStateCfg(pos=(0, 1.0, 1.0)),
...
CYLINDER_CFG = ContactSensorRigidObjectCfg(
    ...
    spawn=sim_utils.CylinderCfg(
        ...
        collision_props=sim_utils.CollisionPropertiesCfg(
            collision_enabled=True, ),
        activate_contact_sensors=True,
        visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.6, 0.4,
                                                                   0.4)),
    ),
...
CAPSULE_CFG = ContactSensorRigidObjectCfg(
    ...
    spawn=sim_utils.CapsuleCfg(
        ...
        collision_props=sim_utils.CollisionPropertiesCfg(
            collision_enabled=True, ),
        activate_contact_sensors=True,
        visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.2, 0.4,
                                                                   0.4)),
    ),
...
CONE_CFG = ContactSensorRigidObjectCfg(
    ...
    spawn=sim_utils.ConeCfg(
        ...
        collision_props=sim_utils.CollisionPropertiesCfg(
            collision_enabled=True, ),
        activate_contact_sensors=True,
        visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.4, 0.2,
                                                                   0.4)),
    ),
...
FLAT_TERRAIN_CFG = TerrainImporterCfg(prim_path="/World/ground",
                                      terrain_type="plane")
...
COBBLESTONE_TERRAIN_CFG = TerrainImporterCfg(
    ...
    terrain_generator=TerrainGeneratorCfg(
        ...
        sub_terrains={
            "random_rough":
            HfRandomUniformTerrainCfg(proportion=1.0,
...
def test_cube_contact_time(setup_simulation, disable_contact_processing):
    ...
    _run_contact_sensor_test(CUBE_CFG, sim_dt, devices, terrains,
                             carb_settings_iface, durations)
...
def test_sphere_contact_time(setup_simulation, disable_contact_processing):
    ...
    carb_settings_iface.set_bool("/physics/disableContactProcessing",
                                 disable_contact_processing)
    _run_contact_sensor_test(SPHERE_CFG, sim_dt, devices, terrains,
                             carb_settings_iface, durations)
...
def test_cube_stack_contact_filtering(setup_simulation, device, num_envs):
    ...
    sim_dt, durations, terrains, devices, carb_settings_iface = setup_simulation
    with build_simulation_context(device=device, dt=sim_dt,
                                  add_lighting=True) as sim:
        sim._app_control_on_stop_handle = None
        # Instance new scene for the current terrain and contact prim.
        scene_cfg = ContactSensorSceneCfg(num_envs=num_envs,
                                          env_spacing=1.0,
        ...
        torch.testing.assert_close(
            contact_sensor_2.data.force_matrix_w[:, :, 0],
            contact_sensor_2.data.net_forces_w)
        # Check that forces are opposite and equal
        torch.testing.assert_close(
            contact_sensor_2.data.force_matrix_w[:, :, 0],
            -contact_sensor.data.force_matrix_w[:, :, 0])
...
def test_no_contact_reporting(setup_simulation):
    ...
    sim_dt, durations, terrains, devices, carb_settings_iface = setup_simulation
    with build_simulation_context(device="cpu", dt=sim_dt,
                                  add_lighting=True) as sim:
        sim._app_control_on_stop_handle = None
        # Instance new scene for the current terrain and contact prim.
        scene_cfg = ContactSensorSceneCfg(num_envs=32,
                                          env_spacing=1.0,
                                          lazy_sensor_update=False)
...
def test_sensor_print(setup_simulation):
    ...
    sim_dt, durations, terrains, devices, carb_settings_iface = setup_simulation
    with build_simulation_context(device="cuda:0",
                                  dt=sim_dt,
                                  add_lighting=False) as sim:
        sim._app_control_on_stop_handle = None
        # Spawn things into stage
        scene_cfg = ContactSensorSceneCfg(num_envs=1,
                                          env_spacing=1.0,
                                          lazy_sensor_update=False)
        scene_cfg.terrain = FLAT_TERRAIN_CFG.replace(prim_path="/World/ground")
        ...
        for terrain in terrains:
            for track_contact_points in [True, False]:
                with build_simulation_context(device=device,
                    ...
                    if (type(shape_cfg.spawn)
                            is sim_utils.SphereCfg) and (terrain.terrain_type
                                                         == "plane"):
                    ...
                    if track_contact_points:
                        if terrain.terrain_type == "plane":
                            filter_prim_paths_expr = [
                                terrain.prim_path +
                                "/terrain/GroundPlane/CollisionPlane"
                            ]
                        elif terrain.terrain_type == "generator":
                            filter_prim_paths_expr = [
                                terrain.prim_path + "/terrain/mesh"
                            ]
                    else:
                        filter_prim_paths_expr = []
...
def _test_sensor_contact(
    ...
    for idx in range(len(durations)):
        ...
        expected_last_test_contact_time = durations[
            idx - 1] + sim_dt if idx > 0 else 0
...
def _test_contact_position(shape: RigidObject, sensor: ContactSensor,
                           mode: ContactTestMode) -> None:
    ...
    if sensor.cfg.track_contact_points:
        ...
        assert sensor._data.contact_pos_w.shape == (sensor.num_instances /
                                                    num_bodies, num_bodies, 1,
                                                    3)
        # check contact positions
        if mode == ContactTestMode.IN_CONTACT:
            contact_position = sensor._data.pos_w + torch.tensor(
                [[0.0, 0.0, -shape.cfg.spawn.radius]],
                device=sensor._data.pos_w.device)
            assert torch.all(
                torch.abs(
                    torch.norm(sensor._data.contact_pos_w -
                               contact_position.unsqueeze(1),
                               p=2,
                               dim=-1)) < 1e-2).item()
        elif mode == ContactTestMode.NON_CONTACT:
...
def _check_prim_contact_state_times(
    ...
    assert pytest.approx(measured_contact_time.item(),
                         0.01) == expected_contact_time
    assert pytest.approx(measured_air_time.item(), 0.01) == expected_air_time
    # check last contact state
    assert pytest.approx(measured_last_contact_time.item(),
                         0.01) == expected_last_contact_time
    assert pytest.approx(measured_last_air_time.item(),
                         0.01) == expected_last_air_time
