...
def setup_camera(
        device) -> tuple[sim_utils.SimulationContext, TiledCameraCfg, float]:
    """Fixture to set up and tear down the camera simulation environment."""
    camera_cfg = TiledCameraCfg(
        height=128,
        width=256,
        offset=TiledCameraCfg.OffsetCfg(pos=(0.0, 0.0, 4.0),
                                        rot=(0.0, 0.0, 1.0, 0.0),
                                        convention="ros"),
...
def test_single_camera_init(setup_camera, device):
    ...
    for _ in range(10):
        ...
        for im_type, im_data in camera.data.output.items():
            if im_type == "rgb":
                assert im_data.shape == (1, camera_cfg.height,
                                         camera_cfg.width, 3)
...
def test_depth_clipping_max(setup_camera, device):
    ...
    camera_cfg = TiledCameraCfg(
        prim_path="/World/Camera",
        offset=TiledCameraCfg.OffsetCfg(pos=(2.5, 2.5, 6.0),
    ...
    assert len(camera.data.output["depth"][torch.isinf(
        camera.data.output["depth"])]) == 0
    assert camera.data.output["depth"].min(
    ) >= camera_cfg.spawn.clipping_range[0]
    assert camera.data.output["depth"].max(
...
def test_depth_clipping_none(setup_camera, device):
    ...
    if len(camera.data.output["depth"]
           [~torch.isinf(camera.data.output["depth"])]) > 0:
        assert (camera.data.output["depth"]
                [~torch.isinf(camera.data.output["depth"])].max()
                <= camera_cfg.spawn.clipping_range[1])
...
def test_multi_camera_init(setup_camera, device):
    ...
    assert camera.is_initialized
    # Check if camera prim is set correctly and that it is a camera prim
    assert camera._sensor_prims[1].GetPath(
    ).pathString == "/World/Origin_1/CameraSensor"
    assert isinstance(camera._sensor_prims[0], UsdGeom.Camera)
    ...
    for _ in range(10):
        ...
        for im_type, im_data in camera.data.output.items():
            if im_type == "rgb":
                assert im_data.shape == (num_cameras, camera_cfg.height,
                                         camera_cfg.width, 3)
                for i in range(4):
...
def test_data_types(setup_camera, device):
    ...
    assert camera_distance._sensor_prims[0].GetPath(
    ).pathString == "/World/CameraDistance"
    assert isinstance(camera_distance._sensor_prims[0], UsdGeom.Camera)
    assert camera_depth._sensor_prims[0].GetPath(
    ).pathString == "/World/CameraDepth"
    assert isinstance(camera_depth._sensor_prims[0], UsdGeom.Camera)
    assert camera_both._sensor_prims[0].GetPath(
    ).pathString == "/World/CameraBoth"
    ...
    assert list(
        camera_both.data.output.keys()) == ["depth", "distance_to_camera"]
...
def test_instance_segmentation_fast_colorize_only_camera(setup_camera, device):
    ...
    for _ in range(10):
        ...
        for _, im_data in camera.data.output.items():
            ...
            for i in range(num_cameras):
                assert (im_data[i] / 255.0).mean() > 0.0
    assert camera.data.output[
        "instance_segmentation_fast"].dtype == torch.uint8
    ...
    for _ in range(10):
        ...
        for _, im_data in camera.data.output.items():
            ...
            for i in range(num_cameras):
                assert (im_data[i] / 255.0).mean() > 0.0
    assert camera.data.output[
        "instance_id_segmentation_fast"].dtype == torch.uint8
...
@pytest.mark.parametrize("device", ["cuda:0", "cpu"])
@pytest.mark.isaacsim_ci
def test_instance_segmentation_fast_non_colorize_only_camera(
        setup_camera, device):
    """Test initialization with only instance_segmentation_fast."""
    sim, camera_cfg, dt = setup_camera
    num_cameras = 9
    ...
    for _ in range(10):
        ...
        for _, im_data in camera.data.output.items():
            ...
            for i in range(num_cameras):
                assert im_data[i].to(dtype=float).mean() > 0.0
    assert camera.data.output[
        "instance_segmentation_fast"].dtype == torch.int32
    assert isinstance(camera.data.info["instance_segmentation_fast"], dict)
    del camera


@pytest.mark.parametrize("device", ["cuda:0", "cpu"])
def test_instance_id_segmentation_fast_non_colorize_only_camera(
        setup_camera, device):
    """Test initialization with only instance_id_segmentation_fast."""
    sim, camera_cfg, dt = setup_camera
    num_cameras = 9
    ...
    for _ in range(10):
        ...
        for _, im_data in camera.data.output.items():
            ...
            for i in range(num_cameras):
                assert im_data[i].to(dtype=float).mean() > 0.0
    assert camera.data.output[
        "instance_id_segmentation_fast"].dtype == torch.int32
    assert isinstance(camera.data.info["instance_id_segmentation_fast"], dict)
    del camera
...
def test_all_annotators_instanceable(setup_camera, device):
    ...
    num_cameras = 10
    for i in range(num_cameras):
        prim_utils.create_prim(f"/World/Origin_{i}",
                               "Xform",
                               translation=(0.0, i, 0.0))
...
def test_output_equal_to_usd_camera_intrinsics(setup_camera, device):
    ...
    camera_tiled_cfg = TiledCameraCfg(
        ...
        offset=TiledCameraCfg.OffsetCfg(pos=offset_pos,
                                        rot=offset_rot,
                                        convention="ros"),
        spawn=sim_utils.PinholeCameraCfg.from_intrinsic_matrix(
    ...
    torch.testing.assert_close(camera_tiled.data.intrinsic_matrices[0],
...
def test_frame_offset_small_resolution(setup_camera, device):
    ...
    for i in range(10):
        ...
        UsdGeom.Gprim(prim).GetOrderedXformOps()[2].Set(Gf.Vec3d(
            1.0, 1.0, 1.0))
    for i in range(100):
        # step simulation
        sim.step()
    ...
    assert torch.abs(image_after - image_before).mean(
    ) > 0.1  # images of same color should be below 0.01
...
def test_frame_offset_large_resolution(setup_camera, device):
    ...
    # check difference is above threshold
    assert torch.abs(image_after - image_before).mean(
    ) > 0.01  # images of same color should be below 0.001
