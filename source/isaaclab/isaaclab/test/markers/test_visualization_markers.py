...
def sim():
    ...
    # Load kit helper
    sim_context = SimulationContext(physics_dt=dt,
                                    rendering_dt=dt,
                                    backend="torch",
                                    device="cuda:0")
    yield sim_context
...
def test_usd_marker_color(sim):
    ...
    config = FRAME_MARKER_CFG.copy()
    config.prim_path = "/World/Visuals/test_frames"
    config.markers["frame"].visual_material = sim_utils.PreviewSurfaceCfg(
...
def test_multiple_prototypes_marker(sim):
    ...
    for count in range(1000):
        ...
        if count % 50 == 0:
            ...
            marker_indices = torch.randint(0,
                                           test_marker.num_prototypes,
                                           (num_frames, ),
                                           device=sim.device)
            # set the marker
            test_marker.visualize(translations=frame_translations,
...
def test_visualization_time_based_on_prototypes(sim):
    ...
    frame_translations = torch.randn(num_frames, 3, device=sim.device)
    marker_indices = torch.randint(0,
                                   test_marker.num_prototypes, (num_frames, ),
                                   device=sim.device)
    # set the marker
