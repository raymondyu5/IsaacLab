...
def test_non_concatenated_obs_groups_contain_all_terms(device):
    ...
    assert isinstance(
        ...
        gym.spaces.Dict), "Top-level observation space should be Dict"
    ...
    assert isinstance(policy_space,
                      gym.spaces.Dict), "Policy group should be Dict space"
    ...
    for term_name in expected_policy_terms:
        ...
        assert isinstance(
            policy_space.spaces[term_name],
            gym.spaces.Box), f"Term '{term_name}' should be Box space"
    ...
    assert isinstance(
        subtask_space,
        gym.spaces.Dict), "Subtask_terms group should be Dict space"
    ...
    for term_name in expected_subtask_terms:
        ...
        assert isinstance(
            subtask_space.spaces[term_name],
            gym.spaces.Box), f"Term '{term_name}' should be Box space"
    ...
    for term_name in expected_policy_terms:
        assert term_name in obs[
            "policy"], f"Term '{term_name}' missing from policy observation"

    for term_name in expected_subtask_terms:
        assert term_name in obs[
            "subtask_terms"], f"Term '{term_name}' missing from subtask_terms observation"
...
def test_obs_space_follows_clip_contraint(env_cfg_cls, device):
    ...
    for group_name, group_space in env.observation_space.spaces.items():
        for term_name, term_space in group_space.spaces.items():
            term_cfg = getattr(getattr(env_cfg.observations, group_name),
                               term_name)
