...
class EmptyObservationWithHistoryCfg:
    ...
    class EmptyObservationGroupWithHistoryCfg(ObsGroup):
        ...
        dummy_term: ObsTerm = ObsTerm(
            func=lambda env: torch.randn(env.num_envs, 1, device=env.device))
...
def get_empty_base_env_cfg(device: str = "cuda:0",
                           num_envs: int = 1,
    ...
    class EmptyEnvCfg(ManagerBasedEnvCfg):
        ...
        # Scene settings
        scene: EmptySceneCfg = EmptySceneCfg(num_envs=num_envs,
...
def get_empty_base_env_cfg_with_history(device: str = "cuda:0",
                                        num_envs: int = 1,
                                        env_spacing: float = 1.0):
    """Generate base environment config based on device"""
    ...
    for group_name in env.observation_manager._group_obs_term_names:
        ...
        for term_name in group_term_names:
            ...
            group_obs[group_name][
                term_name] = env.observation_manager._group_obs_term_history_buffer[
                    group_name][term_name].buffer
    ...
    for group_name in env.observation_manager._group_obs_term_names:
        ...
        for term_name in group_term_names:
            ...
            assert torch.allclose(
                ...
                _group_obs_term_history_buffer[group_name][term_name].buffer,
            )
