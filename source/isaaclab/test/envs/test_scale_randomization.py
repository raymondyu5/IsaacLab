...
class CubeActionTerm(ActionTerm):
    ...
    def __init__(self, cfg: CubeActionTermCfg, env: ManagerBasedEnv):
        ...
        self._processed_actions = torch.zeros(env.num_envs,
                                              3,
                                              device=self.device)
    ...
    def apply_actions(self):
        ...
        self._vel_command[:, :
                          3] = self.p_gain * pos_error + self.d_gain * vel_error
...
def base_position(env: ManagerBasedEnv,
...
class MySceneCfg(InteractiveSceneCfg):
    ...
    light = AssetBaseCfg(
        ...
        spawn=sim_utils.DistantLightCfg(color=(0.75, 0.75, 0.75),
                                        intensity=3000.0),
    )
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        # cube velocity
        position = ObsTerm(func=base_position,
...
class CubeEnvCfg(ManagerBasedEnvCfg):
    ...
    # Scene settings
    scene: MySceneCfg = MySceneCfg(num_envs=10,
                                   env_spacing=2.5,
...
def test_scale_randomization(device):
    ...
    all_prim_paths = sim_utils.find_matching_prim_paths(
    ...
    for i in range(3):
        ...
        scale_spec = prim_spec.GetAttributeAtPath(prim_paths[i] +
                                                  ".xformOp:scale")
