...
def test_interpolation():
    ...
    height_field_downsampled = np.random.choice(height_range,
                                                size=(width_downsampled,
                                                      length_downsampled))
    ...
    xx_upsampled, yy_upsampled = np.meshgrid(x_upsampled,
                                             y_upsampled,
                                             indexing="ij",
                                             sparse=True)
    z_upsampled_RegularGridInterpolator = func_RegularGridInterpolator(
        (xx_upsampled, yy_upsampled))
    # -- method 2: RectBivariateSpline (alternate to interp2d)
    func_RectBiVariate = interpolate.RectBivariateSpline(
        x, y, height_field_downsampled)
    ...
    np.testing.assert_allclose(z_upsampled_RegularGridInterpolator,
                               z_upsampled_RectBivariant,
                               atol=1e-2,
                               rtol=1e-2)
    np.testing.assert_allclose(z_upsampled_RectBivariant,
                               z_upsampled_RegularGridInterpolator,
                               atol=1e-2,
                               rtol=1e-2)
    np.testing.assert_allclose(z_upsampled_RegularGridInterpolator,
                               z_upsampled_RegularGridInterpolator,
