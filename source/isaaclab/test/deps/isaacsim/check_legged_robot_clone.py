...
parser = argparse.ArgumentParser(
    description=
    "This script shows the issue in Isaac Sim with GPU simulation of floating robots."
)
parser.add_argument("--num_robots",
                    type=int,
                    default=128,
                    help="Number of robots to spawn.")
parser.add_argument(
    "--asset",
    type=str,
    default="isaaclab",
...
def main():
    ...
    set_carb_setting(world._settings,
    ...
    world.scene.add_default_ground_plane(prim_path="/World/defaultGroundPlane",
                                         z_position=0.0)
    # Lights-1
    prim_utils.create_prim("/World/Light/GreySphere",
                           "SphereLight",
                           translation=(4.5, 3.5, 10.0))
    # Lights-2
    prim_utils.create_prim("/World/Light/WhiteSphere",
                           "SphereLight",
                           translation=(-4.5, 3.5, 10.0))
    # -- Robot
    # resolve asset
    if args_cli.asset == "isaaclab":
        ...
        raise ValueError(
            f"Invalid asset: {args_cli.asset}. Must be one of: isaaclab, oige."
        )
    # add asset
    ...
    envs_prim_paths = cloner.generate_paths("/World/envs/env",
                                            num_paths=num_envs)
    envs_positions = cloner.clone(source_prim_path="/World/envs/env_0",
                                  prim_paths=envs_prim_paths,
                                  replicate_physics=True)
    # convert environment positions to torch tensor
    envs_positions = torch.tensor(envs_positions,
                                  dtype=torch.float,
                                  device=world.device)
