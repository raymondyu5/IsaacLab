...
def test_camera_init(setup_sim):
    ...
    assert camera.data.image_shape == (camera_cfg.pattern_cfg.height,
    ...
    for _ in range(10):
        ...
        for im_data in camera.data.output.values():
            assert im_data.shape == (1, camera_cfg.pattern_cfg.height,
                                     camera_cfg.pattern_cfg.width, 1)
...
def test_depth_clipping(setup_sim):
    ...
    camera_cfg_zero = RayCasterCameraCfg(
        ...
        mesh_prim_paths=["/World/defaultGroundPlane"],
        offset=RayCasterCameraCfg.OffsetCfg(pos=(2.5, 2.5, 6.0),
                                            rot=(0.9914449, 0.0, 0.1305, 0.0),
                                            convention="world"),
        pattern_cfg=patterns.PinholeCameraPatternCfg().from_intrinsic_matrix(
            focal_length=38.0,
            intrinsic_matrix=[
                380.08, 0.0, 467.79, 0.0, 380.08, 262.05, 0.0, 0.0, 1.0
            ],
            height=540,
            width=960,
        ),
    ...
    assert torch.isnan(
        camera_none.data.output["distance_to_image_plane"]).any()
    assert (
        camera_none.data.output["distance_to_camera"]
        [~torch.isinf(camera_none.data.output["distance_to_camera"])].max()
    ...
    assert (camera_none.data.output["distance_to_image_plane"]
            ...
            max() > camera_cfg_zero.max_distance)

    # zero clipping should result in zero values
    assert torch.all(camera_zero.data.output["distance_to_camera"][torch.isinf(
        camera_none.data.output["distance_to_camera"])] == 0.0)
    assert torch.all(
        camera_zero.data.output["distance_to_image_plane"][torch.isnan(
            camera_none.data.output["distance_to_image_plane"])] == 0.0)
    assert camera_zero.data.output["distance_to_camera"].max(
    ) <= camera_cfg_zero.max_distance
    assert camera_zero.data.output["distance_to_image_plane"].max(
    ) <= camera_cfg_zero.max_distance

    # max clipping should result in max values
    assert torch.all(camera_max.data.output["distance_to_camera"][torch.isinf(
        camera_none.data.output["distance_to_camera"])] ==
                     camera_cfg_zero.max_distance)
    assert torch.all(camera_max.data.output["distance_to_image_plane"][
        torch.isnan(camera_none.data.output["distance_to_image_plane"])] ==
                     camera_cfg_zero.max_distance)
    assert camera_max.data.output["distance_to_camera"].max(
    ) <= camera_cfg_zero.max_distance
    assert camera_max.data.output["distance_to_image_plane"].max(
    ) <= camera_cfg_zero.max_distance
...
def test_camera_init_offset(setup_sim):
    ...
    np.testing.assert_allclose(camera_ros.data.pos_w[0].cpu().numpy(),
                               cam_cfg_offset_ros.offset.pos)
    np.testing.assert_allclose(camera_opengl.data.pos_w[0].cpu().numpy(),
                               cam_cfg_offset_opengl.offset.pos)
    np.testing.assert_allclose(camera_world.data.pos_w[0].cpu().numpy(),
                               cam_cfg_offset_world.offset.pos)

    # check if transform correctly set in output
    np.testing.assert_allclose(camera_ros.data.pos_w[0].cpu().numpy(),
                               cam_cfg_offset_ros.offset.pos,
                               rtol=1e-5)
    np.testing.assert_allclose(camera_ros.data.quat_w_ros[0].cpu().numpy(),
                               QUAT_ROS,
                               rtol=1e-5)
    np.testing.assert_allclose(camera_ros.data.quat_w_opengl[0].cpu().numpy(),
                               QUAT_OPENGL,
                               rtol=1e-5)
    np.testing.assert_allclose(camera_ros.data.quat_w_world[0].cpu().numpy(),
                               QUAT_WORLD,
                               rtol=1e-5)
...
def test_camera_init_intrinsic_matrix(setup_sim):
    ...
    sim.reset()
    intrinsic_matrix = camera_1.data.intrinsic_matrices[0].cpu().flatten(
    ).tolist()
    teardown(sim)
...
def test_camera_set_world_poses(setup_sim):
    ...
    orientation = torch.tensor([QUAT_WORLD],
                               dtype=torch.float32,
                               device=camera.device)
    # set new pose
    camera.set_world_poses(position.clone(),
                           orientation.clone(),
                           convention="world")
...
def test_camera_set_world_poses_from_view(setup_sim):
    ...
    targets = torch.tensor([[0.0, 0.0, 0.0]],
                           dtype=torch.float32,
                           device=camera.device)
    quat_ros_gt = torch.tensor([QUAT_ROS],
                               dtype=torch.float32,
                               device=camera.device)
    # set new pose
...
def test_intrinsic_matrix(setup_sim):
    ...
    rs_intrinsic_matrix = [
        229.31640625, 0.0, 164.810546875, 0.0, 229.826171875, 122.1650390625,
        0.0, 0.0, 1.0
    ]
    rs_intrinsic_matrix = torch.tensor(rs_intrinsic_matrix,
                                       device=camera.device).reshape(
                                           3, 3).unsqueeze(0)
    ...
    for _ in range(10):
        ...
        torch.testing.assert_close(rs_intrinsic_matrix,
...
def test_throughput(setup_sim):
    ...
    for _ in range(5):
        ...
        # update camera
        with Timer(
                f"Time taken for updating camera with shape {camera.image_shape}"
        ):
            camera.update(dt)
        # Save images
        with Timer(
                f"Time taken for writing data with shape {camera.image_shape}   "
        ):
            ...
            camera_data = convert_dict_to_backend(
                {
                    k: v[0]
                    for k, v in camera.data.output.items()
                },
                backend="numpy")
            for key, data, info in zip(camera_data.keys(),
                                       camera_data.values(),
                                       camera.data.info[0].values()):
                if info is not None:
...
def test_output_equal_to_usdcamera_offset(setup_sim):
    ...
    camera_cfg_warp = RayCasterCameraCfg(
        ...
        mesh_prim_paths=["/World/defaultGroundPlane"],
        update_period=0,
        offset=RayCasterCameraCfg.OffsetCfg(pos=(2.5, 2.5, 4.0),
                                            rot=tuple(offset_rot),
                                            convention="ros"),
        debug_vis=False,
        pattern_cfg=camera_pattern_cfg,
        data_types=[
    ...
    camera_cfg_usd = CameraCfg(
        ...
        offset=CameraCfg.OffsetCfg(pos=(2.5, 2.5, 4.0),
                                   rot=(offset_rot[0], offset_rot[1],
                                        offset_rot[2], offset_rot[3]),
                                   convention="ros"),
    )
    camera_usd = Camera(camera_cfg_usd)
...
def test_output_equal_to_usdcamera_prim_offset(setup_sim):
    ...
    camera_cfg_warp = RayCasterCameraCfg(
        ...
        mesh_prim_paths=["/World/defaultGroundPlane"],
        update_period=0,
        offset=RayCasterCameraCfg.OffsetCfg(pos=(0, 0, 2.0),
                                            rot=offset_rot,
                                            convention="ros"),
        debug_vis=False,
        pattern_cfg=camera_pattern_cfg,
    ...
    camera_cfg_usd = CameraCfg(
        ...
        spawn=PinholeCameraCfg(focal_length=24.0,
                               ...
                               horizontal_aperture=20.955,
                               clipping_range=(1e-6, 1.0e5)),
        offset=CameraCfg.OffsetCfg(pos=(0, 0, 2.0),
                                   rot=offset_rot,
                                   convention="ros"),
    ...
    torch.testing.assert_close(camera_warp.data.pos_w[0],
                               camera_usd.data.pos_w[0])
    torch.testing.assert_close(camera_warp.data.quat_w_ros[0],
                               camera_usd.data.quat_w_ros[0])
...
def test_output_equal_to_usd_camera_intrinsics(setup_sim, focal_length):
    ...
    camera_warp_cfg = RayCasterCameraCfg(
        prim_path="/World/Camera_warp",
        mesh_prim_paths=["/World/defaultGroundPlane"],
        offset=RayCasterCameraCfg.OffsetCfg(pos=offset_pos,
                                            rot=offset_rot,
                                            convention="ros"),
        debug_vis=False,
        pattern_cfg=patterns.PinholeCameraPatternCfg.from_intrinsic_matrix(
    ...
    camera_usd_cfg = CameraCfg(
        prim_path="/World/Camera_usd",
        offset=CameraCfg.OffsetCfg(pos=offset_pos,
                                   rot=offset_rot,
                                   convention="ros"),
    ...
    torch.testing.assert_close(camera_warp.data.intrinsic_matrices[0],
                               camera_usd.data.intrinsic_matrices[0])
...
@pytest.mark.parametrize("focal_length_aperture", [(0.193, 0.20955),
                                                   (1.93, 2.0955),
                                                   (19.3, 20.955),
                                                   (0.193, 20.955)])
@pytest.mark.isaacsim_ci
def test_output_equal_to_usd_camera_when_intrinsics_set(
    ...
    camera_cfg_usd = CameraCfg(
        ...
        update_period=0,
        data_types=["distance_to_camera"],
        spawn=PinholeCameraCfg(focal_length=focal_length,
                               focus_distance=400.0,
                               horizontal_aperture=aperture,
                               clipping_range=(1e-4, 1.0e5)),
    )
    ...
    if DEBUG_PLOTS:
        ...
        axs[1].set_title("WARP")
        diff_plt = axs[2].imshow(
            torch.abs(camera_usd.data.output["distance_to_camera"] -
                      camera_warp.data.output["distance_to_camera"])
            [0].cpu().numpy())
        fig.colorbar(diff_plt, ax=axs[2])
        axs[2].set_title("Difference")
