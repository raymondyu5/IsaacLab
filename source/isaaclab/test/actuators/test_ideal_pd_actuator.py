...
def test_ideal_pd_actuator_init_minimum(num_envs, num_joints, device,
                                        usd_default):
    ...
    torch.testing.assert_close(
        ...
        torch.zeros(num_envs, num_joints, device=device))
    torch.testing.assert_close(
    ...
    torch.testing.assert_close(
        ...
        actuator._DEFAULT_MAX_EFFORT_SIM *
        torch.ones(num_envs, num_joints, device=device))
    torch.testing.assert_close(
    ...
    if not usd_default:
        torch.testing.assert_close(
            actuator.stiffness,
            stiffness * torch.ones(num_envs, num_joints, device=device))
        torch.testing.assert_close(
            actuator.damping,
            damping * torch.ones(num_envs, num_joints, device=device))
        torch.testing.assert_close(
            actuator.armature,
            armature * torch.ones(num_envs, num_joints, device=device))
        torch.testing.assert_close(
            actuator.friction,
            friction * torch.ones(num_envs, num_joints, device=device))
    else:
        torch.testing.assert_close(
            actuator.stiffness,
            stiffness_default *
...
def test_ideal_pd_actuator_init_effort_limits(num_envs, num_joints, device,
                                              effort_lim, effort_lim_sim):
    ...
    torch.testing.assert_close(
        actuator.effort_limit,
        effort_lim_expected * torch.ones(num_envs, num_joints, device=device))
    torch.testing.assert_close(
        actuator.effort_limit_sim,
        effort_lim_sim_expected *
        torch.ones(num_envs, num_joints, device=device))
...
def test_ideal_pd_actuator_init_velocity_limits(num_envs, num_joints, device,
                                                velocity_lim,
                                                velocity_lim_sim):
    ...
    torch.testing.assert_close(
        actuator.velocity_limit,
        vel_lim_expected * torch.ones(num_envs, num_joints, device=device))
    torch.testing.assert_close(
        actuator.velocity_limit_sim,
        vel_lim_sim_expected * torch.ones(num_envs, num_joints, device=device))
...
def test_ideal_pd_compute(num_envs, num_joints, device, effort_lim):
    ...
    desired_control_action.joint_velocities = desired_vel * torch.ones(
        num_envs, num_joints, device=device)
    desired_control_action.joint_efforts = torch.zeros(num_envs,
                                                       num_joints,
                                                       device=device)

    expected_comp_joint_effort = stiffness * (
        desired_pos - measured_joint_pos) + damping * (desired_vel -
                                                       measured_joint_vel)
