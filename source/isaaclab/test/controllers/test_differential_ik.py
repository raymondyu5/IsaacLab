...
from isaaclab.utils.math import (  # isort:skip
    compute_pose_error, matrix_from_quat, quat_inv, random_yaw_orientation,
...
def test_franka_ik_pose_abs(sim):
    ...
    _run_ik_controller(robot, diff_ik_controller, "panda_hand",
                       ["panda_joint.*"], sim_context, num_envs,
                       ee_pose_b_des_set)
...
def test_ur10_ik_pose_abs(sim):
    ...
    # Run the controller and check that it converges to the goal
    _run_ik_controller(robot, diff_ik_controller, "ee_link", [".*"],
                       sim_context, num_envs, ee_pose_b_des_set)
...
def _run_ik_controller(
    ...
    ee_pose_b_des = torch.zeros(num_envs,
                                diff_ik_controller.action_dim,
                                device=sim.device)
        ...
        if count % 250 == 0:
            ...
            if count > 0:
                pos_error, rot_error = compute_pose_error(
                    ee_pos_b, ee_quat_b, ee_pose_b_des[:, 0:3],
                    ee_pose_b_des[:, 3:7])
