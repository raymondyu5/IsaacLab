...
def load_test_config(env_name):
    ...
    if "G1" in env_name:
        ...
        raise ValueError(
            f"Unknown environment type in {env_name}. Expected G1 or GR1.")
...
def env_and_cfg(request):
    ...
    variable_input_tasks = [
        task for task in
        env_cfg.actions.upper_body_ik.controller.variable_input_tasks
        if isinstance(task, FrameTask)
    ]
    assert len(variable_input_tasks
               ) == 2, "Expected exactly two FrameTasks (left and right hand)."
...
def test_setup(env_and_cfg):
    ...
    left_target_link_name = env_cfg.actions.upper_body_ik.target_eef_link_names[
        "left_wrist"]
    right_target_link_name = env_cfg.actions.upper_body_ik.target_eef_link_names[
        "right_wrist"]
...
def test_movement_types(test_setup, test_name):
    ...
    if test_name not in test_cfg["tests"]:
        print(
            f"Skipping {test_name} test for {env_cfg.__class__.__name__} environment (test not defined)..."
        )
        pytest.skip(
            f"Test {test_name} not defined for {env_cfg.__class__.__name__}")
        return
    ...
    if requires_waist_bending and not waist_enabled:
        ...
        pytest.skip(
            f"Test {test_name} requires waist bending but waist is not enabled"
        )
        return
...
def run_movement_test(test_setup, test_config, test_cfg, aux_function=None):
    ...
    right_hand_poses = np.array(test_config["right_hand_pose"],
                                dtype=np.float32)
    ...
    with contextlib.suppress(KeyboardInterrupt) and torch.inference_mode():
        ...
        while simulation_app.is_running() and not simulation_app.is_exiting():
            ...
            setpoint_poses = np.concatenate([
                left_hand_poses[curr_pose_idx], right_hand_poses[curr_pose_idx]
            ])
            actions = np.concatenate(
                [setpoint_poses,
                 np.zeros(num_joints_in_robot_hands)])
            actions = torch.tensor(actions,
                                   device=env.device,
                                   dtype=torch.float32)
            # Append base command for Locomanipulation environments with fixed height
            if test_setup[
                    "env_cfg"].__class__.__name__ == "LocomanipulationG1EnvCfg":
                # Use a named variable for base height for clarity and maintainability
                BASE_HEIGHT = 0.72
                base_command = torch.zeros(4,
                                           device=env.device,
                                           dtype=actions.dtype)
...
def calculate_rotation_error(current_rot, target_rot):
    ...
    if isinstance(target_rot, torch.Tensor):
        target_rot_tensor = (target_rot.unsqueeze(0).expand(
            current_rot.shape[0], -1) if target_rot.dim() == 1 else target_rot)
    else:
        target_rot_tensor = torch.tensor(target_rot, device=current_rot.device)
        if target_rot_tensor.dim() == 1:
            target_rot_tensor = target_rot_tensor.unsqueeze(0).expand(
                current_rot.shape[0], -1)

    return axis_angle_from_quat(
        quat_from_matrix(
            matrix_from_quat(target_rot_tensor) *
            matrix_from_quat(quat_inv(current_rot))))


def compute_errors(test_setup, env, left_target_pose, right_target_pose,
                   left_eef_urdf_link_name, right_eef_urdf_link_name):
    ...
    left_hand_pose_setpoint = torch.tensor(left_target_pose,
                                           device=device).unsqueeze(0).repeat(
                                               num_envs, 1)
    right_hand_pose_setpoint = torch.tensor(right_target_pose,
                                            device=device).unsqueeze(0).repeat(
                                                num_envs, 1)
    ...
    left_rot_error = calculate_rotation_error(left_hand_rot,
                                              left_hand_pose_setpoint[:, 3:])
    right_rot_error = calculate_rotation_error(right_hand_rot,
                                               right_hand_pose_setpoint[:, 3:])
    ...
    curr_joints = articulation.data.joint_pos[:,
                                              isaaclab_controlled_joint_ids].cpu(
                                              ).numpy()[0]
    target_joints = action_term.processed_actions[:, :len(
        isaaclab_controlled_joint_ids)].cpu().numpy()[0]

    # Reorder joints for Pink IK (using controlled joint ordering)
    curr_joints = np.array(curr_joints)[
        ik_controller.isaac_lab_to_pink_controlled_ordering]
    ...
    left_curr_pos = test_kinematics_model.get_transform_frame_to_world(
        frame=left_eef_urdf_link_name).translation
    right_curr_pos = test_kinematics_model.get_transform_frame_to_world(
        frame=right_eef_urdf_link_name).translation
    ...
    # Calculate PD errors
    left_pd_error = (torch.tensor(left_target_pos - left_curr_pos,
                                  device=device,
                                  dtype=torch.float32).unsqueeze(0).repeat(
                                      num_envs, 1))
    right_pd_error = (torch.tensor(right_target_pos - right_curr_pos,
                                   device=device,
                                   dtype=torch.float32).unsqueeze(0).repeat(
                                       num_envs, 1))
...
def print_debug_info(errors, test_counter):
    ...
    for hand in ["left", "right"]:
        print(f"Measured {hand} hand position error:",
              errors[f"{hand}_pos_error"])
        print(f"Measured {hand} hand rotation error:",
              errors[f"{hand}_rot_error"])
