...
def test_modify_properties_on_articulation_instanced_usd(setup_simulation):
    ...
    if "4.5" in ISAAC_NUCLEUS_DIR:
        asset_usd_file = asset_usd_file.replace("http",
                                                "https").replace("4.5", "5.0")
    prim_utils.create_prim("/World/asset_instanced",
                           usd_path=asset_usd_file,
                           translation=(0.0, 0.0, 0.62))

    # set properties on the asset and check all properties are set
    schemas.modify_articulation_root_properties("/World/asset_instanced",
                                                arti_cfg)
    ...
    _validate_articulation_properties_on_prim("/World/asset_instanced/base",
                                              arti_cfg, False)
    _validate_rigid_body_properties_on_prim("/World/asset_instanced",
                                            rigid_cfg)
    _validate_mass_properties_on_prim("/World/asset_instanced", mass_cfg)
    _validate_joint_drive_properties_on_prim("/World/asset_instanced",
                                             joint_cfg)
...
def test_modify_properties_on_articulation_usd(setup_simulation):
    ...
    if "4.5" in ISAAC_NUCLEUS_DIR:
        asset_usd_file = asset_usd_file.replace("http",
                                                "https").replace("4.5", "5.0")
    prim_utils.create_prim("/World/asset",
                           usd_path=asset_usd_file,
                           translation=(0.0, 0.0, 0.62))
...
def test_defining_rigid_body_properties_on_prim(setup_simulation):
    ...
    # spawn a prim
    prim_utils.create_prim("/World/cube1",
                           prim_type="Cube",
                           translation=(0.0, 0.0, 0.62))
    # set properties on the asset and check all properties are set
    ...
    # spawn another prim
    prim_utils.create_prim("/World/cube2",
                           prim_type="Cube",
                           translation=(1.0, 1.0, 0.62))
    # set properties on the asset and check all properties are set
...
def test_defining_articulation_properties_on_prim(setup_simulation):
    ...
    prim_utils.create_prim("/World/parent/child",
                           prim_type="Cube",
                           translation=(0.0, 0.0, 0.62))
...
def _validate_articulation_properties_on_prim(prim_path: str,
                                              arti_cfg,
                                              has_default_fixed_root: bool,
                                              verbose: bool = False):
    ...
    for attr_name, attr_value in arti_cfg.__dict__.items():
        ...
        assert root_prim.GetAttribute(prim_prop_name).Get() == pytest.approx(
            attr_value, abs=1e-5), f"Failed setting for {prim_prop_name}"


def _validate_rigid_body_properties_on_prim(prim_path: str,
                                            rigid_cfg,
                                            verbose: bool = False):
    ...
    for link_prim in root_prim.GetChildren():
        if UsdPhysics.RigidBodyAPI(link_prim):
            for attr_name, attr_value in rigid_cfg.__dict__.items():
                # skip names we know are not present
                if attr_name in [
                        "func", "rigid_body_enabled", "kinematic_enabled"
                ]:
                    continue
                # convert attribute name in prim to cfg name
                prim_prop_name = f"physxRigidBody:{to_camel_case(attr_name, to='cC')}"
                # validate the values
                assert link_prim.GetAttribute(
                    prim_prop_name).Get() == pytest.approx(
                        attr_value,
                        abs=1e-5), f"Failed setting for {prim_prop_name}"
        elif verbose:
            print(
                f"Skipping prim {link_prim.GetPrimPath()} as it is not a rigid body."
            )


def _validate_collision_properties_on_prim(prim_path: str,
                                           collision_cfg,
                                           verbose: bool = False):
        ...
        for mesh_prim in link_prim.GetChildren():
            if UsdPhysics.CollisionAPI(mesh_prim):
                for attr_name, attr_value in collision_cfg.__dict__.items():
                    ...
                    # validate the values
                    assert mesh_prim.GetAttribute(
                        prim_prop_name).Get() == pytest.approx(
                            attr_value,
                            abs=1e-5), f"Failed setting for {prim_prop_name}"
            elif verbose:
                print(
                    f"Skipping prim {mesh_prim.GetPrimPath()} as it is not a collision mesh."
                )


def _validate_mass_properties_on_prim(prim_path: str,
                                      mass_cfg,
                                      verbose: bool = False):
    ...
    for link_prim in root_prim.GetChildren():
        if UsdPhysics.MassAPI(link_prim):
            ...
            print(
                f"Skipping prim {link_prim.GetPrimPath()} as it is not a mass api."
            )


def _validate_joint_drive_properties_on_prim(prim_path: str,
                                             joint_cfg,
                                             verbose: bool = False):
    """Validate the mass properties on the prim.
    ...
    for link_prim in root_prim.GetAllChildren():
        for joint_prim in link_prim.GetChildren():
            if joint_prim.IsA(UsdPhysics.PrismaticJoint) or joint_prim.IsA(
                ...
                for attr_name, attr_value in joint_cfg.__dict__.items():
                    ...
                    drive_model = "linear" if joint_prim.IsA(
                        UsdPhysics.PrismaticJoint) else "angular"
                    ...
                    if attr_name == "drive_type":
                        ...
                        # check the value
                        assert attr_value == joint_prim.GetAttribute(
                            prim_attr_name).Get()
                        continue
                    ...
                    prim_attr_value = joint_prim.GetAttribute(
                        prim_attr_name).Get()
                    ...
                    assert prim_attr_value == pytest.approx(
                        attr_value,
                        abs=1e-5), f"Failed setting for {prim_attr_name}"
            elif verbose:
                print(
                    f"Skipping prim {joint_prim.GetPrimPath()} as it is not a joint drive api."
                )
