...
def check_mesh_conversion(mesh_converter: MeshConverter):
    ...
    # Check mesh settings
    pos = tuple(
        prim_utils.get_prim_at_path("/World/Object/geometry").GetAttribute(
            "xformOp:translate").Get())
    assert pos == mesh_converter.cfg.translation
    quat = prim_utils.get_prim_at_path("/World/Object/geometry").GetAttribute(
        "xformOp:orient").Get()
    quat = (quat.GetReal(), quat.GetImaginary()[0], quat.GetImaginary()[1],
            quat.GetImaginary()[2])
...
def check_mesh_collider_settings(mesh_converter: MeshConverter):
    ...
    exp_collision_enabled = (
        mesh_converter.cfg.collision_props is not None
        and mesh_converter.cfg.collision_props.collision_enabled)
    ...
    if exp_collision_enabled:
        if mesh_converter.cfg.mesh_collision_props is not None:
            exp_collision_approximation = (
                mesh_converter.cfg.mesh_collision_props.usd_func(
                    mesh_prim).GetApproximationAttr().Get())
            mesh_collision_api = UsdPhysics.MeshCollisionAPI(mesh_prim)
            collision_approximation = mesh_collision_api.GetApproximationAttr(
            ).Get()
...
def test_no_change(assets):
    ...
    new_time_usd_file_created = os.stat(
        new_mesh_converter.usd_path).st_mtime_ns
...
def test_convert_obj(assets):
    ...
    mesh_config = MeshConverterCfg(
        ...
        scale=(random.uniform(0.1, 2.0), random.uniform(0.1, 2.0),
               random.uniform(0.1, 2.0)),
        translation=(random.uniform(-10.0, 10.0), random.uniform(-10.0, 10.0),
