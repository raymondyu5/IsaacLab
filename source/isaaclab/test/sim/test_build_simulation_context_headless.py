...
def test_build_simulation_context_no_cfg(gravity_enabled, device, dt):
    ...
    with build_simulation_context(gravity_enabled=gravity_enabled,
                                  device=device,
                                  dt=dt) as sim:
        if gravity_enabled:
...
def test_build_simulation_context_ground_plane(add_ground_plane):
    ...
    with build_simulation_context(add_ground_plane=add_ground_plane) as _:
        ...
        assert is_prim_path_valid(
            "/World/defaultGroundPlane") == add_ground_plane
...
def test_build_simulation_context_auto_add_lighting(add_lighting,
    ...
    with build_simulation_context(add_lighting=add_lighting,
                                  auto_add_lighting=auto_add_lighting) as _:
        if add_lighting:
...
def test_build_simulation_context_cfg():
    ...
    with build_simulation_context(sim_cfg=cfg,
