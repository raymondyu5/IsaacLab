...
def test_spawn_mdl_material(sim):
    ...
    assert prim.GetAttribute(
        "inputs:albedo_brightness").Get() == cfg.albedo_brightness


def test_spawn_glass_mdl_material(sim):
    """Test spawning a glass mdl material."""
    cfg = sim_utils.materials.GlassMdlCfg(thin_walled=False,
                                          glass_ior=1.0,
...
def test_spawn_rigid_body_material(sim):
    ...
    # Check properties
    assert prim.GetAttribute(
        "physics:staticFriction").Get() == cfg.static_friction
    assert prim.GetAttribute(
        "physics:dynamicFriction").Get() == cfg.dynamic_friction
    assert prim.GetAttribute("physics:restitution").Get() == cfg.restitution
    assert prim.GetAttribute("physxMaterial:restitutionCombineMode").Get(
    ) == cfg.restitution_combine_mode
    assert prim.GetAttribute(
        "physxMaterial:frictionCombineMode").Get() == cfg.friction_combine_mode
...
def test_spawn_deformable_body_material(sim):
    ...
    # Check properties
    assert prim.GetAttribute(
        "physxDeformableBodyMaterial:density").Get() == cfg.density
    assert prim.GetAttribute("physxDeformableBodyMaterial:dynamicFriction"
                             ).Get() == cfg.dynamic_friction
    assert prim.GetAttribute("physxDeformableBodyMaterial:youngsModulus").Get(
    ) == cfg.youngs_modulus
    assert prim.GetAttribute("physxDeformableBodyMaterial:poissonsRatio").Get(
    ) == cfg.poissons_ratio
    assert prim.GetAttribute("physxDeformableBodyMaterial:elasticityDamping"
                             ).Get() == pytest.approx(cfg.elasticity_damping)
    assert prim.GetAttribute(
        "physxDeformableBodyMaterial:dampingScale").Get() == cfg.damping_scale
...
def test_bind_prim_to_material(sim):
    ...
    visual_material_cfg = sim_utils.GlassMdlCfg(glass_ior=1.0,
                                                thin_walled=True)
    visual_material_cfg.func("/World/Looks/glassMaterial", visual_material_cfg)
    # create a physics material
    physics_material_cfg = sim_utils.RigidBodyMaterialCfg(static_friction=0.5,
                                                          dynamic_friction=1.5,
                                                          restitution=1.5)
    physics_material_cfg.func("/World/Physics/rubberMaterial",
                              physics_material_cfg)
    sim_utils.bind_visual_material("/World/Geometry/box",
                                   "/World/Looks/glassMaterial")
    sim_utils.bind_physics_material("/World/Geometry/box",
                                    "/World/Physics/rubberMaterial")
    ...
    assert material_direct_binding.GetMaterialPath(
    ) == "/World/Looks/glassMaterial"
