...
class DummyResetRecorderTerm(RecorderTerm):
    ...
    def record_pre_reset(
        ...
        return "record_pre_reset", torch.ones(self._env.num_envs,
                                              2,
    ...
    def record_post_reset(
        ...
        return "record_post_reset", torch.ones(self._env.num_envs,
                                               3,
                                               device=self._env.device)
...
class DummyStepRecorderTerm(RecorderTerm):
    ...
    def record_pre_step(self) -> tuple[str | None, torch.Tensor | None]:
        return "record_pre_step", torch.ones(self._env.num_envs,
                                             4,
                                             device=self._env.device)

    def record_post_step(self) -> tuple[str | None, torch.Tensor | None]:
        return "record_post_step", torch.ones(self._env.num_envs,
                                              5,
                                              device=self._env.device)
...
def create_dummy_env(device: str = "cpu") -> ManagerBasedEnv:
    ...
    return namedtuple(
        "ManagerBasedEnv",
        ["num_envs", "device", "sim", "cfg", "termination_manager"])(
...
def test_record(dataset_dir):
    ...
    for device in ("cuda:0", "cpu"):
        ...
        for env_id in range(env.num_envs):
            ...
            assert torch.stack(episode.data["record_post_step"]).shape == (2,
                                                                           5)
        ...
        for env_id in range(env.num_envs):
            episode = recorder_manager.get_episode(env_id)
            assert torch.stack(episode.data["record_post_reset"]).shape == (1,
                                                                            3)
