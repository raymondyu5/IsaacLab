...
def mock_xrcore(mocker):
    ...
    joint_pose_mock.validity_flags = (
        xr_pose_validity_flags_mock.POSITION_VALID
        | xr_pose_validity_flags_mock.ORIENTATION_VALID)
    ...
    mocker.patch.object(device_mod, "XRPoseValidityFlags",
                        xr_pose_validity_flags_mock)
...
def test_xr_anchor(empty_env, mock_xrcore):
    ...
    assert carb.settings.get_settings().get(
        "/persistent/xr/profile/ar/anchorMode") == "custom anchor"
    assert carb.settings.get_settings().get(
        "/xrstage/profile/ar/customAnchor") == "/XRAnchor"
...
def test_get_raw_data(empty_env, mock_xrcore):
    ...
    np.testing.assert_almost_equal(palm_pose[3:],
                                   [0.9, 0.1, 0.2, 0.3])  # Orientation
    ...
    np.testing.assert_almost_equal(head_pose[3:],
                                   [0.9, 0.1, 0.2, 0.3])  # Orientation
