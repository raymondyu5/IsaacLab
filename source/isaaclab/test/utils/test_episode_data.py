...
def test_add_tensors(device):
    ...
    expected_added_data = torch.cat(
        (dummy_data_0.unsqueeze(0), dummy_data_1.unsqueeze(0)))
    episode = EpisodeData()
...
def test_add_dict_tensors(device):
    ...
    dummy_dict_data_0 = {
        ...
        "key_1": {
            "key_1_0": torch.tensor([1], device=device),
            "key_1_1": torch.tensor([2], device=device)
        },
    }
    dummy_dict_data_1 = {
        "key_0": torch.tensor([3], device=device),
        "key_1": {
            "key_1_0": torch.tensor([4], device=device),
            "key_1_1": torch.tensor([5], device=device)
        },
    }
