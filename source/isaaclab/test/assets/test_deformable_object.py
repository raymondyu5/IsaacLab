...
@pytest.mark.parametrize("material_path",
                         [None, "/World/SoftMaterial", "material"])
def test_initialization(sim, num_cubes, material_path):
    ...
    assert cube_object.data.nodal_state_w.shape == (
        num_cubes, cube_object.max_sim_vertices_per_body, 6)
    assert cube_object.data.nodal_kinematic_target.shape == (
    ...
    # Check sim data
    assert cube_object.data.sim_element_quat_w.shape == (
    ...
    assert cube_object.data.sim_element_stress_w.shape == (
...
def test_initialization_with_kinematic_enabled(sim, num_cubes):
    ...
    for _ in range(2):
        ...
        torch.testing.assert_close(cube_object.data.nodal_state_w,
...
@pytest.mark.isaacsim_ci
def test_set_nodal_state_with_applied_transform(sim, num_cubes, randomize_pos,
                                                randomize_rot):
    """Test setting the state of the deformable object with applied transform."""
    carb_settings_iface = carb.settings.get_settings()
    carb_settings_iface.set_bool("/physics/cooking/ujitsoCollisionCooking",
                                 False)

    # Create a new simulation context with gravity disabled
    with build_simulation_context(auto_add_lighting=True,
                                  gravity_enabled=False) as sim:
        sim._app_control_on_stop_handle = None
        cube_object = generate_cubes_scene(num_cubes=num_cubes)
        sim.reset()
        ...
        for _ in range(5):
            ...
            if randomize_pos:
                pos_w = torch.rand(cube_object.num_instances,
                                   3,
                                   device=sim.device)
                pos_w[:, 2] += 0.5
            else:
                pos_w = None
            if randomize_rot:
                quat_w = math_utils.random_orientation(
                    cube_object.num_instances, device=sim.device)
            else:
                quat_w = None
            ...
            if pos_w is None:
                torch.testing.assert_close(mean_nodal_pos_init,
                                           mean_nodal_pos_default,
                                           rtol=1e-5,
                                           atol=1e-5)
            else:
                torch.testing.assert_close(mean_nodal_pos_init,
                                           mean_nodal_pos_default + pos_w,
                                           rtol=1e-5,
                                           atol=1e-5)
...
def test_set_kinematic_targets(sim, num_cubes):
    ...
    nodal_kinematic_targets = cube_object.root_physx_view.get_sim_kinematic_targets(
    ).clone()

    for _ in range(5):
        cube_object.write_nodal_state_to_sim(
            cube_object.data.default_nodal_state_w)
        ...
        nodal_kinematic_targets[
            0, :, :3] = cube_object.data.default_nodal_state_w[0, :, :3]
        cube_object.write_nodal_kinematic_target_to_sim(
            nodal_kinematic_targets[0],
        ...
        for _ in range(20):
            ...
            torch.testing.assert_close(cube_object.data.nodal_pos_w[0],
