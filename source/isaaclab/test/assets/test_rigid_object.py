...
def generate_cubes_scene(
    ...
    """
    origins = torch.tensor([(i * 1.0, 0, height)
                            for i in range(num_cubes)]).to(device)
    ...
    if api == "none":
        ...
        spawn_cfg = sim_utils.UsdFileCfg(
            usd_path=
            f"{ISAACLAB_NUCLEUS_DIR}/Tests/RigidObject/Cube/dex_cube_instanceable_with_articulation_root.usd",
            rigid_props=sim_utils.RigidBodyPropertiesCfg(
                kinematic_enabled=kinematic_enabled),
        )
    else:
...
def test_initialization_with_kinematic_enabled(num_cubes, device):
    ...
    with build_simulation_context(device=device,
        ...
        for _ in range(2):
            ...
            torch.testing.assert_close(cube_object.data.root_state_w,
                                       default_root_state)
...
def test_external_force_buffer(device):
    ...
    with build_simulation_context(device=device,
        ...
        for step in range(5):
            ...
            external_wrench_b = torch.zeros(cube_object.num_instances,
                                            len(body_ids),
                                            6,
                                            device=sim.device)
            external_wrench_positions_b = torch.zeros(
                cube_object.num_instances, len(body_ids), 3, device=sim.device)
            ...
            assert cube_object._external_wrench_positions_b[
                0, 0, 0].item() == position
            assert cube_object._use_global_wrench_frame == (step == 0
                                                            or step == 3)
...
def test_external_force_on_single_body(num_cubes, device):
    ...
    with build_simulation_context(device=device,
        ...
        external_wrench_b[
            0::2, :, 2] = 9.81 * cube_object.root_physx_view.get_masses()[0]
        ...
        for _ in range(5):
            ...
            torch.testing.assert_close(
                cube_object.data.root_pos_w[0::2, 2],
...
def test_external_force_on_single_body_at_position(num_cubes, device):
    ...
    with build_simulation_context(device=device,
        ...
        external_wrench_b = torch.zeros(cube_object.num_instances,
                                        ...
                                        6,
                                        device=sim.device)
        external_wrench_positions_b = torch.zeros(cube_object.num_instances,
                                                  len(body_ids),
                                                  3,
                                                  device=sim.device)
        # Every 2nd cube should have a force applied to it
        ...
        for _ in range(5):
            ...
            assert torch.all(
                torch.abs(cube_object.data.root_ang_vel_b[0::2, 0]) > 0.1)
            # Second object should have fallen, so it's Z height should be less than initial height of 1.0
            assert torch.all(cube_object.data.root_pos_w[1::2, 2] < 1.0)
...
def test_set_rigid_object_state(num_cubes, device):
    ...
    with build_simulation_context(device=device,
        ...
        for state_type_to_randomize in state_types:
            state_dict = {
                "root_pos_w":
                torch.zeros_like(cube_object.data.root_pos_w,
                                 device=sim.device),
                "root_quat_w":
                default_orientation(num=num_cubes, device=sim.device),
                "root_lin_vel_w":
                torch.zeros_like(cube_object.data.root_lin_vel_w,
                                 device=sim.device),
                "root_ang_vel_w":
                torch.zeros_like(cube_object.data.root_ang_vel_w,
                                 device=sim.device),
            }
...
def test_reset_rigid_object(num_cubes, device):
    ...
    with build_simulation_context(device=device,
        ...
        for i in range(5):
            ...
            # Random orientation
            root_state[:, 3:7] = random_orientation(num=num_cubes,
                                                    device=sim.device)
            cube_object.write_root_pose_to_sim(root_state[:, :7])
            cube_object.write_root_velocity_to_sim(root_state[:, 7:])
...
def test_rigid_body_set_material_properties(num_cubes, device):
    ...
    with build_simulation_context(device=device,
        ...
        torch.testing.assert_close(materials_to_check.reshape(num_cubes, 3),
                                   materials)
...
def test_rigid_body_no_friction(num_cubes, device):
    ...
    with build_simulation_context(device=device,
        ...
        cube_object.root_physx_view.set_material_properties(
            cube_object_materials, indices)
        ...
        for _ in range(5):
            ...
            torch.testing.assert_close(cube_object.data.root_lin_vel_w,
                                       initial_velocity[:, :3],
                                       rtol=1e-5,
                                       atol=tolerance)
    ...
    with build_simulation_context(device=device,
        ...
        cfg = sim_utils.GroundPlaneCfg(
            physics_material=materials.RigidBodyMaterialCfg(
                ...
                dynamic_friction=
                static_friction_coefficient,  # This shouldn't be required but is due to a bug in PhysX
            ))
        cfg.func("/World/GroundPlane", cfg)
        ...
        static_friction = torch.Tensor([[static_friction_coefficient]] *
                                       num_cubes)
        dynamic_friction = torch.Tensor([[static_friction_coefficient]] *
                                        num_cubes)
        ...
        for _ in range(100):
            sim.step()
            cube_object.update(sim.cfg.dt)
        cube_object.write_root_velocity_to_sim(
        ...
        for force in "below_mu", "above_mu":
            ...
            external_wrench_b = torch.zeros((num_cubes, 1, 6),
                                            device=sim.device)
            if force == "below_mu":
                external_wrench_b[
                    ...,
                    0] = static_friction_coefficient * cube_mass * gravity_magnitude * 0.99
            else:
                external_wrench_b[
                    ...,
                    0] = static_friction_coefficient * cube_mass * gravity_magnitude * 1.01
            ...
            for _ in range(200):
                ...
                if force == "below_mu":
                    ...
                    torch.testing.assert_close(cube_object.data.root_pos_w,
                                               initial_root_pos,
                                               rtol=2e-3,
                                               atol=2e-3)
            if force == "above_mu":
                assert (cube_object.data.root_state_w[..., 0] -
                        initial_root_pos[..., 0] > 0.02).all()
    ...
    for expected_collision_type in "partially_elastic", "inelastic":
        with build_simulation_context(device=device,
            ...
            if expected_collision_type == "partially_elastic":
                ...
                assert torch.all(
                    torch.le(abs(curr_z_velocity), abs(prev_z_velocity)))
                assert (curr_z_velocity > 0.0).all()
...
def test_rigid_body_set_mass(num_cubes, device):
    ...
    with build_simulation_context(device=device,
        ...
        masses = original_masses + torch.FloatTensor(num_cubes, 1).uniform_(
            4, 8)
        ...
        torch.testing.assert_close(cube_object.root_physx_view.get_masses(),
                                   masses)
    ...
    with build_simulation_context(device=device,
        ...
        if with_offset:
            offset = torch.tensor([0.1, 0.0, 0.0],
                                  device=device).repeat(num_cubes, 1)
        else:
        ...
        for _ in range(100):
            ...
            if not with_offset:
                ...
                torch.testing.assert_close(env_pos + offset,
                                           root_com_state_w[..., :3])
                torch.testing.assert_close(
                    env_pos + offset, body_com_state_w[..., :3].squeeze(-2))
                ...
                torch.testing.assert_close(-offset,
                ...
                torch.testing.assert_close(
                    -offset, body_link_state_pos_rel_com.squeeze(-2))
                ...
                torch.testing.assert_close(com_quat_w, body_com_state_w[...,
                                                                        3:7])
                torch.testing.assert_close(com_quat_w.squeeze(-2),
                                           root_com_state_w[..., 3:7])

                # orientation of link will match root state will always match
                torch.testing.assert_close(root_state_w[..., 3:7],
                                           root_link_state_w[..., 3:7])
                torch.testing.assert_close(body_state_w[..., 3:7],
                                           body_link_state_w[..., 3:7])

                # lin_vel will not match
                # center of mass vel will be constant (i.e. spinning around com)
                torch.testing.assert_close(
                    torch.zeros_like(root_com_state_w[..., 7:10]),
                    root_com_state_w[..., 7:10])
                torch.testing.assert_close(
                    torch.zeros_like(body_com_state_w[..., 7:10]),
                    body_com_state_w[..., 7:10])
...
def test_write_root_state(num_cubes, device, with_offset, state_location):
    ...
    with build_simulation_context(device=device,
        ...
        rand_state[..., 3:7] = torch.nn.functional.normalize(rand_state[...,
                                                                        3:7],
                                                             dim=-1)
        ...
        for i in range(10):
            ...
            if state_location == "com":
                torch.testing.assert_close(rand_state,
                                           cube_object.data.root_com_state_w)
            elif state_location == "link":
                torch.testing.assert_close(rand_state,
                                           cube_object.data.root_link_state_w)
...
@pytest.mark.isaacsim_ci
def test_write_state_functions_data_consistency(num_cubes, device, with_offset,
                                                state_location):
    """Test the setters for root_state using both the link frame and center of mass as reference frame."""
    with build_simulation_context(device=device,
                                  gravity_enabled=False,
        ...
        if state_location == "com":
            ...
            expected_root_link_pose = torch.cat(
                (expected_root_link_pos, expected_root_link_quat), dim=1)
            # test both root_pose and root_link_state_w successfully updated when root_com_state_w updates
            torch.testing.assert_close(
                expected_root_link_pose,
                cube_object.data.root_link_state_w[:, :7])
            ...
            torch.testing.assert_close(
                cube_object.data.root_com_state_w[:, 10:],
                cube_object.data.root_link_state_w[:, 10:])
            torch.testing.assert_close(expected_root_link_pose,
                                       cube_object.data.root_state_w[:, :7])
            torch.testing.assert_close(
                cube_object.data.root_com_state_w[:, 10:],
                cube_object.data.root_state_w[:, 10:])
        elif state_location == "link":
            ...
            expected_com_pose = torch.cat(
                (expected_com_pos, expected_com_quat), dim=1)
            # test both root_pose and root_com_state_w successfully updated when root_link_state_w updates
            torch.testing.assert_close(
                expected_com_pose, cube_object.data.root_com_state_w[:, :7])
            # skip 7:10 because they differs from link frame, this should be fine because we are only checking
            # if velocity update is triggered, which can be determined by comparing angular velocity
            torch.testing.assert_close(
                cube_object.data.root_link_state_w[:, 10:],
            ...
            torch.testing.assert_close(
                cube_object.data.root_state_w[:, 7:],
                cube_object.data.root_com_state_w[:, 7:])
            torch.testing.assert_close(
                cube_object.data.root_state_w[:, :7],
                cube_object.data.root_link_state_w[:, :7])
            torch.testing.assert_close(
