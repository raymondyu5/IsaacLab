...
def generate_cubes_scene(
    ...
    """
    origins = torch.tensor([(i * 3.0, 0, height)
                            for i in range(num_envs)]).to(device)
    # Create Top-level Xforms, one for each cube
    for i, origin in enumerate(origins):
        prim_utils.create_prim(f"/World/Table_{i}",
                               "Xform",
                               translation=origin)
    ...
    for i in range(num_cubes):
        cube_object_cfg = RigidObjectCfg(
            ...
            spawn=spawn_cfg,
            init_state=RigidObjectCfg.InitialStateCfg(pos=(0.0, 3 * i,
                                                           height)),
        )
        cube_config_dict[f"cube_{i}"] = cube_object_cfg
    # create the rigid object collection
    cube_object_collection_cfg = RigidObjectCollectionCfg(
        rigid_objects=cube_config_dict)
    cube_object_colection = RigidObjectCollection(
...
def sim(request):
    ...
    with build_simulation_context(device=device,
                                  ...
                                  gravity_enabled=gravity_enabled) as sim:
...
def test_initialization(sim, num_envs, num_cubes, device):
    ...
    object_collection, _ = generate_cubes_scene(num_envs=num_envs,
                                                num_cubes=num_cubes,
                                                device=device)
    ...
    assert object_collection.data.object_link_pos_w.shape == (num_envs,
                                                              num_cubes, 3)
    assert object_collection.data.object_link_quat_w.shape == (num_envs,
                                                               num_cubes, 4)
    assert object_collection.data.default_mass.shape == (num_envs, num_cubes,
                                                         1)
    assert object_collection.data.default_inertia.shape == (num_envs,
                                                            num_cubes, 9)
...
def test_id_conversion(sim, device):
    ...
    object_collection, _ = generate_cubes_scene(num_envs=2,
                                                num_cubes=3,
                                                device=device)
    ...
    view_ids = object_collection._env_obj_ids_to_view_ids(
        object_collection._ALL_ENV_INDICES,
        object_collection._ALL_OBJ_INDICES[None, 2])
    assert (view_ids == expected[0]).all()
    view_ids = object_collection._env_obj_ids_to_view_ids(
        object_collection._ALL_ENV_INDICES[None, 0],
        object_collection._ALL_OBJ_INDICES[None, 2])
    assert (view_ids == expected[1]).all()
    view_ids = object_collection._env_obj_ids_to_view_ids(
        object_collection._ALL_ENV_INDICES[None, 0],
        object_collection._ALL_OBJ_INDICES)
    assert (view_ids == expected[2]).all()
    view_ids = object_collection._env_obj_ids_to_view_ids(
        object_collection._ALL_ENV_INDICES[None, 1],
        object_collection._ALL_OBJ_INDICES)
...
@pytest.mark.parametrize("num_cubes", [1, 3])
@pytest.mark.parametrize("device", ["cuda:0", "cpu"])
def test_initialization_with_kinematic_enabled(sim, num_envs, num_cubes,
                                               device):
    """Test that initialization for prim with kinematic flag enabled."""
    object_collection, origins = generate_cubes_scene(num_envs=num_envs,
                                                      num_cubes=num_cubes,
    ...
    for _ in range(2):
        ...
        torch.testing.assert_close(object_collection.data.object_link_state_w,
...
def test_initialization_with_no_rigid_body(sim, num_cubes, device):
    ...
    object_collection, _ = generate_cubes_scene(num_cubes=num_cubes,
                                                has_api=False,
                                                device=device)
...
def test_external_force_buffer(sim, device):
    ...
    for step in range(5):
        ...
        external_wrench_b = torch.zeros(object_collection.num_instances,
                                        len(object_ids),
                                        6,
                                        device=sim.device)
        external_wrench_positions_b = torch.zeros(
            object_collection.num_instances,
            len(object_ids),
            3,
            device=sim.device)
        ...
        for i in range(num_envs):
            ...
            assert object_collection._external_torque_b[i, 0,
                                                        0].item() == force
            assert object_collection._external_wrench_positions_b[
                i, 0, 0].item() == position
            assert object_collection._use_global_wrench_frame == (step == 0
                                                                  or step == 3)
...
def test_external_force_on_single_body(sim, num_envs, num_cubes, device):
    ...
    external_wrench_b[:, 0::2,
                      2] = 9.81 * object_collection.data.default_mass[:, 0::2,
                                                                      0]
    ...
    for _ in range(5):
        ...
        assert torch.all(object_collection.data.object_link_pos_w[:, 1::2,
                                                                  2] < 1.0)
...
@pytest.mark.parametrize("num_cubes", [1, 4])
@pytest.mark.parametrize("device", ["cuda:0", "cpu"])
def test_external_force_on_single_body_at_position(sim, num_envs, num_cubes,
                                                   device):
    ...
    external_wrench_b = torch.zeros(object_collection.num_instances,
                                    ...
                                    6,
                                    device=sim.device)
    external_wrench_positions_b = torch.zeros(object_collection.num_instances,
                                              len(object_ids),
                                              3,
                                              device=sim.device)
    # Every 2nd cube should have a force applied to it
    ...
    for _ in range(5):
        ...
        assert torch.all(object_collection.data.object_ang_vel_b[:, 0::2,
                                                                 0] > 0.1)
        # Second object should have fallen, so it's Z height should be less than initial height of 1.0
        assert torch.all(object_collection.data.object_link_pos_w[:, 1::2,
                                                                  2] < 1.0)
...
def test_set_object_state(sim, num_envs, num_cubes, device, gravity_enabled):
    ...
    for state_type_to_randomize in state_types:
        state_dict = {
            "object_pos_w":
            torch.zeros_like(object_collection.data.object_pos_w,
                             device=sim.device),
            "object_quat_w":
            default_orientation(num=num_cubes * num_envs,
                                device=sim.device).view(
                                    num_envs, num_cubes, 4),
            "object_lin_vel_w":
            torch.zeros_like(object_collection.data.object_lin_vel_w,
                             device=sim.device),
            "object_ang_vel_w":
            torch.zeros_like(object_collection.data.object_ang_vel_w,
                             device=sim.device),
        }
        ...
        for _ in range(5):
            ...
            if state_type_to_randomize == "object_quat_w":
                state_dict[state_type_to_randomize] = random_orientation(
                    num=num_cubes * num_envs,
                    device=sim.device).view(num_envs, num_cubes, 4)
            else:
                ...
                if state_type_to_randomize == "object_pos_w":
                    state_dict[state_type_to_randomize][
                        ..., :2] += origins.unsqueeze(1)[..., :2]
...
def test_object_state_properties(sim, num_envs, num_cubes, device, with_offset,
    ...
    cube_object, env_pos = generate_cubes_scene(num_envs=num_envs,
                                                num_cubes=num_cubes,
                                                height=0.0,
                                                device=device)
    ...
    offset = (torch.tensor([0.1, 0.0, 0.0], device=device).repeat(
        num_envs, num_cubes, 1) if with_offset else torch.tensor(
            [0.0, 0.0, 0.0], device=device).repeat(num_envs, num_cubes, 1))

    com = cube_object.reshape_view_to_data(
        cube_object.root_physx_view.get_coms())
    com[..., :3] = offset.to("cpu")
    ...
    torch.testing.assert_close(
        cube_object.reshape_view_to_data(
            cube_object.root_physx_view.get_coms()), com)
    ...
    for i in range(10):
        ...
        cube_object.write_object_velocity_to_sim(
        ...
        if not with_offset:
            ...
            torch.testing.assert_close(com_quat_w, object_com_state_w[...,
                                                                      3:7])

            # orientation of link will match object state will always match
            torch.testing.assert_close(object_state_w[..., 3:7],
                                       object_link_state_w[..., 3:7])
            ...
            lin_vel_rel_object_gt = quat_apply_inverse(
            ...
            torch.testing.assert_close(lin_vel_rel_gt,
                                       lin_vel_rel_object_gt,
                                       atol=1e-4,
                                       rtol=1e-3)

            # ang_vel will always match
            torch.testing.assert_close(object_state_w[..., 10:],
                                       object_com_state_w[..., 10:])
            torch.testing.assert_close(object_state_w[..., 10:],
                                       object_link_state_w[..., 10:])
...
@pytest.mark.parametrize("gravity_enabled", [False])
def test_write_object_state(sim, num_envs, num_cubes, device, with_offset,
    ...
    for i in range(10):
        ...
        if state_location == "com":
            torch.testing.assert_close(rand_state,
                                       cube_object.data.object_com_state_w)
        elif state_location == "link":
            torch.testing.assert_close(rand_state,
                                       cube_object.data.object_link_state_w)
...
def test_reset_object_collection(sim, num_envs, num_cubes, device):
    ...
    for i in range(5):
        ...
        object_state = object_collection.data.default_object_state.clone()
        object_state[..., :3] = torch.randn(num_envs,
                                            num_cubes,
                                            3,
                                            device=sim.device)
        # Random orientation
        object_state[..., 3:7] = random_orientation(num=num_cubes,
                                                    device=sim.device)
        object_collection.write_object_state_to_sim(object_state)
        ...
        if i % 2 == 0:
            ...
            assert torch.count_nonzero(
                object_collection._external_force_b) == 0
            assert torch.count_nonzero(
                object_collection._external_torque_b) == 0
...
def test_set_material_properties(sim, num_envs, num_cubes, device):
    ...
    static_friction = torch.FloatTensor(num_envs, num_cubes,
                                        1).uniform_(0.4, 0.8)
    dynamic_friction = torch.FloatTensor(num_envs, num_cubes,
                                         1).uniform_(0.4, 0.8)
    restitution = torch.FloatTensor(num_envs, num_cubes, 1).uniform_(0.0, 0.2)

    materials = torch.cat([static_friction, dynamic_friction, restitution],
                          dim=-1)
...
def test_gravity_vec_w(sim, num_envs, num_cubes, device, gravity_enabled):
    ...
    for _ in range(2):
        ...
        torch.testing.assert_close(object_collection.data.object_acc_w,
                                   gravity)
...
@pytest.mark.parametrize("gravity_enabled", [False])
def test_write_object_state_functions_data_consistency(sim, num_envs,
                                                       num_cubes, device,
                                                       with_offset,
                                                       state_location,
                                                       gravity_enabled):
    ...
    if state_location == "com":
        ...
        cube_object.write_object_state_to_sim(rand_state,
                                              env_ids=env_ids,
                                              object_ids=object_ids)
    ...
    if state_location == "com":
        expected_root_link_pos, expected_root_link_quat = combine_frame_transforms(
            cube_object.data.object_com_state_w[..., :3].view(-1, 3),
            cube_object.data.object_com_state_w[..., 3:7].view(-1, 4),
            quat_rotate(quat_inv(object_link_to_com_quat),
        ...
        expected_object_link_pose = torch.cat(
            (expected_root_link_pos, expected_root_link_quat),
            dim=1).view(num_envs, -1, 7)
        # test both root_pose and root_link_state_w successfully updated when root_com_state_w updates
        torch.testing.assert_close(
            expected_object_link_pose,
            cube_object.data.object_link_state_w[..., :7])
        ...
        torch.testing.assert_close(
            cube_object.data.object_com_state_w[..., 10:],
            cube_object.data.object_link_state_w[..., 10:])
        torch.testing.assert_close(expected_object_link_pose,
                                   cube_object.data.object_state_w[..., :7])
        torch.testing.assert_close(
            cube_object.data.object_com_state_w[..., 10:],
            cube_object.data.object_state_w[..., 10:])
    elif state_location == "link":
        ...
        expected_object_com_pose = torch.cat(
            (expected_com_pos, expected_com_quat),
            dim=1).view(num_envs, -1, 7)
        # test both root_pose and root_com_state_w successfully updated when root_link_state_w updates
        torch.testing.assert_close(
            expected_object_com_pose,
            cube_object.data.object_com_state_w[..., :7])
        # skip 7:10 because they differs from link frame, this should be fine because we are only checking
        # if velocity update is triggered, which can be determined by comparing angular velocity
        torch.testing.assert_close(
            cube_object.data.object_link_state_w[..., 10:],
        ...
        expected_object_com_pose = torch.cat(
            (expected_object_com_pos, expected_object_com_quat),
            dim=1).view(num_envs, -1, 7)
