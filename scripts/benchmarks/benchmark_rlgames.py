...
parser = argparse.ArgumentParser(
    description="Train an RL agent with RL-Games.")
...
parser.add_argument(
    ...
    choices=[
        "LocalLogMetrics", "JSONFileMetrics", "OsmoKPIFile", "OmniPerfKPIFile"
    ],
...
benchmark = BaseIsaacBenchmark(
    ...
    workflow_metadata={
        "metadata": [
            ...
            {
                "name": "num_envs",
                "data": args_cli.num_envs
            },
            {
                "name": "max_iterations",
                "data": args_cli.max_iterations
            },
        ]
    },
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg,
    ...
    log_root_path = os.path.join("logs", "rl_games",
    ...
    if args_cli.distributed:
        ...
        agent_cfg["params"]["config"][
            "device"] = f"cuda:{app_launcher.local_rank}"
        agent_cfg["params"]["config"][
            "device_name"] = f"cuda:{app_launcher.local_rank}"
    ...
    dump_yaml(os.path.join(log_root_path, log_dir, "params", "env.yaml"),
              env_cfg)
    dump_yaml(os.path.join(log_root_path, log_dir, "params", "agent.yaml"),
              agent_cfg)
    ...
    if world_rank == 0:
        ...
        rl_training_times = {
            "Environment only step time":
            log_data["performance/step_time"],
            "Environment + Inference step time":
            log_data["performance/step_inference_time"],
            "Environment + Inference + Policy update time":
            log_data["performance/rl_update_time"],
            "Environment only FPS":
            log_data["performance/step_fps"],
            "Environment + Inference FPS":
            log_data["performance/step_inference_fps"],
            "Environment + Inference + Policy update FPS":
            log_data["performance/step_inference_rl_update_fps"],
        }

        # log additional metrics to benchmark services
        log_app_start_time(benchmark,
                           (app_start_time_end - app_start_time_begin) / 1e6)
        log_python_imports_time(benchmark,
                                (imports_time_end - imports_time_begin) / 1e6)
        log_task_start_time(
            benchmark, (task_startup_time_end - task_startup_time_begin) / 1e6)
        log_scene_creation_time(benchmark,
                                Timer.get_timer_info("scene_creation") * 1000)
        log_simulation_start_time(
            benchmark,
            Timer.get_timer_info("simulation_start") * 1000)
        log_total_start_time(
            benchmark, (task_startup_time_end - app_start_time_begin) / 1e6)
        log_runtime_step_times(benchmark,
                               rl_training_times,
                               compute_stats=True)
        log_rl_policy_rewards(benchmark, log_data["rewards/iter"])
        log_rl_policy_episode_lengths(benchmark,
                                      log_data["episode_lengths/iter"])
