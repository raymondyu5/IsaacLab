...
parser = argparse.ArgumentParser(
    description=
    "This script can help you benchmark how many cameras you could run.")
"""
...
parser.add_argument(
    ...
    required=False,
    help=
    "Supply this argument to spawn cameras within an known manager-based task environment.",
)
...
parser.add_argument(
    ...
    default=1,
    help=
    "The number of cameras per environment to use when using a known task.",
)

parser.add_argument("--use_fabric",
                    action="store_true",
                    default=False,
                    help="Enable fabric and use USD I/O operations.")
...
parser.add_argument(
    "--autotune_max_camera_count",
    type=int,
    default=4096,
    help="The maximum amount of cameras allowed in an autotune.")
...
parser.add_argument(
    ...
    required=False,
    help=
    "Number of tiled cameras to create. For autotuning, this is how many cameras to start with.",
)
...
parser.add_argument(
    ...
    required=False,
    help=
    "Number of standard cameras to create. For autotuning, this is how many cameras to start with.",
)
...
parser.add_argument(
    ...
    required=False,
    help=
    "Number of ray caster cameras to create. For autotuning, this is how many cameras to start with.",
)
...
parser.add_argument(
    ...
    help=
    "WARNING: Ray Caster can currently only cast against a single, static, object",
)
...
parser.add_argument(
    ...
    required=False,
    help=
    "Number of objects to spawn into the scene when not using a known task.",
)
...
if len(args_cli.ray_caster_visible_mesh_prim_paths) > 1:
    print(
        "[WARNING]: Ray Casting is only currently supported for a single, static object"
    )
...
def create_tiled_cameras(num_cams: int = 2,
                         ...
                         height: int = 100,
                         width: int = 120) -> TiledCamera | None:
...
def create_cameras(num_cams: int = 2,
                   data_types: list[str] | None = None,
                   height: int = 100,
                   width: int = 120) -> Camera | None:
    """Defines the Standard cameras."""
    if data_types is None:
        data_types = ["rgb", "depth"]
    return create_camera_base(camera_cfg=CameraCfg,
                              num_cams=num_cams,
                              data_types=data_types,
                              height=height,
                              width=width)
...
def create_ray_caster_cameras(
    ...
    for idx in range(num_cams):
        prim_utils.create_prim(f"/World/RayCasterCamera_{idx:02d}/RayCaster",
                               "Xform")
...
def design_scene(
    ...
    for i in range(num_objects):
        ...
        common_properties = {
            ...
            "mass_props":
            sim_utils.MassPropertiesCfg(mass=5.0),
            "collision_props":
            sim_utils.CollisionPropertiesCfg(),
            "visual_material":
            sim_utils.PreviewSurfaceCfg(diffuse_color=color, metallic=0.5),
            "semantic_tags": [("class", prim_type)],
        }
        if prim_type == "Cube":
            shape_cfg = sim_utils.CuboidCfg(size=(0.25, 0.25, 0.25),
                                            **common_properties)
        elif prim_type == "Cone":
            shape_cfg = sim_utils.ConeCfg(radius=0.1,
                                          height=0.25,
                                          **common_properties)
        elif prim_type == "Cylinder":
            shape_cfg = sim_utils.CylinderCfg(radius=0.25,
                                              height=0.25,
                                              **common_properties)
        # Rigid Object
...
def get_utilization_percentages(
        reset: bool = False,
        max_values: list[float] = [0.0, 0.0, 0.0, 0.0]) -> list[float]:
    ...
    if torch.cuda.is_available():
        ...
        if args_cli.autotune:
            ...
            for i in range(torch.cuda.device_count()):
                ...
                max_values[2] = max(max_values[2],
                                    gpu_processing_utilization_percent)

                # GPU Memory Usage
                memory_info = pynvml.nvmlDeviceGetMemoryInfo(handle)
                gpu_memory_total = memory_info.total
                gpu_memory_used = memory_info.used
                gpu_memory_utilization_percent = (gpu_memory_used /
                                                  gpu_memory_total) * 100
                max_values[3] = max(max_values[3],
...
def run_simulator(
    ...
    perspective_depth_predicate: Callable = lambda x: x ==
    "distance_to_camera",
    ...
    camera_data_types = [
        tiled_camera_data_types, standard_camera_data_types,
        ray_caster_camera_data_types
    ]
    ...
    while simulation_app.is_running() and timestep < experiment_length:
        print(
            f"On timestep {timestep} of {experiment_length}, with warm start of {warm_start_length}"
        )
        ...
        for camera_list, data_types, label in zip(camera_lists,
                                                  camera_data_types, labels):
    ...
    print(
        f"Average simulation step duration: {avg_sim_step_duration:.6f} seconds"
    )
    print(f"Total simulation time: {sim_step_time:.6f} seconds")
    print("\nSystem Utilization Statistics:")
    print(f"| CPU:{system_utilization_analytics[0]}% | "
    ...
    return {
        ...
        "system_utilization_analytics": system_utilization_analytics
    }
...
def main():
    ...
    if ((args_cli.num_tiled_cameras > 0 and args_cli.num_standard_cameras > 0)
            or (args_cli.num_ray_caster_cameras > 0
                and args_cli.num_standard_cameras > 0)
            or (args_cli.num_ray_caster_cameras > 0
                and args_cli.num_tiled_cameras > 0)):
        print("[WARNING]: You have elected to use more than one camera type.")
        print(
            "[WARNING]: For a benchmark to be meaningful, use ONLY ONE camera type at a time."
        )
        print(
    ...
    if args_cli.task is None:
        ...
        run_simulator(
            ...
            standard_camera_data_types=args_cli.standard_camera_data_types,
            ray_caster_camera_data_types=args_cli.ray_caster_camera_data_types,
            convert_depth_to_camera_to_image_plane=args_cli.
            convert_depth_to_camera_to_image_plane,
        )
    else:
        ...
        max_num_cams = max(args_cli.num_tiled_cameras,
                           args_cli.num_standard_cameras,
                           args_cli.num_ray_caster_cameras)
        ...
        if args_cli.autotune:
            ...
            print(
                "[INFO]: Auto tuning until any of the following threshold are met"
            )
            print(
                f"|CPU: {max_sys_util_thresh[0]}% | RAM {max_sys_util_thresh[1]}% | GPU: {max_sys_util_thresh[2]}% |"
            )
        ...
        ray_caster_camera_cfg = create_ray_caster_camera_cfg(
            "ray_caster_camera")
        camera_name_prefix = ""
        ...
        while (all(
                cur <= max_thresh
                for cur, max_thresh in zip(cur_sys_util, max_sys_util_thresh))
            ...
            analysis = run_simulator(
                ...
                tiled_camera_data_types=args_cli.tiled_camera_data_types,
                standard_camera_data_types=args_cli.standard_camera_data_types,
                ray_caster_camera_data_types=args_cli.
                ray_caster_camera_data_types,
                convert_depth_to_camera_to_image_plane=args_cli.
                convert_depth_to_camera_to_image_plane,
        ...
        print(
            f"[INFO]: If you've made it this far, you can likely simulate {cur_num_cams} {camera_name_prefix}"
        )
        ...
        if not args_cli.autotune:
            print(
                "[WARNING]: GPU Util Statistics only correct while autotuning, ignore above."
            )
