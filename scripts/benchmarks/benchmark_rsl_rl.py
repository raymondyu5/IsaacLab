...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg,
    ...
    benchmark.set_phase("loading",
                        start_recording_frametime=False,
                        start_recording_runtime=True)
    ...
    if world_rank == 0:
        ...
        collection_fps = (1 / (np.array(log_data["Perf/collection time"])) *
                          env.unwrapped.num_envs *
                          agent_cfg.num_steps_per_env * world_size)
        rl_training_times = {
            "Collection Time":
            (np.array(log_data["Perf/collection time"]) / 1000).tolist(),
            "Learning Time":
            (np.array(log_data["Perf/learning_time"]) / 1000).tolist(),
            "Collection FPS":
            collection_fps.tolist(),
            "Total FPS":
            log_data["Perf/total_fps"] * world_size,
        }
        ...
        log_rl_policy_episode_lengths(benchmark,
                                      log_data["Train/mean_episode_length"])
