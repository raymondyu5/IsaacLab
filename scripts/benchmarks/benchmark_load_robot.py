...
parser = argparse.ArgumentParser(
    description="Benchmark loading different robots.")
...
print(
    f"[INFO]: App start time: {(app_start_time_end - app_start_time_begin) / 1e6:.2f} ms"
)
...
print(
    f"[INFO]: Imports time: {(imports_time_end - imports_time_begin) / 1e6:.2f} ms"
)
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    print(
        f"[INFO]: Per step time: {(step_time_end - step_time_begin) / num_steps / 1e6:.2f} ms"
    )
...
def main():
    ...
    print(
        f"[INFO]: Scene creation time: {(setup_time_end - setup_time_begin) / 1e6:.2f} ms"
    )
    ...
    print(
        f"[INFO]: Sim start time: {(reset_time_end - reset_time_begin) / 1e6:.2f} ms"
    )
