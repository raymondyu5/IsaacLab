...
import numpy as np
import asyncio
from scripts.workflows.hand_manipulation.utils.TeleVision.webrtc.zed_server import *
...
class OpenTeleVision:
    ...
    def __init__(self,
        ...
        if stream_mode == "webrtc":
            ...
            self.webrtc_process = Process(target=web.run_app,
                                          args=(app, ),
                                          kwargs={
                                              "host": "0.0.0.0",
                                              "port": 8080,
    ...
    async def on_hand_move(self, event, session, fps=60):
        try:
            ...
            self.left_landmarks_shared[:] = np.array(
                event.value["leftLandmarks"]).flatten()
            self.right_landmarks_shared[:] = np.array(
                event.value["rightLandmarks"]).flatten()
        except:
            pass
...
if __name__ == "__main__":
    ...
    crop_size_h = 270
    resolution_cropped = (resolution[0] - crop_size_h,
                          resolution[1] - 2 * crop_size_w)  # 450 * 600
    ...
    shm = shared_memory.SharedMemory(create=True,
                                     ...
                                     np.uint8().itemsize)
    shm_name = shm.name
    img_array = np.ndarray((img_shape[0], img_shape[1], 3),
                           dtype=np.uint8,
                           buffer=shm.buf)

    tv = OpenTeleVision(resolution_cropped,
                        cert_file="../cert.pem",
                        key_file="../key.pem")
    while True:
