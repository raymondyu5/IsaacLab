...
class TeleopServer:
    ...
    def send_teleop_cmd(self, left_hand_pose, left_hand_joints,
                        right_hand_pose, right_hand_joints, head_pose):

        if self.pub_socket is not None:

            self.pub_socket.send_pyobj(
                dict(left_hand_pose=left_hand_pose,
    ...
    def handle_init_config_request(self):
        try:
            while True:
                ...
                try:
                    init_config = InitializationConfig.from_dict(
                        init_config_dict)
