...
def _get_out_shape(in_shape, layers):
    ...
    return ((nn.Sequential(
        *layers) if isinstance(layers, list) else layers)(x).squeeze(0).shape)
...
class RandomShiftsAug(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.pad = int(
            cfg.img_size /
            21)  # maintain same padding ratio as in original implementation
...
class Episode(object):
    ...
    def __init__(self, cfg, init_obs, init_state=None):
        ...
        self.obs[0] = torch.tensor(init_obs,
                                   dtype=torch.uint8,
                                   device=self.device)
        ...
        self.state[0] = torch.tensor(init_state,
                                     dtype=torch.float32,
                                     device=self.device)
        ...
        self.reward = torch.empty((cfg.episode_length, ),
                                  dtype=torch.float32,
                                  device=self.device)
    ...
    def from_trajectory(cls, cfg, obs, states, action, reward, done=None):
        ...
        episode.obs[1:] = torch.tensor(obs[1:],
                                       dtype=episode.obs.dtype,
                                       device=episode.device)
        episode.state[1:] = torch.tensor(states[1:],
                                         dtype=episode.state.dtype,
                                         device=episode.device)
        episode.action = torch.tensor(action,
                                      dtype=episode.action.dtype,
                                      device=episode.device)
        episode.reward = torch.tensor(reward,
                                      dtype=episode.reward.dtype,
                                      device=episode.device)
    ...
    def add(self, obs, state, action, reward, done):
        self.obs[self._idx + 1] = torch.tensor(obs,
                                               dtype=self.obs.dtype,
                                               device=self.obs.device)
        self.state[self._idx + 1] = torch.tensor(state,
...
def get_demos(cfg):
    fps = glob.glob(
        str(Path(cfg.demo_dir) / "demonstrations" / f"{cfg.task}/*.pt"))
    episodes = []
    for fp in fps:
        data = torch.load(fp)
        frames_dir = Path(os.path.dirname(fp)) / "frames"
        assert frames_dir.exists(), "No frames directory found for {}".format(
            fp)
        frame_fps = [frames_dir / fn for fn in data["frames"]]
        obs = np.stack([np.array(Image.open(fp))
                        for fp in frame_fps]).transpose(0, 3, 1, 2)
        ...
        if cfg.task.startswith("mw-"):
            ...
            if cfg.task == "adroit-door":
                state = np.concatenate([state[:, :27], state[:, 29:32]],
                                       axis=1)
            elif cfg.task == "adroit-hammer":
                state = state[:, :36]
            elif cfg.task == "adroit-pen":
                state = np.concatenate([state[:, :24], state[:, -9:-6]],
                                       axis=1)
            else:
        ...
        if cfg.task.startswith("mw-") or cfg.task.startswith("adroit-"):
            rewards = (np.array(
                [
                    _data["success" if "success" in
                          _data.keys() else "goal_achieved"]
...
class ReplayBuffer(object):
    ...
    def __init__(self, cfg):
        ...
        self._obs = torch.empty((self.capacity + 1, *obs_shape),
                                dtype=torch.uint8,
                                device=self.device)
        ...
        self._action = torch.empty((self.capacity, cfg.action_dim),
                                   dtype=torch.float32,
                                   device=self.device)
        self._reward = torch.empty((self.capacity, ),
                                   dtype=torch.float32,
                                   device=self.device)
        self._state = torch.empty((self.capacity, self._state_dim),
                                  dtype=torch.float32,
                                  device=self.device)
        ...
        self._priorities = torch.ones((self.capacity, ),
                                      dtype=torch.float32,
                                      device=self.device)
        self._eps = 1e-6
        self.idx = 0
    ...
    def add(self, episode: Episode):
        ...
        self._action[self.idx:self.idx +
                     self.cfg.episode_length] = episode.action
        self._reward[self.idx:self.idx +
                     self.cfg.episode_length] = episode.reward
        states = torch.tensor(episode.state, dtype=torch.float32)
        self._state[self.idx:self.idx +
                    self.cfg.episode_length, :self._state_dim] = states[:-1]
        self._last_state[
            self.idx // self.cfg.episode_length, :self._state_dim] = states[-1]
        max_priority = (1.0 if self.idx == 0 else
                        self._priorities[:self.idx].max().to(
                            self.device).item())
        mask = (torch.arange(self.cfg.episode_length)
                >= self.cfg.episode_length - self.cfg.horizon)
        new_priorities = torch.full((self.cfg.episode_length, ),
                                    max_priority,
                                    device=self.device)
        new_priorities[mask] = 0
        self._priorities[self.idx:self.idx +
                         self.cfg.episode_length] = new_priorities
        self.idx = (self.idx + self.cfg.episode_length) % self.capacity

    def update_priorities(self, idxs, priorities):
        self._priorities[idxs] = priorities.squeeze(1).to(
            self.device) + self._eps
    ...
    def sample(self):
        ...
        idxs = torch.from_numpy(
            np.random.choice(total,
                             self.cfg.batch_size,
                             p=probs.cpu().numpy(),
                             replace=True)).to(self.device)
        weights = (total * probs[idxs])**(-self.cfg.per_beta)
        weights /= weights.max()
        obs = (self._get_obs(self._obs, idxs) if self.cfg.frame_stack > 1 else
               self._obs[idxs].cuda(non_blocking=True))
        ...
        for t in range(self.cfg.horizon + 1):
            _idxs = idxs + t
            next_obs[t] = (self._get_obs(self._obs, _idxs +
                                         1) if self.cfg.frame_stack > 1 else
                           self._obs[_idxs + 1].cuda(non_blocking=True))
            action[t] = self._action[_idxs]
            reward[t] = self._reward[_idxs]
            next_state[t] = self._state[_idxs + 1, :self._state_dim]

        mask = (_idxs + 1) % self.cfg.episode_length == 0
        next_obs[-1, mask] = (self._last_obs[_idxs[mask] //
                                             self.cfg.episode_length].to(
                                                 next_obs.device,
                                                 non_blocking=True).float())
        state = state.cuda(non_blocking=True)
        next_state[-1, mask] = (
            self._last_state[_idxs[mask] //
                             self.cfg.episode_length, :self._state_dim].to(
                                 next_state.device).float())
