...
class Sb3VecEnvWrapper(VecEnv):
    ...
    def __init__(self,
                 env: ManagerBasedRLEnv | DirectRLEnvm,
                 args_cli,
                 gpu_buffer=False):
        ...
        if self.env.args_cli.action_framework is not None:
            ...
            action_space = action_space.shape[
                0] - self.env.num_hand_joints * self.num_agents + self.env.num_finger_actions * self.num_agents
            action_space = gym.spaces.Box(low=-1,
                                          high=1,
        ...
        self.action_space = []
        self.shared_observation_space = observation_space
        self.shared_action_space = action_space
        ...
        if args_cli.add_left_hand and args_cli.add_right_hand:
            ...
            single_action_space = gym.spaces.Box(
                low=-1, high=1, shape=(int(action_space.shape[-1] / 2), ))
        ...
        self._ep_rew_buf = torch.zeros(self.num_envs,
                                       self.num_agents,
    ...
    def step_wait(self) -> VecEnvStepReturn:  # noqa: D102
        ...
        if self.args_cli.add_left_hand:
            ...
            does_buffer.append(np.expand_dims(dones, axis=1))
        if self.args_cli.add_right_hand:
            rewards.append(self.env.env.reward_manager.
                           _episode_reward["right_rewards"].unsqueeze(-1))
            does_buffer.append(np.expand_dims(dones, axis=1))
        concat_rew = torch.cat(rewards, dim=1)
        self._ep_rew_buf += concat_rew

        if dones[0]:

            self.reset()

        return obs, obs, concat_rew.cpu().numpy(), np.concatenate(
            does_buffer, axis=1), infos
        ...
        if not concatenate_obs:
            ...
            if not self.gpu_buffer:
                for key, value in obs.items():
                    ...
                    dict_obs[key] = value.cpu().numpy().reshape(
                        self.num_envs, self.num_agents, -1)
                return dict_obs
            else:
                return obs.reshape(self.num_envs, self.num_agents, -1)
        # note: ManagerBasedRLEnv uses torch backend (by default).
        if isinstance(obs, dict):

            obs = torch.cat([v for v in obs.values()],
                            dim=-1).detach().cpu().numpy().reshape(
                                self.num_envs, self.num_agents, -1)
        elif isinstance(obs, torch.Tensor):
            if self.gpu_buffer:
                obs = obs.detach().reshape(self.num_envs, self.num_agents, -1)
            else:
                obs = obs.detach().cpu().numpy().reshape(
                    self.num_envs, self.num_agents, -1)
        else:
    ...
    def _process_extras(self, obs: np.ndarray, terminated: np.ndarray,
        ...
        for idx in range(self.num_envs):
            ...
            if idx in reset_ids:
                ...
                agent_id = 0
                if self.args_cli.add_left_hand:
                    infos[idx]["episode"][f"agent_{agent_id}_r"] = float(
                        self._ep_rew_buf[idx, agent_id])
                    agent_id += 1
                if self.args_cli.add_right_hand:
                    infos[idx]["episode"][f"agent_{agent_id}_r"] = float(
                        self._ep_rew_buf[idx, agent_id])
