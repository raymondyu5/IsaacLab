...
import scripts.workflows.hand_manipulation.env.rl_env.tdmpc_modem_helper as h
...
class TOLD(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self._dynamics = h.mlp(cfg.latent_dim + cfg.action_dim, cfg.mlp_dim,
                               cfg.latent_dim)
...
class TDMPC:
    ...
    def __init__(self, cfg):
        ...
        self.pi_optim = torch.optim.Adam(self.model._pi.parameters(),
                                         lr=self.cfg.lr)
        self.bc_optim = torch.optim.Adam(self.model.parameters(),
                                         lr=self.cfg.lr)
    ...
    def estimate_value(self, z, actions, horizon):
        ...
        G += discount * torch.min(
            *self.model.Q(z, self.model.pi(z, self.cfg.min_std)))
        return G
    ...
    def plan(self, obs, state, eval_mode=False, step=None, t0=True):
        ...
        for _ in range(self.cfg.iterations):
            actions = torch.clamp(
                mean.unsqueeze(1) + std.unsqueeze(1) * torch.randn(
                    horizon,
            ...
            score /= score.sum(0)
            _mean = torch.sum(score.unsqueeze(0) * elite_actions,
            ...
            mean, std = self.cfg.momentum * mean + (
                1 - self.cfg.momentum) * _mean, _std

        # Outputs
        score = score.squeeze(1).cpu().numpy()
        actions = elite_actions[:,
                                np.random.
                                choice(np.arange(score.shape[0]), p=score)]
    ...
    def init_bc(self, buffer):
        ...
        self.model.train()
        for _ in tqdm(range(2 * self.cfg.demos * self.cfg.episode_length),
                      "Pretraining policy"):
    ...
    def _td_target(self, next_obs, next_state, reward):
        ...
        td_target = reward + self.cfg.discount * torch.min(
            *self.model_target.Q(next_z, self.model.pi(next_z,
                                                       self.cfg.min_std)))
        return td_target
    ...
    def update(self, replay_buffer, step=int(1e6), demo_buffer=None):
        ...
        for t in range(self.cfg.horizon):
            ...
            consistency_loss += rho * torch.mean(
                h.mse(z, next_z), dim=1, keepdim=True)
        ...
        if self.cfg.per:
            ...
            replay_buffer.update_priorities(idxs[:self.cfg.batch_size],
                                            priorities[:self.cfg.batch_size])
            demo_buffer.update_priorities(demo_idxs,
                                          priorities[self.cfg.batch_size:])
