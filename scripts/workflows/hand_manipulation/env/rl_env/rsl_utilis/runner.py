...
import rsl_rl
from rsl_rl.algorithms import Distillation, PPO
# from scripts.workflows.hand_manipulation.env.rl_env.rsl_utilis.ppo import PPO
...
class OnPolicyRunner:
    ...
    def __init__(self,
        ...
        if self.training_type == "rl":
            if "critic" in self.env.unwrapped.observation_manager.group_obs_dim:
                self.privileged_obs_type = "critic"  # actor-critic reinforcement learnig, e.g., PPO
            else:
                self.privileged_obs_type = None
        if self.training_type == "distillation":
            if "teacher" in self.env.unwrapped.observation_manager.group_obs_dim:
        ...
        if self.privileged_obs_type is not None:
            num_privileged_obs = extras["observations"][
                self.privileged_obs_type].shape[1]
        else:
        ...
        policy: ActorCritic | ActorCriticRecurrent | StudentTeacher | StudentTeacherRecurrent = policy_class(
            num_obs, num_privileged_obs, self.env.action_space,
        ...
        if "symmetry_cfg" in self.alg_cfg and self.alg_cfg[
                "symmetry_cfg"] is not None:
        ...
        if self.empirical_normalization:
            ...
            self.privileged_obs_normalizer = EmpiricalNormalization(
                shape=[num_privileged_obs], until=1.0e8).to(self.device)
        else:
            self.obs_normalizer = torch.nn.Identity().to(
                self.device)  # no normalization
    ...
    def learn(self,
              ...
              init_at_random_ep_len: bool = False):  # noqa: C901
        # initialize writer
        if self.log_dir is not None and self.writer is None and not self.disable_logs:
            ...
            if self.logger_type == "neptune":
                ...
                self.writer = NeptuneSummaryWriter(log_dir=self.log_dir,
                                                   flush_secs=10,
                                                   cfg=self.cfg)
                self.writer.log_config(self.env.cfg, self.cfg, self.alg_cfg,
                                       self.policy_cfg)
        ...
        if self.alg.rnd:
            ...
            cur_ereward_sum = torch.zeros(self.env.num_envs,
                                          dtype=torch.float,
                                          device=self.device)
            cur_ireward_sum = torch.zeros(self.env.num_envs,
                                          dtype=torch.float,
                                          device=self.device)
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    self.rollout_rewards += rewards.sum().item()
                    if dones[0]:
                        self.num_rollout_steps += self.env.num_envs

                    # Move to device
                    obs, rewards, dones = (obs.to(self.device),
                    ...
                    if self.log_dir is not None:
                        ...
                        rewbuffer.extend(
                            cur_reward_sum[new_ids][:,
                                                    0].cpu().numpy().tolist())
                        lenbuffer.extend(cur_episode_length[new_ids]
                                         [:, 0].cpu().numpy().tolist())
                        cur_reward_sum[new_ids] = 0
                        cur_episode_length[new_ids] = 0
                        # -- intrinsic and extrinsic rewards
                        if self.alg.rnd:
                            erewbuffer.extend(cur_ereward_sum[new_ids]
                                              [:, 0].cpu().numpy().tolist())
                            irewbuffer.extend(cur_ireward_sum[new_ids]
                                              [:, 0].cpu().numpy().tolist())
            ...
            if it == start_iter and not self.disable_logs:
                ...
                git_file_paths = store_code_state(self.log_dir,
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        if len(locs["rewbuffer"]) > 0:
            ...
            if self.alg.rnd:
                ...
                self.writer.add_scalar("Rnd/mean_intrinsic_reward",
                                       ...
                                       locs["it"])
                self.writer.add_scalar("Rnd/weight", self.alg.rnd.weight,
                                       locs["it"])
            # everything else
        ...
        avg_reward = (
            ...
            self.num_rollout_steps) if self.num_rollout_steps > 0 else None
        log_string += (f"""{'Average Reward:':>{pad}} {avg_reward}\n""")
        print(log_string)
    ...
    def get_inference_policy(self, device=None):
        ...
        if self.cfg["empirical_normalization"]:
            ...
            policy = lambda x: self.alg.policy.act_inference(
                self.obs_normalizer(x))  # noqa: E731
        return policy
