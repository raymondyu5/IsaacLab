...
class PPO:
    ...
    def __init__(
        ...
        if symmetry_cfg is not None:
            ...
            use_symmetry = symmetry_cfg[
                "use_data_augmentation"] or symmetry_cfg["use_mirror_loss"]
    ...
    def process_env_step(self, rewards, dones, infos):
        ...
        if self.rnd:
            ...
            self.intrinsic_rewards, rnd_state = self.rnd.get_intrinsic_reward(
                rnd_state)
    ...
    def update(self):  # noqa: C901
        ...
        for (
            ...
            if self.desired_kl is not None and self.schedule == "adaptive":
                with torch.no_grad():
                    ...
                    approx_kl_div = torch.mean((torch.exp(log_ratio) - 1) -
                                               log_ratio)
                    ...
                    #         self.learning_rate = max(1e-5,
                    #                                  self.learning_rate / 1.5)
                    #     elif kl_mean < self.desired_kl / 2.0 and kl_mean > 0.0:
                    #         self.learning_rate = min(1e-2,
                    #                                  self.learning_rate * 1.5)
            ...
            if approx_kl_div > self.desired_kl:
                print(
                    f"Early stopping at iteration {it} due to large KL divergence: {approx_kl_div:.4f} > {self.desired_kl:.4f}"
                )
                break
        ...
        loss_dict["approx_kl_div"] = approx_kl_div.item(
        ) if self.desired_kl is not None else None
    ...
    def reduce_parameters(self):
        ...
        grads = [
            param.grad.view(-1) for param in self.policy.parameters()
            if param.grad is not None
        ]
        if self.rnd:
            grads += [
                param.grad.view(-1) for param in self.rnd.parameters()
                if param.grad is not None
            ]
        all_grads = torch.cat(grads)

        # Average the gradients across all GPUs
        torch.distributed.all_reduce(all_grads,
        ...
        for param in all_params:
            if param.grad is not None:
                ...
                param.grad.data.copy_(all_grads[offset:offset + numel].view_as(
                    param.grad.data))
