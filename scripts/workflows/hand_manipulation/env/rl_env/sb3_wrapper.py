...
class Sb3VecEnvWrapper(VecEnv):
    ...
    def __init__(self,
        ...
        setattr(env, "success_rate", None)

        if isinstance(env.observation_space,
                      gym.spaces.Dict) and not env.args_cli.use_visual_obs:
            keys_to_exclude = ["seg_pc", "rgb"]

            obs_dim = 0
            for key, value in env.observation_space.spaces["policy"].items():
            ...
            observation_space = gym.spaces.Box(
                ...
                dtype=np.float32)
                        ...
                        gym.spaces.Dict) and env.args_cli.use_visual_obs:
        ...
        if args_cli.action_framework is not None:
            framework_action_space = action_space.shape[
                -1]  #- self.env.num_hand_joints + self.env.num_finger_actions
            action_space = gym.spaces.Box(low=-1,
                                          high=1,
                                          shape=np.array(
        ...
        #     if args_cli.use_residual_only:

        #         action_space = gym.spaces.Box(low=-10,
        #                                       high=10,
        #                                       shape=(22, ),
        #                                       dtype=np.float32)
        #     elif not args_cli.use_residual_action:
        #         action_space = gym.spaces.Box(low=-10,
        #                                       high=10,
        #                                       shape=(23, ),
        #                                       dtype=np.float32)
    ...
    def reset(self) -> VecEnvObs:  # noqa: D102
        ...
        self.reset_obs_dict, _ = self.env.reset()
        ...
        return self._process_obs(self.reset_obs_dict,
    ...
    def step_wait(self) -> VecEnvStepReturn:  # noqa: D102
        ...
        if dones[0]:
            ...
            obs = self.reset()
        raw_action = rollouts[5] if len(rollouts) > 5 else None
        if len(rollouts) > 6:
            diffusion_obs = rollouts[6]
        else:
            diffusion_obs = None

        return obs, rew, dones, infos, raw_action, diffusion_obs
