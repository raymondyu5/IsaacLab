...
class DynamixelClient:
    ...
    def __init__(self,
        ...
        self._pos_vel_cur_reader = DynamixelPosVelCurReader(
            ...
            self.motor_ids,
            pos_scale=pos_scale
            if pos_scale is not None else DEFAULT_POS_SCALE,
            vel_scale=vel_scale
            if vel_scale is not None else DEFAULT_VEL_SCALE,
            cur_scale=cur_scale
            if cur_scale is not None else DEFAULT_CUR_SCALE,
        )
    ...
    def write_byte(
        ...
        for motor_id in motor_ids:
            ...
            success = self.handle_packet_result(comm_result,
                                                dxl_error,
                                                motor_id,
                                                context='write_byte')
            if not success:
    ...
    def sync_write(self, motor_ids: Sequence[int],
                   values: Sequence[Union[int,
                                          float]], address: int, size: int):
...
class DynamixelReader:
    ...
    def __init__(self, client: DynamixelClient, motor_ids: Sequence[int],
        ...
        for motor_id in motor_ids:
            success = self.operation.addParam(
                motor_id
            )  #, address, size  for bulkread, but syncread is faster
            if not success:
                raise OSError(
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b',
                        '--baud',
                        default=1000000,
                        help='The baudrate to connect with.')
