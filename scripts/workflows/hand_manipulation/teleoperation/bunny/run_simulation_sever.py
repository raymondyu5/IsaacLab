...
sys.path.append("scripts/workflows/hand_manipulation/teleoperation/bunny/")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        "-k",
        default=
        "source/config/task/hand_env/teleoperation/bunny/kinematics_config/bimanual_franka_leap.yml",
    ...
    parser.add_argument(
        ...
        "-c",
        default=
        "source/config/task/hand_env/teleoperation/bunny/camera_config/visionpro_config.yml",
        help="File path to the camera config.")
    parser.add_argument(
        "--communication-cfg",
        "-comm",
        default=
        "source/config/task/hand_env/teleoperation/bunny/communication_config/sim_web_visualizer_leap.yml",
...
def main():
    ...
    MotionControlConfig.set_default_robot_config_dir(
        (root_dir /
         "source/config/task/hand_env/teleoperation/bunny/robot_config"
         ).resolve())
    ...
    for i, cfg_dict in enumerate([left_config, right_config]):
        ...
        motion_control = control_config.build(
        ) if control_config is not None else None
    ...
    if comm_config is not None:
        ...
        if comm_cfg.viz_type == "sim_web_visualizer":
            ...
            viz = MeshCatVisualizerBase(port=comm_cfg.viz_port,
                                        host=comm_cfg.viz_host)
    ...
    teleop_node = BimanualRobotTeleopNode(
        ...
        robot_viz=robot_viz,
        disable_orientation_control=tuple(cfg.disable_orientation_control
                                          for cfg in bimanual_control_configs),
        motion_scaling_factor=tuple(cfg.motion_scaling_factor
                                    for cfg in bimanual_control_configs),
        low_pass_smoothing_wrist=tuple(cfg.low_pass_alpha
                                       for cfg in bimanual_control_configs),
        verbose=True,
