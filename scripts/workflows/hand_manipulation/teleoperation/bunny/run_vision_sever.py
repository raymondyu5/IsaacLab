...
def launch_detector(cfg: CameraConfig):
    launch_list = []
    executable = Path(
        "scripts/workflows/hand_manipulation/teleoperation/bunny/nodes/bimanual_hand_detector_node.py"
    )
    executable = executable.absolute()
    for config in cfg.configs:
        arguments = dict(visionpro_name=config.name, )
        ...
        argument_list = [
            f"--{k.replace('_', '-')}={v}" for k, v in arguments.items()
