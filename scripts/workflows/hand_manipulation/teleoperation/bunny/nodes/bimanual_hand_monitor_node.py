...
class BimanualMonitorNode(Node):
    ...
    def _is_hand_pose_good_for_init(
        ...
        hand_not_far_away = (np.linalg.norm(hand_wrist_pose[:3] -
                                            last_hand_wrist_pose[:3])
                             < not_far_away_threshold)
        print(np.linalg.norm(hand_wrist_pose[:3] - last_hand_wrist_pose[:3]))
