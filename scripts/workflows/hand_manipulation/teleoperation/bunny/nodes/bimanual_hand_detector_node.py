...
class VisionProHandDetector(Node):
    ...
    def __init__(
        ...
        # Create publisher
        self.detection_pub = self.create_publisher(BimanualHandDetection,
    ...
    def start(self):
        ...
        self.get_logger().info(
            f"Initial data received. VisionPro node is working now.")
    ...
    def fill_detection_msg(self, message: BimanualHandDetection,
        ...
        right_joint_pose = joint_avp2hand(
            transformation["right_fingers"]).astype(np.float32)
        left_joint_pose = joint_avp2hand(
            transformation["left_fingers"]).astype(np.float32)
        ...
        if self.visualize_3d_detection:
            ...
            left_keypoints = (left_joint_pose[:, :3, 3] @ left_wrist_rot.T +
                              left_wrist_trans)

            right_wrist_rot = transformation["right_wrist"][:3, :3]
            right_wrist_trans = transformation["right_wrist"][:3, 3][None, ...]
            right_keypoints = (right_joint_pose[:, :3, 3] @ right_wrist_rot.T +
                               right_wrist_trans)
            self.publish_hand_skeleton(left_keypoints, right_keypoints)

        return message

    def publish_hand_skeleton(self, left_keypoint_3d: np.ndarray,
                              right_keypoint_3d: np.ndarray):
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "Name of the Apple Vision Pro, current not important but reserved for future multiple AVP settings.",
    )
    parser.add_argument(
        "--avp-ip",
        required=True,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        "Do not use this one, just a dirty hack to be compatible with ROS2 node action.",
    )
    ...
    try:
        ...
        hand_detector_node.get_logger().info(
