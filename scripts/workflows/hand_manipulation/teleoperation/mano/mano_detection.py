...
sys.path.append("scripts/workflows/hand_manipulation/utils/")
...
def depth2point_cloud(depth: np.ndarray, intrinsic: np.ndarray):
    ...
    points_camera = uv1 @ np.linalg.inv(intrinsic).T * depth[...,
                                                             None]  # [H, W, 3]
...
class Record3DSingleHandMotionControl:
    ...
    def __init__(self,
        ...
        self.calibrated_shape_norm_dist = scipy.stats.norm(
        ...
        default_checkpoint_hand = "scripts/workflows/hand_manipulation/utils/hand_detector/extra_data/hand_module/pretrained_weights/pose_shape_best.pth"
        default_checkpoint_body_smpl = 'scripts/workflows/hand_manipulation/utils/hand_detector/extra_data/smpl'

        self.hand_mocap = HandMocap(str(default_checkpoint_hand),
                                    str(default_checkpoint_body_smpl))
        ...
        self.previous_offset = {
            "left_hand": np.zeros(3, dtype=np.float32),
            "right_hand": np.zeros(3, dtype=np.float32)
        }
    ...
    def compute_3d_offset(self, mocap_data: Dict, depth: np.ndarray):
        ...
        depth_valid_mask = np.nonzero(
            np.abs(depth_vertices - depth_median) < 0.2)[0]
        ...
        offset_img = mocap_data["pred_joints_img"][0,
                                                   0:2] - self.camera_mat[0:2,
                                                                          2]
        offset = np.concatenate(
            [offset_img / self.focal_length * depth_offset, [depth_offset]])
    ...
    def normal_step(self) -> Tuple[bool, Dict]:
        ...
        # Joint regression
        pred_output = self.hand_mocap.regress(bgr,
                                              hand_bbox_list,
                                              add_margin=False)[0]
        ...
        output = dict(rgb=rgb,
                      depth=depth,
                      origin=self.latest_root_offset,
                      joint=self.compute_operator_space_joint_pos(
                          mocap_data["pred_joints_smpl"]),
                      pose_params=pose_params,
                      bbox=hand_bbox_list[0])
        if self.show_hand:
            vertices, normals = self.compute_operator_space_vertices(
                mocap_data, self.latest_root_offset)
            output.update({
                "vertices": mocap_data["pred_vertices_smpl"],
                "faces": mocap_data["faces"],
                "normals": normals
            })

        # Confidence
        confidence = self.compute_shape_confidence(
            mocap_data["pred_shape_params"][0])
        output.update({"confidence": confidence})

        shape_error = np.linalg.norm(mocap_data["pred_shape_params"][0] -
                                     self.calibrated_shape_params)
        return True, output
    ...
    def init_step(self):
        ...
        mocap_data = pred_output[self.hand_mode]
        offset = self.compute_3d_offset(mocap_data, depth)
        has_init_offset = np.sum(np.abs(
            self.previous_offset[self.hand_mode])) > 1e-2

        # Stop initialization process and clear data
        if np.linalg.norm(offset - self.previous_offset[self.hand_mode]
                          ) > 0.05 and has_init_offset:
            self.init_process = 0
            ...
            self.previous_offset[self.hand_mode] = offset
            hand_pose = mocap_data["pred_hand_pose"][
                3:] + self.hand_mocap.mean_pose
            hand_pose = np.reshape(hand_pose, [15, 3])
            if np.linalg.norm(hand_pose, axis=1).mean() < 0.50:
                self.init_process += 0.02
                self.init_root_pose_list.append(
                    np.concatenate([offset, mocap_data["pred_hand_pose"][:3]]))
                self.init_shape_param_list.append(
                    mocap_data["pred_shape_params"][0])
            else:
                self.init_process = 0
                self.init_root_pose_list.clear()
        ...
        if self.init_process >= 1:
            ...
            weight = (np.arange(num_data) +
                      1) / np.sum(np.arange(num_data) + 1)
            root_axis_angle = init_collect_data[-1, 3:]
            angle = np.linalg.norm(root_axis_angle)
            axis = root_axis_angle / (angle + 1e-6)
            self.calibrated_offset = np.sum(weight[:, None] *
                                            init_collect_data[:, :3],
                                            axis=0).astype(np.float32)
            self.calibrated_rotation = rot_mano2operator(
                transforms3d.axangles.axangle2mat(axis, angle)).T
            self.calibrated_shape_params = np.sum(weight[:, None] *
                                                  self.init_shape_param_list,
                                                  axis=0)
            self.calibrated_shape_norm_dist = scipy.stats.norm(
                self.calibrated_shape_params, self.shape_dist_var)
            print(
                f"Estimated shape params during init of the operator: {self.calibrated_shape_params}"
            )
            print(
                f"The variance of shape params: {np.std(self.init_shape_param_list, axis=0)}"
            )
        ...
        output = dict(rgb=rgb,
                      depth=depth,
                      origin=offset,
                      joint=mocap_data["pred_joints_smpl"],
                      pose_params=pose_params,
                      bbox=hand_bbox_list[0])
        if self.show_hand:
            vertices, normals = self.compute_operator_space_vertices(
                mocap_data, np.zeros(3))
            output.update({
                "vertices": mocap_data["pred_vertices_smpl"],
                "faces": mocap_data["faces"],
                "normals": normals
    ...
    def compute_operator_space_root_pose(self, motion_data: Dict):
        ...
        angle = np.linalg.norm(root_axis_angle)
        axis = root_axis_angle / (angle + 1e-6)
        root_rotation = self.calibrated_rotation @ rot_mano2operator(
            transforms3d.axangles.axangle2mat(axis, angle))
    ...
    def compute_operator_space_joint_pos(self, joint_pos: np.ndarray):
        human_hand_joints = frame_cam2operator(joint_pos +
    ...
    def compute_hand_zero_pos(self):
        if not self.initialized:
            raise RuntimeError(
                f"Can not perform hand shape based computation before initialization"
            )
        shape_params = torch.from_numpy(
            self.calibrated_shape_params.astype(np.float32))[None, :].cuda()
        smplx: SMPLX = self.hand_mocap.model.smplx
        hand_index = [21] + list(range(40, 55)) + list(range(
        ...
        with torch.no_grad():
            output = smplx(body_pose=body_pose,
                           right_hand_pose=-smplx.right_hand_mean,
                           betas=shape_params,
                           return_verts=True)
    ...
    def compute_qpos(self, pose_param: np.ndarray):
        if pose_param.size != 45:
            raise ValueError(f"pose_param should be in shape of 45")
        smplx_to_panoptic = np.array(
            [12, 13, 14, 0, 1, 2, 3, 4, 5, 9, 10, 11, 6, 7, 8])
...
def plot_rgb_real_time(rgb_image):
    plt.ion()
    if not hasattr(plot_rgb_real_time, "fig"):
        plot_rgb_real_time.fig, plot_rgb_real_time.ax = plt.subplots()
        plot_rgb_real_time.im = plot_rgb_real_time.ax.imshow(rgb_image)
        plot_rgb_real_time.ax.axis('off')
    else:
        plot_rgb_real_time.im.set_data(rgb_image)
    plt.pause(0.001)
...
class ManoSever:
    ...
    def send_landmarks(self, landmarks):
        ...
        message = json.dumps(to_serializable(landmarks))
...
def run_hand_tracking(server: ManoSever):
    while True:
        success, motion_data = motion_control.step()
        import pdb
        # pdb.set_trace()
        # mesh = trimesh.Trimesh(vertices=motion_data["vertices"],
        #                        faces=motion_data["faces"])

        if "joint" not in motion_data.keys():
            server.send_landmarks(None)

        else:
            output_data = {"joint": motion_data["joint"].tolist()}
            server.send_landmarks(output_data)

            # demo.display_hand(
            #     {
            #         'verts': torch.as_tensor(
            #             motion_data["vertices"]).unsqueeze(0),
            #         'joints': torch.as_tensor(
            #             motion_data["joint"]).unsqueeze(0),
            #     },
            #     mano_faces=torch.as_tensor(
            #         motion_data["faces"]))  # <-- becomes [1, F, 3]
...
#     success, motion_data = motion_control.step()
#     rgb = motion_data["rgb"]
#     if "pose_params" not in list(motion_data.keys()):
#         continue

#     root_joint_qpos = motion_control.compute_operator_space_root_qpos(
#         motion_data)
