...
class TrainDiffusionUnetLowdimWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, args_cli=None):
        if args_cli is not None:
            ...
            output_dir = args_cli.log_dir + f"/{args_cli.diffusion_name}/horizon_{cfg.horizon}_nobs_{cfg.n_obs_steps}_naction_{cfg.n_action_steps}_ninference_{cfg.policy.num_inference_steps}"
        else:
            output_dir = f"logs/trash/horizon_{cfg.horizon}_nobs_{cfg.n_obs_steps}_naction_{cfg.n_action_steps}_ninference_{cfg.policy.num_inference_steps}"
    ...
    def run(self):
        ...
        dataset = HandLowdimDataset(
            h5py_path=self.args_cli.data_path,
            horizon=cfg.horizon,
...
def main(args_cli):
    ...
    if args_cli.diffusion_name == "DDPM":
        cfg.policy.noise_scheduler = cfg.policy.DDPM_scheduler
        cfg.logging.name = "DDPM"
    elif args_cli.diffusion_name == "DDIM":
        cfg.policy.noise_scheduler = cfg.policy.DDIM_scheduler
        cfg.logging.name = "DDIM"
    cfg.policy.pop("DDPM_scheduler")
    cfg.policy.pop("DDIM_scheduler")
    cfg.policy.noise_scheduler.num_train_timesteps = args_cli.num_inference
    cfg.policy.num_inference_steps = args_cli.num_inference

    # Initialize and run workspace
    workspace = TrainDiffusionUnetLowdimWorkspace(cfg, args_cli)
    workspace.run()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_inference", type=int, default=100)
    parser.add_argument("--diffusion_name", type=str, default="DDIM")
