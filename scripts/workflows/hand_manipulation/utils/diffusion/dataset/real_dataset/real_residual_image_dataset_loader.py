...
class ResidualRealImageDatasetLoader:
    ...
    def load_data(self):
        ...
        for demo_id, path_key in enumerate(self.load_list):
            ...
            for file_id in tqdm(range(min([self.num_demo,
                ...
                image_list = sorted([
                    self.data_path + "/" + path_key + f"/episode_{file_id}/" +
                    f for f in os.listdir(self.data_path + "/" + path_key +
                                          f"/episode_{file_id}/")
                    if f.endswith(".png")
                ])
                os.makedirs(self.data_path + "/" + path_key +
                            f"/episode_{file_id}/processed_images",
                            exist_ok=True)
                ...
                for image_path in image_list:
                    ...
                    img_tensor = torch.as_tensor(np.array(img)).permute(
                        2, 0, 1)  # (H,W,C) -> (C,H,W)
                    out = (self.image_tf(img_tensor).permute(
                        1, 2, 0).numpy()[..., ::-1] * 255).astype(np.uint8)

                    cv2.imwrite(
                        self.data_path + "/" + path_key +
                        f"/episode_{file_id}/processed_images/{os.path.basename(image_path)}",
                        out)

                obs_buffer["image"] += sorted([
                    self.data_path + "/" + path_key +
                    f"/episode_{file_id}/processed_images/" + f for f in
                    os.listdir(self.data_path + "/" + path_key +
                               f"/episode_{file_id}/processed_images/")
                    if f.endswith(".png")
                ])
