...
class RealPCDDatasetLoader:
    ...
    def load_data(self):
        ...
        for demo_id, path_key in enumerate(self.load_list):
            # Stop if we've loaded enough demos
            if demo_count >= self.num_demo:
                break

            if not os.path.isdir(self.data_path + "/" + path_key):
                continue

            # Check if path_key is an episode directory itself (flat structure)
            ...
            if is_flat_structure:
                ...
                for _, cam_id in enumerate(self.camera_id):
                    print("loading camera id ", cam_id)
                    pcd_list = sorted([
                ...
                # Nested structure: data_path/path_key/episode_X/episode_X.npy
