...
class ImageSequenceSampler:
    ...
    def load_image(self, key, value):
        ...
        if self.add_randomizer and self.dino_kwargs is None:

            value = (self.gaussian_randomizer.forward(value / 255) *
                     255).astype(np.uint8)

            value = np.asarray(self.color_randomizer.get_transform()(
                Image.fromarray(value[0].transpose(1, 2, 0).astype(
                    np.uint8)))).transpose(2, 0, 1)[None]

            return value / 255
        elif self.dino_kwargs is not None:
            return value
