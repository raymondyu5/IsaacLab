...
class MultiPCDObsEncoder(ModuleAttrMixin):
    ...
    def forward(self, obs_dict):
        ...
        for key in self.pcd_keys:
            pcd = obs_dict[key][:, :3]  # only use xyz #TODO: add rgb
            pcd = pcd.to(device, non_blocking=True)

            if batch_size is None:
                batch_size = pcd.shape[0]
            else:
                assert batch_size == pcd.shape[0]
            # assert pcd.shape[1:] == self.key_shape_map[key]
        ...
        for key in self.low_dim_keys:
            data = obs_dict[key].to(device, non_blocking=True)
            if batch_size is None:
