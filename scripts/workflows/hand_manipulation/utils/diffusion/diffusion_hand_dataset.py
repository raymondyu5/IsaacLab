...
class ReplayBuffer:
    ...
    def __init__(self, path: Union[zarr.Group, Dict[str, dict]]):
        """
        Dummy constructor. Use copy_from* and create_from* class methods instead.
        """
        self.root = self.h5py2zarr(path)

    def h5py2zarr(self, path):
        ...
        with h5py.File(path, "r") as f:
            ...
            self.root['meta']['episode_ends'] = np.array(episode_ends,
                                                         dtype=np.int64)
            action_buffer = np.concatenate(action_buffer, axis=0).clip(-1, 1)
            ...
            self.root['data']['state'] = state_buffer.clip(-1, 1)
            self.root['data']['hand_joints'] = obs_buffer.clip(-1, 1)
