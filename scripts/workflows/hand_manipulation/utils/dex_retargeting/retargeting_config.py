...
class RetargetingConfig:
    ...
    def __post_init__(self):
        ...
        if self.type == "vector":
            ...
            if len(self.target_task_link_names) != len(
            ...
            if self.target_link_human_indices.shape != (
                    2, len(self.target_origin_link_names)):
                raise ValueError(
            ...
            if self.target_link_names is None:
                raise ValueError(
                    f"Position retargeting requires: target_link_names")
            self.target_link_human_indices = self.target_link_human_indices.squeeze(
            )
            if self.target_link_human_indices.shape != (len(
...
def parse_mimic_joint(
    ...
    return len(
        mimic_joint_names
    ) > 0, source_joint_names, mimic_joint_names, multipliers, offsets
