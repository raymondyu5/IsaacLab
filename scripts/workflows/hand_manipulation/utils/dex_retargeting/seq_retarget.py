...
class SeqRetargeting:
    ...
    def __init__(
        ...
        if has_joint_limits:
            ...
            self.optimizer.set_joint_limit(
                joint_limits[self.optimizer.idx_pin2target])
        self.joint_limits = joint_limits[self.optimizer.idx_pin2target]

        # Temporal information
        self.last_qpos = joint_limits.mean(1)[
            self.optimizer.idx_pin2target].astype(np.float32)
    ...
    def warm_start(
        ...
        if len(wrist_quat) != 4:
            raise ValueError(
                f"Wrist quat: {wrist_quat} is not a 4-dim vector.")

        operator2mano = OPERATOR2MANO[
            hand_type] if is_mano_convention else np.eye(3)
        robot = self.optimizer.robot
        target_wrist_pose = np.eye(4)
        target_wrist_pose[:3, :3] = rotations.matrix_from_quaternion(
            wrist_quat) @ operator2mano.T
        ...
        euler = rotations.euler_from_matrix(target_root_pose[:3, :3],
                                            0,
                                            1,
                                            2,
                                            extrinsic=False)
    ...
    def retarget(self, ref_value, fixed_qpos=np.array([])):
        ...
        qpos = self.optimizer.retarget(
            ...
            last_qpos=np.clip(self.last_qpos, self.joint_limits[:, 0],
                              self.joint_limits[:, 1]),
        )
