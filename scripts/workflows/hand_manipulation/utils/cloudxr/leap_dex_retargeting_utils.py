...
from scripts.workflows.hand_manipulation.utils.dex_retargeting.retarget_utils import get_hand_joint_names, init_leap_hand_retarget, display_hand, _HAND_JOINTS_INDEX, _OPERATOR2MANO_RIGHT, _OPERATOR2MANO_LEFT

from scipy.spatial.transform import Rotation as R


class LeapDexRetargeting:
    ...
    def init_retargeting(self):
        ...
        if self.add_right_hand:
            ...
            self.retarget2sim = np.array([0, 1, 2, 3, 4, 5] + [
                self.right_retargeting.optimizer.target_joint_names.index(j)
                for j in self.robot_joint_name
            ])

        self.retarget2sim = self.retarget2sim[6:]
    ...
    def compute_ref_value(
        ...
        with torch.enable_grad():
            with torch.inference_mode(False):
                retargeted_qpos = retargeting.retarget(
                    ref_value[..., :3])[self.retarget2sim]
