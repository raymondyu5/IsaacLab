...
class LeapRetargeter:
    ...
    def __init__(
        ...
        self.left_hand_pos_offset = np.array([0, 0, 0], dtype=np.float32)
        self.right_hand_pos_offset = np.array([0, 0, 0], dtype=np.float32)
    ...
    def retarget(
        ...
        if self._enable_visualization:
            ...
            joints_position[::2] = np.array([
                pose[:3] + self.left_hand_pos_offset
                for pose in left_hand_poses.values()
            ])
            joints_position[1::2] = np.array([
                pose[:3] + self.right_hand_pos_offset
                for pose in right_hand_poses.values()
            ])

            self._markers.visualize(translations=torch.tensor(
                joints_position, device=self._device))
        ...
        right_wrist = self._retarget_abs(right_wrist)
    ...
    def _retarget_abs(self, wrist: np.ndarray) -> np.ndarray:
        ...
        return np.concatenate([
            usd_right_roll_link_in_world_pos, usd_right_roll_link_in_world_quat
        ])
