...
class ActionManagerWrapper:
    ...
    def init_reset_teleoperation(self):
        ...
        self.arm_motion_env = IKPlanner(self.env, )

        init_ee_pose = torch.as_tensor(
            self.env_cfg["params"]["init_ee_pose"]).to(
                self.device).unsqueeze(0)
        self.num_hand_joint = 1
        self.num_arm_joint = self.env_cfg["params"]["num_arm_joints"]
        ...
        init_pose = torch.cat([
            ...
            torch.zeros(1, self.num_hand_joint).to(self.device)
                              ...
                              dim=1).repeat_interleave(self.env.num_envs,
                                                       dim=0)
        init_arm_qpos = self.arm_motion_env.plan_motion(
            init_ee_pose[:, :3],
            init_ee_pose[:, 3:7])[..., :self.num_arm_joint].reshape(
                -1, self.num_arm_joint).repeat_interleave(self.env.num_envs,
                                                          dim=0)
        ...
        self.env.scene["robot"]._data.reset_joint_pos[
            ..., :self.num_arm_joint] = init_arm_qpos

        self.rightpalm2wrist = torch.as_tensor([[0, 0, 0, 1, 0, 0, 0]]).to(
            self.device).repeat_interleave(self.env.num_envs,
                                           dim=0).to(torch.float32)
        self.leftpalm2wrist = torch.as_tensor([[0, 0, 0, 1, 0, 0, 0]]).to(
            self.device).repeat_interleave(self.env.num_envs,
                                           dim=0).to(torch.float32)
    ...
    def reset(self):
        ...
        if self.add_left_hand:
            self.prev_left_wrist_pose = self.env.scene[
                "panda_link7"]._data.root_state_w[:, :7]
        if self.add_right_hand:
