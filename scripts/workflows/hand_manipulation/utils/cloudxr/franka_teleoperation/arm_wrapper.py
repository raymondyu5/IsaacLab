...
from scripts.workflows.hand_manipulation.utils.cloudxr.franka_teleoperation.franak_arm_motion import ActionManagerWrapper
        ...
        if reset_stopped_teleoperation:  # reset come first
            ...
            if self.add_left_hand:
                left_hand_pose = self.step_action(
                    teleop_data[0][0],
                    torch.as_tensor(teleop_data[0][-1][-1].reshape(-1)).to(
