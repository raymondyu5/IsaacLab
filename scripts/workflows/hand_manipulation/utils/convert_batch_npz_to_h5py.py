...
class Converter:
    ...
    def __init__(
        ...
        self.load_path = args_cli.load_dir
    ...
    def filter_obs_buffer(self, obs_buffer, index, obs_name="obs"):
        ...
        for key, value in obs.items():
            ...
            if not isinstance(value, torch.Tensor) and not isinstance(
                ...
                if key == "dexretargeting_human_data":
                    value = value.unsqueeze(0)
    ...
    def convert(self):
        ...
        all_object_files = os.listdir(f"{self.load_path}")
        all_object_files.sort()

        for object_name in all_object_files:
            object_file = f"{self.load_path}/{object_name}"
            if not os.path.isdir(object_file):
                continue

            files = os.listdir(f"{self.load_path}/{object_name}")

            for file in files:
                if file.endswith(".npz"):

                    data = torch.load(os.path.join(
                        f"{self.load_path}/{object_name}", file),
