...
class DexYCBDatasetLoader:

    def __init__(self, data_dir, hand_side="right", filter_objects=[]):
        ...
        for subject_dir in self._subject_dirs:
            for capture_dir in subject_dir.iterdir():
                ...
                if hand_side not in meta["mano_sides"]:
                    continue
                ...
                if self.use_filter:
                    ...
                    if len(list(set(ycb_ids) & set(filter_ids))) < 1:
                        continue
                    capture_filter = [
                        i for i in range(len(ycb_ids))
                        if ycb_ids[i] in filter_ids
                    ]
    ...
    def __getitem__(self, item):
        ...
        extrinsic_mat = np.array(self._extrinsics[extrinsic_name]["extrinsics"]
                                 ["apriltag"]).reshape([3, 4])
        ...
        if self.use_filter:
            ...
            ycb_ids = [
                ycb_ids[valid_id]
                for valid_id in self._capture_filter[capture_name]
            ]
    ...
    def _filter_object_motion_frame(self,
                                    capture_filter,
                                    object_pose,
                                    frame_margin=40):
        frames = np.arange(0)
        for filter_id in capture_filter:
            ...
            last_frame = len(object_move_list) - object_move_list[::-1].index(
                True) - 1
    ...
    def is_object_move(single_object_pose: np.ndarray):
        ...
        current_move = np.linalg.norm(single_object_trans[1] -
                                      single_object_trans[0]) > 2e-2
        future_move = np.linalg.norm(single_object_trans[future_frame] -
                                     single_object_trans[0]) > 5e-2
        return current_move or future_move

    def _object_mesh_file(self, object_id):
        obj_file = self._data_dir / "models" / YCB_CLASSES[
            object_id] / "textured_simple.obj"
...
def main(dexycb_dir: str):
    ...
    dataset = DexYCBDatasetLoader(dexycb_dir)
    print(len(dataset))
