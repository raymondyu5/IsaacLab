...
class Mesh(trimesh.Trimesh):
    ...
    def __init__(self,
        ...
        super(Mesh, self).__init__(vertices=vertices,
                                   faces=faces,
                                   process=process,
                                   visual=visual)
    ...
    def colors_like(self, color, array, ids):
        ...
        if n_color <= 4:
            new_color[ids, :n_color] = np.repeat(color[np.newaxis],
                                                 n_ids,
                                                 axis=0)
        else:
            new_color[ids, :] = color
...
class MeshViewer(object):
    ...
    def __init__(self,
        ...
        if self.offscreen:
            light = Node(light=DirectionalLight(color=np.ones(3),
                                                intensity=3.0),
                         matrix=camera_pose)
        ...
        camera_pose[:3, :3] = euler([90, 0, 90], 'xzx')
        camera_pose[:3, 3] = np.array([0.0, 1.5, 1.0])
        self.cam = pyrender.Node(
            name='camera',
            camera=pc,
        )
        self.scene.add_node(self.cam)

        # self.update_camera_pose(camera_pose)
