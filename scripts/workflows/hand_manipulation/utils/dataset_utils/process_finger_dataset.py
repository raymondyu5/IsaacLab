...
class ProcessFingerDataset:
    ...
    def extension_actions(self, actions):
        ...
        frozen_init_extension = init_actions[np.newaxis, :].repeat(
            n_frozen_init, axis=0) * 0.0

        # Interpolate init actions
        init_factors = np.arange(0, num_init_frames).reshape(
            -1, 1) / num_init_frames
        ...
        exteneded_final_actions = np.zeros(
            (num_final_frames, actions.shape[1]), dtype=np.float32)

        exteneded_final_actions[:, -16:] = final_factors / num_final_frames * (
            self.joint_limits[:, 1] * 0.6 -
            last_actions[-16:]) + last_actions[-16:]
    ...
    def merge_datasets(self, ):
        for filepath in self.args_cli.input_files:
            if not os.path.exists(filepath):
                raise FileNotFoundError(
                    f"The dataset file {filepath} does not exist.")

        all_npz_files = []
        for file in self.args_cli.input_files:
            all_npz_files.extend(glob.glob(file + "/**/*.npz", recursive=True))
        os.makedirs(self.args_cli.output_file, exist_ok=True)

        with h5py.File(self.args_cli.output_file + f"/raw_right_data.hdf5",
                       "w") as output:
            ...
            for filepath in all_npz_files:
                ...
                extended_actions = self.extension_actions(actions)

                extended_actions[:, -16:] = (
                    (extended_actions[:, -16:] - self.joint_limits[:, 0]) /
                    (self.joint_limits[:, 1] -
                     self.joint_limits[:, 0])) * 2 - 1
                reverted_actions = extended_actions[::-1, :]
                all_actions = np.concatenate(
                    [extended_actions, reverted_actions], axis=0)
        ...
        print(f"Merged dataset saved to {self.args_cli.output_file}")

    def normalize_datasets(self):

        all_npz_files = glob.glob(self.args_cli.input_files[0] + "/*/*.npz",
                                  recursive=True)
        os.makedirs(self.args_cli.output_file, exist_ok=True)

        with h5py.File(
                self.args_cli.output_file + f"/{self.args_cli.save_path}.hdf5",
                "w") as output:
            output_data_group = output.create_group("data")
            episode_idx = 0
            ...
            for filepath in all_npz_files:
                ...
                demo_group = output_data_group.create_group(
                    f"demo_{episode_idx}")
                demo_group.create_dataset("actions", data=all_actions[::1])
                num_demo += len(all_actions[::1])
...
if __name__ == "__main__":
    proccessor = ProcessFingerDataset(args_cli)
    # proccessor.merge_datasets()
    proccessor.normalize_datasets()
