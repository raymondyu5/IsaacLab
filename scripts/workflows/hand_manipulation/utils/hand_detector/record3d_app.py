...
class CameraApp:
    ...
    def connect_to_device(self, dev_idx=0):
        if not os.path.exists(self.rgb_video_file):
            ...
            self.session.connect(
                dev)  # Initiate connection and start capturing
    ...
    def camera_intrinsics(self):
        if self.session is None:
            intrinsic_mat = np.array([[804.5928, 0.0,
                                       357.6741333][0.0, 804.5928,
                                                    474.83026123][0., 0., 1.]])
        else:
            intrinsic_mat = self._get_intrinsic_mat_from_coeffs(
    ...
    def record(self, rgb_filename):
        if len(self.rgb_video_file) > 0:
            raise RuntimeError(
                f"When recoding video, the input file must be empty string")
        saved_rgb = []
        saved_depth = []
