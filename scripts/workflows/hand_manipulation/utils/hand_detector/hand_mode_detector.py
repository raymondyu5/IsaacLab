...
sys.path.append("/home/ensu/Documents/weird/dex-hand-teleop")
...
class SingleHandDetector:
    ...
    def __init__(self,
        ...
        self.detected_hand_type = hand_type if selfie else inverse_hand_dict[
            hand_type]
...
class HandMocap:
    ...
    def __init__(self, checkpoint_path, smpl_dir):
        ...
        self.mean_pose = np.array([
            ...
            0.1885, -0.1181, -0.0509, 0.5296, -0.1437, -0.0552, 0.7049,
            -0.0192, 0.0923, 0.3379, -0.4570, 0.1963, 0.6255, -0.2147, 0.0660,
            0.5069, -0.3697, 0.0603, 0.0795, -0.1419, 0.0859, 0.6355, -0.3033,
            0.0579, 0.6314, -0.1761, 0.1321, 0.3734, 0.8510, -0.2769, 0.0915,
            -0.4998, -0.0266, -0.0529, 0.5356, -0.0460, 0.2774
        ])
        ...
        if not self.model.success_load:
            raise RuntimeError(
                f"Check points {self.opt.checkpoint_path} does not exist")
        self.model.eval()
    ...
    def __process_hand_bbox(self,
        ...
        img_cropped, bbox_scale_ratio, bbox_processed = self.__pad_and_resize(
            raw_image, hand_bbox, add_margin)

        # horizontal Flip to make it as right hand
        if hand_type == 'left_hand':
            img_cropped = np.ascontiguousarray(img_cropped[:, ::-1, :],
                                               img_cropped.dtype)
                ...
                if bbox is None:
                    ...
                    with torch.no_grad():
                        ...
                        pred_output[hand_type][
                            'pred_vertices_smpl'] = pred_vertex_origin - pred_joints[
                                0:1, :]
                        pred_output[hand_type][
                            'pred_joints_smpl'] = pred_joints - pred_joints[
                                0:1, :]
                        pred_output[hand_type]['pred_shape_params'] = pred_res[
                            "pred_shape_params"]
...
def main():
    ...
    hand_mocap = HandMocap(
        str(hand_detector_dir / default_checkpoint_hand),
        str(hand_detector_dir / default_checkpoint_body_smpl))

    vid = cv2.VideoCapture(
        "/home/ensu/Documents/weird/dex-hand-teleop/frankmocap/sample_data/han_hand_long.mp4"
    )
    while True:
        ...
        pred_output = hand_mocap.regress(image_bgr,
                                         hand_bbox_list,
                                         add_margin=False)[0]
