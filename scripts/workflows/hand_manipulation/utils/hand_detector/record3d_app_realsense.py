...
class RealsenseApp:
    ...
    def connect_to_device(self):
        if not os.path.exists(self.rgb_video_file):
            cfg = pyrs.config()
            cfg.enable_stream(pyrs.stream.color, 640, 480, pyrs.format.rgb8,
                              30)
            ...
            cam_int = dev.get_stream(
                pyrs.stream.color).as_video_stream_profile().get_intrinsics()
            self.depth_scale = dev.get_device().first_depth_sensor(
            ).get_depth_scale()
            self.cam_k = np.array([[cam_int.fx, 0, cam_int.ppx],
                                   [0, cam_int.fy, cam_int.ppy], [0, 0, 1]])
            self.frame = self.grab_realsense()
            Thread(target=self.update, args=()).start()
        else:
            print(
                f"Streaming from video file {os.path.abspath(self.rgb_video_file)}"
            )
            self.rgb_stream = cv2.VideoCapture(self.rgb_video_file)
            self.depth_stream = np.load(
                self.rgb_video_file.replace("rgb",
                                            "depth").replace("mp4",
                                                             "npz"))['depth']
    ...
    def record(self, rgb_filename):
        ...
        np.savez(rgb_filename.replace("rgb", "depth").replace("mp4", "npz"),
                 depth=saved_depth)
