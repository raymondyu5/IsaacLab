...
from scripts.workflows.hand_manipulation.utils.vae.reactive_diffusion_policy.utils import get_output_shape
from scripts.workflows.hand_manipulation.utils.vae.reactive_diffusion_policy.vector_quantize_pytorch.residual_vq import ResidualVQ
from scripts.workflows.hand_manipulation.utils.vae.reactive_diffusion_policy.distributions import DiagonalGaussianDistribution
from scripts.workflows.hand_manipulation.utils.vae.reactive_diffusion_policy.utils import *
...
class EncoderCNN(nn.Module):
    ...
    def __init__(self, input_dim, output_dim=16, hidden_dim=128, layer_num=1):
...
class DecoderRNN(nn.Module):
    ...
    def __init__(
        ...
        self.rnn = nn.GRU(global_cond_dim + temporal_cond_dim,
                          hidden_dim,
                          layer_num,
                          batch_first=True)
        self.fc = nn.Linear(hidden_dim, output_dim)
    ...
    def forward(self, global_cond, temporal_cond):
        ...
        global_cond = global_cond.unsqueeze(1).expand(-1,
                                                      temporal_cond.size(1),
                                                      -1)
        # (N, T, A)

        x = torch.cat([global_cond, temporal_cond.to(torch.float32)], dim=-1)
        x, _ = self.rnn(x)
        x = self.fc(x)
...
class VAE:
    ...
    def __init__(
        ...
        if self.use_rnn_decoder:
            ...
            self.rnn_temporal_cond_dim = sum([
                shape_meta['extended_obs'][extented_obs_key]['shape'][0]
        ...
        if self.use_conv_encoder:
            self.encoder = EncoderCNN(input_dim=self.input_dim_w,
                                      output_dim=n_latent_dims,
                                      hidden_dim=conv_latent_dims,
                                      layer_num=conv_layer_num).to(self.device)
        else:
            self.encoder = MLP(input_dim=self.input_dim_w * self.input_dim_h,
                               output_dim=n_latent_dims,
                               layer_num=mlp_layer_num).to(self.device)

        output_shape = get_output_shape(
            (self.input_dim_w * self.input_dim_h, ), self.encoder)
        ...
        decoder_n_latent_dims += state_dim

        if self.use_rnn_decoder:
            self.decoder = DecoderRNN(
                global_cond_dim=decoder_n_latent_dims,
                temporal_cond_dim=self.rnn_temporal_cond_dim,
                output_dim=self.input_dim_w,
                hidden_dim=rnn_latent_dims,
                layer_num=rnn_layer_num).to(self.device)
        else:
            self.decoder = MLP(input_dim=decoder_n_latent_dims,
                               output_dim=self.input_dim_w * self.input_dim_h,
                               layer_num=mlp_layer_num).to(self.device)
        ...
        if self.use_vq:
            ...
            self.quant = torch.nn.Conv1d(self.n_latent_dims, 2 * self.n_embed,
                                         1).to(self.device)
            self.post_quant = torch.nn.Conv1d(self.n_embed, self.n_latent_dims,
                                              1).to(self.device)
        self.embedding_dim = self.n_latent_dims

        self.optim_params = (list(self.encoder.parameters()) +
    ...
    def get_action_from_latent(self, latent, obs):

        obs = self.normalizer['action'].normalize(obs).to(self.device)
        latent = torch.cat([latent, obs], dim=-1)
        output = self.decoder(latent) * self.act_scale
        # if self.input_dim_h == 1:
        #     return einops.rearrange(output,
        #                             "N (T A) -> N T A",
        #                             A=self.input_dim_w)
        # else:
        #     return einops.rearrange(output,
    ...
    def quant_state_with_vq(self, state):
        ...
        if len(state.shape) == 2:
            state = einops.rearrange(state,
                                     "N (T A) -> N T A",
                                     T=self.downsampled_input_h)
    ...
    def postprocess_quant_state_without_vq(self, state_vq):
        state_vq = einops.rearrange(state_vq,
                                    "N (T A) -> N A T",
                                    T=self.downsampled_input_h)
    ...
    def get_temporal_cond(self,
                          extented_obs_data,
                          extended_obs_last_step=None,
                          extend_obs_pad_after_n=None):
        inital_obs = extented_obs_data[:, 0]

        return inital_obs.unsqueeze(1).expand(-1, extented_obs_data.size(1),
                                              -1).to(self.device)
    ...
    def eval_step(self, batch):
        ...
        if self.use_vq:
            state_vq, vq_code, vq_loss_state = self.quant_state_with_vq(
                state_rep)
        else:
        ...
        obs = batch["obs"]
        obs = self.normalizer['action'].normalize(obs).to(self.device)
        state_vq = torch.cat([state_vq, obs[:, 0]], dim=-1)

        if self.use_rnn_decoder:

            temporal_cond = self.get_temporal_cond(
                self.normalizer['action'].normalize(batch["obs"]))
        ...
        return {
            ...
            "latent_mean": posterior.mean.clone(),
            "latent_logvar": posterior.std.clone(),
            "state_vq": state_vq.clone(),
            "vae_recon_loss": vae_recon_loss.item(),
        }

    def reset(self):
        self.state_vq_buffer = []

    def load_latent_state_dict(self, state_dict):
        self.max_latent_value = torch.as_tensor(
            state_dict.get("max_latent_value")).to(self.device)
        self.min_latent_value = torch.as_tensor(
            state_dict.get("min_latent_value")).to(self.device)
    ...
    def compute_loss_and_metric(self, batch):
        ...
        if self.use_vq:
            ...
            state_vq, posterior = self.quant_state_without_vq(state_rep)
            state_vq = self.postprocess_quant_state_without_vq(state_vq)
        self.state_vq_buffer.append(state_vq.clone().detach().cpu().numpy())
        ...
        if self.use_vq:
            rep_loss = encoder_loss * self.encoder_loss_multiplier + (
                vq_loss_state * 5)
            ...
            return_dict.update({
                ...
                vq_code,
                "rep_loss":
                rep_loss.clone().detach().cpu().numpy(),
                "vq_loss_state":
                vq_loss_state.clone().detach().cpu().numpy(),
            })
        else:
            kl_loss = posterior.kl().mean()
            rep_loss = encoder_loss * self.encoder_loss_multiplier + (
                kl_loss * self.kl_multiplier)
            return_dict["loss"] = rep_loss
            return_dict.update({
                "kl_loss":
                kl_loss.clone().detach().cpu().numpy(),
                "rep_loss":
                rep_loss.clone().detach().cpu().numpy(),
            })
    ...
    def state_dict(self):
        ...
        latent_space = np.concatenate(self.state_vq_buffer, axis=0)
        latetent_max = np.percentile(latent_space, 95, axis=0)
        latent_min = np.percentile(latent_space, 5, axis=0)
        ...
        state_dict = {
            ...
            "latent_value": {
                "max_latent_value": latetent_max,
                "min_latent_value": latent_min,
            }
        }
        if self.use_vq:
    ...
    def load_state_dict(self, state_dict):
        ...
        self.load_latent_state_dict(state_dict["latent_value"])
        if self.use_vq:
