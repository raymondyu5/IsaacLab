...
def cdist(x, y):
    ...
    return (rearrange(x2, 'b i -> b i 1') + rearrange(y2, 'b j -> b 1 j') +
            xy).sqrt()
...
def sample_vectors_distributed(local_samples, num):
    ...
    all_samples = all_gather_variably_sized(local_samples,
                                            samples_per_rank,
                                            dim=0)
...
def kmeans(samples,
    ...
    for _ in range(num_iters):
        ...
        new_means = buckets.new_zeros(num_codebooks,
                                      num_clusters,
                                      dim,
                                      dtype=dtype)
        ...
        means = torch.where(rearrange(zero_mask, '... -> ... 1'), means,
                            new_means)
...
class EuclideanCodebook(nn.Module):
    ...
    def __init__(self,
        ...
        self.reset_cluster_size = default(reset_cluster_size,
        ...
        self.register_buffer('codebook_mean',
                             torch.empty(num_codebooks, 1, dim))
        self.register_buffer('codebook_variance_needs_init',
                             torch.Tensor([True]))
        self.register_buffer('codebook_variance',
                             torch.empty(num_codebooks, 1, dim))
    ...
    def update_affine(self, data, embed, mask=None):
        ...
        if self.training:
            self.update_with_decay('codebook_mean',
                                   reduce(embed, 'h n d -> h 1 d', 'mean'),
                                   self.affine_param_codebook_decay)
            self.update_with_decay('codebook_variance',
        ...
        if not self.sync_affine_param:
            self.update_with_decay('batch_mean',
                                   reduce(data, 'h n d -> h 1 d', 'mean'),
                                   self.affine_param_batch_decay)
            self.update_with_decay('batch_variance',
        ...
        self.update_with_decay('batch_mean', batch_mean,
                               self.affine_param_batch_decay)

        # calculate distributed variance

        variance_numer = reduce((data - batch_mean)**2, 'h n d -> h 1 d',
                                'sum')
        distributed.all_reduce(variance_numer)
        batch_variance = variance_numer / num_vectors

        self.update_with_decay('batch_variance', batch_variance,
                               self.affine_param_batch_decay)
    ...
    def replace(self, batch_samples, batch_mask):
        for ind, (samples, mask) in enumerate(
            ...
            sampled = self.sample_fn(rearrange(samples, '... -> 1 ...'),
                                     mask.sum().item())
    ...
    def forward(self,
        ...
        sample_codebook_temp = default(sample_codebook_temp,
        ...
        if self.affine_param:
            ...
            embed = (embed - self.codebook_mean) * (
                batch_std / codebook_std) + self.batch_mean
        ...
        if self.training:
            ...
            quantize = einsum('h b n c, h c d -> h b n d', unpacked_onehot,
                              embed)
        else:
        ...
        if self.training and self.ema_update and not freeze_codebook:
            ...
            if self.affine_param:
                flatten = (flatten - self.batch_mean) * (
                    codebook_std / batch_std) + self.codebook_mean
            ...
            cluster_size = laplace_smoothing(
                ...
                self.eps) * self.cluster_size.sum(dim=-1, keepdim=True)

            embed_normalized = self.embed_avg / rearrange(
                cluster_size, '... -> ... 1')
...
class VectorQuantize(nn.Module):
    ...
    def __init__(
        ...
        in_place_codebook_optimizer: Callable[
            ...,
            Optimizer] = None,  # Optimizer used to update the codebook embedding if using learnable_codebook
        affine_param=False,
        affine_param_batch_decay=0.99,
        affine_param_codebook_decay=0.9,
        ...
        gumbel_sample_fn = partial(gumbel_sample,
                                   stochastic=stochastic_sample_codes,
                                   reinmax=reinmax,
                                   straight_through=straight_through)

        if not exists(sync_codebook):
            sync_codebook = distributed.is_initialized(
        ...
        self.in_place_codebook_optimizer = in_place_codebook_optimizer(
            self._codebook.parameters()) if exists(
    ...
    def forward(self,
        ...
        if self.training:
            ...
            if self.sync_update_v > 0.:
                ...
                quantize = quantize + self.sync_update_v * (quantize -
                                                            quantize.detach())
        ...
        def calculate_ce_loss(codes):
            ...
            ce_loss = F.cross_entropy(rearrange(distances,
                                                dist_einops_eq,
                                                b=shape[0]),
                                      codes,
                                      ignore_index=-1)
            ...
            if self.commitment_weight > 0:
                if self.commitment_use_cross_entropy_loss:
                    if exists(mask):
                        ce_loss_mask = mask
                        if is_multiheaded:
                            ce_loss_mask = repeat(ce_loss_mask,
                                                  'b n -> b n h',
                                                  h=heads)
                    ...
                    if exists(mask):
                        ...
                        loss_mask = mask
                        if is_multiheaded:
                            loss_mask = repeat(loss_mask,
                                               'b n -> c (b h) n',
                                               c=commit_loss.shape[0],
                                               h=commit_loss.shape[1] //
                                               mask.shape[0])
        ...
        if exists(mask):
            quantize = torch.where(rearrange(mask, '... -> ... 1'), quantize,
                                   orig_input)
