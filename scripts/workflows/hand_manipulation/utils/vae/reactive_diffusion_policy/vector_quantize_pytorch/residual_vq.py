...
from scripts.workflows.hand_manipulation.utils.vae.reactive_diffusion_policy.vector_quantize_pytorch.vector_quantize_pytorch import VectorQuantize
...
class ResidualVQ(nn.Module):
    ...
    def __init__(self,
        ...
        self.layers = nn.ModuleList([
            VectorQuantize(dim=codebook_dim,
                           codebook_dim=codebook_dim,
                           accept_image_fmap=accept_image_fmap,
                           **kwargs) for _ in range(num_quantizers)
        ])
    ...
    def get_codes_from_indices(self, indices):
        ...
        if quantize_dim < self.num_quantizers:
            ...
            indices = F.pad(indices, (0, self.num_quantizers - quantize_dim),
                            value=-1)

        # get ready for gathering

        codebooks = repeat(self.codebooks, 'q c d -> q b c d', b=batch)
        gather_indices = repeat(indices,
                                'b n q -> q b n d',
                                d=codebooks.shape[-1])
        ...
        gather_indices = gather_indices.masked_fill(
            mask, 0)  # have it fetch a dummy code to be masked out later
    ...
    def draw_logits_forward(self, encoding_logits):
        ...
        for q in range(encoding_logits.shape[1]):
            quantized += torch.matmul(encoding_logits[:, q],
                                      self.codebooks[q]).to(self.device)
        return quantized
    ...
    def forward(self,
        ...
        if should_quantize_dropout:
            ...
            if quant_dropout_multiple_of != 1:
                rand_quantize_dropout_index = round_up_multiple(
                    rand_quantize_dropout_index + 1,
                    quant_dropout_multiple_of) - 1

            null_indices_shape = (
                x.shape[0],
                *x.shape[-2:]) if self.accept_image_fmap else tuple(
                    x.shape[:2])
            null_indices = torch.full(null_indices_shape,
                                      -1.,
                                      device=device,
                                      dtype=torch.long)
        ...
        all_losses, all_indices = map(partial(torch.stack, dim=-1),
...
class GroupedResidualVQ(nn.Module):
    ...
    def forward(self,
        ...
        out = tuple(
            ...
            for rvq, chunk, chunk_indices in zip_longest(
                self.rvqs, x, indices))
        out = tuple(zip(*out))
