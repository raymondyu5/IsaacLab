        ...
        for index, vae_path in enumerate(self.vae_path):
            ...
            with open(f"{vae_path}/model_config.yaml", "r") as f:
                ...
                self.vae_model_setting.append([
                    ...
                    action_mean, action_std, embedding_dim
                ])
        self.num_hand = min([vae_model.quantizer.num_embeddings, 100])
        import itertools
        # all_codebook_combinations = list(
        #     itertools.product(range(vae_model.quantizer.num_embeddings),
        #                       repeat=embedding_dim))
        all_codebook_combinations = torch.arange(
            vae_model.quantizer.num_embeddings).to(self.device)
        with torch.no_grad():
