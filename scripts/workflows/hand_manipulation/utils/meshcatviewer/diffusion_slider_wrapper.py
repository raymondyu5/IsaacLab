...
from scripts.workflows.hand_manipulation.utils.meshcatviewer.diffusion_slider import DiffusionSliderApp
import math
...
class SliderApp:
    ...
    def init_settings(self):
        ...
        if self.diffusion_path is not None:
            self.num_slider += len(self.diffusion_path)
            self.num_diffusion = len(self.diffusion_path)
            if self.demo_path is not None:
                self.num_slider += 1
        else:
            # Add 1 by default if no VAE path is provided
            self.num_slider += 1
            self.num_diffusion = 0
        ...
        if self.diffusion_path is not None:
            ...
            self.diffusion_app = DiffusionSliderApp(
                self.raw_sliders,
                root=self.root,
                action_buffer=self.action_buffer,
            ...
            self.group_count = self.diffusion_app.group_count
        # else:
        #     self._create_slider_group("VAE Latents [-1, 1]",
        #                               [f"z[{i}]" for i in range(self.diffusion_dim)],
        #                               -2, 2, self.diffusion_sliders)
    ...
    def _get_all_values(self):
        ...
        if self.diffusion_path is not None:
            ...
            diffusion = self.diffusion_app.get_diffusion_values()
        else:
        ...
        return {"raw": raw, "pca": pca, "demo": demo} | diffusion

    def _start_socket_server(self, host='localhost', port=10113):
