...
from scripts.workflows.hand_manipulation.env.rl_env.offline2online_env_wrapper import Offline2OnlineEnvWrapper
import imageio
import os
...
def main():
    ...
    if args_cli.rl_type == "ddpg":
        agent = d3rlpy.algos.DDPGConfig(critic_learning_rate=1e-4).create(
            device=env.unwrapped.device)
    elif args_cli.rl_type == "td3":
        agent = d3rlpy.algos.TD3Config(batch_size=1024).create(
            device=env.unwrapped.device, )

    wrapper = Offline2OnlineEnvWrapper(
        env,
        agent,
        args_cli,
        save_config,
