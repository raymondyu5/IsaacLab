...
CONFIG = {
    "working_dir": script_directory,
    "executable": "/workspace/isaaclab/isaaclab.sh -p"
}
...
def read_cluster_spec(fn: str | None = None) -> list[dict]:
    ...
    if not os.path.exists(cluster_spec_path):
        raise FileNotFoundError(
            f"Cluster spec file not found at {cluster_spec_path}")
    ...
    with open(cluster_spec_path) as f:
        for line in f:
            ...
            print(f"[INFO] Setting {cluster_info['name']}"
                  )  # with {cluster_info['num_gpu']} GPUs.")
...
def submit_job(cluster: dict, job_command: str) -> None:
    ...
    print(
        f"[INFO]: Checking contents of the directory: {CONFIG['working_dir']}")
    try:
    ...
    while status in [
            job_submission.JobStatus.PENDING, job_submission.JobStatus.RUNNING
    ]:
        time.sleep(5)
        status = client.get_job_status(job_id)
...
def submit_jobs_to_clusters(jobs: list[str], clusters: list[dict]) -> None:
    ...
    if len(jobs) < len(clusters):
        print(
            "[INFO]: Less jobs than clusters, some clusters will not receive jobs"
        )
    elif len(jobs) == len(clusters):
        print("[INFO]: Exactly one job per cluster")
    else:
        print(
            "[INFO]: More jobs than clusters, jobs submitted as clusters become available."
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Submit multiple GPU jobs to multiple Ray clusters.")
    parser.add_argument("--config_file",
                        default="~/.cluster_config",
                        help="The cluster config path.")
    parser.add_argument(
        "--aggregate_jobs",
        type=str,
        nargs=argparse.REMAINDER,
        help=
        "This should be last argument. The aggregate jobs to submit separated by the * delimiter.",
    )
