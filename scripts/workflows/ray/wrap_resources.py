...
def wrap_resources_to_jobs(jobs: list[str], args: argparse.Namespace) -> None:
    ...
    job_results = []
    gpu_node_resources = util.get_gpu_node_resources(include_id=True,
                                                     include_gb_ram=True)

    if any([args.gpu_per_worker, args.cpu_per_worker, args.ram_gb_per_worker
            ]) and args.num_workers:
        raise ValueError(
            "Either specify only num_workers or only granular resources(GPU,CPU,RAM_GB)."
        )
    ...
    formatted_node_resources = {
        "gpu_per_worker":
        [gpu_node_resources[i]["GPU"] for i in range(num_nodes)],
        "cpu_per_worker":
        [gpu_node_resources[i]["CPU"] for i in range(num_nodes)],
        "ram_gb_per_worker":
        [gpu_node_resources[i]["ram_gb"] for i in range(num_nodes)],
        "num_workers":
        args.num_workers,  # By default, 1 worker por node
    }
    args = util.fill_in_missing_resources(args,
    ...
    for i, job in enumerate(jobs):
        ...
        print(
            f"[INFO]: Submitting job {i + 1} of {len(jobs)} with job '{job}' to node {gpu_node}"
        )
        print(
            f"[INFO]: Resource parameters: GPU: {args.gpu_per_worker[i]}"
            f" CPU: {args.cpu_per_worker[i]} RAM {args.ram_gb_per_worker[i]}")
        print(
            f"[INFO] For the node parameters, creating {args.num_workers[i]} workers"
        )
        ...
        print(
            f"[INFO]: Requesting {num_gpus=} {num_cpus=} {memory=} id={gpu_node['id']}"
        )
        job = util.remote_execute_job.options(
            num_gpus=num_gpus,
            num_cpus=num_cpus,
            memory=memory,
            scheduling_strategy=NodeAffinitySchedulingStrategy(gpu_node["id"],
                                                               soft=False),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Submit multiple jobs with optional GPU testing.")
    ...
    parser.add_argument(
        "--test",
        action="store_true",
        help=("Run nvidia-smi test instead of the arbitrary job,"
    ...
    parser.add_argument(
        ...
        help=
        "This should be last wrapper argument. Jobs separated by the + delimiter to run on a cluster.",
    )
