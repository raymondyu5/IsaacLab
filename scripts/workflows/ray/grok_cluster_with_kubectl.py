...
def get_clusters(pods: list, cluster_name_prefix: str) -> set:
    ...
    for pod_name, _ in pods:
        ...
        match = re.match(r"(" + re.escape(cluster_name_prefix) + r"[-\w]+)",
                         pod_name)
        if match:
...
def get_mlflow_info(namespace: str = None,
                    cluster_prefix: str = "isaacray") -> str:
    """
    ...
    if len(clusters) > 1:
        raise ValueError(
            "More than one cluster matches prefix, could not automatically determine mlflow info."
        )
    ...
    cmd = [
        "kubectl", "get", "svc", mlflow_name, "-n", namespace, "--no-headers"
    ]
    try:
        output = subprocess.check_output(cmd).decode()
...
def get_ray_address(head_pod: str,
...
def process_cluster(cluster_info: dict, ray_head_name: str = "head") -> str:
    ...
    ray_address = get_ray_address(head_pod,
                                  namespace=namespace,
                                  ray_head_name=ray_head_name)
    if not ray_address:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process Ray clusters and save their specifications.")
    ...
    parser.add_argument(
        "--namespace",
        help=
        "Kubernetes namespace to use. If not provided, will detect from current context."
    )
    ...
    with ThreadPoolExecutor() as executor:
        future_to_cluster = {
            executor.submit(process_cluster, info, args.ray_head_name): info[0]
