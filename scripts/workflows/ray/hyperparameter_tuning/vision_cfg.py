...
class CameraJobCfg(tuner.JobCfg):
    ...
    @staticmethod
    def _get_batch_size_divisors(batch_size: int,
                                 min_size: int = 128) -> list[int]:
        """Get valid batch divisors to combine with num_envs and horizon length"""
        divisors = [
            i for i in range(min_size, batch_size + 1) if batch_size % i == 0
        ]
    ...
    def __init__(self,
        ...
        if vary_env_count:  # Vary the env count, and horizon length, and select a compatible mini-batch size
            ...
            cfg["hydra_args"][
                "agent.params.config.horizon_length"] = selected_horizon

            def get_valid_batch_size(config):
                num_envs = config["runner_args"]["--num_envs"]
                horizon_length = config["hydra_args"][
            ...
            cfg["hydra_args"][
                "agent.params.config.minibatch_size"] = tune.sample_from(
                    get_valid_batch_size)
        ...
        if vary_cnn:  # Vary the depth, and size of the layers in the CNN part of the agent
            ...
            cfg["hydra_args"][
                "agent.params.network.cnn.activation"] = tune.choice(
                    ["relu", "elu"])
            cfg["hydra_args"][
                "agent.params.network.cnn.initializer"] = "{name:default}"
            cfg["hydra_args"][
                "agent.params.network.cnn.regularizer"] = "{name:None}"
            ...
            def get_cnn_layers(_):
                ...
                for _ in range(num_layers.sample()):
                    ...
                    valid_kernels = [
                        k for k in [3, 4, 6, 8, 10, 12] if k <= size
                    ]
                    if not valid_kernels:
                        break

                    kernel = int(
                        tune.choice([str(k) for k in valid_kernels]).sample())
            ...
            cfg["hydra_args"][
                "agent.params.network.cnn.convs"] = tune.sample_from(
                    get_cnn_layers)
        ...
        if vary_mlp:  # Vary the MLP structure; neurons (units) per layer, number of layers,
            ...
            if "env.observations.policy.image.params.model_name" in cfg[
                    "hydra_args"]:
            ...
            def get_mlp_layers(_):
                return [
                    tune.randint(4, max_neurons_per_layer).sample()
                    for _ in range(num_layers.sample())
                ]

            cfg["hydra_args"][
                "agent.params.network.mlp.units"] = tune.sample_from(
                    get_mlp_layers)
            cfg["hydra_args"][
                "agent.params.network.mlp.initializer.name"] = tune.choice(
                    ["default"]).sample()
            cfg["hydra_args"][
                "agent.params.network.mlp.activation"] = tune.choice(
                    ["relu", "tanh", "sigmoid", "elu"]).sample()
...
class ResNetCameraJob(CameraJobCfg):
    ...
    def __init__(self, cfg: dict = {}):
        cfg = util.populate_isaac_ray_cfg_args(cfg)
        cfg["hydra_args"][
            "env.observations.policy.image.params.model_name"] = tune.choice(
