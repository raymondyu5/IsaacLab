...
@dataclass
class FinetuneConfig(BaseJobConfig):
    # fmt: off
...
class OpenvlaJob(BaseJob):

    def __init__(self, job_config: FinetuneConfig, wandb_config: WandbConfig):
    ...
    def _job_call(self):
        ...
        run_dir, adapter_dir = f"{cfg.run_root_dir}/{exp_id}", f"{cfg.adapter_tmp_dir}/{exp_id}"
        ...
        AutoModelForVision2Seq.register(OpenVLAConfig,
        ...
        batch_transform = RLDSBatchTransform(
            ...
            if "v01" not in cfg.vla_path else VicunaV15ChatPromptBuilder,
        )
        ...
        #     if "v01" not in cfg.vla_path else VicunaV15ChatPromptBuilder,
        #     data_path=cfg.
        #     data_root_dir,  #"logs/openvla_eggplant/replay_rl_data.hdf5",
        #     prompt="put the eggplant into the sink",
        #     raw_actions=raw_actions,
        #     resize_images=gs_images,
        ...
        with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
            ...
            for batch_idx, batch in enumerate(dataloader):
                ...
                action_logits = output.logits[:, vla.module.vision_backbone.
                                              featurizer.patch_embed.
                                              num_patches:-1]
                ...
                action_accuracy = correct_preds.sum().float() / mask.sum(
                ).float()
                ...
                print("predict", continuous_actions_pred)
                ...
                smoothened_l1_loss = sum(recent_l1_losses) / len(
                    recent_l1_losses)
...
def generate_train_configs():
    ...
    # search
    def creation_fn(vla_path, data_root_dir, dataset_name, run_root_dir,
                    adapter_tmp_dir, lora_rank, batch_size,
                    grad_accumulation_steps, learning_rate, image_aug,
                    wandb_project, wandb_entity, save_steps):
        job = FinetuneConfig(vla_path=vla_path,
    ...
    params = {
        ...
        "data_root_dir": "logs/bridge_uw_0126",
        "dataset_name": "bridge_uw",
        "run_root_dir": "logs/bridge_uw_0126_test",
        "adapter_tmp_dir": "logs/openvla_finetune",
        "lora_rank": 12,
        "batch_size": 9,
        ...
        "wandb_entity": "entongsu",
        "save_steps": 50
    }
    # get the configs
    configs = grid_search_job_configs(params,
                                      job_cls=None,
...
def main():
    ...
    executor_config = SubmititExecutorConfig(
        root_folder="logs/",  #"/gscratch/weirdlab/Entong/IsaacLab/logs/test",
        slurm_partition="gpu-a40",
        slurm_name="submitit-test",
        timeout_min=120,
        slurm_gpus_per_node="a40:3",
        cpus_per_task=8,
        mem_gb=200,
    )
    ...
    state = SubmititState(job_cls=OpenvlaJob,
