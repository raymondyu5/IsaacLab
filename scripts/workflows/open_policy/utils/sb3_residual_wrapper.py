...
class Sb3VecEnvWrapper(VecEnv):
    ...
    def __init__(self,
        ...
        if self.args_cli.normalize_action:
            self.norm_stats = np.load(self.args_cli.normalize_action_path,
                                      allow_pickle=True).item()["action"]
        ...
        if isinstance(env.observation_space, gym.spaces.Dict):
            ...
            observation_space = gym.spaces.Box(
                -np.inf,
                np.inf,
                shape=(int(obs_dim / num_envs) +
                       7, ),  # Ensure shape is a tuple
                dtype=np.float32)
    ...
    def reset(self) -> VecEnvObs:  # noqa: D102
        ...
        with torch.no_grad():
            self.base_action = self.base_policy.policy.get_action(
                obs_dict["policy"])

        obs_dict["policy"]["base_action"] = self.base_action
        self.raw_obs_dict = obs_dict
    ...
    def proccess_actions(self, actions):
        ...
        actions[:, :6] /= self.env.env.step_dt
        clip_actions = ((actions - self.env.lower_bound) /
                        (self.env.upper_bound - self.env.lower_bound)) * 2 - 1
    ...
    def step_wait(self) -> VecEnvStepReturn:  # noqa: D102
        ...
        if self.args_cli.normalize_action:
            ...
            obs_dict, rew, terminated, truncated, extras, _ = self.env.step(
                self._async_actions,
                self.unnormalize(self.base_action, self.norm_stats) *
                self.args_cli.base_scale_factor)

        else:
            obs_dict, rew, terminated, truncated, extras, _ = self.env.step(
                self._async_actions,
                self.base_action * self.args_cli.base_scale_factor)
        self.base_policy.policy.nets.eval()
        with torch.no_grad():
            self.base_action = self.base_policy.policy.get_action(
                obs_dict["policy"])
