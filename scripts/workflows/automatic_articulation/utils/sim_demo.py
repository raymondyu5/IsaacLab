...
def add_box_mesh_revolute(stage,
                          ...
                          xform_rotation=None,
                          interior_mesh_paths=None,
                          door_mesh_paths=None):
    ...
    internal_box_mesh_trimesh = trimesh.Trimesh(
        interior_mesh_paths[0],
        interior_mesh_paths[1],
    )
    ...
    texCoords = UsdGeom.PrimvarsAPI(internal_box_mesh).CreatePrimvar(
        "st", Sdf.ValueTypeNames.TexCoord2fArray, UsdGeom.Tokens.faceVarying)
    texCoords.Set(
        Vt.Vec2fArray.FromNumpy(interior_mesh_paths[2].reshape(-1, 2)))

    internal_box_mesh.CreateDisplayColorPrimvar("vertex")
    internal_box_mesh.GetPrim().ApplyAPI(UsdShade.MaterialBindingAPI)
    UsdShade.MaterialBindingAPI(internal_box_mesh).Bind(interior_mesh_paths[3])
    ...
    stage = create_xform(
        stage, (1, 1, 1),
        (xform_translation[0], xform_translation[1], xform_translation[2]),
        xform_rotation, new_path + "_door_back")

    door_back_mesh = UsdGeom.Mesh.Define(stage,
                                         new_path + "_door_back/door_back")
    door_back_mesh_trimesh = trimesh.Trimesh(
        door_mesh_paths[0],
        door_mesh_paths[1],
    )
    points = np.array(door_back_mesh_trimesh.vertices).reshape(-1, 3)
    vertex_counts = np.ones_like(
        door_back_mesh_trimesh.faces[:, 0]).reshape(-1).astype(np.int32) * 3
    faces = np.array(door_back_mesh_trimesh.faces).reshape(-1, 3)

    door_back_mesh.CreatePointsAttr(Vt.Vec3fArray.FromNumpy(points))
    door_back_mesh.CreateFaceVertexIndicesAttr(Vt.IntArray.FromNumpy(faces))
    door_back_mesh.CreateFaceVertexCountsAttr(
        Vt.IntArray.FromNumpy(vertex_counts))
    door_back_mesh.CreateExtentAttr([(-10, -10, -10), (10, 10, 10)])

    texCoords = UsdGeom.PrimvarsAPI(door_back_mesh).CreatePrimvar(
        "st", Sdf.ValueTypeNames.TexCoord2fArray, UsdGeom.Tokens.faceVarying)
    texCoords.Set(Vt.Vec2fArray.FromNumpy(door_mesh_paths[2].reshape(-1, 2)))

    door_back_mesh.CreateDisplayColorPrimvar("vertex")
    door_back_mesh.GetPrim().ApplyAPI(UsdShade.MaterialBindingAPI)
    UsdShade.MaterialBindingAPI(door_back_mesh).Bind(door_mesh_paths[3])

    prim = stage.GetPrimAtPath(new_path + "_door_back")
    create_rigid_collision(prim, set_sdf=False)
...
def add_box_mesh_prismatic(stage,
    ...
    box_mesh = trimesh.util.concatenate([box_mesh])
...
def create_drawer(stage, path, scale, inverse, internal_inverse, box_grid_mn,
                  ...
                  interior_mesh_paths, door_mesh_paths):
    sy = scale[1]
    sz = scale[2]
    ...
    stage = add_box_mesh_revolute(stage, (1e-4, sy, sz), (0, 0, 0),
                                  ...
                                  interior_mesh_paths=interior_mesh_paths,
                                  door_mesh_paths=door_mesh_paths)
...
class AssembleArticulation:
    ...
    def init_setting(self):
        self.full_mesh_path = os.path.join(
            self.ckpt_dir, "separate/main_tex_mesh_inpainted.obj")
        self.full_mesh_tex_path = os.path.join(
            self.ckpt_dir, "separate/main_tex_mesh_inpainted.png")
        self.root = '/kitchen'
        ...
        self.door_texture_dir = os.path.join(self.ckpt_dir, "texture_doors")
    ...
    def assembly(self):
        ...
        for drawer_i in range(0, self.total_drawers):
            ...
            interior_mesh_paths = self.load_obj_and_material(
                os.path.join(self.door_texture_dir, f"{drawer_i:0>2d}",
                             f"mesh_{drawer_i:0>2d}.obj"),
                os.path.join(self.door_texture_dir, f"{drawer_i:0>2d}",
                             f"mesh_{drawer_i:0>2d}.png"),
            )

            door_mesh_paths = self.load_obj_and_material(
                os.path.join(self.door_texture_dir, f"{drawer_i:0>2d}",
...
# articulated = AssembleArticulation("./cs_kitchen_n")
# articulated.assembly()
usd_file = "/home/ensu/Documents/weird/IsaacLab/tools/auto_articulation/asset/uw_kitchen"
usd_name = "kitchen"
articulated = AssembleArticulation(usd_file, )
articulated.assembly()
