...
ckpt_dir = "/media/lme/data4/weird/IsaacLab/tools/auto_articulation/asset"
...
def add_revolute(stage, joint_path, main_path, door_path, rel0, rel1, inverse):
    ...
    angularDriveAPI = UsdPhysics.DriveAPI.Apply(stage.GetPrimAtPath(jointPath),
                                                "angular")
...
def add_box_mesh_revolute(stage,
    ...
    if drawer_mesh_handle is not None:
        stage = create_xform(stage, (1, 1, 1), (0, 0, 0), (1, 0, 0, 0),
                             path + "_handle")
        mesh = UsdGeom.Mesh.Define(stage, path + "_handle/handle")
        ...
        prim = stage.GetPrimAtPath(path + "_handle")
...
def create_drawer(stage, path, scale, inverse, internal_inverse, box_grid_mn,
                  drawer_mesh_door, drawer_mesh_handle):
    sy = scale[1]
    sz = scale[2]
    joint_pos = [0, -0.5 * sy, 0]
    print("drawer_mesh_door: ", drawer_mesh_door[0].max(axis=0),
          drawer_mesh_door[0].min(axis=0))
    print("sy: ", sy)
    print("sz: ", sz)
    ...
    stage = add_cube(stage, (0.001, 0.001, 0.001),
                     ...
                     f"{path}/drawer_help",
                     no_collision=False)
    stage = add_revolute(stage, f"{path}/drawer_joint", f"{path}/drawer_help",
                         f"{path}/drawer",
                         np.array([-1e-4, 0,
                                   0]).reshape(3), (0, -0.5 * sy, 0), inverse)
    if drawer_mesh_handle is not None:
        stage = add_revolute(stage, f"{path}/drawer_joint_handle",
                             f"{path}/drawer_help", f"{path}/drawer_handle",
                             np.array([-1e-4, 0, 0]).reshape(3),
                             (0, -0.5 * sy, 0), inverse)
    return stage
...
def create_drawer_with_joint(stage,
    ...
    stage = create_drawer(stage, path, scale, inverse, internal_inverse,
                          box_grid_mn, drawer_mesh_door, drawer_mesh_handle)
...
def create_drawer_prismatic(stage, path, scale, drawer_mesh_door,
    ...
    stage = add_cube(stage, (0.001, 0.001, 0.001),
                     ...
                     f"{path}/drawer_help",
                     no_collision=False)
    stage = add_prismatic(stage, f"{path}/drawer_joint", f"{path}/drawer_help",
                          f"{path}/drawer",
                          np.array([-1e-4, 0, 0]).reshape(3), (0, 0, 0))
    if drawer_mesh_handle is not None:
        stage = add_prismatic(stage, f"{path}/drawer_joint_handle",
                              f"{path}/drawer_help", f"{path}/drawer_handle",
                              np.array([-1e-4, 0, 0]).reshape(3), (0, 0, 0))
    return stage
...
for drawer_i in range(total_drawers):
    ...
    if np.any(drawer_handles_local):
        drawer_mesh_handle = filter_mesh_from_vertices(
            drawer_handles_local, full_mesh_verts, full_mesh_faces,
        ...
        components = trimesh.graph.connected_components(edges,
                                                        min_len=1,
                                                        engine='scipy')
        largest_cc = np.argmax(np.array([comp.shape[0]
                                         for comp in components]).reshape(-1),
                               axis=0)
        ...
        v_filtered, f_filtered, _ = filter_mesh_from_vertices(
            keep, filtered_mesh.vertices.copy(), filtered_mesh.faces.copy(),
            None)

        trimesh.exchange.export.export_mesh(
            trimesh.Trimesh(v_filtered, f_filtered),
            os.path.join(drawers_handle_save_dir,
        ...
        handle_pos_pad = np.pad(handle_pos.reshape(1, 3), ((0, 0), (0, 1)),
                                constant_values=(0, 1))
        ...
        drawer_mesh_handle = filter_mesh_from_vertices(
            drawer_handles_local, full_mesh_verts_transformed, full_mesh_faces,
            full_mesh_faces_tex_pos)
        ...
        drawer_mesh_handle = (v_filtered, f_filtered,
                              tex_pos_filtered, material,
...
stage = add_mesh(stage, no_drawer_mesh_verts, no_drawer_mesh_faces,
                 no_drawer_mesh_tex_pos, f"{root}/main", material)
