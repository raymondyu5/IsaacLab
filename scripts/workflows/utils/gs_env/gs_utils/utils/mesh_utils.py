...
def post_process_mesh(mesh, cluster_to_keep=1000):
    ...
    """
    import copy
    print("post processing the mesh to have {} clusterscluster_to_kep".format(
        cluster_to_keep))
...
class GaussianExtractor(object):
    ...
    def reconstruction(self, viewpoint_stack):
        ...
        for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack),
            ...
            normal = torch.nn.functional.normalize(render_pkg['rend_normal'],
                                                   dim=0)
    ...
    def extract_mesh_bounded(self,
        ...
        for i, cam_o3d in tqdm(enumerate(to_cam_open3d(self.viewpoint_stack)),
            ...
            if mask_backgrond and (self.viewpoint_stack[i].gt_alpha_mask
                                   is not None):
            ...
            rgbd = o3d.geometry.RGBDImage.create_from_color_and_depth(
                o3d.geometry.Image(
                    np.asarray(
                        np.clip(rgb.permute(1, 2, 0).cpu().numpy(), 0.0, 1.0) *
                        255,
                        order="C",
                        dtype=np.uint8)),
                o3d.geometry.Image(
                    np.asarray(depth.permute(1, 2, 0).cpu().numpy(),
                               order="C")),
    ...
    def extract_mesh_unbounded(self, resolution=1024):
        ...
        def compute_sdf_perframe(i, points, depthmap, rgbmap, viewpoint_cam):
            ...
            mask_proj = ((pix_coords > -1.) & (pix_coords < 1.) &
                         (z > 0)).all(dim=-1)
            ...
            sampled_rgb = torch.nn.functional.grid_sample(
                ...
                mode='bilinear',
                padding_mode='border',
                align_corners=True).reshape(3, -1).T
            sdf = (sampled_depth - z)
            return sdf, sampled_rgb, mask_proj

        def compute_unbounded_tsdf(samples,
                                   inv_contraction,
                                   voxel_size,
                                   return_rgb=False):
            """
            ...
            if inv_contraction is not None:
                ...
                sdf_trunc[mask] *= 1 / (2 - torch.linalg.norm(
                    samples, dim=-1)[mask].clamp(max=1.9))
            ...
            for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack),
                ...
                sdf = torch.clamp(sdf / sdf_trunc, min=-1.0,
                                  max=1.0)[mask_proj]
                w = weights[mask_proj]
                wp = w + 1
                tsdfs[mask_proj] = (tsdfs[mask_proj] * w + sdf) / wp
                rgbs[mask_proj] = (rgbs[mask_proj] * w[:, None] +
                                   rgb[mask_proj]) / wp[:, None]
                # update weight
        ...
        sdf_function = lambda x: compute_unbounded_tsdf(
            x, inv_contraction, voxel_size)
        from utils.mcube_utils import marching_cubes_with_contraction
        R = contract(normalize(
            self.gaussians.get_xyz)).norm(dim=-1).cpu().numpy()
        ...
        _, rgbs = compute_unbounded_tsdf(torch.tensor(np.asarray(
            mesh.vertices)).float().cuda(),
    ...
    def export_image(self, path):
        ...
        for idx, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack),
                                       desc="export images"):
