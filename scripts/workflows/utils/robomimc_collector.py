...
def fps_points(point_clouds, downsample_points=1024):
    ...
    # dgl farthest point sampling
    # pcd = torch.cat(pc_buffer, dim=0)
    ...
    # pytorch3d  #
    # output, indices = torch3d_ops.sample_farthest_points(
    #     points=point_clouds, K=6000)
...
def sample_train_test(h5_file):
    ...
    data = h5_file["data"]
    num_demo = len(data)
    training_set_size = int(0.8 * num_demo)
...
class RobomimicDataCollector:
    ...
    def add(self, key: str, value: np.ndarray | torch.Tensor):
        ...
        if self._is_first_interaction:
            carb.log_warn(
                "Please call reset before adding new data. Calling reset...")
            self.reset()
        if self._is_stop:
            carb.log_warn(
        ...
        for i in range(value.shape[0]):
            ...
            if num_sub_keys == 2:
                ...
                if sub_keys[1] not in self._dataset[f"env_{i}"][sub_keys[0]]:
                    self._dataset[f"env_{i}"][sub_keys[0]][
                        sub_keys[1]] = list()
                # add data to key
                self._dataset[f"env_{i}"][sub_keys[0]][sub_keys[1]].append(
                    value[i])
            else:
                # create keys
                if sub_keys[0] not in self._dataset[f"env_{i}"]:
    ...
    def flush(self, env_ids: Iterable[int] = (0, )):
        ...
        for index in env_ids:
            ...
            for key, value in env_dataset.items():
                if isinstance(value, dict):
                    ...
                    for sub_key, sub_value in value.items():
                        try:
                            key_group.create_dataset(sub_key,
                                                     data=np.array(sub_value))
                        except:
                            import pdb
                            pdb.set_trace()
                else:
            ...
            self._h5_data_group.attrs["total"] += h5_episode_group.attrs[
                "num_samples"]
    ...
    def close(self):
        ...
        if not self._is_stop:
            print(
                f">>> Closing recording of data. Collected demos: {self._demo_count} / {self._num_demos}"
            )
