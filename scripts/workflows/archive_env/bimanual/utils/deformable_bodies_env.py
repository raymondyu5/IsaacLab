...
class DeformableBodiesEnv:

    def __init__(self):
        self.deformable_assets = []
        for object_name in self.env.scene.keys():
            if "deform_object" in object_name:
                self.deformable_assets.append(self.env.scene[object_name])

        self.env_ids = torch.arange(self.num_envs, device=self.device)

    def reset_deformable_object_pose(
        self,
        explore_type,
        ...
        if reset_gripper and self.has_gripper:
            self.reset_gripper_for_sample_pc()
