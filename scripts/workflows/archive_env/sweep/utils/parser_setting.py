...
def update_deformable_setting(object_config, deformable_list, fix_params,
    ...
    for index, object_name in enumerate(deformable_list):
        env_config = f"source/config/task/sweep/deformable_object/{object_name}_sweep.yaml"
        ...
        attachment_name = object_name + "_" + [
            *config["params"]["RigidObject"].keys()
        ][0]
        raw_attachment_name = [*config["params"]["RigidObject"].keys()][0]
        object_config["params"]["RigidObject"][attachment_name] = {}
        object_config["params"]["RigidObject"][attachment_name].update(
            config["params"]["RigidObject"][raw_attachment_name])
        object_config["params"]["RigidObject"][attachment_name][
            "pos"] = config["params"]["DeformableObject"]["deform_object"][
                "env_setting"]["pos"]
        object_config["params"]["RigidObject"][attachment_name][
            "rot"] = config["params"]["DeformableObject"]["deform_object"][
                "env_setting"]["rot"]
        object_config["params"]["RigidObject"][attachment_name][
            "scale"] = config["params"]["DeformableObject"]["deform_object"][
                "env_setting"]["scale"]

        object_count += 1
        # if fix_params is None:
        #     object_config["params"]["DeformableObject"][object_name][
        #         "physical_params"]["fix_params"] = None
...
def merge_config():
    with open("source/config/task/sweep/default_config.yaml", 'r') as file:
        # Load the contents of the file
        object_config = yaml.safe_load(file)
    with open("source/config/task/sweep/default_camera.yaml", 'r') as file:
        # Load the contents of the file
        camera_config = yaml.safe_load(file)
    with open("source/config/task/sweep/default_assets.yaml", 'r') as file:
        # Load the contents of the file
        assets_config = yaml.safe_load(file)
    with open("source/config/task/sweep/default_deformable.yaml", 'r') as file:
        # Load the contents of the file
        deformable_config = yaml.safe_load(file)
    with open("source/config/task/sweep/default_rigid.yaml", 'r') as file:
        # Load the contents of the file
