...
def bottle(func, *tensors):
    ...
    t, n = tensors[0].shape[:2]
    #out = func(*[x.view(n*t, *x.shape[2:]) for x in tensors])
    out = func(*[x.reshape(t * n, *x.shape[2:]) for x in tensors])
    return out.view(t, n, *out.shape[1:])
...
def save_video(frames, path, name):
    ...
    #)
    writer = cv2.VideoWriter(str(pathlib.Path(path) / f'{name}.mp4'),
...
def visualize_episode(frames, episode, path, name, mapping=None):
    ...
    if W == 64 * 2:
        writer = cv2.VideoWriter(str(pathlib.Path(path) / f'{name}.mp4'),
                                 cv2.VideoWriter_fourcc(*'mp4v'), 0.5,
                                 (256 * 2, 256), True)
    else:
        writer = cv2.VideoWriter(str(pathlib.Path(path) / f'{name}.mp4'),
                                 cv2.VideoWriter_fourcc(*'mp4v'), 0.5,
    ...
    for i, frame in enumerate(frames[..., ::-1]):
        ...
        cv2.putText(vis,
                    'Action: %s, Reward: %.2f' % (mapping[action], reward),
                    (10, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 0), 1,
                    cv2.LINE_AA)
