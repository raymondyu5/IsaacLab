...
class STN(nn.Module):
    ...
    def __init__(self, input_dims=1, dataset="mnist"):
        ...
        self.localization = nn.Sequential(
            nn.Conv2d(input_dims, 8, kernel_size=7), nn.MaxPool2d(2, stride=2),
            nn.ReLU(True), nn.Conv2d(8, 10, kernel_size=5),
        ...
        self.fc_loc = nn.Sequential(nn.Linear(self.fc_loc_inp_dim, 32),
...
class decoder_cnn(nn.Module):
    ...
    def __init__(self, dataset='mnist', input_dim=62):
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(256,
                      32 * (self.input_height // 4) * (self.input_width // 4)),
            nn.BatchNorm1d(32 * (self.input_height // 4) *
                           (self.input_width // 4)),
            nn.ReLU(),
        )
...
class VRNN(nn.Module):
    ...
    def __init__(self,
        ...
        self.phi_h = nn.Sequential(nn.Linear(z_dim, h_dim), nn.ReLU())
        ...
        if (spectral):
            self.enc = nn.Sequential(
                SpectralNorm(nn.Linear(enc_dec_dim, h_dim)), nn.ReLU(),
                SpectralNorm(nn.Linear(h_dim, h_dim)), nn.ReLU())
            self.enc_mean = SpectralNorm(nn.Linear(h_dim, z_dim))
            self.enc_std = nn.Sequential(SpectralNorm(nn.Linear(h_dim, z_dim)),
                                         nn.Softplus())

            #prior
            self.prior = nn.Sequential(
                SpectralNorm(nn.Linear(prior_dim, h_dim)), nn.ReLU())
            self.prior_mean = SpectralNorm(nn.Linear(h_dim, z_dim))
            self.prior_std = nn.Sequential(
                SpectralNorm(nn.Linear(h_dim, z_dim)), nn.Softplus())
        else:
            self.enc = nn.Sequential(nn.Linear(enc_dec_dim, h_dim), nn.ReLU(),
                                     nn.Linear(h_dim, h_dim), nn.ReLU())
            ...
            #prior
            self.prior = nn.Sequential(nn.Linear(prior_dim, h_dim), nn.ReLU())
        ...
        if (decoder):
            ...
            self.dec = nn.Sequential(nn.Linear(enc_dec_dim, h_dim), nn.ReLU(),
                                     nn.Linear(h_dim, h_dim), nn.ReLU())
    ...
    def reset(self):
        #feature-extracting transformations
        self.phi_x = nn.Sequential(nn.Linear(self.x_dim,
                                             self.h_dim), nn.ReLU(),
        ...
        #encoder
        self.enc = nn.Sequential(
            nn.Linear(self.h_dim + self.h_dim, self.h_dim), nn.ReLU(),
    ...
    def forward(self,
                ...
                epoch=0,
                sw=None):
        all_enc_mean, all_enc_std = torch.zeros(
            x.size(0), x.size(1), self.z_dim).to(self.device), torch.zeros(
                x.size(0), x.size(1), self.z_dim).to(self.device)
        all_dec_mean, all_dec_std = torch.zeros(x.shape).to(
            self.device), torch.zeros(x.shape).to(self.device)
        dec_std_t = 1
        dec_mean_t = 1
        all_z_t_mean = torch.zeros(x.size(0), x.size(1),
                                   self.z_dim).to(self.device)
        all_z_t_var = torch.zeros(x.size(0), x.size(1),
                                  self.z_dim).to(self.device)
        ...
        phys_loss = 0
        if h is None:
            h1 = Variable(torch.zeros(self.n_layers, x.size(0),
                                      self.h_dim)).to(self.device)
            c1 = Variable(torch.zeros(self.n_layers, x.size(0),
                                      self.h_dim)).to(self.device)
        else:
            h1 = h[0]
            c1 = h[1]
        ...
        z_prev = None
        z_current = None
        for t in range(start, x.size(0)):

            if (pred is None):
                #phi_x_t = self.phi_x(x[:, t])

                phi_x_t = self.phi_x.conv(x[t, :])

                # Flatten the tensor
                phi_x_t = phi_x_t.view(
                    phi_x_t.size(0),
                    -1)  # x.size(0) is the batch size, -1 infers the rest

                # Pass through fully connected layers
                phi_x_t = self.phi_x.fc(phi_x_t)
            ...
            if (pred is None):
                #computing losses
                kld_loss += self.beta * self._kld_gauss(
                    enc_mean_t, enc_std_t, prior_mean_t, prior_std_t)
                if (self.decoder):

                    nll_loss += self.gamma * F.binary_cross_entropy(
                        dec_mean_t, x[t, :], reduction='sum')
                else:
                    nll_loss = 0

                all_t = params.to(self.device)
                phys_loss += self.alpha * self._nll_gauss(
                    enc_mean_t[:, :3], enc_std_t[:, :3], all_t[:, :3])
    ...
    def forward_eval(self, x, get_h=None, h=None, max_time=0, pred=False):
        if (h is None):
            all_z_t_mean = torch.zeros(x.size(0), x.size(1),
                                       self.z_dim).to(self.device)
            all_z_t_std = torch.zeros(x.size(0), x.size(1),
                                      self.z_dim).to(self.device)
            h1 = Variable(torch.zeros(self.n_layers, x.size(0),
                                      self.h_dim)).to(self.device)
            c1 = Variable(torch.zeros(self.n_layers, x.size(0),
            ...
            h1 = h[0]
            c1 = h[1]
            all_z_t_mean = torch.zeros(x.size(0), max_time,
                                       self.z_dim).to(self.device)
            all_z_t_std = torch.zeros(x.size(0), max_time,
                                      self.z_dim).to(self.device)
        ...
        for t in range(0, max_time):
            if (pred):
                ...
                time_eval[t] = time_eval[max(
                    0, t - 1)] + timeit.default_timer() - start_time
            else:
                start_time = timeit.default_timer()
                phi_x_t = self.phi_x(x[:, t])
    ...
    def _kld_gauss(self, mean_1, std_1, mean_2, std_2):
        ...
        return 0.5 * torch.mean(kld_element, dim=0).sum()

    def _nll_gauss(self, mean, std, x):

        neg_log_p = ((mean - x)**2 / (2 * torch.pow(std, 2))) + torch.log(
            torch.pow(std, 2)) / 2 + torch.log(
                torch.from_numpy(np.array(2 * np.pi))) / 2

        return torch.mean(neg_log_p, dim=0).mean()
