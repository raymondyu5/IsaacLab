...
class BaseCallback(ABC):
    ...
    def on_training_start(self, locals_: Dict[str, Any],
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.save_freq == 0:
            ...
            if self.save_replay_buffer and hasattr(
                ...
                replay_buffer_path = self._checkpoint_path("replay_buffer_",
                                                           extension="pkl")
                self.model.save_replay_buffer(
                    replay_buffer_path)  # type: ignore[attr-defined]
                if self.verbose > 1:
                    print(
            ...
            if self.save_vecnormalize and self.model.get_vec_normalize_env(
                ...
                vec_normalize_path = self._checkpoint_path("vecnormalize_",
                                                           extension="pkl")
                self.model.get_vec_normalize_env().save(
                    vec_normalize_path)  # type: ignore[union-attr]
                if self.verbose >= 2:
...
class EvalCallback(EventCallback):
    ...
    def _log_success_callback(self, locals_: Dict[str, Any],
                              globals_: Dict[str, Any]) -> None:
        """
