...
def save_image(rgb_image, seg_image, dir_name, gs_name, frame_name, img_idx):
...
def save_frames_info(rgb_dataset,
    ...
    for img_idx in range(rgb_dataset.shape[0]):
        ...
        save_image(rgb_image, seg_image, dir_name, gs_name, frame_name,
                   img_idx)
...
def save_static_images(file, dir_name, gs_name, skip_frames, out,
                       target_mesh_path):

    rgb_data = file["data"]["demo_0"]["obs"]["rgb"][0][skip_frames:, ..., :3]
    extrinsic_params = file["data"]["demo_0"]["obs"]["extrinsic_params"][0][
        skip_frames:]
    intrinsic_params = file["data"]["demo_0"]["obs"]["intrinsic_params"][0][
        skip_frames:]
    seg_data = file["data"]["demo_0"]["obs"]["seg_rgb"][0][skip_frames:,
                                                           ..., :3]
    ...
    save_frames_info(rgb_dataset, seg_data_dataset, dir_name, gs_name,
                     extrinsic_dataset, out, intrinsic_dataset,
                     target_mesh_path)
...
def save_sperate(dir_name, dataset_type, skip_frames, target_mesh_path):
...
def convert_h5py(dir_name, dataset_type, skip_frames, target_mesh_path):
    save_sperate(dir_name, dataset_type, skip_frames, target_mesh_path)
