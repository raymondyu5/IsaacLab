...
from scripts.workflows.sysID.ASID.tool.optimizers import AdaptiveOptimizer
from scripts.workflows.sysID.ASID.eppo.eppo import ePPO
# import imageio
...
def main():
    ...
    os.makedirs(logdir + "/result", exist_ok=True)
    # reset environment
    ...
    buffer = DataBuffer(env, "cuda:0",
                        [*env.observation_space["policy"].keys()])
    ...
    while simulation_app.is_running():
        ...
        for num_loop in range(loop_num):
            ...
            for eppo_index in range(num_eppo):

                if abs(
                        torch.exp(eppo.distribution._log_sigma.data[0]).detach(
                        ).cpu().numpy()) < 0.0001:
                    continue
                rollout_count = 0
                buffer.clear_buffer(buffer_type="train")

                print(f"Begin sampling,itr {eppo_index},loop {num_loop}")
                #sample new data at each cem iteraction

                sample_parms = eppo.distribution.sample(
                    num_env=args_cli.num_envs *
                    rollout_num).detach().cpu().numpy()
                ...
                while True:
                    if target_num > target_count:
                        ...
                        print(
                            f"Real: num_eppo: {eppo_index}, mean:{real_data}, std:0.0"
                        )

                        mu = eppo.distribution._mu.data[0].detach().cpu(
                        ).numpy()
                        sigma = torch.exp(eppo.distribution._log_sigma.data[0]
                                          ).detach().cpu().numpy()
                        print(
                            f"Rollout: num_eppo:{eppo_index}, mean:{mu}, std:{sigma}"
                        )

                        break  #exit after the optimization

            target_propoerties.append(real_data.squeeze().cpu().numpy())
            rollout_properties.append(mu)
            rollout_properties_std.append(sigma)

            np.save(
                f"{logdir}/result/{num_loop}.npy",
                np.concatenate([[real_data.squeeze().cpu().numpy()], [mu],
                                [sigma]]))
