...
from scripts.workflows.sysID.ASID.reps.REPS import REPS
import imageio
...
def main():
    ...
    while simulation_app.is_running():
        ...
        for num_loop in range(loop_num):
            ...
            for reps_index in range(num_reps):

                if reps.distribution._std[0] < 0.0001:
                    continue
                rollout_count = 0
                buffer.clear_buffer(buffer_type="train")

                print(f"Begin sampling,itr {reps_index},loop {num_loop}")
                #sample new data at each cem iteraction

                sample_parms = reps.distribution.sample(args_cli.num_envs *
                                                        rollout_num)
                ...
                while True:
                    if target_num > target_count:
                        ...
                        print(
                            f"Real: num_reps: {reps_index}, mean:{real_data}, std:0.0"
                        )
                        print(
                            f"Rollout: num_reps:{reps_index}, mean:{reps.distribution._mu}, std:{reps.distribution._std}"
                        )

                        break  #exit after the optimization

            target_propoerties.append(real_data.squeeze().cpu().numpy())
            rollout_properties.append(reps.distribution._mu)
            rollout_properties_std.append(reps.distribution._std)
            np.save(
                f"{logdir}/result/{num_loop}.npy",
                np.concatenate([[real_data.squeeze().cpu().numpy()],
                                reps.distribution._mu,
                                reps.distribution._std]))
