...
class GaussianDistribution():
    ...
    def con_wmle(self, theta, weights, eps, *args):
        ...
        res = minimize(GaussianDistribution._lagrangian_eta,
                       eta_start,
        ...
        self._mu = GaussianDistribution._compute_mu_from_lagrangian(
            weights, theta, mu, eta_opt)
    ...
    @staticmethod
    def _kl_constraint(mu, mu_new, sigma, sigma_new, sigma_inv, sigma_new_inv,
                       logdet_sigma, logdet_sigma_new, n_dims):

        return 0.5 * (np.trace(sigma_new_inv @ sigma) - n_dims +
    ...
    def _lagrangian_eta(lag_array, weights, theta, mu, sigma, n_dims, eps):
        ...
        mu_new = GaussianDistribution._compute_mu_from_lagrangian(
            weights, theta, mu, eta)
        ...
        sum1 = np.sum([
            w_i * (-0.5 * (theta_i - mu_new)[:, np.newaxis].T @ sigma_inv
                   @ (theta_i - mu_new)[:, np.newaxis] - 0.5 * logdet_sigma -
                   0.5 * c) for w_i, theta_i in zip(weights, theta)
        ])
        sum2 = eta * (eps - GaussianDistribution._kl_constraint(
            mu, mu_new, sigma, sigma, sigma_inv, sigma_inv, logdet_sigma,
            logdet_sigma, n_dims))
...
class GaussianDiagonalDistribution():
    ...
    def sample(self, num_envs, context=None):
        sigma = np.diag(self._std**2)
        return np.random.multivariate_normal(self._mu, sigma, size=num_envs)
    ...
    def con_wmle(self, theta, weights, eps, kappa):
        ...
        res = minimize(GaussianDiagonalDistribution._lagrangian_eta_omega,
                       eta_omg_start,
                       bounds=((np.finfo(np.float32).eps, np.inf),
                               (np.finfo(np.float32).eps, np.inf)),
    ...
    def _compute_mu_sigma_from_lagrangian(weights, theta, mu, sigma, eta, omg):
        ...
        mu_new = (weights @ theta + eta * mu) / (weights_sum + eta)
        sigma_new = np.sqrt((np.sum([
            w_i * (theta_i - mu_new)**2
            for theta_i, w_i in zip(theta, weights)
        ],
                                    axis=0) + eta * sigma**2 + eta *
                             (mu_new - mu)**2) / (weights_sum + eta - omg))
    ...
    @staticmethod
    def _lagrangian_eta_omega(lag_array, weights, theta, mu, sigma, n_dims,
                              eps, kappa):
        ...
        sum1 = np.sum([
            w_i * (-0.5 * (theta_i - mu_new)[:, np.newaxis].T @ sigma_new_inv
                   @ (theta_i - mu_new)[:, np.newaxis] -
                   0.5 * logdet_sigma_new - c / 2)
            for w_i, theta_i in zip(weights, theta)
        ])

        sum2 = eta * (eps - GaussianDiagonalDistribution._kl_constraint(
            mu, mu_new, sigma, sigma_new, sigma_inv, sigma_new_inv,
            logdet_sigma, logdet_sigma_new, n_dims))

        sum3 = omg * (
            GaussianDiagonalDistribution._entropy(logdet_sigma_new, n_dims) -
            (GaussianDiagonalDistribution._entropy(logdet_sigma, n_dims) -
             kappa))
...
class GaussianCholeskyDistribution():
    ...
    def con_wmle(self, theta, weights, eps, kappa):
        ...
        eta_omg_start = np.array([1000, 0])
        res = minimize(GaussianCholeskyDistribution._lagrangian_eta_omega,
                       eta_omg_start,
                       bounds=((np.finfo(np.float32).eps, np.inf),
                               (np.finfo(np.float32).eps, np.inf)),
                       args=(weights, theta, mu, sigma, n_dims, eps, kappa),
    ...
    def _compute_mu_sigma_from_lagrangian(weights, theta, mu, sigma, eta, omg):
        ...
        sigma_new = (
            sigmawa + eta * sigma + eta *
            (mu_new - mu)[:, np.newaxis] @ (mu_new - mu)[:, np.newaxis].T) / (
                weights_sum + eta - omg)
    ...
    def _lagrangian_eta_omega(lag_array, weights, theta, mu, sigma, n_dims,
        ...
        sum1 = np.sum([
            w_i * (-0.5 * (theta_i - mu_new)[:, np.newaxis].T @ sigma_new_inv
                   @ (theta_i - mu_new)[:, np.newaxis] -
                   0.5 * logdet_sigma_new - 0.5 * c)
            for w_i, theta_i in zip(weights, theta)
        ])

        sum2 = eta * (eps - GaussianCholeskyDistribution._kl_constraint(
            mu, mu_new, sigma, sigma_new, sigma_inv, sigma_new_inv,
            logdet_sigma, logdet_sigma_new, n_dims))

        sum3 = omg * (
            GaussianCholeskyDistribution._entropy(logdet_sigma_new, n_dims) -
            (GaussianCholeskyDistribution._entropy(logdet_sigma, n_dims) -
             kappa))
