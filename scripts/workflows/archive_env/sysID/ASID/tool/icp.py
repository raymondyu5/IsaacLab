...
def iterative_closest_point(
    ...
    if ((num_points_Y < Yt.shape[1]).any() or
        (num_points_X < Xt.shape[1]).any()) and (num_points_Y
                                                 != num_points_X).any():
        # we have a heterogeneous input (e.g. because X/Y is
        # an instance of Pointclouds)
        mask_X = (torch.arange(size_X, dtype=torch.int64,
                               device=Xt.device)[None]
                  < num_points_X[:, None]).type_as(Xt)
    else:
    ...
    if init_transform is not None:
        ...
        try:
            ...
            assert (R.shape == torch.Size(
                (b, dim, dim)) and T.shape == torch.Size((b, dim))
            ...
            raise ValueError(
                ...
                "of scalars of shape (minibatch,).") from None
    ...
    for iteration in range(max_iterations):
        Xt_nn_points = knn_points(Xt,
                                  Yt,
                                  lengths1=num_points_X,
                                  lengths2=num_points_Y,
                                  K=1,
                                  return_nn=True).knn[:, :, 0, :]
        ...
        if trim:
            diff_thresholds = Xt_sq_diff[mask_X.bool()].quantile(1 -
                                                                 trim_fraction)
        ...
        if verbose:
            rmse_msg = (f"ICP iteration {iteration}: mean/max rmse = " +
    ...
    return ICPSolution(converged, rmse, Xt, SimilarityTransform(R, T, s),
                       t_history)
...
def corresponding_points_alignment(
    ...
    if (S.abs() <= AMBIGUOUS_ROT_SINGULAR_THR).any() and not (num_points <
                                                              (dim + 1)).any():
        warnings.warn(
...
def _apply_similarity_transform(X: torch.Tensor, R: torch.Tensor,
                                T: torch.Tensor,
