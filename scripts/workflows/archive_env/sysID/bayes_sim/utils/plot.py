...
def plot_trajectories(sim_traj_states):
     #Create a 3D plot for each environment
    fig = plt.figure(figsize=(12, 10))

    for i in range(sim_traj_states.shape[0]):
        ax = fig.add_subplot(2, 2, i+1, projection='3d')
        ax.plot(
            sim_traj_states[i, :, 0],  # X-axis
            sim_traj_states[i, :, 1],  # Y-axis
            sim_traj_states[i, :, 2],  # Z-axis
            marker='o'
        )
