...
def pad_states_actions(states, actions, tgt_actions_len=None):
    ...
    assert (len(
        actions.shape) == 3), 'Need actions: ntraj x n_steps x state_dim'
...
def summary_start(states, actions, max_t=50):
...
def summary_signatory(states, actions):
    ...
    """
    assert (len(
        states.shape) == 3), 'states should be batch x time x state_dim'
