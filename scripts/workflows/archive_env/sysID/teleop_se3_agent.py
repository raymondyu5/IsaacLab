...
def pre_process_actions(delta_pose: torch.Tensor,
    ...
    if "Reach" in args_cli.task:
        ...
        gripper_vel = torch.zeros(delta_pose.shape[0],
                                  1,
                                  device=delta_pose.device)

        if isinstance(gripper_command, np.ndarray):
            gripper_vel[:] = -gripper_command.astype(np.float64)[0]
            # compute actions
            return torch.concat(
                [delta_pose,
                 torch.concat([gripper_vel, gripper_vel], dim=1)],
                dim=1)
...
def main():
    ...
    if args_cli.teleop_device.lower() == "keyboard":
        ...
        raise ValueError(
            f"Invalid device interface '{args_cli.device}'. Supported: 'keyboard', 'spacemouse'."
        )
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            delta_pose = torch.tensor(delta_pose,
                                      device=env.unwrapped.device).repeat(
