...
class MotionPlanReach:
    ...
    def reset_env(self, reset_joint=True):
        self.env.reset()
        if reset_joint:
            self.reset_robot_ee_pose = self.add_noise_to_position(
                self.init_ee_pose.clone(), )
        # self.reset_robot_ee_pose = mdp.add_noise_to_position(self.init_ee_pose.clone(),self.position_range)

        self.reset_joint_pos = self.ik_planner.plan_motion(
            self.reset_robot_ee_pose[:, :3],
            self.reset_robot_ee_pose[:, 3:7])[:, 0]
        self.env.scene["robot"].root_physx_view.set_dof_positions(
            self.reset_joint_pos, self.env_indices)

        self.target_ee_pose = self.env.command_manager.get_command(
            "target_ee_pose")

        result = self.ik_planner.ik_solver.fk(self.reset_joint_pos)
        reset_ee_position = result.ee_pose.position
        reset_ee_quat = result.ee_pose.quaternion
    ...
    def step_motion(self, last_obs):
        ...
        for i in range(3):
            ...
            cur_robot_jpose = self.env.scene["robot"]._data.joint_pos
            if self.env.num_envs > 1:
                ee_pose, traj = self.planner.plan_batch_motion(
                    cur_robot_jpose, self.target_ee_pose[:, :3],
                    self.target_ee_pose[:, 3:7])
            else:
            ...
            if ee_pose is None:
                return
            ee_position = ee_pose.ee_position
            ee_quaternion = ee_pose.ee_quaternion

            # if not self.use_ik_pose:
            target_poses = torch.cat([
                ee_position, ee_quaternion,
                torch.zeros((len(ee_quaternion), 1)).to(self.device)
            ],
                                     dim=1)

            for index, action in enumerate(target_poses):

                if self.use_ik_pose:
                    action = process_action(action.unsqueeze(0), True,
                                            self.robot, self.device)[0]
            ...
            self.env.command_manager.reset(self.env_indices)

            self.target_ee_pose = self.env.command_manager.get_command(
                "target_ee_pose")
        final_ee_pose, _ = get_robottip_pose(self.robot,
                                             self.device,
                                             use_gripper_offset=False)
        dist = torch.linalg.norm(final_ee_pose[0, :3] - target_poses[-1, :3])
        if dist < 0.05:
