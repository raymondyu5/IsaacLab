...
class TD3_BC:
    ...
    def __init__(self,
        ...
        self.algo = algo
        self.log_dir = self.log_dir.split(
            self.algo)[0] + self.algo + '/' + self.datatype + '/'
        self.data_dir = self.log_dir.split(self.algo)[0].split('logs')
        self.data_dir = self.data_dir[0] + 'data' + self.data_dir[
            1] + self.datatype + '/'
    ...
    def run(self, num_learning_iterations, log_interval=1):
        ...
        policy = TD3_BC_Model(state_dim, action_dim, max_action, self.device,
                              self.discount, self.tau, policy_noise,
                              noise_clip, self.policy_freq, self.alpha)
        ...
        for t in range(int(self.max_timesteps / self.iterations) + 1):
            ...
            reward_sum = []
            cur_reward_sum = torch.zeros(self.vec_env.num_envs,
                                         dtype=torch.float,
                                         device=self.device)
            current_obs = self.vec_env.reset()
            ...
            self.reward_log.write(
                str(sum(reward_sum) / len(reward_sum)) + '\n')
