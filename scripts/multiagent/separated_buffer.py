...
class SeparatedReplayBuffer(object):
    ...
    def __init__(self, config, obs_space, share_obs_space, act_space, device):
        ...
        self.rnn_hidden_size = config["hidden_size"][-1]
        ...
        self.share_obs = torch.zeros(self.episode_length + 1,
                                     self.n_rollout_threads,
                                     *share_obs_shape,
                                     device=self.device)
        self.obs = torch.zeros(self.episode_length + 1,
                               self.n_rollout_threads,
                               *obs_shape,
                               device=self.device)

        self.rnn_states = torch.zeros(self.episode_length + 1,
                                      self.n_rollout_threads,
                                      self.recurrent_N,
        ...
        self.value_preds = torch.zeros(self.episode_length + 1,
                                       self.n_rollout_threads,
                                       1,
                                       device=self.device)
        self.returns = torch.zeros(self.episode_length + 1,
                                   self.n_rollout_threads,
                                   1,
                                   device=self.device)

        if act_space.__class__.__name__ == 'Discrete':
            self.available_actions = torch.ones(self.episode_length + 1,
                                                self.n_rollout_threads,
                                                act_space.n,
                                                device=self.device)
        else:
        ...
        self.actions = torch.zeros(self.episode_length,
                                   self.n_rollout_threads,
                                   act_shape,
                                   device=self.device)
        self.action_log_probs = torch.zeros(self.episode_length,
                                            self.n_rollout_threads,
                                            act_shape,
                                            device=self.device)
        self.rewards = torch.zeros(self.episode_length,
                                   self.n_rollout_threads,
                                   1,
                                   device=self.device)

        self.masks = torch.ones(self.episode_length + 1,
                                self.n_rollout_threads,
                                1,
                                device=self.device)
        ...
        self.factor = torch.ones(self.episode_length,
                                 self.n_rollout_threads,
                                 1,
                                 device=self.device)
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        for start_ind in range(0, n_rollout_threads, num_envs_per_batch):
            ...
            rnn_states_batch = torch.stack(rnn_states_batch, 1).reshape(
                N, *self.rnn_states.shape[2:])
            rnn_states_critic_batch = torch.stack(
                rnn_states_critic_batch,
