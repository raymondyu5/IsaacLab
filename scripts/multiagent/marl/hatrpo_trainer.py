...
from scripts.multiagent.marl.utils.popart import PopArt
from scripts.multiagent.marl.utils.util import check
from scripts.multiagent.marl.actor_critic import Actor, Critic
    ...
    def trpo_update(self, sample, update_actor=True):
        ...
        for i in range(self.ls_step):
            ...
            ratio = torch.exp((action_log_probs -
                               old_action_log_probs_batch).sum(dim=-1,
                                                               keepdim=True))
            if self._use_policy_active_masks:
                new_loss = (torch.sum(
                    ratio * factor_batch * adv_targ, dim=-1, keepdim=True) *
