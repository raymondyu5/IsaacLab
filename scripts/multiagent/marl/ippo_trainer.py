...
from scripts.multiagent.marl.utils.util import get_gard_norm, huber_loss, mse_loss
from scripts.multiagent.marl.utils.valuenorm import ValueNorm
from scripts.multiagent.marl.utils.util import check
...
class IPPO():
    ...
    def __init__(self, config, policy, device=torch.device("cpu")):
        ...
        self._use_popart = config["use_popart"]  # false in IPPO
        self._use_value_active_masks = config[
            "use_value_active_masks"]  # false in IPPO
        self._use_policy_active_masks = config[
            "use_policy_active_masks"]  # false in IPPO
    ...
    def ppo_update(self, sample, update_actor=True):
        ...
        value_preds_batch = value_preds_batch.detach()
        return_batch = return_batch.detach()
        active_masks_batch = active_masks_batch.detach()
        ...
        imp_weights = torch.exp(
            (action_log_probs - old_action_log_probs_batch).sum(dim=-1,
                                                                keepdim=True))
