...
class ReplayBuffer:
    ...
    def __init__(self,
        ...
        self.device = device
        self.sampler = config["sampler"] if config[
            "sampler"] is not None else "random"
        ...
        # Core
        self.obs = torch.zeros(num_transitions_per_env,
                               num_envs,
                               *obs_shape,
                               device=self.device)

        # share observation = share_obs
        self.share_obs = torch.zeros(num_transitions_per_env,
                                     num_envs,
                                     *share_obs_shape,
                                     device=self.device)

        self.rewards = torch.zeros(num_transitions_per_env,
                                   num_envs,
                                   1,
                                   device=self.device)
        self.next_observations = torch.zeros(num_transitions_per_env,
                                             num_envs,
                                             *obs_shape,
                                             device=self.device)

        # next share observation = next_share_obs
        self.next_share_obs = torch.zeros(num_transitions_per_env,
                                          num_envs,
                                          *share_obs_shape,
                                          device=self.device)

        self.actions = torch.zeros(num_transitions_per_env,
                                   num_envs,
                                   *actions_shape,
                                   device=self.device)

        #share actions
        self.joint_actions = torch.zeros(num_transitions_per_env,
                                         num_envs,
                                         joint_act_dim,
                                         device=self.device)
    ...
    def add_transitions(self, observations, share_obs, actions, joint_actions,
    ...
    def mini_batch_generator(self, num_mini_batches):
        ...
        for _ in range(num_mini_batches):
            if self.fullfill:
                subset = random.sample(range(self.num_transitions_per_env),
                                       mini_batch_size)
            else:
