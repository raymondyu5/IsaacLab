...
from scripts.multiagent.marl.maddpg import MADDPG as TrainAlgo
from scripts.multiagent.marl.maddpg import MADDPG_policy as Policy
from scripts.multiagent.marl.maddpg import ReplayBuffer
...
class Runner:
    ...
    def run(self):
        ...
        train_episode_rewards = torch.tensor(train_episode_rewards,
                                             dtype=torch.float,
                                             device=self.device)
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                next_obs, next_share_obs, rewards, dones, infos, _ = self.envs.step(
                    actions)
            ...
            if len(done_episodes_rewards) != 0:
                aver_episode_rewards = torch.mean(
                    torch.stack(done_episodes_rewards))
    ...
    def collect(self, step):
        ...
        for agent_id in range(self.num_agents):
            ...
            action = self.trainer.policy[agent_id].act(
                self.buffer[agent_id].obs[step], deterministic=False)
    ...
    def insert(self, data):
        ...
        for agent_id in range(self.num_agents):
            self.buffer[agent_id].add_transitions(
                obs[:, agent_id], share_obs[:, agent_id], actions[agent_id],
                joint_actions[:], rewards[:, agent_id], next_obs[:, agent_id],
