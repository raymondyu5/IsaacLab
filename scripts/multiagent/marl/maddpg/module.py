...
from scripts.multiagent.marl.utils.util import get_gard_norm
...
class Actor(nn.Module):
    ...
    def __init__(self,
        ...
        self.pi = MLPActLayer(obs_dim, act_dim, hidden_sizes, activation,
                              act_limit)
...
class Critic(nn.Module):
    ...
    def __init__(self,
        ...
        # build policy and value functions
        self.q = MLPQFunction(share_obs_dim, share_act_dim, hidden_sizes,
                              activation)
...
class MADDPG_policy:
    ...
    def __init__(self,
        ...
        self.actor = Actor(self.obs_space, self.act_space, self.hidden_size,
                           self.activation, self.device)
        self.critic = Critic(self.share_obs_space, self.share_act_space,
                             self.hidden_size, self.activation, self.device)
    ...
    def act(self, obs, deterministic=False):
        ...
        if deterministic == True:
            ...
            actions = torch.clamp(
                actions +
                self.act_noise * torch.randn(actions.shape).to(self.device),
...
class MADDPG():
    ...
    def __init__(self, config, policy, num_agents, device=torch.device("cpu")):
    ...
    def ddpg_update(self, samples):
        ...
        for id in range(self.num_agents):
            ...
            loss_q.backward()
            nn.utils.clip_grad_norm_(self.policy[id].critic.parameters(),
                                     self.max_grad_norm)
            ...
            loss_pi.backward()
            nn.utils.clip_grad_norm_(self.policy[id].actor.parameters(),
                                     self.max_grad_norm)
            ...
            with torch.no_grad():
                for p, p_targ in zip(
                        self.policy[id].critic.q.parameters(),
                        self.policy[id].critic_targ.q.parameters()):
                ...
                for p, p_targ in zip(
                        self.policy[id].actor.pi.parameters(),
                        self.policy[id].actor_targ.pi.parameters()):
                    # NB: We use an in-place operations "mul_", "add_" to update target
                    # params, as opposed to "mul" and "add", which would make new tensors.
                    p_targ.data.mul_(self.polyak)
                    p_targ.data.add_((1 - self.polyak) * p.data)
