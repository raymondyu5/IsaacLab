...
from scripts.multiagent.separated_buffer import SeparatedReplayBuffer
...
class Runner:
    ...
    def __init__(self, vec_env, config, model_dir=""):
        ...
        self.hidden_size = config["hidden_size"][-1]
        ...
        self.log_dir = str(self.run_dir + '/' + self.env_name + '/' +
                           self.algorithm_name +
                           '/logs_seed{}'.format(self.seed))
        if not os.path.exists(self.log_dir):
            os.makedirs(self.log_dir)
        self.writter = SummaryWriter(self.log_dir)
        self.save_dir = str(self.run_dir + '/' + self.env_name + '/' +
                            self.algorithm_name +
                            '/models_seed{}'.format(self.seed))
        if not os.path.exists(self.save_dir):
            os.makedirs(self.save_dir)

        if self.algorithm_name == "happo":

            from scripts.multiagent.marl.happo_trainer import HAPPO as TrainAlgo
            from scripts.multiagent.marl.happo_policy import HAPPO_Policy as Policy
        if self.algorithm_name == "hatrpo":
            from scripts.multiagent.marl.hatrpo_trainer import HATRPO as TrainAlgo
            from scripts.multiagent.marl.hatrpo_policy import HATRPO_Policy as Policy
        if self.algorithm_name == "mappo":
            from scripts.multiagent.marl.mappo_trainer import MAPPO as TrainAlgo
            from scripts.multiagent.marl.mappo_policy import MAPPO_Policy as Policy
    ...
    def run(self, num_learning_iterations):
        self.warmup()
        ...
        train_episode_rewards = torch.zeros(1,
        ...
        for episode in range(episodes):
            ...
            all_rewards = torch.zeros(self.n_rollout_threads,
                                      self.num_agents,
                                      device=self.device)
            ...
            for step in range(self.episode_length):
                ...
                obs, share_obs, rewards, dones, infos, _ = self.envs.step_wait(
                    actions)
            ...
            if episode % self.log_interval == 0:
                ...
                print(
                    ...
                    .format(self.algorithm_name, self.experiment_name, episode,
                            episodes, total_num_steps, self.num_env_steps,
            ...
            if len(done_episodes_rewards) != 0:
                ...
                aver_episode_rewards = torch.stack(
                    done_episodes_rewards).mean()
                print("some episodes done, average rewards: ",
                      aver_episode_rewards.item())

                print(f"time: {end - start}")
                self.writter.add_scalars(
                    "train_episode_rewards",
                    {"aver_rewards": aver_episode_rewards}, total_num_steps)
    ...
    def insert(self, data):
        ...
        rnn_states[dones_env == True] = torch.zeros((dones_env == True).sum(),
                                                    self.num_agents,
                                                    self.recurrent_N,
                                                    self.hidden_size,
                                                    device=self.device)
        ...
        masks = torch.ones(self.n_rollout_threads,
                           ...
                           1,
                           device=self.device)
        masks[dones_env == True] = torch.zeros((dones_env == True).sum(),
                                               self.num_agents,
                                               1,
                                               device=self.device)

        active_masks = torch.ones(self.n_rollout_threads,
                                  self.num_agents,
                                  1,
                                  device=self.device)
        active_masks[dones == True] = torch.zeros((dones == True).sum(),
                                                  1,
                                                  device=self.device)
        active_masks[dones_env == True] = torch.ones((dones_env == True).sum(),
                                                     self.num_agents,
                                                     1,
                                                     device=self.device)
        ...
        for agent_id in range(self.num_agents):
            self.buffer[agent_id].insert(
                ...
                masks[:, agent_id], None, active_masks[:, agent_id], None)
    ...
    def train(self):
        ...
        action_dim = 1
        factor = torch.ones(self.episode_length,
                            self.n_rollout_threads,
                            action_dim,
                            device=self.device)
        ...
        for agent_id in torch.randperm(self.num_agents):
            ...
            action_prod = torch.exp((new_actions_logprob.detach() -
                                     old_actions_logprob.detach()).reshape(
                                         self.episode_length,
                                         self.n_rollout_threads,
                                         action_dim).sum(dim=-1, keepdim=True))
    ...
    def eval(self, total_num_steps):
        ...
        eval_rnn_states = torch.zeros(self.n_eval_rollout_threads,
                                      self.num_agents,
                                      self.recurrent_N,
                                      self.hidden_size,
                                      device=self.device)
        eval_masks = torch.ones(self.n_eval_rollout_threads,
                                self.num_agents,
                                1,
                                device=self.device)
        ...
        while True:
            ...
            # Obser reward and next obs
            eval_obs, eval_share_obs, eval_rewards, eval_dones, eval_infos, _ = self.eval_envs.step_wait(
                eval_actions)
            ...
            for eval_i in range(self.n_eval_rollout_threads):
                if eval_dones_env[eval_i]:
                    ...
                    eval_episode_rewards.append(
                        torch.sum(torch.cat(one_episode_rewards[eval_i]),
                                  dim=0))
            ...
            if eval_episode >= self.eval_episodes:
                ...
                eval_env_infos = {
                    ...
                    torch.max(eval_episode_rewards)
                }
