...
from scripts.multiagent.marl.utils.util import init, check
from scripts.multiagent.utils.cnn import CNNBase
from scripts.multiagent.utils.mlp import MLPBase
from scripts.multiagent.utils.rnn import RNNLayer
from scripts.multiagent.utils.act import ACTLayer
from scripts.multiagent.marl.utils.util import get_shape_from_obs_space
...
class Actor(nn.Module):
    ...
    def __init__(self,
        ...
        if self._use_naive_recurrent_policy or self._use_recurrent_policy:
            self.rnn = RNNLayer(self.hidden_size[-1], self.hidden_size[-1],
                                self._recurrent_N, self._use_orthogonal)

        self.act = ACTLayer(action_space, self.hidden_size[-1],
...
class Critic(nn.Module):
    ...
    def __init__(self, config, cent_obs_space, device=torch.device("cuda:0")):
        ...
        self.v_out = init_(nn.Linear(self.hidden_size[-1], 1))
