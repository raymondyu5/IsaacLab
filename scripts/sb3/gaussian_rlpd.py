...
from scripts.sb3.gaussian import GaussianModel

log = logging.getLogger(__name__)
from scripts.sb3.mlp_gaussian import Gaussian_MLP, MLP
...
class RLPD_Gaussian(GaussianModel):
    ...
    def critic_wrapper(self, params, buffers, data):
        """for vmap"""
        return torch.func.functional_call(self.base_model, (params, buffers),
                                          data)
    ...
    def loss_actor(self, obs, alpha):
        ...
        current_q = torch.vmap(self.critic_wrapper,
                               in_dims=(0, 0,
                                        None))(self.ensemble_params,
                                               self.ensemble_buffers,
                                               (obs, action))  # (n_critics, B)
    ...
    def update_target_critic(self, tau):
        ...
        for target_ind, target_critic in enumerate(self.target_networks):
            for target_param_name, target_param in target_critic.named_parameters(
                ...
                source_param = self.ensemble_params[target_param_name][
                    target_ind]
                target_param.data.copy_(target_param.data * (1.0 - tau) +
                                        source_param.data * tau)


actor = MLP(dim_list=[256, 128, 64, 7])
actor.moduleList
critic = MLP(dim_list=[256, 128, 64, 1])
rlpd_network = RLPD_Gaussian(
    actor,
    critic,
    n_critics=5,
)
...
class RLPDModel(nn.Module):

    def __init__(self, rlpd_network, device="cuda"):
        super(RLPDModel, self).__init__()
        self.device = device

        self.critic = rlpd_network.critic_networks.to(self.device)

        # target critic
        self.critic_target = copy.deepcopy(self.critic)

        # reassign actor
        self.actor = rlpd_network.actor.to(self.device)
