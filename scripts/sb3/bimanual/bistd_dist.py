...
SelfDiagGaussianDistribution = TypeVar("SelfDiagGaussianDistribution",
                                       bound="DiagGaussianDistribution")
SelfSquashedDiagGaussianDistribution = TypeVar(
    "SelfSquashedDiagGaussianDistribution",
    bound="SquashedDiagGaussianDistribution")
SelfCategoricalDistribution = TypeVar("SelfCategoricalDistribution",
                                      bound="CategoricalDistribution")
SelfMultiCategoricalDistribution = TypeVar(
    "SelfMultiCategoricalDistribution", bound="MultiCategoricalDistribution")
SelfBernoulliDistribution = TypeVar("SelfBernoulliDistribution",
                                    bound="BernoulliDistribution")
SelfStateDependentNoiseDistribution = TypeVar(
    "SelfStateDependentNoiseDistribution",
    bound="StateDependentNoiseDistribution")
...
class Distribution(ABC):
    ...
    @abstractmethod
    def proba_distribution(self: SelfDistribution, *args,
                           **kwargs) -> SelfDistribution:
...
class DiagGaussianDistribution(Distribution):
    ...
    def proba_distribution_net(self,
        ...
        log_left_std = nn.Parameter(th.ones(int(self.action_dim / 2)) *
                                    log_std_init,
                                    requires_grad=True)
        log_right_std = nn.Parameter(th.ones(int(self.action_dim / 2)) *
                                     log_std_init,
                                     requires_grad=True)
        return mean_actions, log_left_std, log_right_std

    def proba_distribution(
            self: SelfDiagGaussianDistribution, mean_actions: th.Tensor,
            log_left_std: th.Tensor,
            log_right_std: th.Tensor) -> SelfDiagGaussianDistribution:
        """
        ...
        :return:
        """
        left_action_std = th.ones_like(
            mean_actions[..., :int(self.action_dim / 2)]) * log_left_std.exp()
        self.left_distribution = Normal(
            mean_actions[..., :int(self.action_dim / 2)], left_action_std)
        right_action_std = th.ones_like(
            mean_actions[...,
                         int(self.action_dim / 2):]) * log_right_std.exp()
        self.right_distribution = Normal(
            mean_actions[..., int(self.action_dim / 2):], right_action_std)
    ...
    def log_prob(self, actions: th.Tensor) -> th.Tensor:
        ...
        :return:
        """
        left_log_prob = self.left_distribution.log_prob(
            actions[..., :int(self.action_dim / 2)])
        right_log_prob = self.right_distribution.log_prob(
            actions[..., int(self.action_dim / 2):])

        return sum_independent_dims(
            th.cat([left_log_prob, right_log_prob], dim=-1))

    def entropy(self) -> Optional[th.Tensor]:

        left_entropy = self.left_distribution.entropy()
        right_entropy = self.right_distribution.entropy()
        return sum_independent_dims(
            th.cat([left_entropy, right_entropy], dim=-1))

    def sample(self) -> th.Tensor:
        # Reparametrization trick to pass gradients

        left_sample = self.left_distribution.rsample()
        right_sample = self.right_distribution.rsample()
        return th.cat((left_sample, right_sample), dim=-1)

    def mode(self) -> th.Tensor:

        left_mean = self.left_distribution.mean
        right_mean = self.right_distribution.mean

        return th.cat((left_mean, right_mean), dim=-1)
...
class SquashedDiagGaussianDistribution(DiagGaussianDistribution):
    ...
    def proba_distribution(
            ...
            mean_actions: th.Tensor,
            log_std: th.Tensor) -> SelfSquashedDiagGaussianDistribution:
...
class CategoricalDistribution(Distribution):
    ...
    def proba_distribution(
            ...
            action_logits: th.Tensor) -> SelfCategoricalDistribution:
...
class MultiCategoricalDistribution(Distribution):
    ...
    def log_prob(self, actions: th.Tensor) -> th.Tensor:
        ...
        return th.stack([
            dist.log_prob(action) for dist, action in zip(
                self.distribution, th.unbind(actions, dim=1))
        ],
                        dim=1).sum(dim=1)
    ...
    def mode(self) -> th.Tensor:
        return th.stack(
            [th.argmax(dist.probs, dim=1) for dist in self.distribution],
            dim=1)
...
class BernoulliDistribution(Distribution):
    ...
    def proba_distribution(
            ...
            action_logits: th.Tensor) -> SelfBernoulliDistribution:
...
class StateDependentNoiseDistribution(Distribution):
    ...
    def proba_distribution(
            self: SelfStateDependentNoiseDistribution, mean_actions: th.Tensor,
            log_std: th.Tensor,
            latent_sde: th.Tensor) -> SelfStateDependentNoiseDistribution:
        """
        Create the distribution given its parameters (mean, std)
...
def kl_divergence(dist_true: Distribution,
                  dist_pred: Distribution) -> th.Tensor:
    """
    ...
    if isinstance(dist_pred, MultiCategoricalDistribution):
        assert isinstance(
            dist_true,
            MultiCategoricalDistribution)  # already checked above, for mypy
        assert np.allclose(
    ...
    # Use the PyTorch kl_divergence implementation
        ...
        return th.distributions.kl_divergence(dist_true.distribution,
