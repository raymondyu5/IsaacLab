...
from scripts.sb3.on_policy_algorithm import OnPolicyAlgorithm
...
SelfPPOBC = TypeVar("SelfPPOBC", bound="PPOBC")


class PPOBC(OnPolicyAlgorithm):
    """
    ...
    def compute_bc_loss(self):
        ...
        indices = np.random.choice(self.bc_buffer.buffer_size,
                                   size=self.bc_batch_size)
        obs = self.bc_buffer.observations[indices]
        act = self.bc_buffer.actions[indices]

        obs = th.as_tensor(obs).to(self.device)
        act = th.as_tensor(act).to(self.device)

        dist = self.policy.get_distribution(obs)
        mode = dist.distribution.mode
        mse_loss = th.nn.MSELoss(reduction='none')
        bc_policy_loss = mse_loss(mode, act)
        return th.mean(bc_policy_loss)
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                loss = policy_loss + self.ent_coef * entropy_loss + self.vf_coef * value_loss + bc_loss * self.bc_coef
        ...
        #     self.rollout_buffer.values.cpu().numpy().flatten(),
        #     self.rollout_buffer.returns.cpu().numpy().flatten())
    ...
    def _excluded_save_params(self) -> list[str]:
        return super()._excluded_save_params() + ["bc_buffer"]
