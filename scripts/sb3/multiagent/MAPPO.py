...
from scripts.sb3.multiagent.multiagent_on_policy_algorithm import OnPolicyAlgorithm
...
class MAPPO:
    ...
    def __init__(self,
        ...
        self.share_policy = args_cli.share_policy
        
        if self.share_policy:
            self.num_policies = 1
            self.train = self.train_shared_policy
        else:
            self.num_policies = 2
            self.train = self.train_multi_policy
        
        for _ in range(self.num_policies):

            hand_policy = OnPolicyAlgorithm(
                policy_aliases[policy],
                env,
                learning_rate=learning_rate,
                n_steps=n_steps *
                np.min([2, self.num_agents]) if self.share_policy else n_steps,
                gamma=gamma,
                ...
                share_policy=args_cli.share_policy,
            ...
            self.policies.append(hand_policy)
    ...
    def train_shared_policy(self) -> None:
        """
            ...
            for epoch in range(self.n_epochs):
                ...
                for rollout_data in hand_policy.rollout_buffer.get(
                    ...
                    values, log_prob, entropy = hand_policy.policy.evaluate_actions(
                        ...
                        rollout_data.shared_observations, actions)
    ...
    def train_multi_policy(self) -> None:
        """
        Update policy using the currently gathered rollout buffer.
        """
        # Switch to train mode (this affects batch norm / dropout)
    ...
    def _dump_logs(self, iteration: int) -> None:
        ...
        if self.share_policy:
            ...
            for agent_id in range(self.num_agents):
                ...
                hand_policy.logger.record(
                    f"rollout/agent_{agent_id}_ep_rew_mean",
                    self.last_per_hand_rollout_reward[agent_id].tolist())
            ...
            for _, hand_policy in enumerate(self.policies):
                time_elapsed = max(
                    (time.time_ns() - hand_policy.start_time) / 1e9,
                    sys.float_info.epsilon)
                fps = int((hand_policy.num_timesteps -
                           hand_policy._num_timesteps_at_start) / time_elapsed)
                hand_policy.logger.record("time/iterations",
    ...
    def collect_rollouts(
        ...
        self.last_per_hand_rollout_reward = np.zeros(self.num_agents, )
        num_rollouts = 0
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                values = []
                log_probs = []
                all_obs = obs_as_tensor(
                        self._last_obs.reshape(self._last_obs.shape[0], -1),
                        self.device)
                ...
                if self.share_policy:
                    ...
                    for agent_id in range(self.num_agents):
                        ...
                        action, value, log_prob = hand_policy.policy(
                            obs_tensor, all_obs)
            ...
            clipped_actions = np.clip(actions,
                                      self.policies[-1].action_space.low,
                                      self.policies[-1].action_space.high)
            ...
            for idx in range(len(dones)):  # loop over envs
                ...
                with th.no_grad():
                    ...
                    if self.share_policy:
                        ...
                        agent_terminal_obs = terminal_obs.unsqueeze(0)
                        terminal_value = self.policies[0].policy.predict_values(
                            agent_terminal_obs)[0].clone()

                        for agent_id in range(self.num_agents):
                            rewards[idx][agent_id] += self.gamma * terminal_value
                    else:
                        for agent_id in range(len(self.policies)):
                            # Predict value for bootstrapping
                            agent_terminal_obs = terminal_obs.unsqueeze(0)
                            terminal_value = self.policies[agent_id].policy.predict_values(
                                agent_terminal_obs)[0].clone()
            ...
            if not self.share_policy:
                ...
                for agent_id in range(self.num_agents):
                    self.policies[0].rollout_buffer.add(
                        self._last_obs[:, agent_id],  # type: ignore[arg-type]
                        self._last_obs.reshape(self._last_obs.shape[0], -1),
                        actions[:, agent_id],  # type: ignore[arg-type]
                        actions.reshape(actions.shape[0], -1),
                    ...
                    self.policies[0]._last_episode_starts = dones[:, agent_id]

            self._last_obs = new_obs  # type: ignore[assignment]

            self.last_rollout_reward += rewards.sum()

            self.last_per_hand_rollout_reward += np.mean(rewards, axis=0)

        self.last_rollout_reward /= num_rollouts

        with th.no_grad():

            # Compute value for the last timesteps
            if self.share_policy:

                new_obs_tensor = obs_as_tensor(
                    new_obs.reshape(new_obs.shape[0], -1), self.device).clone()

                # Compute value for the last timestep
                values = self.policies[0].policy.predict_values(new_obs_tensor)

                self.policies[0].rollout_buffer.compute_returns_and_advantage(
