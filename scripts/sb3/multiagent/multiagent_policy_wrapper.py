...
class BaseModel(nn.Module):
    ...
    def __init__(
        ...
        shared_observation_space: spaces.Space,
    ...
    def make_features_extractor(self,
                                observation_space) -> BaseFeaturesExtractor:
        """Helper method to create a features extractor."""
        return self.features_extractor_class(observation_space,
...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(
        ...
        self.pi_features_extractor = self.make_features_extractor(
            self.observation_space)
        self.policy_features_dim = self.pi_features_extractor.features_dim
    ...
    def _build_mlp_extractor(self) -> None:
        ...
        self.mlp_policy_extractor = MlpExtractor(
            self.policy_features_dim,
            net_arch=self.net_arch,
            activation_fn=self.activation_fn,
            device=self.device,
        )
        if not self.use_multi_agent:
            self.mlp_vf_extractor = self.mlp_policy_extractor

        else:
            self.mlp_vf_extractor = MlpExtractor(
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        latent_dim_pi = self.mlp_policy_extractor.latent_dim_pi
        ...
        self.value_net = nn.Linear(self.mlp_vf_extractor.latent_dim_vf, 1)
        ...
        if self.ortho_init:
            ...
            module_gains = {
                ...
                self.mlp_policy_extractor: np.sqrt(2),
                self.mlp_vf_extractor: np.sqrt(2),
        ...
        # else:
        #     self.actor_parameters = list(self.pi_features_extractor.parameters()) + \
        #                             list(self.mlp_policy_extractor.parameters()) + \
        #                             list(self.action_net.parameters())

        #     # DiagGaussian case (log_std is a learnable parameter)
        #     if isinstance(self.action_dist, (DiagGaussianDistribution, StateDependentNoiseDistribution)):
        #         self.actor_parameters += [self.log_std]

        #     self.critic_parameters = list(self.vf_features_extractor.parameters()) + \
        #                             list(self.mlp_vf_extractor.parameters()) + \
        #                             list(self.value_net.parameters())

        #     self.actor_optimizer = self.optimizer_class(
        #         self.actor_parameters, **self.optimizer_kwargs)
        #     self.critic_optimizer = self.optimizer_class(
        #         self.critic_parameters, **self.optimizer_kwargs)
    ...
    def forward(self,
        ...
        features = self.extract_features(obs, all_obs)

        pi_features, vf_features = features
        latent_pi = self.mlp_policy_extractor.forward_actor(pi_features)
        latent_vf = self.mlp_vf_extractor.forward_critic(vf_features)
    ...
    def extract_features(  # type: ignore[override]
        ...
        vf_features = super().extract_features(shared_obs,
    ...
    def evaluate_actions(
        self, obs: PyTorchObs, shared_obs: PyTorchObs, actions: th.Tensor
        ...
        """
        # Preprocess the observation if needed
        features = self.extract_features(obs, shared_obs)
    ...
    def get_distribution(self, obs: PyTorchObs) -> Distribution:
        ...
        latent_pi = self.mlp_policy_extractor.forward_actor(features)
    ...
    def predict_values(self, obs: PyTorchObs) -> th.Tensor:
        ...
        latent_vf = self.mlp_vf_extractor.forward_critic(features)
