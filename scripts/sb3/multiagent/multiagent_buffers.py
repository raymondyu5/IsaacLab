...
class RolloutBufferSamples(NamedTuple):
    ...
    shared_observations: th.Tensor
    actions: th.Tensor
    shared_actions: th.Tensor
...
class BaseBuffer(ABC):
    ...
    def __init__(
        ...
        shared_action_space: spaces.Space,
        ...
        self.shared_obs_shape = get_obs_shape(shared_observation_space)
        self.shared_action_space = shared_action_space

        self.action_dim = get_action_dim(action_space)
        self.shared_action_dim = get_action_dim(shared_action_space)
        self.pos = 0
        self.full = False
    ...
    def sample(self,
        ...
        return self._get_samples(batch_inds, env=env, use_numpy=use_numpy)
...
class RolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.advantages = np.zeros((self.buffer_size, self.n_envs),
                                   dtype=np.float32)
        self.shared_observations = np.zeros(
            (self.buffer_size, self.n_envs, *self.shared_obs_shape),
            dtype=np.float32)
        self.shared_actions = np.zeros(
            (self.buffer_size, self.n_envs, self.shared_action_dim),
            dtype=np.float32)
        self.generator_ready = False
        super().reset()
    ...
    def add(
        ...
        self.shared_actions[self.pos] = np.array(shared_action)
        self.shared_observations[self.pos] = np.array(shared_obs)
    ...
    def _get_samples(
        ...
        data = (
            ...
            self.shared_observations[batch_inds],
            self.actions[batch_inds],
            self.shared_actions[batch_inds],
