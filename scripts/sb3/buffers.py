...
class ReplayBuffer(BaseBuffer):
    ...
    def sample(self,
               batch_size: int,
               env: Optional[VecNormalize] = None,
               use_numpy=False) -> ReplayBufferSamples:
        """
    ...
    def _get_samples(self,
        ...
        if not use_numpy:
            ...
            return RolloutBufferNumpySamples(*data)
    ...
    def compute_returns_and_advantage(self, last_values: th.Tensor,
        ...
        for step in reversed(range(self.buffer_size)):
            if step == self.buffer_size - 1:
                if self.gpu_buffer:
                    next_non_terminal = 1.0 - dones.to(dtype=torch.float32)
                else:
    ...
    def add(
        ...
        if self.gpu_buffer:
            ...
            self.episode_starts[self.pos] = th.tensor(
                episode_start).clone().to(self.device)
    ...
    def add(  # type: ignore[override]
        ...
        for key in self.observations.keys():
            if not self.gpu_buffer:
                ...
                # Reshape needed when using multiple envs with discrete observations
                # as numpy cannot broadcast (n_discrete,) to (n_discrete, 1)
                if isinstance(self.observation_space.spaces[key],
                              spaces.Discrete):
        ...
        if self.gpu_buffer:
            ...
            self.episode_starts[self.pos] = th.tensor(
                episode_start).clone().to(self.device)
            self.values[self.pos] = value.clone().clone().flatten()
            self.log_probs[self.pos] = log_prob.clone()
        else:
            self.actions[self.pos] = np.array(action)
            self.rewards[self.pos] = np.array(reward)
