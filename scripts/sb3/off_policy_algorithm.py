...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            ...
            if self.num_timesteps > 0 and self.num_timesteps > self.learning_starts and self.num_timesteps % self.train_interval == 0:
    ...
    def _store_transition(
        ...
        diffusion_obs: Optional[np.ndarray] = None,
    ) -> None:
        """
        ...
        self.last_diffusion_obs = None

        if diffusion_obs is not None:
            diffusion_obs = self.env.env.wrapper.get_diffusion_obs()
            self.last_diffusion_obs = self.env.env.wrapper.last_diffusion_obs
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps,
            ...
            new_obs, rewards, dones, infos, raw_action, diffusion_obs = env.step(
                actions)
            ...
            self._store_transition(replay_buffer, buffer_actions, new_obs,
                                   rewards, dones, infos, raw_action,
                                   diffusion_obs)  # type: ignore[arg-type]
