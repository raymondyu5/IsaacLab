...
class DSRL(OffPolicyAlgorithm):
    ...
    def __init__(
        ...
        self.diffusion_policy = diffusion_model

        self.diffusion_act_chunk = diffusion_action_space.shape[0]
        self.diffusion_act_dim = diffusion_action_space.shape[1]
        self.diffusion_obs_space = diffusion_obs_space
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        if self.actor_gradient_steps < 0:
            actor_gradient_idx = np.linspace(0,
                                             gradient_steps - 1,
                                             gradient_steps,
                                             dtype=int)
        else:
        ...
        for gradient_step in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                next_actions = th.tensor(
                    self.policy.unscale_action(next_actions.cpu().numpy())).to(
                        self.device)  # noise aliased action

                next_diffused_actions = self.diffusion_policy.predict_action(
                    {"obs": replay_data.next_diffusion_observations},
                    next_actions.reshape(-1, self.diffusion_act_chunk,
                                         self.diffusion_act_dim),
                )["action_pred"]  # reshape back to original action

                next_diffused_actions = next_diffused_actions.reshape(
                    -1, self.diffusion_act_chunk * self.diffusion_act_dim)
                # Compute the next Q values: min over all critics targets

                next_q_values = th.cat(self.critic_target(
                    replay_data.next_observations, next_diffused_actions),
                                       dim=1)
            ...
            # actions = self.diffusion_policy.predict_action( {"obs": replay_data.diffusion_observations},replay_data.actions.reshape(-1, self.diffusion_act_chunk,   self.diffusion_act_dim), )["action_pred"]

            current_q_values = self.critic(
                replay_data.observations,
                replay_data.diffused_actions.reshape(
                    -1, self.diffusion_act_chunk * self.diffusion_act_dim).to(
                        th.float32),
            )
            ...
            if gradient_step in actor_gradient_idx:
                ...
                q_values_pi = th.cat(self.critic_noise(
        ...
        for gradient_step in range(self.noise_critic_grad_steps):
            ...
            replay_data = self.replay_buffer.sample(
                batch_size,
                env=self._vec_normalize_env)  # type: ignore[union-attr]
            critic_distill_loss = critic_distill_loss + self.update_noise_critic(
                replay_data)
        ...
        self.logger.record("train/noise_critic_loss",
                           np.mean(noise_critic_losses))
        if len(ent_coef_losses) > 0:
            self.logger.record("train/ent_coef_loss", np.mean(ent_coef_losses))

    def update_noise_critic(self, replay_data):
        with th.no_grad():
            noise_actions = th.randn(replay_data.actions.shape[0],
                                     self.diffusion_act_chunk,
                                     self.diffusion_act_dim).to(self.device)

            diffused_actions = self.diffusion_policy.predict_action(
                {"obs": replay_data.diffusion_observations},
                noise_actions,
            )["action_pred"]

            diffused_actions = diffused_actions.reshape(
                -1, self.diffusion_act_chunk * self.diffusion_act_dim)
            current_q_values = self.critic(replay_data.observations,
                                           diffused_actions)
        noise_actions = noise_actions.reshape(
            -1, self.diffusion_act_chunk *
            self.diffusion_act_dim).detach().cpu().numpy()
        noise_actions = th.tensor(self.policy.scale_action(noise_actions)).to(
            self.device)
        current_q_noise_vals = self.critic_noise(replay_data.observations,
                                                 noise_actions)
        ...
        for i in range(len(current_q_values)):
            ...
            critic_distill_loss = critic_distill_loss + 0.5 * F.mse_loss(
                current_q.detach(), current_q_noise)
    ...
    def _get_torch_save_params(self) -> tuple[list[str], list[str]]:
        state_dicts = [
            "policy", "actor.optimizer", "critic.optimizer", "critic_noise"
        ]
        if self.ent_coef_optimizer is not None:
