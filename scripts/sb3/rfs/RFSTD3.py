...
class RFSTD3(OffPolicyAlgorithm):
    """
    ...
    def __init__(
        ...
        replay_buffer_kwargs.update(
            raw_action_space=getattr(env, "raw_action_space", None))
        replay_buffer_kwargs.update(
            diffusion_obs_space=getattr(env, "diffusion_obs_space", None))
    ...
    def _create_aliases(self) -> None:
        self.critic_noise = self.policy.critic
            ...
            if not self.freeze_critic:
                ...
                with th.no_grad():
                    ...
                    with th.no_grad():
                        ...
                        reconstructed_hand_actions, residual_hand_action = self.env.env.wrapper.reconstruct_hand_action(
                            replay_data.next_difusion_observations,
                            next_noise_actions)

                        next_actions = self.env.env.wrapper.get_diffused_action(
                            next_noise_actions, reconstructed_hand_actions,
                            residual_hand_action, 0).detach()
                ...
                #     reconstructed_hand_actions, residual_hand_action = self.env.env.wrapper.reconstruct_hand_action(
                #         replay_data.difusion_observations, replay_data.actions)

                #     actions = self.env.env.wrapper.get_diffused_action(
                #         replay_data.actions, reconstructed_hand_actions,
                #         residual_hand_action, 0)
                actions = replay_data.raw_actions.to(th.float32)
    ...
    def update_noise_critic(self, replay_data):
        with th.no_grad():
            ...
            noise_actions = th.randn(*replay_data.actions.shape).to(
                self.device)
            reconstructed_hand_actions, residual_hand_action = self.env.env.wrapper.reconstruct_hand_action(
                replay_data.difusion_observations, noise_actions)

            diffused_actions = self.env.env.wrapper.get_diffused_action(
                replay_data.actions, reconstructed_hand_actions,
                residual_hand_action, 0).detach()
            current_q_values = self.critic(replay_data.observations,
                                           diffused_actions)

        noise_actions = noise_actions.detach().cpu().numpy()
