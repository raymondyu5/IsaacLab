...
class DSRLTD3(OffPolicyAlgorithm):
    """
    ...
    def __init__(
        ...
        self.diffusion_action_space = diffusion_action_space
        ...
        for _ in range(gradient_steps):
            ...
            if not self.freeze_critic:
                ...
                current_q_values = self.critic(
                    ...
                    replay_data.diffused_actions.reshape(
                        ...
                        self.diffusion_act_dim).to(th.float32),
                )
            ...
            if not self.freeze_actor:
                ...
                if self._n_updates % self.policy_delay == 0:
                    # Compute actor loss
                    noise_actions = self.actor(replay_data.observations)

                    diffused_actions = self.diffusion_policy.predict_action(
                        {"obs": replay_data.diffusion_observations},
                        noise_actions.reshape(-1, self.diffusion_act_chunk,
                                              self.diffusion_act_dim),
                    )["action_pred"]

                    actor_loss = -self.critic.q1_forward(
                        replay_data.observations,
                        diffused_actions.reshape(
                            -1, self.diffusion_act_chunk *
                            self.diffusion_act_dim).to(th.float32)).mean()
