...
class OnPolicyAlgorithm(BaseAlgorithm):
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            if not rollout_buffer.gpu_buffer:
            ...
            if isinstance(self.action_space, spaces.Box):
                if self.policy.squash_output:
                    ...
                    if rollout_buffer.gpu_buffer:
                        clipped_actions = th.clip(
                            actions,
                            th.as_tensor(self.action_space.low).to(
                                self.device),
                            th.as_tensor(self.action_space.high).to(
                                self.device))
        ...
        self.last_rollout_reward /= self.num_rollouts
    ...
    def _dump_logs(self, iteration: int) -> None:
        ...
        self.logger.dump(step=self.num_iter)
