...
from scripts.sb3.mlp import MLP, ResidualMLP
...
class Gaussian_VisionMLP(nn.Module):
    ...
    def __init__(
        ...
        self.logvar_min = torch.nn.Parameter(torch.log(torch.tensor(
            std_min**2)),
                                             requires_grad=False)
        self.logvar_max = torch.nn.Parameter(torch.log(torch.tensor(
            std_max**2)),
                                             requires_grad=False)
    ...
    def forward(self, cond):
        ...
        out_mean = torch.tanh(out_mean).view(
            B,
            self.horizon_steps * self.action_dim)  # tanh squashing in [-1, 1]

        if self.learn_fixed_std:
            out_logvar = torch.clamp(self.logvar, self.logvar_min,
                                     self.logvar_max)
            ...
            out_logvar = torch.clamp(out_logvar, self.logvar_min,
                                     self.logvar_max)
            out_scale = torch.exp(0.5 * out_logvar)
...
class Gaussian_MLP(nn.Module):
    ...
    def forward(self, cond):
        ...
        if self.learn_fixed_std:
            ...
            out_logvar = self.logvar_min + 0.5 * (
                self.logvar_max - self.logvar_min) * (
                    out_logvar + 1)  # put back to full range
            out_scale = torch.exp(0.5 * out_logvar)
        return out_mean, out_scale
