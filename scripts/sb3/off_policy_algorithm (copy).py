...
SelfOffPolicyAlgorithm = TypeVar("SelfOffPolicyAlgorithm",
                                 bound="OffPolicyAlgorithm")
...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def _convert_train_freq(self) -> None:
        ...
        if not isinstance(self.train_freq, TrainFreq):
            ...
            self.train_freq = TrainFreq(
                *train_freq)  # type: ignore[assignment,arg-type]

    def _setup_model(self) -> None:
        self._setup_lr_schedule()
        if not self.use_buffer_only:
    ...
    def load_replay_buffer(
        ...
        if not hasattr(self.replay_buffer,
                       "handle_timeout_termination"):  # pragma: no cover
            self.replay_buffer.handle_timeout_termination = False
            self.replay_buffer.timeouts = np.zeros_like(
                self.replay_buffer.dones)
    ...
    def _setup_learn(
        ...
        if not use_buffer_only:
            ...
            truncate_last_traj = (self.optimize_memory_usage
                                  ...
                                  and (replay_buffer.full
                                       or replay_buffer.pos > 0))
            ...
            if (self.action_noise is not None and self.env.num_envs > 1 and
                    not isinstance(self.action_noise, VectorizedActionNoise)):
                self.action_noise = VectorizedActionNoise(
                    self.action_noise, self.env.num_envs)

        return super()._setup_learn(total_timesteps, callback,
                                    reset_num_timesteps, tb_log_name,
                                    progress_bar, use_buffer_only)
    ...
    def learn(
        ...
        if self.use_buffer_only:
            return self.learn_buffer_only(total_timesteps, callback,
                                          log_interval, tb_log_name,
                                          reset_num_timesteps, progress_bar)
        else:

            return self.learn_offline(total_timesteps, callback, log_interval,
                                      tb_log_name, reset_num_timesteps,
                                      progress_bar)

    def learn_buffer_only(
        self: SelfOffPolicyAlgorithm,
        total_timesteps: int,
        callback: MaybeCallback = None,
        log_interval: int = 4,
        ...
        total_timesteps, callback = self._setup_learn(
            ...
            progress_bar, self.use_buffer_only)

        callback.on_training_start(locals(), globals())

        assert self.env is not None, "You must set the environment before calling learn()"
        assert isinstance(self.train_freq,
                          TrainFreq)  # check done in _setup_learn()
    ...
    def learn_offline(
        ...
        # for buffer_index in tqdm(range(self.warmup_steps)):
        #     self.train(batch_size=self.batch_size,
        #                gradient_steps=1,
        #                train_buffer=True,
        #                buffer_index=buffer_index)
        # callback._on_rollout_end(save_path=True)
    ...
    def train(self, gradient_steps: int, batch_size: int, **args) -> None:
        """
    ...
    def _dump_logs(self) -> None:
        ...
        if len(self.ep_info_buffer) > 0 and len(self.ep_info_buffer[0]) > 0:
            ...
            # Ignore the warning about the deprecated `safe_mean`
            self.logger.record(
                "rollout/ep_rew_mean",
        ...
        self.logger.dump(step=self.train_iter)
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps,
            ...
            #TODO: check if this is correct using the buffer actions
            ...
            self._store_transition(replay_buffer, buffer_actions, new_obs,
                                   rewards, dones,
            ...
            for idx, done in enumerate(dones):
                if done:
                    ...
                    if action_noise is not None:
                        kwargs = dict(
                            indices=[idx]) if env.num_envs > 1 else {}
                        action_noise.reset(**kwargs)

            if done:

                print("Rollout time: ", time.time() - self.start_time)
                self.start_time = time.time()
                # Log training infos
                if log_interval is not None and self._episode_num % log_interval % env.num_envs == 0:
                    self._dump_logs()
