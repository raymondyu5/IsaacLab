...
class TrainConfig:
    # Experiment
    device: str = "cuda"
    env: str = "TD3BC"  # OpenAI gym environment name
    seed: int = 0  # Sets Gym, PyTorch and Numpy seeds
    eval_freq: int = 10000  # How often (time steps) we evaluate
    ...
    name: str = "TD3_BC"
    state_dim: int = 0  # to be set
    action_dim: int = 0  # to be set
...
def wandb_init(config: dict) -> None:
    ...
    wandb.run.save("log/wandb")


@torch.no_grad()
def eval_actor(env: gym.Env, actor: nn.Module, device: str, n_episodes: int,
               seed: int) -> np.ndarray:
    env.seed(seed)
    actor.eval()
    episode_rewards = []
...
class TD3_BC:
    ...
    def train(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            bc_loos = F.mse_loss(pi, action)

            actor_loss = -lmbda * q.mean() + bc_loos
            log_dict["actor_loss"] = actor_loss.item()
            log_dict["bc_loss"] = bc_loos.item()
            # Optimize the actor
...
def load_td3_model(ckpt_path: str, ):
...
def train(config: TrainConfig, demo_data, use_image, low_dim_obs_shape,
    ...
    action_dim = env_info["action_shape"]
    ...
    for t in trange(int(config.max_timesteps), ncols=80):
...
if __name__ == "__main__":
    ...
    train(TrainConfig(checkpoints_path="logs/offlinerl/td3bc"), demo_data,
