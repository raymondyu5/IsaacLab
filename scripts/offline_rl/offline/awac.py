...
class TrainConfig:
    ...
    def __post_init__(self):
        ...
        #     self.checkpoints_path = os.path.join(self.checkpoints_path,
        #                                          self.name)
...
class Actor(nn.Module):
    ...
    def _get_policy(self,
                    state: torch.Tensor) -> torch.distributions.Distribution:
...
class AdvantageWeightedActorCritic:
    ...
    def _actor_loss(self, states, actions):
        with torch.no_grad():
            ...
            v = torch.min(self._critic_1(states, pi_action),
                          self._critic_2(states, pi_action))

            q = torch.min(self._critic_1(states, actions),
                          self._critic_2(states, actions))
            adv = q - v
            weights = torch.clamp_max(torch.exp(adv / self._awac_lambda),
                                      self._exp_adv_max)
    ...
    def update(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        critic_loss = self._update_critic(states, actions, rewards, dones,
                                          next_states)
...
@torch.no_grad()
def eval_actor(env: gym.Env, actor: Actor, device: str, n_episodes: int,
               seed: int) -> np.ndarray:
    env.seed(seed)
    actor.eval()
    episode_rewards = []
...
def wandb_init(config: dict) -> None:
    ...
    wandb.run.save("logs/wandb_run")


import yaml


def load_awac_model(ckpt_path: str, ) -> AdvantageWeightedActorCritic:
    print(f"Loading model from {ckpt_path}")
    config = TrainConfig()
    yaml_path = os.path.join(os.path.dirname(ckpt_path), "config.yaml")
    yaml_info = yaml.safe_load(open(yaml_path, "r"))

    actor_critic_kwargs = {
        "state_dim": yaml_info["state_dim"],
        "action_dim": yaml_info["action_dim"],
    ...
    awac.load_state_dict(torch.load(ckpt_path))
...
def train(config: TrainConfig, demo_data, use_image, low_dim_obs_shape,
          capacity):

    actions = demo_data["actions"]
    config.state_dim = low_dim_obs_shape
    ...
    replay_buffer.load_dataset(demo_data)
    env_info = {
        "obs_shape": low_dim_obs_shape,
        "action_shape": actions.shape[-1]
    }

    actor_critic_kwargs = {
        "state_dim": low_dim_obs_shape,
    ...
    if config.checkpoints_path is not None:
        ...
        with open(os.path.join(config.checkpoints_path, "config.yaml"),
                  "w") as f:
    ...
    for t in trange(config.num_train_ops, ncols=80):
        ...
        if (t + 1) % config.eval_frequency == 0:
            ...
            if config.checkpoints_path is not None:
                torch.save(
                    awac.state_dict(),
                    os.path.join(config.checkpoints_path,
                                 f"checkpoint_{t}.pt"),
                )
...
if __name__ == "__main__":
    ...
    train(TrainConfig(checkpoints_path="logs/offlinerl/awac"), demo_data,
          use_image, low_dim_obs_shape, capacity)
