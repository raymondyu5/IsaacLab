...
def randomize_shape_color(prim_path_expr: str):
    ...
    with Sdf.ChangeBlock():
        for prim_path in prim_paths:
            ...
            color_spec.default = Gf.Vec3f(random.random(), random.random(),
                                          random.random())
...
def run_simulator(sim: SimulationContext, scene: InteractiveScene):
    ...
    while simulation_app.is_running():
        ...
        if count % 250 == 0:
            ...
            rigid_object_collection.write_object_link_pose_to_sim(
                object_state[..., :7])
            rigid_object_collection.write_object_com_velocity_to_sim(
                object_state[..., 7:])
            # robot
            # -- root state
...
def main():
    ...
    # Design scene
    scene_cfg = MultiObjectSceneCfg(num_envs=args_cli.num_envs,
                                    env_spacing=2.0,
