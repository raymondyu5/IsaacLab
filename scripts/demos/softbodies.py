...
def define_origins(num_origins: int, spacing: float) -> list[list[float]]:
    ...
    xx, yy = torch.meshgrid(torch.arange(num_rows),
                            ...
                            indexing="xy")
    env_origins[:, 0] = spacing * xx.flatten()[:num_origins] - spacing * (
        num_rows - 1) / 2
    env_origins[:, 1] = spacing * yy.flatten()[:num_origins] - spacing * (
        num_cols - 1) / 2
...
def design_scene():
    ...
    for idx, origin in enumerate(origins):
        prim_utils.create_prim(f"/World/Origin{idx:02d}",
                               "Xform",
                               translation=origin)
    ...
    for idx, origin in tqdm.tqdm(enumerate(origins), total=len(origins)):
        ...
        # randomize the color
        obj_cfg.visual_material.diffuse_color = (random.random(),
                                                 random.random(),
                                                 random.random())
        # spawn the object
        obj_cfg.func(f"/World/Origin.*/Object{idx:02d}",
                     obj_cfg,
                     translation=origin)
