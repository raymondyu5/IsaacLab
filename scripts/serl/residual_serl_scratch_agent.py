...
if FLAGS.actor:  #(FLAGS.learner and FLAGS.debug) or FLAGS.debug or FLAGS.actor
    ...
    os.environ["XLA_PYTHON_CLIENT_PREALLOCATE"] = "false"
    # os.environ["XLA_PYTHON_CLIENT_MEM_FRACTION"] = "0.8"  # optional
...
from scripts.serl.env.serl_wrapper import SerlEnvWrapper, StateCustomSerlEnvWrapper, ImageCustomSerlEnvWrapper
...
def main():
    ...
    if FLAGS.actor:  # LAGS.debug or FLAGS.actor or (FLAGS.learner and FLAGS.debug)
        ...
        if args_cli.rl_type in ["sac", "rlpd"]:
            rl_agent_env = StateCustomSerlEnvWrapper()
        elif args_cli.rl_type in ["drq"]:
    ...
    with open(config_file, 'r') as file:
        serl_env_config = yaml.safe_load(file)
    ...
    if args_cli.rl_type == "sac":
        ...
        agent = SerlRLPDAgent
    elif "drq" in args_cli.rl_type:
        agent = DrqAgent

    serl_agent = agent(rl_agent_env,
                       FLAGS,
                       env_config=serl_env_config,
                       use_diffusion_model=True
                       if args_cli.action_framework is not None else False)
