...
from scripts.serl.utils.replay_buffer import ReplayBuffer
...
class MemoryEfficientReplayBuffer(ReplayBuffer):
    ...
    def insert(self, data_dict: DatasetDict):
        ...
        for pixel_key in self.pixel_keys:
            ...
            next_obs_pixels[pixel_key] = data_dict["next_observations"].pop(
                pixel_key)

        if self._first:
            for i in range(self._num_stack):
                for pixel_key in self.pixel_keys:
                    data_dict["observations"][pixel_key] = obs_pixels[
                        pixel_key][i]
