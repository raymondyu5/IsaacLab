...
from scripts.serl.utils.memory_efficient_replay_buffer import (
...
class MemoryEfficientReplayBufferDataStore(MemoryEfficientReplayBuffer,
    ...
    def __init__(
        ...
        self.gemini_rewards = gemini_rewards
    ...
    def insert(self, data):
        ...
        self.store_data_buffer.append(data)

        if len(self.store_data_buffer) >= self.flush_every or data["dones"]:

            for data in self.store_data_buffer:
                self.flush(data)
            self.store_data_buffer = []
