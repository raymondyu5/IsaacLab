...
class ResidualSACAgent(flax.struct.PyTreeNode):
    """
    ...
    def forward_target_critic(
        ...
        Pass grad_params to use non-default parameters (e.g. for gradients).
        """
        return self.forward_critic(observations,
                                   actions,
                                   rng=rng,
    ...
    def forward_temperature(
            self,
            *,
            grad_params: Optional[Params] = None) -> distrax.Distribution:
        """
    ...
    def _process_action(self, action, base_action):
        if self.config.get("critic_input") == "res":
            return action

        elif self.config.get("critic_input", "sum") == "sum":
            hand_action_boud = base_action[:, -16:]
            delta_hand_action = ((action[:, -1][..., None].clip(-1, 1) + 1) / 2 
                                * 2 * hand_action_boud - hand_action_boud)

            if action.shape[-1] > 1:
                arm_action_boud = base_action[:, -22:-6]
                arm_action = action[:, :6]
                delta_arm_action = ((arm_action.clip(-1, 1) + 1) / 2 
                                    * 2 * arm_action - arm_action)
                concat_action = jnp.concatenate([delta_arm_action, delta_hand_action], axis=-1)
                new_base_action = base_action[:, :22] + concat_action
            else:
                new_base_action = base_action[:, :22].at[..., -16:].add(delta_hand_action[..., -16:])

            return new_base_action

        elif self.config.get("critic_input") == "concat":
            actions = jnp.concatenate([action, base_action[:, :22]], axis=-1)
            return actions
    ...
    def critic_loss_fn(self, batch, params: Params, rng: PRNGKey):
        ...
        # if self.config.get("critic_input", "sum") == "res":
        #     next_actions = next_actions
        # elif self.config.get("critic_input", "sum") == "sum":
        #     action_boud = batch["next_base_actions"][:,-22:]
        #     delta_action = (next_actions.clip(-1,1)+1)/2 *2*action_boud-action_boud
        #     next_actions = batch["next_base_actions"][:,:22] + delta_action
        # elif self.config.get("critic_input", "concat"):
        #     next_actions = jnp.concatenate(
        #         [next_actions, batch["base_actions"][:,:22]], axis=-1)
        next_actions = self._process_action(next_actions,batch["next_base_actions"])
      
        # Evaluate next Qs for all ensemble members (cheap because we're only doing the forward pass)
        target_next_qs = self.forward_target_critic( batch["next_observations"],next_actions,   rng=rng,)  # (critic_ensemble_size, batch_size)
        ...
        # elif self.config.get("critic_input", "sum") == "sum":
        #     action_boud = batch["base_actions"][:,-22:]
        #     delta_action = (batch["actions"].clip(-1,1)+1)/2 *2*action_boud-action_boud
        #     actions = batch["base_actions"][:,:22] + delta_action
        # elif self.config.get("critic_input", "concat"):
        #     actions = jnp.concatenate(
        #         [batch["actions"], batch["base_actions"][:,:22]], axis=-1)
        actions = self._process_action(batch["actions"],batch["base_actions"])

        predicted_qs = self.forward_critic(
            batch["observations"],
            actions,  # add base action to critic input
            rng=rng,
            grad_params=params)

        chex.assert_shape(predicted_qs,
                          (self.config["critic_ensemble_size"], batch_size))
        target_qs = target_q[None].repeat(self.config["critic_ensemble_size"],
                                          axis=0)
    ...
    def policy_loss_fn(self, batch, params: Params, rng: PRNGKey):
        ...
        action_distributions = self.forward_policy(batch["observations"],
                                                   rng=policy_rng,
                                                   grad_params=params)
        ...
        #     delta_action = (actions.clip(-1,1)+1)/2 *2*action_boud-action_boud
        #     actions = batch["base_actions"][:,:22] + delta_action

        # elif self.config.get("critic_input", "concat"):
        #     actions = jnp.concatenate([actions, batch["base_actions"][:,:22]],
        #                               axis=-1)
        actions = self._process_action(batch["actions"],batch["base_actions"])
    ...
    def update(
        ...
        *,
        ...
        new_state, info = self.state.apply_loss_fns(loss_fns,
                                                    pmap_axis=pmap_axis,
                                                    has_aux=True)
    ...
    def create_pixels(
        ...
        **kwargs,
        ...
        critic_def = partial(Critic,
                             encoder=encoders["critic"],
                             network=critic_backbone)(name="critic")
    ...
    def create_states(
        ...
        **kwargs,
        ...
        critic_def = ensemblize(critic_cls,
                                critic_ensemble_size)(name="critic")
        temperature_def = GeqLagrangeMultiplier(
            init_value=temperature_init,
            constraint_shape=(),
            constraint_type="geq",
    ...
    def update_high_utd(
        ...
        *,
        ...
        (agent, ), critic_infos = jax.lax.scan(scan_body, (self, ),
                                               (minibatches, ))

        critic_infos = jax.tree_map(lambda x: jnp.mean(x, axis=0),
                                    critic_infos)
