...
from scripts.serl.agent.residual_sac import ResidualSACAgent


class ResidualDrQAgent(ResidualSACAgent):
    ...
    def create(
        ...
        import numpy as np
        critic_action = np.zeros(22)
        if critic_input in ['sum']:
            params = model_def.init(
                init_rng,
                actor=[observations],
                critic=[observations, critic_action],
                temperature=[],
            )["params"]
        elif critic_input == 'concat':
            import numpy as np
            concat_actions = np.concatenate([actions, critic_action], axis=-1)
            params = model_def.init(
                init_rng,
                actor=[observations],
                critic=[observations, concat_actions],
                temperature=[],
            )["params"]
        elif critic_input == 'res':
    ...
    def data_augmentation_fn(self, rng, observations):
        for pixel_key in self.config["image_keys"]:
            observations = observations.copy(
                add_or_replace={
                    pixel_key:
                    batched_random_crop(observations[pixel_key],
                                        rng,
                                        padding=4,
                                        num_batch_dims=2)
                })
    ...
    def update_high_utd(
        ...
        *,
        ...
        next_obs = self.data_augmentation_fn(next_obs_rng,
        ...
        return SACAgent.update_high_utd(new_agent,
                                        batch,
                                        utd_ratio=utd_ratio,
                                        pmap_axis=pmap_axis)
