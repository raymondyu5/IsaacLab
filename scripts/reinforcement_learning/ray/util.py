    ...
    def get_latest_scalars(path: str) -> dict:
        ...
        try:
            ...
            if event_acc.Tags()["scalars"]:
                return {
                    replace_invalid_chars(tag):
                    event_acc.Scalars(tag)[-1].value
        ...
        for key, value in args.items():
            if not is_hydra:
                if key.endswith("_singleton"):
                    ...
                    target_list.append(
                        f"{key} {value}")  # Space instead of = for runner args
                else:
...
@ray.remote
def remote_execute_job(job_cmd: str,
                       identifier_string: str,
                       test_mode: bool = False,
                       extract_experiment: bool = False) -> str | dict:
    """This method has an identical signature to :meth:`execute_job`, with the ray remote decorator"""
    return execute_job(job_cmd=job_cmd,
                       identifier_string=identifier_string,
                       test_mode=test_mode,
                       extract_experiment=extract_experiment)
...
def execute_job(
    ...
    result_details.append(
        f"{identifier_string}:[INFO]: Invocation {job_cmd} \n")
    node_id = ray.get_runtime_context().get_node_id()
    result_details.append(
        f"{identifier_string}:[INFO]: Ray Node ID: {node_id} \n")
    ...
    if test_mode:
        ...
        try:
            result = subprocess.run(
                [
                    "nvidia-smi", "--query-gpu=name,memory.free,serial",
            ...
            result_details.append(
                f"{identifier_string}[INFO]: Detected GPUs from PyTorch: {num_gpus_detected} \n"
            )
            ...
            if cuda_visible_devices:
                ...
                visible_devices_count = len(
                    output
                )  # All GPUs visible if CUDA_VISIBLE_DEVICES is not set
                result_details.append(
            ...
            if num_gpus_detected != len(output):
                result_details.append(
                    ...
                    " GPUs visible. \n")
                result_details.append(
                    f"{identifier_string}[INFO]: This shows that GPU resources were isolated.\n"
                )
                os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(
                    [str(i) for i in range(len(output))])
        ...
        process = subprocess.Popen(job_cmd,
                                   shell=True,
        ...
        logdir = None
        experiment_info_pattern = re.compile(
            "Exact experiment name requested from command line: (.+)")
        logdir_pattern = re.compile(
            r"\[INFO\] Logging experiment in directory: (.+)$")
        ...
        while True:
            new_line_ready, _, _ = select.select(
                [process_file_descriptor], [], [],
                1.0)  # Wait up to 1s for stdout
            if new_line_ready:
                ...
                if extract_experiment:
                    ...
                    if err_match:
                        raise ValueError(
                            f"Encountered an error during trial run. {' '.join(result_details)}"
                        )
            ...
            if extract_experiment:  # if we are looking for experiment info, check for timeouts and line limits
                ...
                if search_duration > max_time_to_search_logs:
                    print(
                        f"[ERROR]: Could not find experiment logs within {max_time_to_search_logs} seconds."
                    )
                    break
                if lines_read >= max_lines_to_search_logs:
                    print(
                        f"[ERROR]: Could not find experiment logs within first {max_lines_to_search_logs} lines."
                    )
                    break
        ...
        if extract_experiment and not (experiment_name and logdir):
            ...
            raise LogExtractionError(
                "Could not extract experiment_name/logdir from training workflow output."
            )
        process.wait()
...
def ray_init(ray_address: str = "auto",
    ...
    if not ray.is_initialized():
        ...
        print(
            "[WARNING]: Attempting to initialize Ray but it is already initialized!"
        )
...
def get_gpu_node_resources(
    ...
    if not ray.is_initialized():
        raise RuntimeError(
            "Ray must be initialized before calling get_gpu_node_resources().")
    nodes = ray.nodes()
    node_resources = []
    total_cpus = 0
    total_gpus = 0
    ...
    for node in nodes:
        if node["Alive"] and "GPU" in node["Resources"]:
            ...
            total_gpus += gpus
            total_memory += memory
    node_resources = sorted(
        node_resources,
        key=lambda x: (-x["GPU"], -x["CPU"], -x["memory"], x.get("id", "")))
...
def add_resource_arguments(
    ...
    arg_parser.add_argument(
        ...
        default=defaults[0],
        help=
        "Number of GPUs per worker node. Supply more than one for heterogeneous resources",
    )
    arg_parser.add_argument(
        "--cpu_per_worker",
        nargs="+",
        ...
        default=defaults[1],
        help=
        "Number of CPUs per worker node. Supply more than one for heterogeneous resources",
    )
    arg_parser.add_argument(
        "--ram_gb_per_worker",
        nargs="+",
        ...
        default=defaults[2],
        help=
        "RAM in GB per worker node. Supply more than one for heterogeneous resources.",
    )
    arg_parser.add_argument(
        "--num_workers",
        nargs="+",
        ...
        default=defaults[3],
        help=
        "Number of desired workers. Supply more than one for heterogeneous resources.",
    )
    return arg_parser
...
def fill_in_missing_resources(args: argparse.Namespace,
    ...
    print(
        "[INFO]: Filling in missing command line arguments with best guess...")
    ...
    for key, value in resources.items():
        ...
        if extension_length > 0:  # Only extend if the current list is shorter than max_length
            ...
            print(
                f"[INFO] Filling in {extension_length} missing values with {max_value}"
            )
            print(
                f"[INFO] To avoid auto-filling, provide {extension_length} more {key} value(s)"
            )
    ...
    def to_opt(self, nodes: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if self.specific is None or self.specific == "any":
            ...
            if self.hostname is None:
                raise ValueError(
                    "Hostname must be specified when specific is 'hostname'")
            for node in nodes:
                if node["NodeManagerHostname"] == self.hostname:
                    if node["alive"] is False:
                        raise ValueError(f"Node {node['NodeID']} is not alive")
                    opt["scheduling_strategy"] = NodeAffinitySchedulingStrategy(
                        node_id=node["NodeID"], soft=False)
                    return opt
            raise ValueError(
                f"Hostname {self.hostname} not found in nodes: {nodes}")
        elif self.specific == "node_id":
            if self.node_id is None:
                raise ValueError(
                    "Node ID must be specified when specific is 'node_id'")
            for node in nodes:
                if node["NodeID"] == self.node_id:
                    ...
                    return opt
            raise ValueError(
                f"Node ID {self.node_id} not found in nodes: {nodes}")
        else:
            raise ValueError(
                f"Invalid specific value: {self.specific}. Must be 'any', 'hostname', or 'node_id'."
            )
...
class JobActor:
    ...
    def run(self):
        """Run the job."""
        cmd = self.job.cmd if self.job.cmd else " ".join(
            [sys.executable, *self.job.py_args.split()])
        return execute_job(
            job_cmd=cmd,
...
def submit_wrapped_jobs(
    ...
    if not ray.is_initialized():
        raise Exception(
            "Ray is not initialized. Please initialize Ray before submitting jobs."
        )
    nodes = ray.nodes()
    actors = []
