...
def parse_args() -> argparse.Namespace:
    ...
    arg_parser = argparse.ArgumentParser(
        description=
        "Script to apply manifests to create Kubernetes objects for Ray clusters.",
    ...
    arg_parser.add_argument("--service_acount_name",
                            type=str,
                            required=False,
                            default="default",
                            help="The service account name to use.")
    ...
    arg_parser.add_argument(
        ...
        default=["nvidia-l4"],
        help=
        "GPU accelerator name. Supply more than one for heterogeneous resources.",
    )

    arg_parser = util.add_resource_arguments(arg_parser,
    ...
    arg_parser.add_argument("--head_ram_gb",
                            type=int,
                            default=8,
                            help="How many gigs of ram to give the Ray head")
...
def main():
    ...
    if "head" in args.name:
        raise ValueError(
            "For compatibility with other scripts, do not include head in the name"
        )
