...
class IsaacLabTuneTrainable(tune.Trainable):
    ...
    def step(self) -> dict:
        if self.experiment is None:  # start experiment
            ...
            print(
                f"[INFO]: Invoking experiment as first step with {self.invoke_cmd}..."
            )
            try:
                experiment = util.execute_job(
                    ...
                    identifier_string="",
                    extract_experiment=
                    True,  # Keep this as True to return a valid dictionary
                    persistent_dir=BASE_DIR,
                    max_lines_to_search_logs=
        ...
        if proc_status is not None:  # process finished, signal finish
            ...
            if self.data is not None:
                ...
                self_data_ = {
                    k: v
                    for k, v in self.data.items() if k != "done"
                }
                unresponsiveness_start_time = time()
                while util._dicts_equal(data_, self_data_):
                    self.time_since_last_proc_response = time(
                    ) - unresponsiveness_start_time
                    data = util.load_tensorboard_logs(self.tensorboard_logdir)
                    data_ = {k: v for k, v in data.items() if k != "done"}
                    proc_status = self.proc.poll()
                    if proc_status is not None:
                    ...
                    if self.time_since_last_proc_response > PROCESS_RESPONSE_TIMEOUT:
                        self.time_since_last_proc_response = 0.0
                        print(
                            "[WARNING]: Training workflow process is not responding, terminating..."
                        )
                        self.proc.terminate()
                        try:
                            ...
                            print(
                                "[ERROR]: The process did not terminate within timeout duration."
                            )
                            self.proc.kill()
                            self.proc.wait()
                        self.data = data
                        ...
                        return self.data
                    sleep(
                        2)  # Lazy report metrics to avoid performance overhead
    ...
    def default_resource_request(self):
        ...
        return tune.PlacementGroupFactory(
            [{
                "CPU": resources["CPU"] / NUM_WORKERS_PER_NODE,
                "GPU": resources["GPU"] / NUM_WORKERS_PER_NODE
            }],
...
class LogExtractionErrorStopper(tune.Stopper):
    ...
    def stop_all(self):
        ...
        if self.error_count > self.max_errors:
            print(
                "[FATAL]: Encountered LogExtractionError more than allowed, aborting entire tuning run... "
            )
            return True
        else:
            return False
...
class ProcessCleanupCallback(Callback):
    ...
    def _cleanup_trial(self, trial):
        ...
        try:
            subprocess.run([
                "pkill", "-9", "-f",
                f"rid {trial.config['runner_args']['-rid']}"
            ],
                           check=False)
            sleep(5)
...
def invoke_tuning_run(
    ...
    cfg["runner_args"]["-rid"] = tune.sample_from(
        lambda _: str(random.randint(int(1e9),
                                     int(1e10) - 1)))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Tune Isaac Lab hyperparameters.")
    parser.add_argument("--ray_address",
                        type=str,
                        default="auto",
                        help="the Ray address.")
    parser.add_argument(
        "--cfg_file",
        type=str,
    ...
    parser.add_argument(
        ...
        default="remote",
        help=("Set to local to use ./isaaclab.sh -p python, set to "
    ...
    parser.add_argument(
        ...
        help=
        "The absolute path of the workflow to use for the experiment. By default, RL Games is used.",
    )
    parser.add_argument(
        "--mlflow_uri",
        type=str,
    ...
    parser.add_argument(
        ...
        default=1,
        help=
        "Number of workers to run on each GPU node. Only supply for parallelism on multi-gpu nodes",
    )

    parser.add_argument("--metric",
                        type=str,
                        default="rewards/time",
                        help="What metric to tune for.")
    ...
    parser.add_argument(
        ...
        help=
        "Max number of lines to search for experiment logs before terminating the training workflow process.",
    )
    parser.add_argument(
        ...
        help=
        "Max number number of LogExtractionError failures before we abort the whole tuning run.",
    )
    parser.add_argument(
        "--stopper",
        type=str,
        default=None,
        help=
        "A stop criteria in the cfg_file, must be a tune.Stopper instance.",
    )

    args = parser.parse_args()
    PROCESS_RESPONSE_TIMEOUT = args.process_response_timeout
    MAX_LINES_TO_SEARCH_EXPERIMENT_LOGS = int(
        args.max_lines_to_search_experiment_logs)
    print(
    ...
    print(
        f"[INFO]: Attempting to use sweep config from {file_path=} {class_name=}"
    )
    ...
    if hasattr(module, class_name):
        ...
        print(
            f"[INFO]: Successfully instantiated class '{class_name}' from {file_path}"
        )
        cfg = instance.cfg
        print(
            f"[INFO]: Grabbed the following hyperparameter sweep config: \n {cfg}"
        )
        ...
        if args.stopper and hasattr(module, args.stopper):
            ...
            if isinstance(stopper, type) and issubclass(stopper, tune.Stopper):
                ...
                raise TypeError(
                    f"[ERROR]: Unsupported stop criteria type: {type(stopper)}"
                )
