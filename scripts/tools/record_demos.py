...
def create_environment(
        env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg) -> gym.Env:
    ...
    try:
        if hasattr(
            ...
            if args_cli.teleop_device.lower() == "keyboard":
                ...
                logger.error(
                    "Supported devices: keyboard, spacemouse, handtracking")
                exit(1)
...
def process_success_condition(env: gym.Env, success_term: object | None,
                              success_step_count: int) -> tuple[int, bool]:
...
def handle_reset(env: gym.Env, success_step_count: int,
            ...
            if running_recording_instance:
                ...
                if subtasks is not None:
                    if subtasks == {}:
                        ...
                        show_subtask_instructions(instruction_display,
                                                  subtasks, obv, env.cfg)
            else:
                env.sim.render()
...
def main() -> None:
    ...
    env_cfg, success_term = create_environment_config(output_dir,
                                                      output_file_name)
    ...
    # Run simulation loop
    current_recorded_demo_count = run_simulation_loop(env, None, success_term,
                                                      rate_limiter)
