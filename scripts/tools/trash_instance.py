...
def create_parent_xforms(asset_usd_path, source_prim_path, save_as_path=None):
    ...
    while len(prims) > 0:
        ...
        if prim.GetTypeName() in ["Mesh", "Capsule", "Sphere", "Box"]:
            new_xform = UsdGeom.Xform.Define(stage,
                                             str(prim.GetPath()) + "_xform")
            print(prim, new_xform)
            edits.Add(
                Sdf.NamespaceEdit.Reparent(prim.GetPath(), new_xform.GetPath(),
                                           0))
            continue
...
def convert_asset_instanceable(asset_usd_path,
                               source_prim_path,
                               save_as_path=None,
                               create_xforms=True):
    ...
    instance_usd_path = ".".join(
        asset_usd_path.split(".")[:-1]) + "_meshes.usd"
            ...
            if prim.GetTypeName() in ["Mesh", "Capsule", "Sphere", "Box"]:
                ...
                if parent_prim and not parent_prim.IsInstance():
                    parent_prim.GetReferences().AddReference(
                        assetPath=instance_usd_path,
                        primPath=str(parent_prim.GetPath()))
...
convert_asset_instanceable(
    asset_usd_path=
    "/home/ensu/Documents/weird/IsaacLab_assets/assets/robot/franka_leap/franka_right_leap.usd",
    source_prim_path="/World",
    save_as_path=
    "/home/ensu/Documents/weird/IsaacLab_assets/assets/robot/franka_leap/franka_right_leap_instanceable.usd",
)
