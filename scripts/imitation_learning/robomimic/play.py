...
parser.add_argument("--horizon",
                    type=int,
                    default=800,
                    help="Step horizon of each rollout.")
...
parser.add_argument("--enable_pinocchio",
                    default=False,
                    action="store_true",
...
def rollout(policy, env, success_term, horizon, device):
    ...
    for i in range(horizon):
        ...
        if args_cli.norm_factor_min is not None and args_cli.norm_factor_max is not None:
            actions = ((actions + 1) *
                       (args_cli.norm_factor_max - args_cli.norm_factor_min)
                       ) / 2 + args_cli.norm_factor_min

        actions = torch.from_numpy(actions).to(device=device).view(
...
def main():
    ...
    for trial in range(args_cli.num_rollouts):
        ...
        terminated, traj = rollout(policy, env, success_term, args_cli.horizon,
                                   device)
