...
class PreStepDatagenInfoRecorder(RecorderTerm):
    ...
    def record_pre_step(self):
        ...
        for eef_name in self._env.cfg.subtask_configs.keys():
            eef_pose_dict[eef_name] = self._env.get_robot_eef_pose(
                eef_name=eef_name)

        datagen_info = {
            "object_pose":
            self._env.get_object_poses(),
            "eef_pose":
            eef_pose_dict,
            "target_eef_pose":
            self._env.action_to_target_eef_pose(
...
def main():
    ...
    output_file_name = os.path.splitext(os.path.basename(
        args_cli.output_file))[0]
    ...
    if not args_cli.auto or (args_cli.auto
                             and not args_cli.annotate_subtask_start_signals):
    ...
    env: ManagerBasedRLMimicEnv = gym.make(args_cli.task,
    ...
    if args_cli.auto:
        ...
        for eef_name, eef_subtask_configs in env.cfg.subtask_configs.items():
            ...
            ] if args_cli.annotate_subtask_start_signals else [])
            ...
            if args_cli.annotate_subtask_start_signals:
                if any(name in (None, "")
                       for name in subtask_start_signal_names[eef_name]):
                    raise ValueError(
...
def annotate_episode_in_auto_mode(
    ...
    global skip_episode
    skip_episode = False
    is_episode_annotated_successfully = replay_episode(env, episode,
                                                       success_term)
    if skip_episode:
    ...
    if not is_episode_annotated_successfully:
        ...
        if args_cli.annotate_subtask_start_signals:
            subtask_start_signal_dict = annotated_episode.data["obs"][
                "datagen_info"]["subtask_start_signals"]
            for signal_name, signal_flags in subtask_start_signal_dict.items():
                if not torch.any(signal_flags):
                    is_episode_annotated_successfully = False
                    print(
                        f'\tDid not detect start for the subtask "{signal_name}".'
                    )
...
def annotate_episode_in_manual_mode(
    ...
    for eef_name, eef_subtask_term_signal_names in subtask_term_signal_names.items(
        ...
        if len(eef_subtask_term_signal_names) == 0 and len(
                eef_subtask_start_signal_names) == 0:
            continue
        ...
        while True:
            ...
            expected_subtask_signal_count = len(
                eef_subtask_term_signal_names) + len(
            ...
            if task_success_result and expected_subtask_signal_count == len(
                ...
                for marked_signal_index in range(
                        expected_subtask_signal_count):
                    if args_cli.annotate_subtask_start_signals and marked_signal_index % 2 == 0:
                        subtask_start_signal_action_indices[
                            eef_subtask_start_signal_names[int(
                                marked_signal_index /
                                2)]] = marked_subtask_action_indices[
                                    marked_signal_index]
                    if not args_cli.annotate_subtask_start_signals:
                        ...
                        subtask_term_signal_action_indices[
                            eef_subtask_term_signal_names[math.floor(
                                marked_signal_index /
                                2)]] = marked_subtask_action_indices[
                                    marked_signal_index]
                break
            ...
            if expected_subtask_signal_count != len(
                    marked_subtask_action_indices):
                print(
    ...
    for (
        ...
        subtask_signals = torch.ones(len(episode.data["actions"]),
                                     dtype=torch.bool)
