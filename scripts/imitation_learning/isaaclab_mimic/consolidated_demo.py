...
parser = argparse.ArgumentParser(
    description=
    "Record demonstrations and run mimic dataset generation for Isaac Lab environments."
)
...
async def run_teleop_robot(env,
                           env_id,
                           env_action_queue,
    ...
    while True:
        ...
        # convert to torch
        delta_pose = torch.tensor(delta_pose,
                                  dtype=torch.float,
                                  device=env.device).repeat(1, 1)
        # compute actions based on environment
                ...
                if success_step_count >= args_cli.num_success_steps:
                    env.recorder_manager.set_success_to_episodes(
                        ...
                        torch.tensor([[True]],
                                     dtype=torch.bool,
                                     device=env.device))
                    teleop_episode = env.unwrapped.recorder_manager.get_episode(
                        env_id)
                    await shared_datagen_info_pool.add_episode(teleop_episode)

                    recorded_episode_dataset_file_handler.write_episode(
                        teleop_episode)
...
async def run_data_generator(env,
                             env_id,
                             env_action_queue,
                             shared_datagen_info_pool,
                             success_term,
    ...
    while True:
        ...
        results = await data_generator.generate(
            ...
            select_src_per_subtask=env.unwrapped.cfg.datagen_config.
            generation_select_src_per_subtask,
            transform_first_robot_pose=env.unwrapped.cfg.datagen_config.
            generation_transform_first_robot_pose,
            interpolate_from_last_target_pose=env.unwrapped.cfg.datagen_config.
...
def env_loop(env, env_action_queue, shared_datagen_info_pool,
             asyncio_event_loop):
...
def main():
    ...
    if args_cli.generated_output_file:
        ...
        generated_output_file_name = os.path.splitext(
            os.path.basename(args_cli.generated_output_file))[0]
        ...
        if args_cli.teleop_env_index is not None and i == args_cli.teleop_env_index:
            data_generator_asyncio_tasks.append(
                asyncio_event_loop.create_task(
                    run_teleop_robot(env, i, env_action_queue,
                                     shared_datagen_info_pool, success_term,
    ...
    env_loop(env, env_action_queue, shared_datagen_info_pool,
             asyncio_event_loop)
