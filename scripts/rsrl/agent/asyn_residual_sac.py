...
try:
    from scripts.rsrl.agent.async_offline_wrapper import OffPolicyAlgorithm
except:
    from isaac_scripts.rsrl.agent.offline_wrapper import OffPolicyAlgorithm
...
class AsynResidualSAC(OffPolicyAlgorithm):
    """
    ...
    def __init__(
        ...
        actor_update_ratio: int = 1,
        use_residual_schedule: bool = False,
        residual_schedule_horizon: int = 10,
        max_residual_schedule: float = 100,
        ...
        super().__init__(policy,
                         ...
                         use_residual_schedule=use_residual_schedule,
                         residual_schedule_horizon=residual_schedule_horizon,
                         max_residual_schedule=max_residual_schedule,
        ...
        self.actor_update_ratio = actor_update_ratio
    ...
    def _setup_model(self) -> None:
        ...
        if self.target_entropy == "auto":
            ...
            self.target_entropy = float(-np.prod(
                self.action_space.shape).astype(np.float32))  # type: ignore
        else:
            # Force conversion
