...
class TrainerServer:
    ...
    def __init__(self,
        ...
        self._last_dict: Optional[dict] = None
        ...
        def __callback_impl(data: dict) -> dict:
            ...
            if _type == "datastore":
                ...
                if store_name not in self.data_stores:
                    return {
                        "success": False,
                        "message": f"Invalid datastore name {store_name}"
                    }
            ...
            # get last update id of the data store
                ...
                if store_name not in self.data_stores or store_name not in self.last_update_id_map:
                    return {
                        "success": False,
                        "message": f"Invalid datastore name"
                    }
                return {
                    "success": True,
                    "payload": self.last_update_id_map.get(store_name, -1)
                }
            ...
            # custom request
                ...
                return request_callback(_type,
                                        _payload) if request_callback else {}

            return {"success": False, "message": "Invalid type or payload"}

        ############################################################################

        self.req_rep_server = ReqRepServer(config.port_number,
                                           __callback_impl,
                                           log_level=log_level)
        ...
        if config.experimental_pipeline_port:
            ...
            self.consumer = Consumer(_pipe_callback_impl,
                                     config.experimental_pipeline_port)
    ...
    def last_received_dict(self) -> Optional[dict]:
...
class TrainerClient:
    ...
    def __init__(
        ...
        self.req_rep_client = ReqRepClient(server_ip,
                                           config.port_number,
                                           log_level=log_level,
        ...
        if compute_hash(config_json) != compute_hash(res["payload"]):
            raise Exception(f"Incompatible config with hash with server. "
                            "Please check the config of the server and client")

        # NOTE: experimental feature to use pipeline for data update
        if config.experimental_pipeline_port:
            self.producer = Producer(ip=server_ip,
    ...
    def recv_network_callback(self, callback: Callable[[dict], None]):
        ...
        self.broadcast_client = BroadcastClient(self.server_ip,
...
def make_trainer_config(port_number: int = 5555, broadcast_port: int = 5556):
    return TrainerConfig(
        port_number=port_number,
        broadcast_port=broadcast_port,
        request_types=["send-stats", "send_dict", "init_rl"],
...
def _start_server():
    ...
    server = TrainerServer(cfg, log_level=logging.INFO)
    try:
        server.start(threaded=False)  # blocking run
    except KeyboardInterrupt:
        print("\n[SERVER] Shutting down...")
        server.stop()


def make_shared_trainer_config(port: int = 5555, broadcast: int = 5556):
    """
    Build a TrainerConfig used by both server and client.
    Important: order of request_types must be the same everywhere.
    """
    cfg = make_trainer_config(port_number=port, broadcast_port=broadcast)
    # enforce deterministic request_types
    cfg.request_types = ["send-stats", "send_dict"]
    return cfg
