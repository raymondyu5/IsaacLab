...
try:
    ...
    from scripts.rsrl.agent.buffers import DictReplayBuffer, ReplayBuffer
except:
    from isaac_scripts.rsrl.agent.algo import BaseAlgorithm

    from isaac_scripts.rsrl.agent.buffers import DictReplayBuffer, ReplayBuffer
...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        ...
        residual_schedule_horizon: int = 10,
        max_residual_schedule: float = 20000,
        ...
        self.use_residual_schedule = use_residual_schedule
        ...
        self.iteractions = 0
        self.residual_schedule_horizon = residual_schedule_horizon
        self.max_residual_schedule = max_residual_schedule
        self.num_rollouts = 0
    ...
    def _setup_learn(
        ...
        truncate_last_traj = (self.optimize_memory_usage
                              ...
                              and replay_buffer is not None and
                              (replay_buffer.full or replay_buffer.pos > 0))
        ...
        # if (self.action_noise is not None and self.env.num_envs > 1
        #         and not isinstance(self.action_noise, VectorizedActionNoise)):
        #     self.action_noise = VectorizedActionNoise(self.action_noise,
        #                                               self.env.num_envs)
    ...
    def update_residual_schedule(self) -> None:
        ...
        """
        if self.iteractions > self.max_residual_schedule:
        ...
        # Optional: log
        self.logger.record("exploration/epsilon_residual",
    ...
    def _sample_action(
        ...
        # if self.use_residual_schedule:
        #     random_seed = np.random.rand(n_envs)
    ...
    def dump_logs(self) -> None:
        ...
        self.logger.record("rollout/iteractions", self.iteractions)
        ...
        if self.use_sde:
            self.logger.record(
                "train/std",
                (self.actor.get_std()).mean().item())  # type: ignore[operator]

        if len(self.ep_success_buffer) > 0:
            self.logger.record("rollout/success_rate",
                               safe_mean(self.ep_success_buffer))
        # Pass the number of timesteps for tensorboard
        self.logger.dump(step=self.iteractions)
