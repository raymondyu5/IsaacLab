...
class DictReplayBufferSamples(NamedTuple):
    ...
    dones: th.Tensor
    rewards: th.Tensor
    robot_actions: th.Tensor
    base_actions: th.Tensor
    cartesian_actions: th.Tensor
...
class ReplayBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        self.dones = np.zeros((self.buffer_size, self.n_envs),
                              dtype=np.float32)
        self.base_action = np.zeros(
            (self.buffer_size, self.n_envs, 22),
            dtype=self._maybe_cast_dtype(action_space.dtype))
    ...
    def batch_insert(self, batch_data):
        self.data_buffer += batch_data

        if len(self.data_buffer) >= self.flush_every_n_steps:

            for data in batch_data:

                self.add(obs=data['obs'],
                         next_obs=data['next_obs'],
                         action=data['action'],
                         reward=data['reward'],
                         done=data['done'],
                         infos=data['infos'])
                self.latest_data_id = data['latest_data_id']
            self.data_buffer = []
    ...
    def add(
        ...
        if base_action is not None:
            self.base_action[self.pos] = np.array(base_action)
    ...
    def _get_samples(
        ...
        next_actions = np.array([
            np.zeros_like(self.actions[batch_inds[i], env_indices[i], :]) if
            (batch_inds[i] + 1 >= self.buffer_size
             or self.dones[batch_inds[i], env_indices[i]]) else
            self.actions[batch_inds[i] + 1, env_indices[i], :]
            for i in range(len(batch_inds))
        ])
        next_base_action = np.array([
            np.zeros_like(self.base_action[batch_inds[i],
                                           env_indices[i], :]) if
            (batch_inds[i] + 1 >= self.buffer_size
             or self.dones[batch_inds[i], env_indices[i]]) else
            self.base_action[batch_inds[i] + 1, env_indices[i], :]
            for i in range(len(batch_inds))
        ])
        data = (
            ...
            next_actions,
            # self.base_action[batch_inds, env_indices, :],
            # next_base_action
        )
...
class DictReplayBuffer(ReplayBuffer):
    ...
    def __init__(
        ...
        if save_buffer:
            os.makedirs('logs/replay_buffer/', exist_ok=True)
        ...
        self.base_action = np.zeros(
            (self.buffer_size, self.n_envs, 23),
            dtype=self._maybe_cast_dtype(action_space.dtype))

        self.actions = np.zeros(
            (self.buffer_size, self.n_envs, self.action_dim),
            dtype=self._maybe_cast_dtype(action_space.dtype))
        self.robot_actions = np.zeros(
            (self.buffer_size, self.n_envs, 23),
            dtype=self._maybe_cast_dtype(action_space.dtype))
        self.rewards = np.zeros((self.buffer_size, self.n_envs),
                                dtype=np.float32)
        self.dones = np.zeros((self.buffer_size, self.n_envs),
                              dtype=np.float32)
        self.cartesian_actions = np.zeros(
            (self.buffer_size, self.n_envs, 23),
            dtype=self._maybe_cast_dtype(action_space.dtype))
    ...
    def add(  # type: ignore[override]
        ...
        base_action: Optional[np.ndarray] = None,
        cartesian_action: Optional[np.ndarray] = None,
    ) -> None:
        # Copy to avoid modification by reference
        ...
        if robot_action is not None:
            robot_action = robot_action.reshape((self.n_envs, 23))
            self.robot_actions[self.pos] = np.array(robot_action)

        self.actions[self.pos] = np.array(action)
        self.rewards[self.pos] = np.array(reward)
        if cartesian_action is not None:
            self.cartesian_actions[self.pos] = np.array(cartesian_action)
        self.dones[self.pos] = np.array(done)
        if base_action is not None:
            self.base_action[self.pos] = np.array(base_action)
        if self.handle_timeout_termination:
        ...
        if self.save_buffer:
            ...
            with open(f'logs/replay_buffer/replay_buffer_{self.pos}.pkl',
                      'wb') as f:
                ...
                pickle.dump(
                    {
                        ...
                        'cartesian_actions': cartesian_action,
                    }, f)

            # print(f'Saved replay buffer at step {self.pos}')
        ...
        return DictReplayBufferSamples(
            ...
            rewards=self.to_torch(
                self._normalize_reward(
                    ...
                    env)),
            robot_actions=self.to_torch(self.robot_actions[batch_inds,
                                                           env_indices]),
            base_actions=self.to_torch(self.base_action[batch_inds,
                                                        env_indices]),
            cartesian_actions=self.to_torch(
                self.cartesian_actions[batch_inds, env_indices]),
        )
...
class DictRolloutBuffer(RolloutBuffer):
    ...
    def reset(self) -> None:
        ...
        self.actions = np.zeros(
            (self.buffer_size, self.n_envs, self.action_dim), dtype=np.float32)
        self.robot_actions = np.zeros((self.buffer_size, self.n_envs, 23),
                                      dtype=np.float32)
        self.rewards = np.zeros((self.buffer_size, self.n_envs),
                                dtype=np.float32)
        self.returns = np.zeros((self.buffer_size, self.n_envs),
    ...
    def add(  # type: ignore[override]
        ...
        self.values[self.pos] = value.clone().cpu().numpy().flatten()
        self.log_probs[self.pos] = log_prob.clone().cpu().numpy()
        self.robot_actions[self.pos] = np.array(
            np.zeros_like(robot_action) if robot_action is
            None else robot_action)
        self.pos += 1
        if self.pos == self.buffer_size:
            self.full = True
