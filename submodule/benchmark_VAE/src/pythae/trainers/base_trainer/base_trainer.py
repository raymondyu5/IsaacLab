...
class BaseTrainer:
    ...
    def __init__(
        ...
        if self.distributed:
            ...
            device = ("cuda" if torch.cuda.is_available()
                      and not self.training_config.no_cuda else "cpu")

        self.amp_context = (torch.autocast("cuda") if self.training_config.amp
                            else contextlib.nullcontext())

        if (hasattr(model.model_config, "reconstruction_loss")
    ...
    def get_train_dataloader(
            self, train_dataset: BaseDataset) -> torch.utils.data.DataLoader:
        if self.distributed:
    ...
    def set_scheduler(self):
        if self.training_config.scheduler_cls is not None:
            scheduler_cls = getattr(lr_scheduler,
                                    self.training_config.scheduler_cls)
    ...
    def _set_output_dir(self):
        ...
        if not os.path.exists(
                self.training_config.output_dir) and self.is_main_process:
            os.makedirs(self.training_config.output_dir, exist_ok=True)
            logger.info(
                f"Created {self.training_config.output_dir} folder since did not exist.\n"
            )

        self._training_signature = (str(datetime.datetime.now())[0:19].replace(
            " ", "_").replace(":", "-"))
    ...
    def _get_file_logger(self, log_output_dir):
        ...
        f_handler = logging.FileHandler(
            os.path.join(log_dir,
                         f"training_logs_{self._training_signature}.log"))
    ...
    def _setup_callbacks(self):
        ...
        self.callback_handler = CallbackHandler(callbacks=self.callbacks,
                                                model=self.model)
    ...
    def train(self, log_output_dir: str = None):
        ...
        for epoch in range(1, self.training_config.num_epochs + 1):
            ...
            if (self.training_config.steps_predict is not None
            ...
            # save checkpoints
            if (self.training_config.steps_saving is not None
                    and epoch % self.training_config.steps_saving == 0):
                if self.is_main_process:
                    self.save_checkpoint(model=best_model,
                                         dir_path=self.training_dir,
                                         epoch=epoch)
    ...
    def predict(self, model: BaseAE):
        ...
        with self.amp_context:
            ...
            if self.distributed:
                ...
                normal_generation = model.decoder(
                    z).reconstruction.detach().cpu()
