...
def rename_logs(logs):
    ...
    for metric_name in logs.keys():
        if metric_name.startswith(train_prefix):
            clean_logs[metric_name.replace(train_prefix,
                                           "train/")] = logs[metric_name]

        if metric_name.startswith(eval_prefix):
            clean_logs[metric_name.replace(eval_prefix,
                                           "eval/")] = logs[metric_name]
...
class TrainingCallback:
    ...
    def on_train_step_begin(self, training_config: BaseTrainerConfig,
                            **kwargs):
        """
...
class WandbCallback(TrainingCallback):  # pragma: no cover
    ...
    def on_prediction_step(self, training_config: BaseTrainerConfig, **kwargs):
        ...
        column_names = [
            "images_id", "truth", "reconstruction", "normal_generation"
        ]
        ...
        if (true_data is not None and reconstructions is not None
                and generations is not None):
            for i in range(len(true_data)):
                data_to_log.append([
                    f"img_{i}",
                    self._wandb.Image(
                        np.moveaxis(true_data[i].cpu().detach().numpy(), 0,
                                    -1)),
                    self._wandb.Image(
                        np.clip(
                            np.moveaxis(
                                reconstructions[i].cpu().detach().numpy(), 0,
                                -1),
                            0,
                            255.0,
                        )),
                    self._wandb.Image(
                        np.clip(
                            np.moveaxis(generations[i].cpu().detach().numpy(),
                                        0, -1),
                            0,
                            255.0,
                        )),
            ...
            val_table = self._wandb.Table(data=data_to_log,
...
class MLFlowCallback(TrainingCallback):  # pragma: no cover
    ...
    def __del__(self):
        ...
        if (callable(getattr(self._mlflow, "active_run", None))
...
class CometCallback(TrainingCallback):  # pragma: no cover
    ...
    def setup(
        ...
        **kwargs,
        ...
        if not offline_run:
            experiment = self._comet_ml.Experiment(api_key=api_key,
    ...
    def on_prediction_step(self, training_config: BaseTrainerConfig, **kwargs):
        ...
        if (true_data is not None and reconstructions is not None
            ...
            for i in range(len(true_data)):
                ...
                experiment.log_image(
                    np.clip(
                        np.moveaxis(generations[i].cpu().detach().numpy(), 0,
                                    -1),
                        0,
                        255.0,
                    ),
