...
class HierarchicalResidualQuantizer(nn.Module):
    ...
    def __init__(self, model_config: HRQVAEConfig):
        ...
        for hix, embedding in enumerate(self.embeddings):
            scale = init_scale * init_decay_weight**hix / sqrt(
                self.embedding_dim)
            embedding.weight.data.uniform_(-1.0 * scale, scale)

            # Normalise onto sphere
            embedding.weight.data = (
                embedding.weight.data / torch.linalg.vector_norm(
                    embedding.weight, dim=1, keepdim=True) * init_scale *
    ...
    def forward(self, z: torch.Tensor, epoch: int, uses_ddp: bool = False):
        ...
        for head_ix, embedding in enumerate(self.embeddings):
            ...
            distances = -1.0 * (torch.sum(resid_error**2, dim=-1, keepdim=True)
                                + torch.sum(embedding.weight**2, dim=-1) - 2 *
                                torch.matmul(resid_error, embedding.weight.T))
            ...
            if self.training:
                ...
                sample_onehot = F.gumbel_softmax(distances,
                                                 tau=gumbel_temp,
                                                 hard=True,
                                                 dim=-1)
            else:
                ...
                sample_onehot = F.one_hot(
                    indices, num_classes=self.num_embeddings).float()

            probs = F.softmax(distances / gumbel_temp, dim=-1)

            # KL loss
            prior = (torch.ones_like(distances).detach() /
                     torch.ones_like(distances).sum(-1, keepdim=True).detach())
            kl_loss = torch.nn.KLDivLoss(reduction="none")
            kl = kl_loss(nn.functional.log_softmax(distances, dim=-1),
                         prior).sum(dim=-1)
        ...
        if self.model_config.norm_loss_weight is not None:
            upper_norms = torch.linalg.vector_norm(quantized[:, :-1, :],
                                                   dim=-1)
        ...
        quantized_indices = quantized_indices.reshape(*input_shape[:-1],
                                                      self.num_levels)
