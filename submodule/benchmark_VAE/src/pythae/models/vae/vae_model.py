...
class VAE(BaseAE):
    ...
    def forward(self, inputs: BaseDataset, **kwargs):
        ...
        if isinstance(x, dict):
            ...
            loss, recon_loss, kld = self.loss_function(recon_x,
                                                       x["action_chunk"], mu,
                                                       log_var, z)
        else:
            recon_x = self.decoder(z)["reconstruction"]

            loss, recon_loss, kld = self.loss_function(recon_x, x, mu, log_var,
                                                       z)
    ...
    def loss_function(self, recon_x, x, mu, log_var, z):
        ...
        return (recon_loss +
                KLD).mean(dim=0), recon_loss.mean(dim=0), KLD.mean(dim=0)
    ...
    def get_nll(self, data, n_samples=1, batch_size=100):
        ...
        for i in range(len(data)):
            ...
            for j in range(n_full_batch):
                ...
                log_q_z_given_x = -0.5 * (
                    log_var + (z - mu)**2 / torch.exp(log_var)).sum(dim=-1)
