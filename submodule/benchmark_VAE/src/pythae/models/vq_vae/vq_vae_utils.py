...
class Quantizer(nn.Module):
    ...
    def forward(self, z: torch.Tensor, uses_ddp: bool = False):
        ...
        one_hot_encoding = (F.one_hot(closest,
                                      num_classes=self.num_embeddings).type(
                                          torch.float).squeeze(1))
...
class QuantizerEMA(nn.Module):
    ...
    def forward(self, z: torch.Tensor, uses_ddp: bool = False):
        ...
        quantized_indices = closest.reshape(z.shape[0], -1)
        ...
        if self.training:
            ...
            self.cluster_size = self.cluster_size * self.decay + n_i * (
                1 - self.decay)
            ...
            self.embeddings.data.copy_(ema_embed /
