...
class INFOVAE_MMD(VAE):
    ...
    def __init__(
        ...
        self.scales = model_config.scales if model_config.scales is not None else [
            1.0
        ]
    ...
    def loss_function(self, recon_x, x, z, z_prior, mu, log_var):
        ...
        mmd_z_prior = (k_z_prior - k_z_prior.diag().diag()).sum() / (
            (N - 1) * N)
        mmd_cross = k_cross.sum() / (N**2)

        mmd_loss = mmd_z + mmd_z_prior - 2 * mmd_cross

        loss = (recon_loss + (1 - self.alpha) * KLD +
                (self.alpha + self.lbd - 1) * mmd_loss)
    ...
    def imq_kernel(self, z1, z2):
        ...
        Cbase = (2.0 * self.model_config.latent_dim *
                 self.model_config.kernel_bandwidth**2)
