...
class AutoModel(nn.Module):
    ...
    @classmethod
    def load_from_folder(cls, dir_path: str, device="cpu"):
        ...
        if model_name == "Adversarial_AE_Config":
            ...
            model = AE.load_from_folder(dir_path=dir_path, device=device)
            ...
            model = BetaVAE.load_from_folder(dir_path=dir_path, device=device)
            ...
            model = HVAE.load_from_folder(dir_path=dir_path, device=device)

        elif model_name == "HRQVAEConfig":
            from ..hrq_vae import HRQVAE

            model = HRQVAE.load_from_folder(dir_path=dir_path, device=device)
            ...
            model = IWAE.load_from_folder(dir_path=dir_path, device=device)
            ...
            model = RAE_GP.load_from_folder(dir_path=dir_path, device=device)

        elif model_name == "RAE_L2_Config":
            from ..rae_l2 import RAE_L2

            model = RAE_L2.load_from_folder(dir_path=dir_path, device=device)

        elif model_name == "RHVAEConfig":
            from ..rhvae import RHVAE

            model = RHVAE.load_from_folder(dir_path=dir_path, device=device)

        elif model_name == "SVAEConfig":
            from ..svae import SVAE

            model = SVAE.load_from_folder(dir_path=dir_path, device=device)

        elif model_name == "VAEConfig":
            from ..vae import VAE

            model = VAE.load_from_folder(dir_path=dir_path, device=device)

        elif model_name == "VAEGANConfig":
            from ..vae_gan import VAEGAN

            model = VAEGAN.load_from_folder(dir_path=dir_path, device=device)

        elif model_name == "VAE_IAF_Config":
            from ..vae_iaf import VAE_IAF

            model = VAE_IAF.load_from_folder(dir_path=dir_path, device=device)
            ...
            model = VAMP.load_from_folder(dir_path=dir_path, device=device)

        elif model_name == "VQVAEConfig":
            from ..vq_vae import VQVAE

            model = VQVAE.load_from_folder(dir_path=dir_path, device=device)

        elif model_name == "WAE_MMD_Config":
            from ..wae_mmd import WAE_MMD

            model = WAE_MMD.load_from_folder(dir_path=dir_path, device=device)

        elif model_name == "MAFConfig":
            from ..normalizing_flows import MAF

            model = MAF.load_from_folder(dir_path=dir_path, device=device)

        elif model_name == "IAFConfig":
            from ..normalizing_flows import IAF

            model = IAF.load_from_folder(dir_path=dir_path, device=device)
            ...
            model = MADE.load_from_folder(dir_path=dir_path, device=device)

        elif model_name == "PixelCNNConfig":
            from ..normalizing_flows import PixelCNN

            model = PixelCNN.load_from_folder(dir_path=dir_path, device=device)
            ...
            model = CIWAE.load_from_folder(dir_path=dir_path, device=device)

        elif model_name == "MIWAEConfig":
            from ..miwae import MIWAE

            model = MIWAE.load_from_folder(dir_path=dir_path, device=device)

        elif model_name == "PIWAEConfig":
            from ..piwae import PIWAE

            model = PIWAE.load_from_folder(dir_path=dir_path, device=device)
    ...
    @classmethod
    def load_from_hf_hub(cls,
                         hf_hub_path: str,
                         allow_pickle: bool = False):  # pragma: no cover
        ...
        config_path = hf_hub_download(repo_id=hf_hub_path,
        ...
        if model_name == "Adversarial_AE_Config":
            ...
            model = PoincareVAE.load_from_hf_hub(hf_hub_path=hf_hub_path,
                                                 allow_pickle=allow_pickle)

        elif model_name == "CIWAEConfig":
            from ..ciwae import CIWAE

            model = CIWAE.load_from_hf_hub(hf_hub_path=hf_hub_path,
                                           allow_pickle=allow_pickle)

        elif model_name == "MIWAEConfig":
            from ..miwae import MIWAE

            model = MIWAE.load_from_hf_hub(hf_hub_path=hf_hub_path,
                                           allow_pickle=allow_pickle)

        elif model_name == "PIWAEConfig":
            from ..piwae import PIWAE

            model = PIWAE.load_from_hf_hub(hf_hub_path=hf_hub_path,
                                           allow_pickle=allow_pickle)
