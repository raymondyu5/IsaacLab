...
class BaseAE(nn.Module):
    ...
    def interpolate(
        ...
        intep_line = (torch.kron(starting_z.reshape(starting_z.shape[0], -1),
                                 (1 - t).unsqueeze(-1)) +
                      torch.kron(ending_z.reshape(ending_z.shape[0], -1),
                                 t.unsqueeze(-1))
                      ).reshape((starting_z.shape[0] * t.shape[0], ) +
                                (starting_z.shape[1:]))
    ...
    def save(self, dir_path: str):
        ...
        if not self.model_config.uses_default_encoder:
            with open(os.path.join(dir_path, "encoder.pkl"), "wb") as fp:
                cloudpickle.register_pickle_by_value(
                    inspect.getmodule(self.encoder))
        ...
        if not self.model_config.uses_default_decoder:
            with open(os.path.join(dir_path, "decoder.pkl"), "wb") as fp:
                cloudpickle.register_pickle_by_value(
                    inspect.getmodule(self.decoder))
    ...
    @classmethod
    def _load_model_weights_from_folder(cls, dir_path, device="cpu"):
        file_list = os.listdir(dir_path)
    ...
    @classmethod
    def _load_custom_encoder_from_folder(cls, dir_path, device="cpu"):
        file_list = os.listdir(dir_path)
    ...
    @classmethod
    def _load_custom_decoder_from_folder(cls, dir_path, device="cpu"):
        file_list = os.listdir(dir_path)
        cls._check_python_version_from_folder(dir_path=dir_path)
    ...
    @classmethod
    def load_from_folder(cls, dir_path, device="cpu"):
        ...
        model_weights = cls._load_model_weights_from_folder(dir_path, device)

        if not model_config.uses_default_encoder:
            encoder = cls._load_custom_encoder_from_folder(dir_path, device)

        else:
            encoder = None

        if not model_config.uses_default_decoder:
            decoder = cls._load_custom_decoder_from_folder(dir_path, device)
    ...
    def load_from_hf_hub(cls,
                         hf_hub_path: str,
                         allow_pickle=False):  # pragma: no cover
        ...
        if (cls.__name__ + "Config" != model_config.name
                and cls.__name__ + "_Config" != model_config.name):
            warnings.warn(f"You are trying to load a "
        ...
        if (not model_config.uses_default_encoder
                or not model_config.uses_default_decoder) and not allow_pickle:
            warnings.warn(
            ...
            if not model_config.uses_default_encoder:
                _ = hf_hub_download(repo_id=hf_hub_path,
