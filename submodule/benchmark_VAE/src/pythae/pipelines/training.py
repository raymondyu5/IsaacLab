...
from ..data.datasets import collate_dataset_output, ActionChunkDataset
...
class TrainingPipeline(Pipeline):
    ...
    def __init__(
        ...
        if training_config is None:
            ...
            if not isinstance(training_config,
    ...
    def __call__(
        ...
        if isinstance(train_data, torch.utils.data.DataLoader):
            ...
            logger.info("Checking train dataset...")
            self._check_dataset(train_dataset)
            train_dataloader = ActionChunkDataset(train_dataset[0],
                                                  train_dataset[1])
        ...
        if eval_data is not None:
            if isinstance(eval_data, torch.utils.data.DataLoader):
                ...
                logger.info("Checking eval dataset...")
                self._check_dataset(eval_dataset)
                eval_dataloader = ActionChunkDataset(eval_dataset[0],
                                                     eval_dataset[1])
        ...
        if isinstance(self.training_config, CoupledOptimizerTrainerConfig):
        ...
        elif isinstance(self.training_config,
                        CoupledOptimizerAdversarialTrainerConfig):
            ...
            trainer = BaseTrainer(
                ...
                if train_dataloader is not None else train_dataset,
                eval_dataset=eval_dataloader
                if eval_dataloader is not None else eval_dataset,
                training_config=self.training_config,
                callbacks=callbacks,
            )
        else:
