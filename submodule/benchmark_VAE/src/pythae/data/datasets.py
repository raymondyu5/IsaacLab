...
class ActionChunkDataset(Dataset):
    ...
    def __init__(self, action_chunk, state):
        self.action_chunk = action_chunk.float()  # (N, seq, action_dim)
        self.state = state.float()  # (N, state_dim)

    def __len__(self):
        return len(self.action_chunk)

    def __getitem__(self, idx):
        return {
            "data": {
                "action_chunk": self.action_chunk[idx],  # (seq, action_dim)
                "state": self.state[idx]  # (state_dim,)
            }
        }
