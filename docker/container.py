...
def parse_cli_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Utility for using Docker with Isaac Lab.")
    ...
    parent_parser.add_argument(
        ...
        default="base",
        help=
        "Optional container profile specification. Example: 'base' or 'ros'.")
    parent_parser.add_argument(
        "--files",
        nargs="*",
        default=None,
    ...
    subparsers.add_parser(
        "start",
        help=
        "Build the docker image and create the container in detached mode.",
        parents=[parent_parser],
    )
    subparsers.add_parser(
        "enter",
        help="Begin a new bash process within an existing Isaac Lab container.",
        parents=[parent_parser])
    ...
    config.add_argument(
        ...
        nargs="?",
        default=None,
        help="Yaml file to write config output to. Defaults to None.")
    subparsers.add_parser(
        "copy",
        help=
        "Copy build and logs artifacts from the container to the host machine.",
        parents=[parent_parser])
    subparsers.add_parser("stop",
                          help="Stop the docker container and remove it.",
                          parents=[parent_parser])
...
def main(args: argparse.Namespace):
    ...
    if args.command == "start":
        ...
        raise RuntimeError(
            f"Invalid command provided: {args.command}. Please check the help message."
        )
